   ...
   string weightBranchBkg)
{
   ...
   float FOM[300];
   TH2F *myfom = new TH2F("S/sqrt(S+B)", "myfom",100, 0.0, 1.0, 100 , 0.0, 280.0);
...
//      FOM[i]=(scalefacsig*getEntries( tSig, extracut+" && "+bdtvar + " < " + d2s(mincut+(i/(1.*n))*d), weightBranchSig ))/sqrt((scalefacbkg*getEntries( tBkg, extracut+" && "+bdtvar + " < " + d2s(mincut+(i/(1.*n))*d), weightBranchBkg )) + (scalefacsig*getEntries( tSig, extracut+" && "+bdtvar + " < " + d2s(mincut+(i/(1.*n))*d), weightBranchSig )));
  FOM[i]=(scalefacsig*getEntries( tSig, extracut+" && "+bdtvar + " < " + d2s(mincut+(i/(1.*n))*d), weightBranchSig ))/sqrt((scalefacbkg*getEntries( tBkg, extracut+" && "+bdtvar + " < " + d2s(mincut+(i/(1.*n))*d), weightBranchBkg )) + (scalefacsig*getEntries( tSig, extracut+" && "+bdtvar + " < " + d2s(mincut+(i/(1.*n))*d), weightBranchSig )));
      ...
      cout<<"Index of cut:"<<i<<" Sig Eff:"<<sigEff[i]<<" Bkg Rej:"<<bkgRej[i]<<" Nsigleft:"<<nSigLeft[i]<<" Nbkgleft:"<<nBkgLeft[i]<<endl;
      cout<<"is there a problem?: "<<nSigLeft[i]/sqrt(nBkgLeft[i]+nSigLeft[i])<<endl;
   }
...
//         cout<<"This is BDTTrain:"<<BDTTrain<<endl;
         if(classIDTrain == 1) hsigtrain.Fill(BDTTrain, w);
         if(classIDTrain == 0) hbkgtrain.Fill(BDTTrain, w);
      }
