...
//      ("Bplus_TAU")
      ...
      ("Bplus_FD_CHI2")
//      ("(sqrt((mu1_PX-mu3_PX)^2+(mu1_PY-mu3_PY)^2+(mu1_PZ-mu3_PZ)^2)/sqrt((mu1_PX+mu3_PX)^2+(mu1_PY+mu3_PY)^2+(mu1_PZ+mu3_PZ)^2))")
//      ("(sqrt((mu2_PX-mu3_PX)^2+(mu2_PY-mu3_PY)^2+(mu2_PZ-mu3_PZ)^2)/sqrt((mu2_PX+mu3_PX)^2+(mu2_PY+mu3_PY)^2+(mu2_PZ+mu3_PZ)^2))")
//      ("(sqrt((mu1_PX-mu2_PX)^2+(mu1_PY-mu2_PY)^2+(mu1_PZ-mu2_PZ)^2)/sqrt((mu1_PX+mu2_PX)^2+(mu1_PY+mu2_PY)^2+(mu1_PZ+mu2_PZ)^2))")
...
//  );
//CombinatorialNormalBDTNTrees60_MinNodeSize2_MaxDepth3_SeparationTypeGiniIndex_PruneMethodNoPruning_DoPreselection  !H:!V:BoostType=AdaBoost:NTrees=60:MinNodeSize=2:MaxDepth=3:SeparationType=GiniIndex:nCuts=-1:PruneMethod=NoPruning:DoPreselection
...
//   kf.run("MisIDNormalBDTNTrees60_MinNodeSize2_MaxDepth3_SeparationTypeGiniIndex_PruneMethodNoPruning_DoPreselection", "!H:!V:BoostType=AdaBoost:NTrees=60:MinNodeSize=2:MaxDepth=3:SeparationType=GiniIndex:nCuts=-1:PruneMethod=NoPruning:DoPreselection", "all");

//   cout<<"Run name"<<args.runame<<endl;
