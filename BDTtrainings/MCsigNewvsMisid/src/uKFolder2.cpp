#define CLASS_UKFOLDER2_H_CXX

#include "uKFolder2.hpp"
#include "TTreeFormula.h"


uKFolder2::Writer::Writer() :
  _reader(NULL),
  _all(false),
  _n(0),
  _methodname("") {
...
uKFolder2::Writer::Writer(TMVA::Reader* reader, const std::string& methodname, const int& n, const bool& all) :
  _reader(reader),
  _all(all),
  _n(n),
...
bool uKFolder2::Writer::initialize(TTree* tree, std::string branchname) {
  if (!branchname.size()) {
...
void uKFolder2::Writer::fill(const int& k) {
...
uKFolder2::uKFolder2(const std::string& sig, const std::string& bkg) :
  _filename_sig(sig),
  _filename_bkg(bkg),
  _n(10) {
...
void uKFolder2::setSigFile(const std::string& filename_sig) {
  _filename_sig = filename_sig;
}
void uKFolder2::setBkgFile(const std::string& filename_bkg) {
  _filename_bkg = filename_bkg;
}
void uKFolder2::setOutFile(const std::string& filename_out) {
  _filename_out = filename_out;
  boost::replace_all(_filename_out, ".root", "");
}
void uKFolder2::setNFolds(const int& n) {
  _n = n;
}
void uKFolder2::addTrainingVar(const std::string& tvar) {
  _training.push_back(tvar);
}
void uKFolder2::addSpectatorVar(const std::string& svar) {
  _spectator.push_back(svar);
}
void uKFolder2::addTrainingVars(const std::vector<std::string>& tvars) {
  _training = tvars;
}
void uKFolder2::addSpectatorVars(const std::vector<std::string>& svars) {
  _spectator = svars;
}
void uKFolder2::addTrainingOpt(const std::string& name, const std::string& opt) {
  _trainopts[name] = opt;
}
void uKFolder2::addSigCut(const std::string& cut) {
  _cut_sig = TCut(cut.c_str());
}
void uKFolder2::addBkgCut(const std::string& cut) {
  _cut_bkg = cut;
}


void uKFolder2::_trainFold(
    TTree*& tree_sig,
    TTree*& tree_bkg,
    const int& k) {
...
bool uKFolder2::run(const std::string& filename, const std::string& bdtname, const bool& all) {
...
void uKFolder2::setBranch(const std::string& bname) {
  _branches_to_set.push_back(bname);
}

void uKFolder2::setBranches(std::vector<std::string> bnames) {
  _branches_to_set = bnames;
}

void uKFolder2::_setBranches(TTree*& tree) {
