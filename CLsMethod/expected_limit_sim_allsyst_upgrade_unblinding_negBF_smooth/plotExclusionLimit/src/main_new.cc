...
int main(int argc, char *argv[]){
	...
	clsmed=CLsMed2("info_new.root", "CLsValues", "CLsMedian");
	double* myclsval=&clsmed[0];
        int sizeofvec=clsmed.size(); 

        
	vector<double> clsobs;
	clsobs=CLsMed2("info_new.root", "CLsValues", "CLsObserved");
...
//        mg->SetTitle("CLs Method");
	...
	TGraph *grfirst =  new TGraph(n,mybrval,myclsval);
        ...
        clsplusone=CLsMed2("info_new.root", "CLsValues", "CLsOneSigma");
        double* myclsvalpone=&clsplusone[0];


        //Plot Minus One sigma//
        vector<double> clsminusone;
        clsminusone=CLsMed2("info_new.root", "CLsValues", "CLsMinusOneSigma");
        double* myclsvalmone=&clsminusone[0];


        //Plot One Sigma//
	vector<double> clsplustwo;
	clsplustwo=CLsMed2("info_new.root", "CLsValues", "CLsTwoSigma");
	double* myclsvalptwo=&clsplustwo[0];


	//Plot Minus One sigma//
	vector<double> clsminustwo;
	clsminustwo=CLsMed2("info_new.root", "CLsValues", "CLsMinusTwoSigma");
...
//        leg->AddEntry(h1,"Histogram filled with random numbers","f");
	...
	c1->SaveAs("ClsExclusion_new.pdf");

        c1->SetLogy();
        c1->Update();

	c1->SaveAs("ClsExclusion_new_Log.pdf");
	return(0);
