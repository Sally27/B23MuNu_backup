...
//   double CLsBF = 7;
    ...
    TFile* f = new TFile("myworkspace_fitallafterMCnSharedRemovedFullUMSBIncludeLMSB.root");
   ...
   // RooAbsPdf* pdf_sb = ws->pdf("modelsb");
    ...
    RooAbsData* data = ws->data("normsigpdfData");
...
//    return(1);
	...
	rfres.cd();
        double backgroundDLL = fitTwiceOld(BF,pdf_sb,background_datasets[i],nToys,CLsBF,mass,&bkgll,update);

	s.cd();
        hbackground_DLL->Fill(backgroundDLL);
	rfres.cd();
        double signalDLL = fitTwiceOld(BF,pdf_sb,signal_datasets[i],nToys,CLsBF,mass,&sigll,update);
	update = true;
        s.cd();
