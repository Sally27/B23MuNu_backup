...
double fitgaussianandplot(TH1F * hsignal_DLL, string category, std::ofstream& myfile, int nToys, double CLsBF){
      ...
      TF1* rufit = new TF1("Gaussian","gaus",-20.0,20.0);
...
double calculateCLs(TH1F * hsignal_DLL, TH1F * hbackground_DLL, double value, std::ofstream& myfile, double CLsBF){
      double CLb = hbackground_DLL->Integral(hbackground_DLL->FindBin(-20),hbackground_DLL->FindBin(value))/hbackground_DLL->GetSumOfWeights();
      double CLs_b = hsignal_DLL->Integral(hsignal_DLL->FindBin(-20),hsignal_DLL->FindBin(value))/hsignal_DLL->GetSumOfWeights();
...
std::vector<RooDataSet*> generateData(RooRealVar* BF,double BFval,RooAbsPdf* model_LL,RooRealVar* mass,RooCategory cat, int nToys) 
{
        ...
        for (int i = 0; i < nToys; ++i)
        {
          ...
          //      RooDataSet* dataset_DD = model_DD->generate(RooArgSet(*mass),RooFit::Extended(true));
                RooDataSet* dataset = new RooDataSet("data","combined data",*mass,RooFit::Index(cat),RooFit::Import("NOFCME",*dataset_LL));//,RooFit::Import("HighFCME",*dataset_DD));
...
//    cat.defineType("HighFCME") ;
    ...
    data->plotOn(frame1,Cut("sample==sample::NOFCME")) ;
    cout<<"PLOT FINISH in fit old"<<endl;
    model->plotOn(frame1,Slice(*cat,"NOFCME"),ProjWData(*cat,*data));
...
//    treename->Write();
 ...
 //   rfrestree.Branch("CLsOneSigma",&dataresult_free->minNll(), "CLsOneSigma/D");
    ...
    RooPlot* frame3 = Bplus_Corrected_Mass->frame(Title("secondfitNOFCME"),Bins(60)) ;
    data->plotOn(frame3,Cut("sample==sample::NOFCME"));
    model->plotOn(frame3,Slice(*cat,"NOFCME"),ProjWData(*cat,*data));
