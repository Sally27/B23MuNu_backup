...
//   double CLsBF = 7;
   ...
   // RooAbsPdf* pdf = ws->pdf("modelsb");
    ...
    //RooCategory* tracktype = (RooCategory*)ws->var("tracktype");
    RooAbsPdf* pdf_LL = ws->pdf("normsigpdf");
...
//    RooAbsPdf* pdf = ws2->pdf("fLL");
 ...
 //   return(1);
    ...
    RooPlot* frame1 = mass->frame(Title("firstfitNOFCME"),Bins(60)) ;
    data->plotOn(frame1,Cut("sample==sample::NOFCME")) ;
    cout<<"PLOT FINISH 1"<<endl;
    pdf->plotOn(frame1,Slice(*cat,"NOFCME"),ProjWData(*cat,*data));
...
//    return(1);
    ...
    pdf->plotOn(frame3,Slice(*cat,"NOFCME"),ProjWData(*cat,*data));
...
//    return(1);
    ...
    std::vector<RooDataSet*> background_datasets = generateData(BF,0.0,pdf_LL,mass,*cat,nToys);
    std::vector<RooDataSet*> signal_datasets = generateData(BF,CLsBF,pdf_LL,mass,*cat,nToys);
