...
//    TCanvas* canvas= new TCanvas("fit", "Fit",800,800);
   ...
   // RooAbsPdf* pdf_sb = ws->pdf("modelsb");
    ...
    RooFitResult* data_intresult_free = pdf_sb_int->fitTo(*data_int,RooFit::Extended(true),RooFit::Range("low,high"),RooFit::Save(true));
    ...
    RooFitResult* data_intresult_fixed = pdf_sb_int->fitTo(*data_int,RooFit::Extended(true),RooFit::Range("low1,high1"),RooFit::Save(true));
...
//    return(1);
	...
	rfres.cd();
        double backgroundDLL = fitTwiceOld(BF,pdf_sb,background_datasets[i],nToys,CLsBF,mass,&bkgll,update, i);

	s.cd();
        hbackground_DLL->Fill(backgroundDLL);
	rfres.cd();
        double signalDLL = fitTwiceOld(BF,pdf_sb,signal_datasets[i],nToys,CLsBF,mass,&sigll,update, i);
	update = true;
        s.cd();
