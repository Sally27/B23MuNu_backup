...
//    RooAbsPdf* ses_2016_con = ws->pdf("ses_2016_con");
    ...
    RooArgSet constraints(*npr_con, *ses_con);
...
//    return(1);
	...
	rfres.cd();
        double backgroundDLL = fitTwiceOld(BF,pdf_sb,background_datasets[i],nToys,CLsBF,mass,&bkgll,update, i, constraints);

	s.cd();
        hbackground_DLL->Fill(backgroundDLL);
	rfres.cd();
        double signalDLL = fitTwiceOld(BF,pdf_sb,signal_datasets[i],nToys,CLsBF,mass,&sigll,update, i, constraints);
	update = true;
        s.cd();
