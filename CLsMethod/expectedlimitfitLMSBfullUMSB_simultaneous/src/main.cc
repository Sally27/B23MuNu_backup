...
//    RooCategory* cat = (RooCategory*)ws->var("sample");
...
// Sallys code
    ...
    for (int i = 0; i < nToys; ++i)
    {
	    ...
	    rfres.cd();
	    double backgroundDLL = fitTwiceOld(BF,pdf,background_datasets[i],nToys,CLsBF,mass,&bkgll,update);

	    s.cd();
	    hbackground_DLL->Fill(backgroundDLL);
	    rfres.cd();
	    double signalDLL = fitTwiceOld(BF,pdf,signal_datasets[i],nToys,CLsBF,mass,&sigll,update);
	    update = true;
	    s.cd();
