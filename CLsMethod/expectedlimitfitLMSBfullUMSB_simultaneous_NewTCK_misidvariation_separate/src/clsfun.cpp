...
std::vector<RooDataSet*> generateData_2(double BFval,int nToys) 
{
   ...
   RooRealVar* BF_low = ws->var("BR_low");
   RooRealVar* BF_high = ws->var("BR_high");
   RooRealVar* mass = ws->var("Bplus_Corrected_Mass");

        std::vector<RooDataSet*> datasets;
        for (int i = 0; i < nToys; ++i)
        {
                BF_low->setConstant(false);
                BF_low->setVal(BFval);
                BF_high->setConstant(false);
                BF_high->setVal(BFval);
...
//RooRealVar mu_sig4_l(("mu_sig4_"+typelow).c_str(),("mu_sig4_"+typelow).c_str(),(ws->var(("mu_sig4_"+typelow).c_str())->getVal()));
//RooRealVar mu_cbmeanr4_l(("mu_cbmeanr4_"+typelow).c_str(),("mu_cbmeanr4_"+typelow).c_str(),(ws->var(("mu_cbmeanr4_"+typelow).c_str())->getVal()));
//RooRealVar mu_cbsigmar4_l(("mu_cbsigmar4_"+typelow).c_str(),("mu_cbsigmar4_"+typelow).c_str(),(ws->var(("mu_cbsigmar4_"+typelow).c_str())->getVal()));

RooExtendPdf two_ext5_l(("two_ext5_"+typelow).c_str(),"two_ext5",twocbs3_l,misid_scaled_l);


//TMatrixDSym cov_chi_new_l;


RooMultiVarGaussian* mvg_l=(RooMultiVarGaussian*)ws->pdf(("mvg_"+typelow).c_str());

RooProdPdf misidModel_l(("misidModel_"+typelow).c_str(), ("misidModel_"+typelow).c_str(), RooArgSet(two_ext5_l,*mvg_l));
...
//RooRealVar mu_sig4_h(("mu_sig4_"+typehigh).c_str(),("mu_sig4_"+typehigh).c_str(),(ws->var(("mu_sig4_"+typehigh).c_str())->getVal()));
//RooRealVar mu_cbmeanr4_h(("mu_cbmeanr4_"+typehigh).c_str(),("mu_cbmeanr4_"+typehigh).c_str(),(ws->var(("mu_cbmeanr4_"+typehigh).c_str())->getVal()));
//RooRealVar mu_cbsigmar4_h(("mu_cbsigmar4_"+typehigh).c_str(),("mu_cbsigmar4_"+typehigh).c_str(),(ws->var(("mu_cbsigmar4_"+typehigh).c_str())->getVal()));


RooExtendPdf two_ext5_h(("two_ext5_"+typehigh).c_str(),"two_ext5",twocbs3_h,misid_scaled_h);


RooMultiVarGaussian* mvg_h=(RooMultiVarGaussian*)ws->pdf(("mvg_"+typehigh).c_str());
//RooMultiVarGaussian mvg_h(("mvg_"+typehigh).c_str(), ("mvg_"+typehigh).c_str(),(ws->pdf("mvg_"+typehigh).c_str()));
RooProdPdf misidModel_h(("misidModel_"+typehigh).c_str(), ("misidModel_"+typehigh).c_str(), RooArgSet(two_ext5_h,*mvg_h));
...
RooRealVar BR1("BR_low","BR_low",1.0e-8);
RooRealVar BR2("BR_high","BR_high",1.0e-8);
RooRealVar ses_low("ses_LowFCME","ses_LowFCME",seslowrun1,double(seslowrun1-1.91e-10),double(seslowrun1+1.91e-10));
RooRealVar ses_high("ses_HighFCME","ses_HighFCME",seshighrun1,double(seshighrun1-1.91e-10),double(seshighrun1+1.91e-10));
RooRealVar ses_low_2016("ses_LowFCME_2016","ses_LowFCME_2016",seslow2016,double(seslow2016-3.75e-10),double(seslow2016+3.75e-10));
RooRealVar ses_high_2016("ses_HighFCME_2016","ses_HighFCME_2016",seshigh2016,double(seshigh2016-5.34e-10),double(seshigh2016+5.34e-10));

RooFormulaVar nsigform_low("nsigform_LowFCME","nsigform_LowFCME","(@2/@1)+(@2/@0)",RooArgList(ses_low,ses_low_2016,BR1));
RooFormulaVar nsigform_high("nsigform_HighFCME","nsigform_HighFCME","(@2/@1)+(@2/@0)",RooArgList(ses_high,ses_high_2016,BR2));
...
RooSimultaneous simPdfsig("simPdfsig","simPdfsig",sample) ;
...
//    m_sigma->Print("s");
//    m_sigma_high->Print("s");


    BF_low->setConstant(false);
    BF_low->setVal(CLsBF);
    BF_low->removeError();
    BF_low->setConstant(true);


    BF_high->setConstant(false);
    BF_high->setVal(CLsBF);
    BF_high->removeError();
    BF_high->setConstant(true);
...
//    cat.defineType("HighFCME") ;
   ...
   Bplus_Corrected_Mass->setRange("high3",4001,7000);
   Bplus_Corrected_Mass->setRange("low3",4000,4001);
   Bplus_Corrected_Mass->setRange("full3",4000,7000);

   Bplus_Corrected_Mass->setRange("high2",4001,7000);
   Bplus_Corrected_Mass->setRange("low2",4000,4001);
...
//    model->plotOn(frame2,Slice(*cat,"HighFCME"),ProjWData(*cat,*data));
    ...
    double la=dataresult_free->minNll();
    double BFresult2 = BF_low->getVal();
    double statusa= dataresult_free->status();
