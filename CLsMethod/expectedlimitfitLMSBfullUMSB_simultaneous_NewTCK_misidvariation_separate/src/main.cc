...
//   double CLsBF = 7;
    ...
    RooRealVar* BF_int_low = ws_int->var("BR_low");
    RooRealVar* BF_int_high = ws_int->var("BR_high");
    ...
    std::cout << "Value of BF_int_low 0:" <<BF_int_low->getVal()<<endl;
    std::cout << "Value of BF_int_high 0:" <<BF_int_high->getVal()<<endl;
    BF_int_low->setVal(CLsBF);
    BF_int_low->setConstant(true);
    BF_int_high->setVal(CLsBF);
    BF_int_high->setConstant(true);
    std::cout << "Value of BF_int_low 1:" <<BF_int_low->getVal()<<endl;
    std::cout << "Value of BF_int_high 1:" <<BF_int_high->getVal()<<endl;
    ...
    firstfit->SaveAs(("FirstFitRanges_numoftoys_"+i2s(nToys)+"_injectsig"+ cleanNameString(d2s(BF_int_low->getVal()))+".pdf").c_str());
    delete firstfit;


    std::cout << "Value of BF_int_low 2:" <<BF_int_low->getVal()<<endl; 
    BF_int_low->setConstant(false);
    BF_int_low->setVal(0.0);
    BF_int_low->setConstant(true);
    std::cout << "Value of BF_int_low 3:" <<BF_int_low->getVal()<<endl;
    std::cout << "Value of BF_int_high 2:" <<BF_int_high->getVal()<<endl; 
    BF_int_high->setConstant(false);
    BF_int_high->setVal(0.0);
    BF_int_high->setConstant(true);
    std::cout << "Value of BF_int_high 3:" <<BF_int_high->getVal()<<endl;
    ...
    secondfit->SaveAs(("FirstFitRanges_numoftoys"+i2s(nToys)+"_injectsig"+ cleanNameString(d2s(BF_int_low->getVal()))+".pdf").c_str());
    delete secondfit;
//
//
...
//		    RooAbsPdf* m_sigma_high=ws->pdf("fixcon_sigma_HighFCME");
                    ...
                    cout<<"These are scratch function params "<<endl;
...
//	    RooAbsPdf* m_sigma_high=ws->pdf("fixcon_sigma_HighFCME");
            ...
            cout<<"Everything Ok starting from regional fit "<<endl;
	    pdf->getParameters(*mass)->Print("v") ; 
//	    std::vector<RooDataSet*> background_datasets = generateData_2(0.0,nToys);
//	    std::vector<RooDataSet*> signal_datasets = generateData_2(CLsBF,nToys);
	    RooAbsData* data2 = signal_datasets.at(0);
	    ...
	    delete canvasprefit;
            ...
            //minuit 2//
	    RooAbsReal* nll = pdf->createNLL(*data2,RooFit::Extended(true),RooFit::NumCPU(1));//;(,"high_lol"));//,RooFit::NumCPU(8));//RooFit::Save(true),RooFit::PrintLevel(-1));
	    RooMinimizer m(*nll);
	    m.minimize("Minuit2");
...
//	    RooFitResult* cool = pdf->fitTo(*data2,RooFit::Extended(true),RooFit::Save(true),RooFit::NumCPU(1));//,ExternalConstraints(RooArgSet(*m_num,*m_num_high,*m_sigma,*m_sigma_high,*m_mean_high,*m_mean)));
	    ...
	    RooAbsData* data3 = signal_datasets.at(0);
            ...
            //minuit 2//
//	    RooAbsReal* nll = pdf->createNLL(*data3,RooFit::Extended(true),RooFit::NumCPU(1));//;(,"high_lol"));//,RooFit::NumCPU(8));//RooFit::Save(true),RooFit::PrintLevel(-1));
//	    RooMinimizer m(*nll);
//	    m.minimize("Minuit2");
//	    RooMinuit m(*nll);
...
//	    migradStatus = m.migrad();
//	    hesseStatus = m.hesse();
//	    RooFitResult* data_intresult_fixed = m.save();
//	    data_intresult_fixed ->Print("v");
 ...
 //   return(1);
    ...
    for (int i = 0; i < nToys; ++i)
    {
	    ...
	    rfres.cd();
	    double backgroundDLL = fitTwiceOld2_con_new(background_datasets[i],i,CLsBF,&bkgll,update);

	    s.cd();
	    hbackground_DLL->Fill(backgroundDLL);
	    rfres.cd();
	    double signalDLL = fitTwiceOld2_con_new(signal_datasets[i],i,CLsBF,&sigll,update);
	    update = true;
	    s.cd();
