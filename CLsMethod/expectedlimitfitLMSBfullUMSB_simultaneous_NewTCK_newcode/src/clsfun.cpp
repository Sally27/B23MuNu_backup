...
//    model->setNormRange("full");
    model->getParameters(*Bplus_Corrected_Mass)->Print("v") ; 
    RooFitResult* dataresult_free = model->fitTo(*data,ExternalConstraints(RooArgSet(*m_num,*m_num_high,*m_sigma,*m_sigma_high, *m_mean_high,*m_mean)),RooFit::Extended(true),RooFit::Range("low2,high2"),RooFit::Save(true));
//    RooAbsReal* nll = model->createNLL(*data,RooFit::Extended(true),RooFit::Save(true));//,RooFit::PrintLevel(-1));
//    RooMinuit m(*nll);
...
//    treename->Write();
 ...
 //   rfrestree.Branch("CLsOneSigma",&dataresult_free->minNll(), "CLsOneSigma/D");
    ...
    RooFitResult* dataresult_fixed = model->fitTo(*data,RooFit::Range("low3,high3"),ExternalConstraints(RooArgSet(*m_num,*m_num_high,*m_sigma,*m_sigma_high,*m_mean_high,*m_mean)),RooFit::Extended(true),RooFit::Save(true));
