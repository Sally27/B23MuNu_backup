...
void hist()
{
  ...
  DataSample hist1("/vols/lhcbdisk05/ss4314/Gauss_v45r9/ParticleGun/higgstesting/tau_e26mass0p1width2p085/MassHiggsRun.root", "KstarMass");
 
  gROOT->ProcessLine(".x lhcbStyle.C");
   
  TCanvas* mp3 = new TCanvas("mp3", "mp3", 600, 600);
  TH1F* a = new TH1F("KstarMassfinal2", "KstarMassfinal2", 200, -10 , 6200);
  hist1.t->Draw("q2 >> KstarMassfinal2");
  double nSel1 = hist1.t->GetEntries();
  /*print nSel1;*/
  
 DataSample hist2("/vols/lhcbdisk05/ss4314/Gauss_v45r9/ParticleGun/higgstesting/tau_e26mass0p2width2p085/MassHiggsRun.root", "KstarMass");
  TH1F* b = new TH1F("KstarMassfinal3", "KstarMassfinal3", 200, -10 , 6200);
  hist2.t->Draw("q2 >> KstarMassfinal3");
 
 DataSample hist3("/vols/lhcbdisk05/ss4314/Gauss_v45r9/ParticleGun/higgstesting/tau_e26mass0p89width2p085/MassHiggsRun.root", "KstarMass");
  TH1F* c = new TH1F("KstarMassfinal4", "KstarMassfinal4", 200, -10 , 6200);
  hist3.t->Draw("q2 >> KstarMassfinal4");

 DataSample hist4("/vols/lhcbdisk05/ss4314/Gauss_v45r9/ParticleGun/higgstesting/tau_e26mass2p0width2p085/MassHiggsRun.root", "KstarMass");
 TH1F* d = new TH1F("KstarMassfinal4", "KstarMassfinal4", 200, -10 , 6200);
  hist4.t->Draw("q2 >> KstarMassfinal4");
 ...
 *        */
   ...
   d->SetTitle("q(#mu^{+},#mu^{-}) mass distribution for lifetime 1.3*10^{-17} nsec");
   d->SetXTitle("q(#mu^{+} #mu^{-}) [MeV/c^{2}]");
   d->GetYaxis()->SetTitleOffset(1.35);
...
//   d->SetTitle("q(#mu^{+},#mu^{-}) mass distribution ");
 ...
 *               ha->Draw("same");*/
    ...
    Double_t xl1=.45, yl1=0.25, xl2=xl1+.3, yl2=yl1+.325;
    TLegend *leg = new TLegend(xl1,yl1,xl2,yl2);
    leg->SetTextSize(0.035);
//    leg->AddEntry(a,"q mass with lifetime 1.3*10^-17 nsec and mass 0.1 gev","l");   // h1 and h2 are histogram pointers
//    leg->AddEntry(b,"q mass with lifetime 1.3*10^-17 nsec and mass 0.2 gev","l");
//    leg->AddEntry(c,"q mass with lifetime 1.3*10^-17 nsec and mass 0.89 gev","l");
//    leg->AddEntry(d,"q mass with lifetime 1.3*10^-17 nsec and mass 2.0 gev","l");
    leg->SetHeader("LHCb Simulation");
...
/*   mp3->Update();*/
   mp3->SaveAs("reporttrialqpres.pdf");
