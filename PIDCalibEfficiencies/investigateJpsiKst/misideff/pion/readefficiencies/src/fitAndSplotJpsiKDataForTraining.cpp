...
void FitAndSplotJpsiKDataForTraining::prepare2DhistMyAndPIDCalibRatio(string pidfile, string pidcut)
{
...
//   delete f_pid; 
   ...
   TFile* f_pidmine = new TFile("/vols/lhcb/ss4314/fitjpsikst/fitjpsikst_nolambdapkmumu/NOFCME/PIDtables/PID_2D.root","READ");
   ...
   for (int i(1); i<=before->GetNbinsX();i++ ){ 
	   ...
	   for (int j(1); j<=before->GetNbinsY();j++ ){
		   cout<<"PIDCalib "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<< eff->GetBinContent(i,j)<<" \\\\ " <<endl;
                   ...
                   out2<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<<eff->GetBinContent(i,j)<<" & "<<myeff->GetBinError(i,j)<<" & "<<ratio->GetBinError(i,j)<<" \\\\ " <<endl;
	   }
   }
