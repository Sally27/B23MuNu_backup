...
// TCanvas* mp3 = new TCanvas("mp3", "mp3", 700, 600);
  ...
  int nbins=100;
//  printLHCb("L","Prelim","");
  TFile *f = new TFile("/vols/lhcb/ss4314/cutonBDTs/MisidAfterCombiRestripEta1and3/B23MuNuMCallvar_MCtruth_trigger_Jpsi_mu1nShared_mu2nShared_mu3nShared_qmincut_CombBasic_MisidBasic.root");
...
//
  ...
  TFile *f2 = new TFile("/vols/lhcb/ss4314/cutonBDTs/MisidAfterCombiRestripEta1and3/B23MuNuFakeSStotal_mu3isNotMuon_mu3inMuonAcc_trigger_Jpsi_mu1nShared_mu2nShared_qmincut_CombBasic_MisidBasic.root");
 ...
 hs->Add(hm,"nostack");
  ...
  TFile *f3 = new TFile("/vols/lhcb/ss4314/cutonBDTs/MisidAfterCombiRestripEta1and3/BuKMuMuMerged_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc_trigger_mu1nShared_mu2nShared_qmincut_KaonPID_CombBasic_MisidBasic.root");
  TTree *t3 = (TTree*)f3->Get("DecayTree");
  cout<<"lala"<<endl;
  string binforpid="mu3_P >"+d2s(lowPbin)+"&& mu3_P < "+d2s(highPbin)+" && mu3_ETA >"+d2s(lowETAbin)+"&& mu3_ETA < "+d2s(highETAbin);
...
//  t3->SetBranchAddress("nsig_sw",&sweight);
 ...
 TH1F *diff = new TH1F("diff", "my distribution", nbins, low, high);
 hm->Sumw2();
 ho->Sumw2();
 diff->Add(hm,ho,-1);
 if (passed2!=0 && passed3!=0){
 TF1 constant("constant","[0]");
 constant.SetParName(0,"myp0");
 constant.FixParameter(0,0);
 diff->Fit("constant"); 

// diff->Fit("pol0","B",0);
 TF1 *fit= diff->GetFunction("constant");
 chi2= fit->GetChisquare();
 Double_t myp2=fit->GetParameter(0);         
 cout<<"Chi2 is "<<chi2<<endl;
 cout<<"fit with: "<< myp2<<endl;

 diff->Draw();
 mp4->SaveAs(("variable_DIFF_"+pidcalibvar+"COMPARE_JPSIKDATA_BINNED_P_"+d2s(lowPbin)+"_"+d2s(highPbin)+"_ETA_"+d2s(lowETAbin)+"_"+d2s(highETAbin)+".pdf").c_str());   
 }
 else{
 chi2=-1;
 }
...
//  DataSample hist2("/vols/lhcb/ss4314/cutonBDTs/MisidAfterCombiRestripEta1and3/SSMisIDsmalldatasample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut_PionPID_60TreesComb_60TreesMisID.root", "DecayTree");
...
//    
//  DataSample hist4("/vols/lhcb/ss4314/PIDCalibSamples/converted_DSt_Pi_MagDown_Strip20_31.root", "RSDStCalib");
...
//   
    ...
    leg->AddEntry(ho,"JPSIK DATA","l");    

    leg->Draw("same");

    mp3->SetLogx();  
//   mp3->Update();
   mp3->SaveAs(("variable"+pidcalibvar+"COMPARE_JPSIKDATA_BINNED_P_"+d2s(lowPbin)+"_"+d2s(highPbin)+"_ETA_"+d2s(lowETAbin)+"_"+d2s(highETAbin)+".pdf").c_str());
