...
//------------------------------------ MUON ID PID calib sample and cuts-----------------------------------------------------//
	...
	cuts3.push_back("Mu_(DLLmu > 0.0) && ((DLLmu - DLLK) > 0.0) && (nShared==0) && (V2ProbNNmu > 0.35)_All");
 	...
 	ofstream out;
 	out.open(("PIDEfficiency_"+maintag+polarity+stripping+".tex").c_str());
 	...
 	for(int i=0;i<16;i++)
 	{
 		...
 		double f;
 		double g;
 		e=calaveffofasamplemu1(mcmuonsample,"DecayTree",pidmuonsample,(cuts3.at(i)).c_str(),"PIDCalib",("newmc"+stripping+polarity+tags.at(i)+"mu1.root").c_str());
 		f=calaveffofasamplemu2(mcmuonsample,"DecayTree",pidmuonsample,(cuts3.at(i)).c_str(),"PIDCalib",("newmc"+stripping+polarity+tags.at(i)+"mu2.root").c_str());
                g=calaveffofasampleKaon(mcmuonsample, "DecayTree", idPIDcalib, idcuts, "PIDCalib",("newmc"+stripping+polarity+tags.at(i)+"kaon.root").c_str());
 	...
 	out<<"\\end{center}"<<endl;
 	out<<"\\caption{PIDEfficiency_"<<maintag+polarity+stripping<<".txt}"<<endl;
 	out<<"\\end{table}"<<endl;
