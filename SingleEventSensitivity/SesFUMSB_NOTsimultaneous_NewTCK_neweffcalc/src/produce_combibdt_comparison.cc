...
int main(){
	...
	out<<"\\hline"<<endl;
	out<<"\\hline"<<endl;
        out<<" \\multicolumn{1}{|c|}{} & \\multicolumn{2}{c|}{$B^{+} \\rightarrow \\mu^{+} \\mu^{-} \\mu^{+} \\nu$ } & \\multicolumn{2}{c|}{$B^{+} \\rightarrow J/\\psi\\ K^{+}$}"<<" \\\\ "<<endl;
        out<<" TCK & $\\epsilon_{combibdt}$ & $\\epsilon_{misidbdt}$ & $\\epsilon_{combibdt}$ & $\\epsilon_{misidbdt}$ " <<" \\\\ "<<endl;	
        out<<"\\hline"<<endl;
	...
	for(int z(0);z<mytcks.size();z++){
		...
		effrecostrsig=returnEff(("../efficiencytuples_sig/Efficiencies_"+i2s(years.at(2))+"_"+i2s(mytcks.at(z))+"_Misid.root").c_str(), ("MC_Sig_"+i2s(years.at(2))).c_str(), "misidbdt");
		decprodcutsig=returnEff(("../efficiencytuples_sig/Efficiencies_"+i2s(years.at(2))+"_"+i2s(mytcks.at(z))+"_Combi.root").c_str(), ("MC_Sig_"+i2s(years.at(2))).c_str(), "combibdt");
//                effmyrecosig=returnEff(("../efficiencytuples_sig/Efficiencies_"+i2s(years.at(2))+"_"+i2s(mytcks.at(z))+"_Combi.root").c_str(), ("MC_Sig_"+i2s(years.at(2))).c_str(), "trigger");
		...
		effrecostrjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+"_Misid.root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "misidbdt");
		decprodcutjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+"_Combi.root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "combibdt");
//		effmyrecostrjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+"_Combi.root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "trigger");


		out<<mytcks.at(z)<<" & " << round_to_digits(decprodcutsig,3)<<" & "<<round_to_digits(effrecostrsig,3)<<" & "<<round_to_digits(decprodcutjpsik,3) <<" & "<<round_to_digits(effrecostrjpsik,3)<<"  "<<" \\\\ "<<endl;
...
//	effmyrecostrjpsik=inner_product( effmyrecostrjpsik_vec.begin(),  effmyrecostrjpsik_vec.end(), begin(weights), 0.0);
	...
	out<<"\\hline"<<endl;
        out<<"Weighted efficiency" <<" & " << round_to_digits(decprodcutsig,3)<<" & "<<round_to_digits(effrecostrsig,3)<<" & "<<round_to_digits(decprodcutjpsik,3) <<" & "<<round_to_digits(effrecostrjpsik,3)<<" \\\\ "<<endl;
	out<<"\\hline"<<endl;
	...
	out <<"\\hline"<<endl;
	out << "$\\epsilon_{combibdt}$ &" << round_to_digits(decprodcutsig,3) << " & " << round_to_digits(decprodcutjpsik,3) <<" \\\\ "<<endl;
	out << "$\\epsilon_{misidbdt}$ &" << round_to_digits(effrecostrsig,3) << " & " << round_to_digits(effrecostrjpsik,3) <<" \\\\ "<<endl;
	out<<"\\hline"<<endl;
	out<<"\\end{tabular}"<<endl;
	out<<"\\end{center}"<<endl;
