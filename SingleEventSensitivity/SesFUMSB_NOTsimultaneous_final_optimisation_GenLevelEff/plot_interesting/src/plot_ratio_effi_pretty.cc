...
//			TFile* s = new TFile(filename.c_str());
		...
		grfirst[z]->SetTitle((supernice.at(z)).c_str());
	...
	mg->Draw("alp");
	mg->GetXaxis()->SetTitle("year");
	mg->GetYaxis()->SetTitle("R_{NOFCME}(#varepsilon)");
	//c1->Update();
        c1->BuildLegend(0.45, 0.25, 0.65, 0.8); 
	c1->SaveAs("Plot_ALL_Efficiencies_Ratio_Overview_Pretty.pdf");
	delete c1;
