...
//        out<<"\\hline"<<endl;
	...
	vector<double> crosscheckjpsikmchigherr_vec;//still need to add on error from dec prodcut
        ...
        vector<ValError> finalprod_sig_low_vec;
        vector<ValError> finalprod_jpsik_low_vec;

        vector<ValError> finalprod_sig_high_vec;
        vector<ValError> finalprod_jpsik_high_vec;
...
//		effmyrecostrjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "trigger");
		...
		double toteffjpsikmchigh=decprodcutjpsik*effrecostrjpsik*effmyrecostrjpsik*combibdtjpsik*misidbdtjpsik*cormjpsik*highFCMEjpsik*highFCMEjpsikPID;
                ...
                cout<<"Values "<<decprodcutjpsik<<"*"<<effrecostrjpsik<<"*"<<effmyrecostrjpsik<<"*"<<combibdtjpsik<<"*"<<misidbdtjpsik<<"*"<<cormjpsik<<"*"<<highFCMEjpsik<<"*"<<highFCMEjpsikPID<<" is "<<toteffjpsikmchigh<<endl;
		...
		ValError low_sig_pid(0.624,0.008);
		ValError low_jpsik_pid(0.6756,0.0012);
		...
		ValError high_jpsik_othereff_total(number_bkk_nor,sqrt(number_bkk_nor));
		...
		//------------PID estuff no error so far-------//
		ValError high_sig_pid(0.624,0.009);
		ValError high_jpsik_pid(0.6767,0.0017);
		...
		out<<"\\hline"<<endl;
		out<<"$\\epsilon_{neweffapp_nodecprodcut_nopid_low}$ &"<<low_sig_othereff <<" & "<<low_jpsik_othereff<<" \\\\ "<<endl;
		out<<"$\\epsilon_{neweffapp_nodecprodcut_nopid_high}$ &"<<high_sig_othereff <<" & "<<high_jpsik_othereff<<" \\\\ "<<endl;
		out<<"$\\epsilon_{neweffapp_decprodcut}$ &"<< high_sig_decprod <<" & "<< high_jpsik_decprod<<" \\\\ "<<endl;

		out<<"$\\epsilon_{neweffapp_pid_low}$ &"<<low_sig_pid <<" & "<<low_jpsik_pid<<" \\\\ "<<endl;
		out<<"$\\epsilon_{neweffapp_pid_high}$ &"<<high_sig_pid <<" & "<<high_jpsik_pid<<" \\\\ "<<endl;
		out<<"$\\epsilon_{neweffapp_low}$ &"<<finalprod_low_sig <<" & "<<finalprod_low_jpsik<<" \\\\ "<<endl;
		out<<"$\\epsilon_{neweffapp_high}$ &"<<finalprod_high_sig <<" & "<<finalprod_high_jpsik<<" \\\\ "<<endl;
		...
		crosscheckjpsikmchigherr_vec.push_back(crosscheckjpsikmchigherr);//still need to add on error from dec prodcut
                ...
                finalprod_sig_low_vec.push_back(finalprod_low_sig);
                finalprod_jpsik_low_vec.push_back(finalprod_low_jpsik);

                finalprod_sig_high_vec.push_back(finalprod_high_sig);
                finalprod_jpsik_high_vec.push_back(finalprod_high_jpsik);
	...
	ValError finalprod_low_sig(0,0);
	ValError finalprod_high_sig(0,0);
	ValError finalprod_low_jpsik(0,0);
	ValError finalprod_high_jpsik(0,0);
	...
	crosscheckjpsikmchigherr=inner_product( crosscheckjpsikmchigherr_vec.begin(),  crosscheckjpsikmchigherr_vec.end(), begin(weights), 0.0);
        ...
        finalprod_low_sig=getInnerProduct(finalprod_sig_low_vec, weights); 
        finalprod_low_jpsik=getInnerProduct(finalprod_jpsik_low_vec, weights);

        finalprod_high_sig=getInnerProduct(finalprod_sig_high_vec, weights); 
        finalprod_high_jpsik=getInnerProduct(finalprod_jpsik_high_vec, weights);
	...
	out << "$\\epsilon_{newstuff_low}$ &" << finalprod_low_sig<<" & "<< finalprod_low_jpsik<<" \\\\ "<<endl;
	out << "$\\epsilon_{newstuff_high}$ &" << finalprod_high_sig<<" & "<< finalprod_high_jpsik<<" \\\\ "<<endl;
...
//DCB        double numberofevents_high = 55147.0;
        double numberofevents_low = 65239.0; //Hypa
        double numberofevents_high = 29998.0; //Hypa
        ...
        ValError numberofobsevents_low(65239,259);

        ValError newses_low=((finalprod_low_jpsik*b2jpsikjpsik2mumu)/(finalprod_low_sig*numberofobsevents_low));

        ValError numberofobsevents_high(29998,177);

        ValError newses_high=((finalprod_high_jpsik*b2jpsikjpsik2mumu)/(finalprod_high_sig*numberofobsevents_high));
	...
	fillinfotree.Branch("effi_off_norm",&effi_off_norm, "effi_off_norm/D");
        ...
        effi_sig=finalprod_low_sig.val;
        effi_norm=finalprod_low_jpsik.val;
        sensitivity=newses_low.val;
	effi_ratio=double(finalprod_low_jpsik.val/finalprod_low_sig.val);
	effi_numobs=double(finalprod_low_jpsik.val/(finalprod_low_sig.val*numberofobsevents_low.val));
	effi_corr_lumi=double(numberofobsevents_low.val/(finalprod_low_jpsik.val*luminosity));
	...
	fillinfotree2.Branch("effi_off_norm",&effi_off_norm, "effi_off_norm/D");
        ...
        effi_sig=finalprod_high_sig.val;
        effi_norm=finalprod_high_jpsik.val;
        sensitivity=newses_high.val;
	effi_ratio=double(finalprod_high_jpsik.val/finalprod_high_sig.val);
	effi_numobs=double(finalprod_high_jpsik.val/(finalprod_high_sig.val*numberofobsevents_high.val));
	effi_corr_lumi=double(numberofobsevents_high.val/(finalprod_high_jpsik.val*luminosity));
        effi_gen_sig=decprodcutsig;
        effi_gen_norm=decprodcutjpsik;
        effi_rec_sig=effrecostrsig;
        effi_rec_norm=effrecostrjpsik;
	...
	fillinfotree2.Write("",TObject::kOverwrite);
        ...
        out<<"NEW STUFF"<<endl;
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu Low is "<< finalprod_low_jpsik/finalprod_low_sig;
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu High is "<< finalprod_high_jpsik/finalprod_high_sig;
        out<<"\\newline"<<endl; 
        ...
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu Low SCIEN is "<< (finalprod_low_jpsik/finalprod_low_sig).val<<"$\\pm$"<<(finalprod_low_jpsik/finalprod_low_sig).err<<endl;
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu High SCIEN is "<< (finalprod_high_jpsik/finalprod_high_sig).val<<"$\\pm$"<<(finalprod_high_jpsik/finalprod_high_sig).err<<endl;
        out<<"\\newline"<<endl; 
        ...
        out<<"\\newline"<<endl;
        out<<"FORANANNOTE"<<endl;
        out<<"\\newline"<<endl;
        out<<"\\begin{equation}"<<endl;
	out<<"R^{26}_{lowFCME}(\\epsilon)=\\frac{"<<finalprod_low_sig.val<<"\\pm"<<finalprod_low_sig.err<<"}{"<<finalprod_low_jpsik.val<<"\\pm"<<finalprod_low_jpsik.err<<"}="<<(finalprod_low_sig/finalprod_low_jpsik).val <<"\\pm"<< (finalprod_low_sig/finalprod_low_jpsik).err<<endl;
	out<<"\\end{equation}"<<endl;
	out<<"\\begin{equation}"<<endl;
	out<<"R^{26}_{highFCME}(\\epsilon)=\\frac{"<<finalprod_high_sig.val<<"\\pm"<<finalprod_high_sig.err<<"}{"<<finalprod_high_jpsik.val<<"\\pm"<<finalprod_high_jpsik.err<<"}="<<(finalprod_high_sig/finalprod_high_jpsik).val <<"\\pm"<< (finalprod_high_sig/finalprod_high_jpsik).err<<endl;
	out<<"\\end{equation}"<<endl;

	out<<"\\begin{equation}"<<endl;
	out<<"\\alpha^{26}_{lowFCME}=\\frac{\\mathcal{B}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+})}{N^{26}_{lowFCME}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+}) \\times R^{26}_{lowFCME}(\\epsilon)}="<<newses_low.val<<"\\pm"<<newses_low.err <<endl;
	out<<"\\end{equation}"<<endl;

	out<<"\\begin{equation}"<<endl;
	out<<"\\alpha^{26}_{highFCME}=\\frac{\\mathcal{B}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+})}{N^{26}_{highFCME}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+}) \\times R^{26}_{highFCME}(\\epsilon)}="<<newses_high.val<<"\\pm"<<newses_high.err <<endl;
	out<<"\\end{equation}"<<endl;
