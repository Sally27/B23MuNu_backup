...
//DCB        double numberofevents_high = 55147.0;
	...
	fillinfotree2.Write("",TObject::kOverwrite);
        ...
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu Low is "<< finalprod_low_jpsik/finalprod_low_sig<<endl;
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu High is "<< finalprod_high_jpsik/finalprod_high_sig<<endl;
        out<<"\\newline"<<endl; 
        ...
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu Scien Low is "<< (finalprod_low_jpsik/finalprod_low_sig).val <<"$\\pm$"<< (finalprod_low_jpsik/finalprod_low_sig).err<<endl;
        out<<"\\newline"<<endl;
        out<<"Efficiency ratio, i.e efftotjpsik/efftot3mu Scien High is "<< (finalprod_high_jpsik/finalprod_high_sig).val<<"$\\pm$"<< (finalprod_high_jpsik/finalprod_high_sig).err<<endl;
        out<<"\\newline"<<endl; 
	...
	out<<"\\begin{equation}"<<endl;
	out<<"R^{21}_{lowFCME}(\\epsilon)=\\frac{"<<finalprod_low_sig.val<<"\\pm"<<finalprod_low_sig.err<<"}{"<<finalprod_low_jpsik.val<<"\\pm"<<finalprod_low_jpsik.err<<"}="<<(finalprod_low_sig/finalprod_low_jpsik).val <<"\\pm"<< (finalprod_low_sig/finalprod_low_jpsik).err<<endl;
	out<<"\\end{equation}"<<endl;

	out<<"\\begin{equation}"<<endl;
	out<<"R^{21}_{highFCME}(\\epsilon)=\\frac{"<<finalprod_high_sig.val<<"\\pm"<<finalprod_high_sig.err<<"}{"<<finalprod_high_jpsik.val<<"\\pm"<<finalprod_high_jpsik.err<<"}="<<(finalprod_high_sig/finalprod_high_jpsik).val <<"\\pm"<< (finalprod_high_sig/finalprod_high_jpsik).err<<endl;
	out<<"\\end{equation}"<<endl;
