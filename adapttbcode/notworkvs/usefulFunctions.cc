...
//   TH2F th2f("th2f", "th2f", 50,-1e5,1e5,50,-1e5,1e5);
   TH2F th2f("th2f", "th2f", 100,2400,10000,100, float(mincut) , float(maxcut));
...
//          cout<<"Var1:"<< var1<<endl;
//          cout<<"nSig:"<<nSig<<endl;
          cout<<"Cut Value:"<<double(getEntries( dirname.c_str() , extracut + " && " + var2name.c_str() + " > " + d2s(currentcut), weightBranchSig ));
//          cout<<"check mincut: "<<(mincut+(i/(1.*n))*d)<<endl;
   ...
   cout<<"in between drawing and iteration"<<endl;

      if(draw)
   {

      cout<<"in the drawing"<<endl;     
