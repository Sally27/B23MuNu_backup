...
//   ontarget = "True";
 ...
 //  float maxPunzi(0.0);
   ...
   for(int i(0); i<n; ++i)
   {
      ...
      FOM[i]=(scalefacsig*getEntries( tSig, extracut+" && "+bdtvar + " > " + d2s(mincut+(i/(1.*n))*d), weightBranchSig )/nSig)/(sqrt((scalefacbkg*getEntries( tBkg, extracut+" && "+bdtvar + " > " + d2s(mincut+(i/(1.*n))*d), weightBranchBkg ))) + float(3/2));
