...
//    }
   ...
   ofstream myfiletex;
   myfiletex.open (("BDTsummary"+scaling+nameplot+punzi+".tex").c_str());
   myfiletex<<"\\documentclass[a4paper,11pt]{article}"<<endl;
   myfiletex<<"\\usepackage[pdftex]{graphicx}"<<endl;
   myfiletex<<"\\usepackage{url}"<<endl;
   myfiletex<<"\\usepackage{mathtools}"<<endl;
   myfiletex<<"\\usepackage{amsmath}"<<endl;
   myfiletex<<"\\usepackage{graphicx}"<<endl;
   myfiletex<<"\\usepackage[table]{xcolor}"<<endl;
   myfiletex<<"\\usepackage{amsmath,amssymb}"<<endl;
   myfiletex<<"\\usepackage[top=25mm,bottom=25mm,left=25mm,right=25mm]{geometry}"<<endl;
   myfiletex<<"\\usepackage{colortbl}"<<endl;
   myfiletex<<"\\begin{document}"<<endl;
   myfiletex<<"\\begin{table}[ht]"<<endl;
   myfiletex<<"\\small"<<endl;
   myfiletex<<"\\begin{center}"<<endl;
   myfiletex<<"\\begin{tabular}{| l | c |}"<<endl;
   myfiletex<<"\\hline"<<endl;
   myfiletex<<"Property & Value  \\\\ "<<endl;
   myfiletex<<"\\hline"<<endl;


//   myfiletex<<"-------------------------BDT summary----------------------"<<endl;
//   myfiletex<<""<<endl;
   myfiletex<<"Signal sample size & "<<nSig<<" \\\\ "<<endl;
   myfiletex<<"Background sample size & "<<nBkg<<" \\\\ "<<endl;
   myfiletex<<"\\hline"<<endl;
   myfiletex<<"Target On & "<< fa <<" \\\\ "<<endl;
   myfiletex<<"Target signal size & "<<trgSigEvt<<" \\\\ "<<endl;
   myfiletex<<"Target background size & "<<trgBkgEvt<<" \\\\ "<<endl;
   myfiletex<<"\\hline"<<endl;
   myfiletex<<"\\hline"<<endl;
   myfiletex<<"Max FOM(S/sqrt(S+B)) & "<<*std::max_element(FOM,FOM+n)<<" \\\\ "<<endl;
   myfiletex<<"Optimal cut & "<<(mincut+((cutindex-1)/(1.*n))*d)<<" \\\\ "<<endl;
   myfiletex<<"Min cut & "<<mincut<<" \\\\ "<<endl;
   myfiletex<<"Max cut & "<<maxcut<<" \\\\ "<<endl;
   myfiletex<<"Index of cut & "<<cutindex<<" \\\\ "<<endl;
   myfiletex<<"\\hline"<<endl;
   myfiletex<<"Signal Efficiency & "<<sigEff[cutindex]<<" \\\\ "<<endl;
   myfiletex<<"Background Rejection & "<<bkgRej[cutindex]<<" \\\\ "<<endl;
   myfiletex<<"Number of signal left & "<<nSigLeft[cutindex]<<" \\\\ "<<endl;
   myfiletex<<"Number of background left & "<<nBkgLeft[cutindex]<<" \\\\ "<<endl;
   myfiletex<<"\\hline"<<endl;
   myfiletex<<"\\end{tabular}"<<endl;
   myfiletex<<"\\end{center}"<<endl;
   myfiletex<<"\\caption{Properties of BDT}"<<endl;
   myfiletex<<"\\end{table}"<<endl;
   myfiletex<<"\\end{document}"<<endl;
...
//    }
   ...
   ofstream myfiletex;
   myfiletex.open (("BDTsummary"+scaling+nameplot+".tex").c_str());
   myfiletex<<"\\documentclass[a4paper,11pt]{article}"<<endl;
   myfiletex<<"\\usepackage[pdftex]{graphicx}"<<endl;
   myfiletex<<"\\usepackage{url}"<<endl;
   myfiletex<<"\\usepackage{mathtools}"<<endl;
