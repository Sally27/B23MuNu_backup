...
//   TH2D eff("eff", "eff", 100,2400,10000,100, 0.0 , 1.0);
   ...
   if(cuts != "") setBranchStatusTTF(t, cuts);
   if(cuts == "") cuts = "1"; 
   TH2F th2f("th2f", "th2f", 100,-0.1,0.7,100, -1.0 ,1.0);
//   TH2F th2f("th2f", "th2f", 50,-1e5,1e5,50,-1e5,1e5);
   TH2D eff("eff", "eff", 100,-0.1,0.7,100, -1.0 , 1.0);
//   TH2D eff("eff", "eff", 50,-1e5,1e5,50,-1e5,1e5);
   ...
   if(!isSeparable1 && !isSeparable2) eff.SetCanExtend(TH1::kAllAxes);
   if(!isSeparable1 && isSeparable2)
   {
      if(draw) eff.SetBins(50,-1e5,1e5, 50, var2min, var2max);
      if(draw) eff.SetCanExtend(TH1::kXaxis);
      cuts += (" && " + var2name + " > " + d2s(var2min) + " && " + var2name + " < " + d2s(var2max));
   }
   if(isSeparable1 && !isSeparable2)
   {
      if(draw) eff.SetBins(50, var1min, var1max, 50,-1e5,1e5);
      if(draw) eff.SetCanExtend(TH1::kYaxis);
      cuts += (" && " + var1name + " > " + d2s(var1min) + " && " + var1name + " < " + d2s(var1max));
   }
   if(isSeparable1 && isSeparable2)
   {
      if(draw) eff.SetBins(50, var1min, var1max,50, var2min, var2max);
      cuts += (" && " + var2name + " > " + d2s(var2min) + " && " + var2name + " < " + d2s(var2max));
      cuts += (" && " + var1name + " > " + d2s(var1min) + " && " + var1name + " < " + d2s(var1max));
   }
...
//      profile->GetXaxis()->SetRange((minx+(0.1*minx)),(maxx+(0.1*maxx)));
...
//    TCanvas canv("canv_nice", "canv2", 1500, 600);

    TCanvas canv("canv2", "canv2", 1500, 600);
//    canv.Divide(2,2,0.02,0.02);
    canv.Divide(2);//2,i0.02,0.02);
...
//      canv->Divide(2,2,0,0);
      ...
      th2f.GetXaxis()->SetTitle(var1name_nice.c_str());
//      th2f.GetYaxis()->SetTitle(var2name.c_str());
      th2f.GetYaxis()->SetTitle("BDT output");
      th2f.SetTitle(( var1name+"_vs_"+var2name).c_str());
      gStyle->SetPalette(62);
      th2f.Draw("colz");
      canv.Print( (filename+"_"+cleanNameString(var1name)+"_vs_"+cleanNameString(var2name)+"_nice.pdf").c_str());
//      canv.Print( (filename+cleanNameString(var1name+"_vs_"+var2name)+".pdf").c_str());
...
//   return correl;
    ...
    if(draw)
   {
    //  TCanvas canv3("canv3", "canv3", 800, 600);
      ...
      prof->GetYaxis()->SetTitleOffset(1.15);
      prof->GetXaxis()->SetTitle(var1name_nice.c_str());
 //     prof->GetYaxis()->SetTitle(var2name.c_str());
      prof->GetYaxis()->SetTitle("BDT output");
  ...
  //    TProfile *prof = th2f.ProfileX();  
      ...
      double minx;
      cout<<"This is x min"<<endl;
      minx=*min_element(vectorvar1.begin(),vectorvar1.end());
      cout<<"This is x min"<<minx<<endl;
      double maxx;
      maxx=*max_element(vectorvar1.begin(),vectorvar1.end());
      cout<<"This is x max"<<maxx<<endl;
      prof->GetXaxis()->SetRangeUser((minx+(0.1*minx)),(maxx+(0.1*maxx)));
      canv.Update();
      prof->Draw();
...
//      canv3.Print( (filename+"_ProfileX_"+var1name+"_vs_"+var2name+".pdf").c_str());
  ...
  canv.Print((filename+"_ProfileX_of_"+cleanNameString(var1name)+"_vs_"+cleanNameString(var2name)+"_nice.pdf").c_str());
...
//      th2f.GetYaxis()->SetTitle(var2name.c_str());
      ...
      th2f.SetTitle(( var1name+"_vs_"+var2name).c_str());
      th2f.Draw("colz");
      canv.Print( (filename+"_"+cleanNameString(var1name)+"_vs_"+cleanNameString(var2name)+".pdf").c_str());
//      canv.Print( (filename+cleanNameString(var1name+"_vs_"+var2name)+".pdf").c_str());
//    canv3.Print( (filename+"_ProfileX_"+var1name+"_vs_"+var2name+".pdf").c_s|      th2f.SetTitle(( var1name+"_vs_"+var2name).c_str());                     
//     canv.Print( (filename+"_"+var1name+"_vs_"+var2name+".pdf").c_str());
...
//   return correl;
    ...
    if(draw)
   {
    //  TCanvas canv3("canv3", "canv3", 800, 600);
      TProfile *prof = th2f.ProfileX();
      prof->GetYaxis()->SetTitleOffset(1.05);
      prof->GetXaxis()->SetTitle(var1name.c_str());
 //     prof->GetYaxis()->SetTitle(var2name.c_str());
      prof->GetYaxis()->SetTitle("Average BDT value");
      prof->SetTitle(( "ProfileX_"+var1name+"_vs_"+var2name).c_str());
  //    TProfile *prof = th2f.ProfileX();  
      canv.cd(2);
...
//      canv3.Print( (filename+"_ProfileX_"+var1name+"_vs_"+var2name+".pdf").c_str());
  ...
  canv.Print((filename+"_ProfileX_of_"+cleanNameString(var1name)+"_vs_"+cleanNameString(var2name)+".pdf").c_str());
...
double getCorrelationCoefficient_nice(string filename, string var1name, string var2name, string var1name_nice, string var2name_nice ,string cuts, bool draw, string dirname, string newplotname, double mincut, double maxcut, int nstep)
{
   ...
   getCorrelationCoefficient_nice(t, var1name, var2name, var1name_nice, var2name_nice ,cuts, draw, dirname, newplotname, mincut, maxcut , nstep);   
...
void getCorrelationMatrix(vector<string> vars, string namefile, string nameoutput, string cuts, bool draw2d)
{
   ...
   TH2I megamatrix("megamatrix", ("CorrMatrix"+namefile).c_str(), n,0,n,n,0,n);
...
//
   ...
   for(int i(0); i<n;++i)
   {
     ...
     cout<<"computation of big matrix 2"<<endl;
     for(int j(i); j<n; ++j)
      {
         cout<<"computation of big matrix 3"<<endl;
         cout<<count<<endl;
         cout<<tot<<endl;
         cout<<j<<endl; 
//         if(count % (tot/10) == 0) cout<<100*count/tot<<" \% processed"<<endl;
         coef = getCorrelationCoefficientold(t, vars[i], vars[j], cuts, draw2d, nameoutput);
      ...
      )
{
   ...
   {
      currentcut = mincut+i*step;
      cout<<"Current cut of bdt: "<< currentcut << endl;
...
//   leg->SetTextSize(0.02);
   ...
   if(nameplot.size()>1) canv.Print(("Scaled"+nameplot).c_str());
...
//   reth.SetXTitle(bdtvar.c_str());
   reth.SetXTitle("BDT variable");
   reth.SetYTitle("#varepsilon_{S}/(#sqrt{B}+(3/2))");
   TCanvas canv("canv", "canv", 600,600);
   gPad->SetLeftMargin(0.20);
   reth.GetYaxis()->SetTitleOffset(1.4);
   reth.Draw();
      ...
      )
{
   ...
   double scalesig;
   double scalebkg;
   scalesig=nsigexp/nSig;
   scalebkg=nbkgexp/nBkg;
   ...
   for(int i(0); i<nstep; ++i)
   {
      ...
      S = wSig*getEntries( tSig, str+d2s(currentcut), weightBranchSig);
      B = wBkg*getEntries( tBkg, str+d2s(currentcut), weightBranchBkg);
      if(S+B == 0) {reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), 0); reth2.SetBinContent(reth.GetXaxis()->FindBin(currentcut),0);}
      else {reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), ((scalesig*S)/(scalesig*nSig))/(sqrt(B*scalebkg)+(3/2)));reth2.SetBinContent(reth.GetXaxis()->FindBin(currentcut), ((scalesig*S)/(scalesig*nSig))/(sqrt(B*scalebkg)+(5/2)));}
   }
   gStyle->SetOptStat(0);
//   reth.SetTitle(("FOM with SigFile: "+fileSig+" bkgFile: "+fileBkg+"BDT var: "+bdtvar).c_str());
   reth.SetTitle(("Punzi FOM")); 
   reth2.SetLineColor(kRed); 
//   gPad->SetLeftMargin(1.5);
//   reth.GetYaxis()->SetTitleOffset(1.4);
   reth.SetXTitle("BDT output");
   reth.SetYTitle("Punzi FOM");

   reth.GetYaxis()->SetRange(0,reth.GetYaxis()->GetXmax()+ (0.3*(reth.GetYaxis()->GetXmax())));
   reth2.GetYaxis()->SetRange(0,reth.GetYaxis()->GetXmax()+ (0.3*(reth.GetYaxis()->GetXmax())));
   reth2.SetTitle(("Punzi FOM")); 
//   reth2.SetLineColor(kRed); 
//   gPad->SetLeftMargin(1.5);
//   reth.GetYaxis()->SetTitleOffset(1.4);
   reth2.SetXTitle("BDT output");
   reth2.SetYTitle("Punzi FOM");
   TCanvas canv("canv", "canv", 700,600);
   
   gPad->SetLeftMargin(0.2);

   float cutoffyaxis(0.045);

   reth.GetYaxis()->SetTitleOffset(1.4);
   reth.GetYaxis()->SetRange(0,0.045);
   reth2.GetYaxis()->SetRange(0,0.045);
   reth.Draw();
   reth2.Draw("same");

   canv.Update();
   cout<<"The other "<<reth.GetYaxis()->GetXmax()+ (0.3*(reth.GetYaxis()->GetXmax()))<<endl;
   cout<<"gPad->GetUymax()"<<gPad->GetUymax()<<endl;

   TLine* myline=new TLine(0.54, 0.0,0.54,gPad->GetUymax());
...
//   myline2->SetLineColor(9);
   ...
   TLegend *leg = new TLegend(0.25,0.75,0.6,0.88);
   leg->AddEntry("reth","#sigma=3, 2016 ","l");
   leg->AddEntry("reth2","#sigma=5, 2016","l");
//   leg->SetFillColorAlpha(kWhite,1.0);
   leg->Draw();

   if(nameplot.size()>1) 
   {canv.Print(("Scaled_nice_"+punzi+nameplot).c_str());
    canv.SaveAs(("Scaled_nice_"+punzi+"FOM.root").c_str());
    }
      ...
      )
{
   ...
   for(int i(0); i<nstep; ++i)
   {
      ...
      B = wBkg*getEntries( tBkg, str+d2s(currentcut), weightBranchBkg);
      if(S+B == 0) reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), 0);
      else reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), ((scalesig*S)/(scalesig*nSig))/(sqrt(B*scalebkg)+(3/2)));
   }
   gStyle->SetOptStat(0);
//   reth.SetTitle(("FOM with SigFile: "+fileSig+" bkgFile: "+fileBkg+"BDT var: "+bdtvar).c_str());
   reth.SetTitle(("Punzi FOM")); 
...
//   reth.GetYaxis()->SetTitleOffset(1.4);
   ...
   reth.Draw();
   TLegend *leg = new TLegend(0.3,0.2,0.7,0.4);
   leg->SetHeader(("Optimal Cut: "+d2s(reth.GetXaxis()->GetBinLowEdge(reth.GetMaximumBin()))).c_str());
//   leg->SetFillColorAlpha(kWhite,1.0);
   leg->Draw();

   if(nameplot.size()>1) 
   {canv.Print(("Scaled"+punzi+nameplot).c_str());
    canv.SaveAs(("Scaled"+punzi+"FOM.root").c_str());
    }
      ...
      )
{
   ...
   for(int i(0); i<nstep; ++i)
   {
    ...
    //  currentbin = ((scalesig*S)/nSig);
    //  cout<<"Current bin: " << currentbin; 
      if(S+B == 0) {reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), 0);}
      else {reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), (((scalesig*S)/(scalesig*nSig)))/(sqrt(B*scalebkg)+(3/2)));}
    
      FOM.push_back((S/nSig)/((sqrt(scalebkg*B)+(3/2))));
      sigeff.push_back(S/nSig);
       
      if (int((sigeff.at(i)*10))==tageff)
      {targeteff=currentcut;}
...
//      else {reth.SetBinContent(reth.GetXaxis()->FindBin(currentcut), ((S)/nSig));}
   ...
   gStyle->SetOptStat(0);
   double* fombin=&FOM[0];
   double* sigeffbin=&sigeff[0];
   double target[2];

//   target[0]=reth.GetXaxis()->GetBinCenter(0.3);
      

   TGraph *gr = new TGraph(nstep,sigeffbin,fombin);   
   gr->SetTitle("FOM as function of Signal Efficiency");
   gPad->SetLeftMargin(0.20);
   gr->GetYaxis()->SetTitleOffset(1.4);
   gr->GetYaxis()->SetTitle("#varepsilon_{s}/(#sqrt{B}+(3/2))");
   gr->GetXaxis()->SetTitle("#varepsilon_{s}");
   gr->SetMarkerColor(1);
   gr->SetLineColor(1);
   ...
   leg->SetHeader(("Cut(30 \% Sig Eff) : "+d2s(targeteff)).c_str());
//   leg->SetTextSize(0.02);
   leg->Draw();

   if(nameplot.size()>1) canv.Print(("Scaled"+punzi+"asfnofSigeff"+i2s(tageff)+nameplot).c_str());
...
void plotCutEffVsVarNonBDT(string fileTree,
      string cut,
      string cutvar,
      string var,
      double minVar,
      ...
      string extracuts)
{
 ...
 //  canv.cd(1);
   ...
   ha->Sumw2(); hb->Sumw2();
//   TH1D* hc(0);
   ha->Divide(ha,hb,1,1,"B");
   ha->GetYaxis()->SetTitle("Efficiency");
   ha->GetXaxis()->SetTitle(var.c_str());
   ha->Draw();
   canv.Print(("CutEfficiency_"+cleanNameString(var)+nameplot+".pdf").c_str());


   TCanvas canvgn("canvgn", "canvgn", 600,600);
   canvgn.Divide(2,2,0.03,0.03);
   canvgn.cd(1);
   ha->SetTitle(fileTree.c_str());
   ha->GetXaxis()->SetTitle(var.c_str());
   ha->GetYaxis()->SetTitle("Efficiency");
   ha->Draw();
...
//   leg->SetTextSize(0.02);
   ...
   canvgn.cd(4);
   t->Draw(var.c_str());
   canvgn.Print(("CutEfficiency_"+cleanNameString(var)+nameplot+"_complex.pdf").c_str());


 //-------------------Make nice histograms----------------------//
   
   TCanvas nice("nice", "nice", 1400,600);
   nice.Divide(2);
   nice.cd(1);
   ha->SetTitle(fileTree.c_str());
   if (var=="Bplus_Corrected_Mass")
   {
	   ha->GetXaxis()->SetTitle("Corrected #mu^{+} #mu^{-} #mu^{+} mass [MeV/c^{2}]");
   } 
   else
   {
	   ha->GetXaxis()->SetTitle(var.c_str());
   }
   ha->GetYaxis()->SetTitle("#varepsilon");
   gPad->SetLeftMargin(0.15);    
   ha->GetYaxis()->SetTitleOffset(1.05);
   ha->Draw();
   ...
   gPad->ls();
   TH1F *temp = (TH1F*)gPad->GetPrimitive("htemp");
   ...
   if (var=="Bplus_Corrected_Mass")
   {
	   temp->GetXaxis()->SetTitle("Corrected #mu^{+} #mu^{-} #mu^{+} mass [MeV/c^{2}]");
   } 
   else
   {
	   temp->GetXaxis()->SetTitle(var.c_str());
   }

   t->SetLineColor(kRed);
   t->Draw(var.c_str(), cut.c_str(),"same");//,LineColor(kRed));//,"kRed");

   TLegend *leg2 = new TLegend(0.5,0.6,0.9,0.9);
   if (nameplot=="Misid")
   {
   leg2->SetHeader("MisID sample");
   leg2->AddEntry((TObject*)0, ("#varepsilon_{CombiBDT} : "+d2s(round_to_digits(eff,3))).c_str(), ""); 
   cout<<"true is misid"<<endl;
   }
   else 
   {
   leg2->SetHeader(("#varepsilon_{CombiBDT} : "+d2s(round_to_digits(eff,3))).c_str());
   }
   leg2->AddEntry(hb,"Before CombiBDT","l");
   leg2->AddEntry(t,"After CombiBDT","l");
   leg2->SetTextSize(0.05);
   leg2->Draw();
   nice.Update();


   nice.Print(("CutEfficiency_"+cleanNameString(var)+nameplot+"_nice.pdf").c_str());
