...
//   ontarget = "True";
 ...
 //  float maxPunzi(0.0);
   ...
   for(int i(0); i<n; ++i)
   {
      ...
      FOM[i]=(scalefacsig*getEntries( tSig, extracut+" && "+bdtvar + " > " + d2s(mincut+(i/(1.*n))*d), weightBranchSig ))/(sqrt((scalefacbkg*getEntries( tBkg, extracut+" && "+bdtvar + " > " + d2s(mincut+(i/(1.*n))*d), weightBranchBkg ))) + float(3/2));
...
//   double maxCut(1.0);
   ...
   {
      if(getOneRocCurvePunzi(fsigname, fbkgname, bdtvars[i], extracut, sigEff, bkgRej, n, fa, nameplot, mincut, maxcut, punzi, weightBranchSig, weightBranchBkg))
...
//   double maxCut(1.0);
   ...
   for(int i(0); i < bdtvars.size(); ++i)
   {
      if(getOneRocCurve(fsigname, fbkgname, bdtvars[i], extracut, sigEff, bkgRej, n, fa, nameplot, mincut, maxcut, weightBranchSig, weightBranchBkg))
