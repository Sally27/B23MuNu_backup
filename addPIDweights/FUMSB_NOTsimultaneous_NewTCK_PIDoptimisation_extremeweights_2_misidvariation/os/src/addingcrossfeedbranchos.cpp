...
vector<float> crossmisid(string filename, string cuts, int p, int eta, string polarity, string stripping)
{
	...
	for (int i=1; i<(p+1); i++) {
        ...
        hname->Draw("TEXT89COLZ");
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
        c1->SaveAs(("PID_HeatMap_Type2_"+polarity+"_"+stripping+"_"+cleanNameString(cuts)+".pdf").c_str());
        delete c1;
...
vector<float> efficiency(string filename, string cuts, int p, int eta, string  polarity, string stripping)
{
	TFile s(filename.c_str());
	TH2F *hname =(TH2F*)s.Get(cuts.c_str());
	TH2F *updated_version =(TH2F*)hname->Clone();
	vector<float> effi;
	Double_t myaverage(0);
	Int_t z(0);
        double correct(0);
	...
	for (int i=1; i<(p+1); i++) {
		for (int j=1; j<(eta+1); j++) {
			...
			correct=hname->GetBinContent(i,j);
			int u(0);
			while (correct==0)
			{
				cout<<"Using "<<u<<" dozadu"<<endl;
				correct=hname->GetBinContent(i-u,j);
                                updated_version->SetBinContent(i,j,correct);
				u++;
				if ((i-u)==0)
				{
					cout<<"Reached the limit"<<endl;
					correct=hname->GetBinContent(i+1,j);
                                        updated_version->SetBinContent(i,j,correct); 
				}
			}
			...
			if (correct<0)
			{
				cout<<"Negative weight set to 0.00001"<<endl;
				correct=0.000001;
                                updated_version->SetBinContent(i,j,correct);
			}
			...
			if (correct>1)
			{
				cout<<"Weight more than 1 set to 1"<<endl;
				correct=1.0;
                                updated_version->SetBinContent(i,j,correct);
			}


			effi.push_back(correct);
			cout<<filename<<cuts<<"ID rate: " << effi.at(z) << " in a bin i , j , k:" << i << " " << j << endl;
                        cout<<"Old "<<hname->GetBinContent(i,j)<<" New "<<correct<<endl;
			cout<<"This is z:" << z <<endl;
			myaverage+=effi.at(z);
			//  if (effi[z]<0)
			//       {
        ...
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
        c1->SaveAs(("PID_HeatMap_Type2_"+stripping+"_"+polarity+"_"+cleanNameString(cuts)+".pdf").c_str());

        TCanvas *upd = new TCanvas("upd","upd",600,600);
        updated_version->Draw("TEXT89COLZ");
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
        upd->SaveAs(("PID_HeatMap_Type2_"+stripping+"_"+polarity+"_"+cleanNameString(cuts)+"_updated.pdf").c_str());

	TFile *F1 = new TFile("append.root","UPDATE");
	updated_version->Write("", TObject::kOverwrite);
	F1->Close();
...
int isnan_double(double value)
{

	if (value!=value)
	{
		cout<<"Having nan value"<<endl;
	}

       else
       {
       
		cout<<"Having other value"<<endl;
       }
...
void readvalue(string filename, string decaytree,string varname)
{
	...
	Double_t mu2_P, mu2_ETA, Bplus_Corrected_Mass, eff;
	...
	{
		t->GetEntry(i);
		cout<<"CorM "<<Bplus_Corrected_Mass<<" P: "<< mu2_P <<" ETA: "<< mu2_ETA<< varname<<": "<<eff<<endl;
                k=eff;
                isnan_double(k);
	}
...
vector<float> newbinmydata(string filename, string decaytree, string species) {
	...
	for(int n=0; n<t->GetEntries(); ++n)
	{
		...
		cout<<"mu2_P:"<<mu2_P<<endl;
		//		int resultx;
		//		int resulty;
		//		int resultz;

		if (mu2_P<binningx.at(0) || mu2_P>binningx.at(18) || mu2_ETA<binningy.at(0) || mu2_ETA>binningy.at(4)) 
		{
...
//	Double_t* yedges = &binningy[0];
	...
	for(int n=0; n<t->GetEntries(); ++n)
	{
		...
		if (mu2_P<binningx.at(0) || mu2_P>binningx.at(18)) 
		{
			cout<< "Need to thow away this"<<endl;
			cout<< "Num of thrown away events: "<<numberofthrownevents++<<endl;
...
//	Double_t* yedges = &binningy[0];
	...
	TH2F *hname =(TH2F*)s->Get(wfilecuts.c_str());
        TH2F *updated_version=(TH2F*)hname->Clone();
        updated_version->SetName("UpdatedVersion");
	vector<float> effi;
        float correct(0);
	Int_t z(0);
	...
	for (int i=1; i<(p+1); i++) {
        ...
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
        c1->SaveAs(("PID_HeatMap_"+polarity+"_"+stripping+"_"+cleanNameString(wfilecuts)+".pdf").c_str());
 
        TCanvas *upd = new TCanvas("upd","upd",600,600);
        updated_version->Draw("TEXT89COLZ");
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
        upd->SaveAs(("PID_HeatMap_"+polarity+"_"+stripping+"_"+cleanNameString(wfilecuts)+"_updated.pdf").c_str());
...
void extremevaluesexchange(string filename, string decaytree, string newfilename)
{
	...
	Double_t mu2_P, mu2_ETA, Bplus_Corrected_Mass;
        ...
        t->SetBranchStatus("mu2_P",0);
        t->SetBranchStatus("mu2_ETA",0);
//	t->SetBranchAddress("mu2_P_new", &mu2_P_new);
//	t->SetBranchAddress("mu2_ETA_new", &mu2_ETA_new);

	TFile *g = new TFile(("New_"+newfilename).c_str(), "RECREATE");
	TTree *newtree = t->CloneTree(0);
...
void extremevaluesFinal(string filename, string decaytree, string newfilename) {

	TFile* f = new TFile(filename.c_str());
	TTree* t = (TTree*)f->Get(decaytree.c_str());

	Double_t mu2_P_new, mu2_ETA_new; 
	...
	Double_t mu2_P;
	Double_t mu2_ETA;
        newtree->Branch("mu2_P",&mu2_P,"mu2_P/D");
	newtree->Branch("mu2_ETA", &mu2_ETA, "mu2_ETA/D");
...
void extremevalues(string filename, string decaytree, string newfilename) {
	...
	Double_t mu2_P_new;
	Double_t mu2_ETA_new;
        newtree->Branch("mu2_P_new",&mu2_P_new,"mu2_P_new/D");
	newtree->Branch("mu2_ETA_new", &mu2_ETA_new, "mu2_ETA_new/D");
...
//	Double_t* yedges = &binningy[0];
	...
	for(int n=0; n<t->GetEntries(); ++n)
	{
		...
		int resulty(20);
                ...
                if (mu2_P<binningx.at(0))
                {
                cout<<"Old value of momentum "<<mu2_P<<endl;

                (mu2_P_new=(binningx.at(0)+1));
                mu2_ETA_new=mu2_ETA;
                cout<<"New value of momentum "<<mu2_P_new<<endl;
		newtree->Fill();
                }

                if (mu2_P>binningx.at(18))
                {
                cout<<"Old value of momentum "<<mu2_P<<endl;
                (mu2_P_new=(binningx.at(18)-1));
                mu2_ETA_new=mu2_ETA;
                cout<<"New value of momentum "<<mu2_P_new<<endl;
		newtree->Fill();
                }

                if (mu2_ETA>binningy.at(4))
                {
                cout<<"Old value of eta "<<mu2_ETA<<endl;
                (mu2_ETA_new=(binningy.at(4)-0.01));
                mu2_P_new=mu2_P;
                cout<<"New value of eta "<<mu2_ETA_new<<endl;
		newtree->Fill();
                }
                if (mu2_ETA<binningy.at(0))
                {
                cout<<"Old value of eta "<<mu2_ETA<<endl;
                (mu2_ETA_new=(binningy.at(0)+0.01));
                mu2_P_new=mu2_P;
                cout<<"New value of eta "<<mu2_ETA_new<<endl;
		newtree->Fill();
                }

		if (mu2_P_new<binningx.at(0) || mu2_P_new>binningx.at(18) || mu2_ETA_new<binningy.at(0) || mu2_ETA_new>binningy.at(4)) 
		{
			cout<< "Need to thow away this"<<endl;
			cout<< "Num of thrown away events: "<<numberofthrownevents++<<endl;
			cout<< "Borders:"<< binningx.at(0) <<" "<<binningx.at(18)<< " "<<binningy.at(0)<< " "<<binningy.at(4) << " "<<endl;
	...
	cout<<"Number of events:" << t->GetEntries()<< endl;
	cout<<"Final numberofthrown event of pion or kaon:  "<< numberofthrownevents<<endl;
	g->Write("",TObject::kOverwrite);
...
//	Double_t* yedges = &binningy[0];
	...
	for(int n=0; n<t->GetEntries(); ++n)
	{
		...
		int resulty(20);
                ...
                (mu2_P=(binningx.at(0)+1));
                cout<<"New value of momentum "<<mu2_P<<endl;
                }

                if (mu2_P>binningx.at(18))
                {
                cout<<"Old value of momentum "<<mu2_P<<endl;
                (mu2_P=(binningx.at(18)-1));
                cout<<"New value of momentum "<<mu2_P<<endl;
                }
                ...
                {
                cout<<"Old value of eta "<<mu2_ETA<<endl;
                (mu2_ETA=(binningy.at(4)-0.01));
                cout<<"New value of eta "<<mu2_ETA<<endl;
                }
                if (mu2_ETA<binningy.at(0))
                {
                cout<<"Old value of eta "<<mu2_ETA<<endl;
                (mu2_ETA=(binningy.at(0)+0.01));
                cout<<"New value of eta "<<mu2_ETA<<endl;
                }
		...
		else
		{
			...
			weight=hname->GetBinContent(resultx,resulty);
                        ...
                        {
                        cout<<"Using "<<u<<" dozadu"<<endl;
                        weight=hname->GetBinContent(resultx-u,resulty);
                        u++;
                        }
                        ...
                        if (weight<0)
                        {
                        cout<<"Setting negative weight to 0.000001"<<endl;
                        weight=0.000001;
                        }

                        if (weight>1)
                        {
                        cout<<"Setting weight of 1.0 more to 1.0"<<endl;
                        weight=1.0;
                        }
...
vector<float> binmydatacorrm(string filename, string decaytree, string species, Double_t lowcorrm, Double_t highcorrm){
	...
	for(int b(0); b < t->GetEntries(); ++b)
	{
		...
		if ((Bplus_Corrected_Mass > lowcorrm) && (Bplus_Corrected_Mass < highcorrm))
		{
			...
			//cout<<mu2_P<<","<<mu2_ETA<<","<<nTracks;
			h3->Fill(mu2_P,mu2_ETA,float(nTracks));
		}
	}
...
float numofpionafteriteration(vector<float> pionbinmydata , vector<float> myktopimisid, vector<float> truekinematicaldistributionkaon, vector<float> pionideff, vector<float> misidpiontomuon, int number, string species, string misidcuts, string polarity, string stripping)
{
	...
	TH2* myhisto = new TH2F("pion", "pion", XBINS, xedges, YBINS, yedges);
	//const Int_t ZBINS = 4;
	...
	for (int i=0; i<XBINS; i++) {
		for (int j=0; j<YBINS; j++) {
			...
			//      jop2.push_back(newkinematicaldiskaon.at(c)*misidkaontomuon.at(c));
                        myhisto->SetBinContent(i+1,j+1,jop1.at(c));
			c++;
			//      }
		}
	}
        ...
        myhisto->Draw("TEXT89COLZ");
        myhisto->SetTitle(("NumOfMisid_after_"+i2s(number)+"_iteration_pion_numberofpart_"+d2s(std::accumulate(jop1.begin(),jop1.end(),0.0))).c_str());
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
    //    gStyle->SetTextAngle(90);
        c4->SaveAs(("NumberOfMisidafter_"+i2s(number)+"_iteration_HeatMap_pion_"+species+"_"+polarity+"_"+stripping+"_"+misidcuts+".pdf").c_str());
        delete c4;
        delete myhisto;
...
float numofkaonafteriteration(vector<float> kaonbinmydata , vector<float> mypitokmisid, vector<float> truekinematicaldistributionpion, vector<float> kaonideff, vector<float> misidkaontomuon, int number, string species, string misidcuts, string polarity, string stripping)
{
	...
	TH2* myhisto = new TH2F("kaon", "kaon", XBINS, xedges, YBINS, yedges);
	...
	for (int i=0; i<XBINS; i++) {
		for (int j=0; j<YBINS; j++) {
			...
			newkinematicaldiskaon.push_back((kaonbinmydata.at(c)-(mypitokmisid.at(c)*truekinematicaldistributionpion.at(c)))/kaonideff.at(c));
			cout<<"NewKinematicalkaon : "<<newkinematicaldiskaon.at(c)<< endl;
//                        myhisto->SetBinContent(i+1,j+1,newkinematicaldiskaon.at(c));
			jop2.push_back(newkinematicaldiskaon.at(c)*misidkaontomuon.at(c));                
                        myhisto->SetBinContent(i+1,j+1,jop2.at(c));
			c++;
			//      }
		}
	}
        ...
        TCanvas *c4 = new TCanvas("c4","c4",600,600);
        myhisto->Draw("TEXT89COLZ");
        myhisto->SetTitle(("NumOfMisid_after_"+i2s(number)+"_iteration_kaon_numberofpart_"+d2s(std::accumulate(jop2.begin(),jop2.end(),0.0))).c_str());
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
    //    gStyle->SetTextAngle(90);
        c4->SaveAs(("NumberOfMisidafter_"+i2s(number)+"_iteration_HeatMap_kaon_"+species+"_"+polarity+"_"+stripping+"_"+misidcuts+".pdf").c_str());
        delete c4;
        delete myhisto;
...
float totmisidproton(string filename, string species, string misidPIDcalib, string misidcuts, string idPIDcalib, string idcuts, string tags, string stripping, string polarity, int ntoy) {
	string decaytree ="DecayTree";

	///------------ ADD Weights ------------///

	TFile f1((filename).c_str());
	TTree* t1 = (TTree*)f1.Get(decaytree.c_str());
	if(t1 && t1->GetEntries()!=0)
	{ 
		addweighttotree(filename, decaytree, idPIDcalib, idcuts, "idPIDcalib", ("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename).c_str());
                plotheatmapPID(filename, decaytree, idPIDcalib, idcuts, "idPIDcalib", polarity, stripping);
		addweighttotreeJACK(("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename).c_str(), decaytree, species, "jackweight", ("modified_"+i2s(ntoy)+"jack"+stripping+polarity+filename).c_str());
		addweighttotree(("modified_"+i2s(ntoy)+"jack"+stripping+polarity+filename).c_str(), decaytree, misidPIDcalib, misidcuts, "misidPIDcalibold", ("modified_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str());
                plotheatmapPID(("modified_"+i2s(ntoy)+"jack"+stripping+polarity+filename).c_str(), decaytree, misidPIDcalib, misidcuts, "misidPIDcalibold",polarity, stripping);

		///------------Cut the stupid entries------///
		cutTree(("modified_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, ("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), "(idPIDcalib != 3.0  && misidPIDcalibold != 3.0)");


		//----Include Jack's weight--------//

		addproductbranches(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "misidPIDcalibold", "jackweight", "misidPIDcalib");

		///----------- Calculate the probability of misid ----///

		addratiofbranches(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "misidPIDcalib", "idPIDcalib", "misidoverid");
		...
		int eta=4;
                ...
                vector<float> protonbinmydata;
                protonbinmydata=newbinmydata(filename, decaytree, species);
       
                         
		vector<float> protonideff;
		protonideff = efficiency(idPIDcalib,idcuts, p, eta, polarity, stripping);


		vector<float> misidprotontomuon;
		misidprotontomuon = efficiency(misidPIDcalib, misidcuts, p, eta, polarity, stripping);// species2);


		vector<float> truekinematicaldistributionproton;
		truekinematicaldistributionproton=newmyfavkin(protonbinmydata , protonideff, species);
	        

                TFile s("append.root");

		TH2F *numproton =(TH2F*)s.Get(("bindata"+species).c_str());


		TCanvas *c3 = new TCanvas("c3","c3",600,600);
		numproton->Draw("TEXT89COLZ");
		gPad->SetRightMargin(0.15);
		gStyle->SetOptStat(0);
		//  gStyle->SetTextAngle();
		c3->SaveAs(("NumberOfParticles_HeatMap_"+species+"_"+polarity+"_"+stripping+"_"+cleanNameString(misidcuts)+".pdf").c_str());
		delete c3;
		...
		TH2F *misproton =(TH2F*)s.Get(misidcuts.c_str());


		float o = misproton->Integral();

		cout<<"misproton:"<<o<<endl;

		//--FIRST TRUE KINEMATICAL DISTRIBUTIONS--//

		TH2F *kinproton =(TH2F*)s.Get(("kindistr"+species).c_str());

		//--------FIRST TRUE KINEMATICAL DISTRIBUTIONS* MISIDEFFICIENCY--------//

		TH2F firstproton=((*misproton)*(*kinproton));

		TCanvas *c1 = new TCanvas("c1","c1",600,600);
		firstproton.Draw("TEXT89COLZ");
		gPad->SetRightMargin(0.15);
		gStyle->SetOptStat(0);
		//  gStyle->SetTextAngle();
		c1->SaveAs(("MisidNumberAfter_zeroth_iteration_HeatMap_"+species+"_"+polarity+"_"+stripping+"_"+cleanNameString(misidcuts)+".pdf").c_str());
		delete c1;


		///---------- Reweigh Corrected Mass ------//
		addrealweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species);


		TFile f2(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str());
		TTree* t2 = (TTree*)f2.Get(decaytree.c_str());

		//   TFile f1(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str());
		//        //   TTree* t1 = (TTree*)f1.Get(decaytree2.c_str());
		//
		if(t2 && t2->GetEntries()!=0)
		{

			cout<<"I will do this"<<endl;
			///---------- Add it normalized ----------///
			addnormalbranchweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "misidoverid", "normalizedmisidoverid");
		}
	}
	return(3.1);
...
float totmisidnobinofcorrmimp(string filename, string filename2, string species, string species2 ,string misidPIDcalib, string misidcuts, string misidPIDcalib2, string misidcuts2, string idPIDcalib, string idcuts,  string idPIDcalib2, string idcuts2, string ktopimisidfilename, string ktopimisidcuts, string pitokmisidfilename, string pitokmisidcuts, string tags, string stripping, string polarity, int ntoy) {
	...
	addweighttotree(filename, decaytree, idPIDcalib, idcuts, "idPIDcalib", ("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename).c_str());
        plotheatmapPID(filename, decaytree, idPIDcalib, idcuts, "idPIDcalib",polarity, stripping);
	addweighttotree(("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename).c_str(), decaytree, misidPIDcalib, misidcuts, "misidPIDcalib", ("modified_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str());
        plotheatmapPID(("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename).c_str(), decaytree, misidPIDcalib, misidcuts, "misidPIDcalib",polarity, stripping);

	addweighttotree(filename2, decaytree2, idPIDcalib2, idcuts2, "idPIDcalib", ("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename2).c_str());
        plotheatmapPID(filename2, decaytree2, idPIDcalib2, idcuts2, "idPIDcalib",polarity, stripping);
	addweighttotree(("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename2).c_str(), decaytree2, misidPIDcalib2, misidcuts2, "misidPIDcalib", ("modified_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str());
        plotheatmapPID(("modified_"+i2s(ntoy)+"_"+stripping+polarity+filename2).c_str(), decaytree2, misidPIDcalib2, misidcuts2, "misidPIDcalib",polarity, stripping);
	//addweighttotree(filename, decaytree); 

	///------------Cut the stupid entries------///
	cutTree(("modified_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, ("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), "(idPIDcalib != 3.0  && misidPIDcalib != 3.0)");
	cutTree(("modified_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str(), decaytree, ("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str(), "(idPIDcalib != 3.0  && misidPIDcalib != 3.0)");
	...
	addratiofbranches(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "misidPIDcalib", "idPIDcalib", "misidoverid");
	addratiofbranches(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str(), decaytree2, "misidPIDcalib", "idPIDcalib", "misidoverid");
	...
	///---------- Reweigh Corrected Mass ------//
	addrealweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species);
	addrealweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species2);
	...
	TFile f1(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str());
	TTree* t1 = (TTree*)f1.Get(decaytree.c_str());
	if(t2 && t2->GetEntries()!=0 && t1 && t1->GetEntries()!=0)
	{
		...
		///---------- Add it normalized ----------///
		addnormalbranchweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, "misidoverid", "normalizedmisidoverid");
		addnormalbranchweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str(), decaytree2, "misidoverid", "normalizedmisidoverid");
	...
	kaonideff = efficiency(idPIDcalib,idcuts, p, eta, polarity, stripping);

	vector<float> pionideff;
	pionideff = efficiency(idPIDcalib2,idcuts2, p, eta, polarity, stripping);


	///------------ MIS-ID EFFICIENCIES-------------//

	vector<float> misidpiontomuon;
	misidpiontomuon = efficiency(misidPIDcalib2, misidcuts2, p, eta, polarity, stripping);// species2);

	vector<float> misidkaontomuon;
	misidkaontomuon = efficiency(misidPIDcalib, misidcuts, p, eta, polarity, stripping);// species);
...
//	string ktopimisidcuts="K_IsMuon==0.0 && DLLK < 0.0 && DLLp < 5.0_All";
	myktopimisid = crossmisid(ktopimisidfilename,ktopimisidcuts, p, eta, polarity, stripping);
...
//	string pitokmisidcuts="Pi_IsMuon==0.0 && DLLK > 0.0 && ((DLLp - DLLK) < 5.0)_All";
	mypitokmisid = crossmisid(pitokmisidfilename,pitokmisidcuts, p, eta, polarity, stripping);
	...
	TH2F *numpion =(TH2F*)s.Get(("bindata"+species2).c_str());
      ...
      //  gStyle->SetTextAngle();
        ...
        TCanvas *c4 = new TCanvas("c4","c4",600,600);
        numpion->Draw("TEXT89COLZ");
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
    //    gStyle->SetTextAngle(90);
        c4->SaveAs(("NumberOfParticles_HeatMap_"+species+"_"+polarity+"_"+stripping+"_"+cleanNameString(misidcuts2)+".pdf").c_str());
        delete c4;
	...
	TH2F firstpion =((*mispion)*(*kinpion));
        ...
        firstpion.Draw("TEXT89COLZ");
        gPad->SetRightMargin(0.15);
        gStyle->SetOptStat(0);
    //    gStyle->SetTextAngle(90);
        c2->SaveAs(("MisidNumberAfter_zeroth_iteration_HeatMap_"+species2+"_"+polarity+"_"+stripping+"_"+cleanNameString(misidcuts2)+".pdf").c_str());
        delete c2;
	...
	cout<<"numofkaonafteriteration:"<< number << "is: " <<numofkaonafteriteration(kaonbinmydata , mypitokmisid, truekinematicaldistributionpion, kaonideff, misidkaontomuon, number, species, misidcuts, polarity, stripping)<<endl;
	cout<<"numofpionafteriteration:"<< number << "is: " <<  numofpionafteriteration(pionbinmydata , myktopimisid, truekinematicaldistributionkaon, pionideff, misidpiontomuon, number, species2, misidcuts2, polarity, stripping)<<endl;



	kaonvector.push_back(numofkaonafteriteration(kaonbinmydata , mypitokmisid, truekinematicaldistributionpion, kaonideff, misidkaontomuon,number,species, misidcuts, polarity, stripping));
	pionvector.push_back(numofpionafteriteration(pionbinmydata , myktopimisid, truekinematicaldistributionkaon, pionideff, misidpiontomuon,number,species2, misidcuts2, polarity, stripping));
	...
	cout<<"numofkaonafteriteration: "<< number << "is: " << numofkaonafteriteration(kaonbinmydata , mypitokmisid, firstiterationpion, kaonideff, misidkaontomuon, number2, species, misidcuts, polarity, stripping)<<endl;
	cout<<"numofpionafteriteration: "<< number << "is: " << numofpionafteriteration(pionbinmydata , myktopimisid, firstiterationkaon, pionideff, misidpiontomuon, number2, species2, misidcuts2, polarity, stripping)<<endl;

	kaonvector.push_back(numofkaonafteriteration(kaonbinmydata , mypitokmisid, firstiterationpion, kaonideff, misidkaontomuon,number2, species, misidcuts, polarity, stripping));
	pionvector.push_back(numofpionafteriteration(pionbinmydata , myktopimisid, firstiterationkaon, pionideff, misidpiontomuon,number2, species2, misidcuts2, polarity, stripping));
	...
	cout<<"Z+2: " << overallvector.at(z+2) << endl;
        cout<<"Filename: "<<("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str()<<endl;
	...
	while (abs(overallvector.at(z+2)-overallvector.at(z+1))>(0.001*(overallvector.at(z+1))))
	{
		...
		cout<<"numofkaonafteriteration:"<< numofkaonafteriteration(kaonbinmydata , mypitokmisid, seconditerationpion, kaonideff, misidkaontomuon, z+3, species, misidcuts, polarity, stripping)<<endl;
		cout<<"numofpionafteriteration:"<< numofpionafteriteration(pionbinmydata , myktopimisid, seconditerationkaon, pionideff, misidpiontomuon, z+3, species2, misidcuts2, polarity, stripping)<<endl;

		kaonvector.push_back(numofkaonafteriteration(kaonbinmydata , mypitokmisid, seconditerationpion, kaonideff, misidkaontomuon,z+3, species, misidcuts, polarity, stripping));
		pionvector.push_back(numofpionafteriteration(pionbinmydata , myktopimisid, seconditerationkaon, pionideff, misidpiontomuon,z+3, species2, misidcuts2, polarity, stripping));
		...
		if(abs(overallvector.at(z+2)-overallvector.at(z+1))<(0.001*(overallvector.at(z+1))))
		{
			...
			for(int l=0; l<finalkaonit.size(); l++)
			{
				...
				//                addweighttotreespecial(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, weightfinalkaon, "crossfeedweight", "try.root"); 
			}       
			addweighttotreespecial(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename).c_str(), decaytree, weightfinalkaon, "crossfeedweight");
			addweighttotreespecial(("modified_andcut_"+i2s(ntoy)+"_"+tags+stripping+polarity+filename2).c_str(), decaytree, weightfinalpion, "crossfeedweight");
...
float totmisidnobinofcorrm(string filename, string filename2, string species, string species2 ,string misidPIDcalib, string misidcuts, string misidPIDcalib2, string misidcuts2, string tags, int ntoy) {
	//string filename="B23MuNuFakeSameSignMuonSmallDataSample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut_KaonPID_PUNZIoptBDT.root";
	string decaytree ="DecayTree";
	//string misidPIDcalib ="/vols/lhcb/ss4314/week26oct/vetojpsismallsamplenshared/PIDnshared/KisMuonNSharedzeroDLLmumorethanzeroDLLmuminusKmorethanzero/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root";
	//string misidcuts ="K_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 0.0) && (nShared==0)_All";
	...
	addweighttotree(filename, decaytree, idPIDcalib, idcuts, "idPIDcalib", ("modified_"+i2s(ntoy)+"_"+filename).c_str());
	addweighttotree(("modified_"+i2s(ntoy)+"_"+filename).c_str(), decaytree, misidPIDcalib, misidcuts, "misidPIDcalib", ("modified_"+i2s(ntoy)+"_"+tags+filename).c_str());

	addweighttotree(filename2, decaytree2, idPIDcalib2, idcuts2, "idPIDcalib", ("modified_"+i2s(ntoy)+"_"+filename2).c_str());
	addweighttotree(("modified_"+i2s(ntoy)+"_"+filename2).c_str(), decaytree2, misidPIDcalib2, misidcuts2, "misidPIDcalib", ("modified_"+i2s(ntoy)+"_"+tags+filename2).c_str());
	//addweighttotree(filename, decaytree); 

	///------------Cut the stupid entries------///
	cutTree(("modified_"+i2s(ntoy)+"_"+tags+filename).c_str(), decaytree, ("modified_andcut_"+i2s(ntoy)+"_"+tags+filename).c_str(), "(idPIDcalib != 3.0  && misidPIDcalib != 3.0)");
	cutTree(("modified_"+i2s(ntoy)+"_"+tags+filename2).c_str(), decaytree, ("modified_andcut_"+i2s(ntoy)+"_"+tags+filename2).c_str(), "(idPIDcalib != 3.0  && misidPIDcalib != 3.0)");



	///----------- Calculate the probability of misid ----///

	addratiofbranches(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename).c_str(), decaytree, "misidPIDcalib", "idPIDcalib", "misidoverid");
	addratiofbranches(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename2).c_str(), decaytree2, "misidPIDcalib", "idPIDcalib", "misidoverid");
...
//	exit;
	...
	///---------- Reweigh Corrected Mass ------//
	addrealweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species);
	addrealweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename2).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species2);


	///---------- Add it normalized ----------///
	addnormalbranchweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename).c_str(), decaytree, "misidoverid", "normalizedmisidoverid");
	addnormalbranchweight(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename2).c_str(), decaytree2, "misidoverid", "normalizedmisidoverid");
	...
	//pionbinmydata=binmydatacorrm(filename2, decaytree2, species2, 2500, 10000);
       ...
       string polarity="balh";
       string stripping="blah2";
	...
	vector<float> misidpiontomuon;
	misidpiontomuon = efficiency(misidPIDcalib2, misidcuts2, p, eta,  polarity, stripping);

	vector<float> misidkaontomuon;
	misidkaontomuon = efficiency(misidPIDcalib, misidcuts, p, eta,  polarity, stripping);
 ...
 //       string polarity="blah";
  //      string stripping="blah2";
	...
	cout<<"numofkaonafteriteration: "<< number2 << "is: " << numofkaonafteriteration(kaonbinmydata , mypitokmisid, firstiterationpion, kaonideff, misidkaontomuon,number2, species, misidcuts, polarity, stripping)<<endl;
	cout<<"numofpionafteriteration: "<< number2 << "is: " << numofpionafteriteration(pionbinmydata , myktopimisid, firstiterationkaon, pionideff, misidpiontomuon,number2, species2, misidcuts2, polarity, stripping)<<endl;
	...
	while (abs(overallvector.at(z+2)-overallvector.at(z+1))>(0.001*(overallvector.at(z+1))))
	{
		...
		if(abs(overallvector.at(z+2)-overallvector.at(z+1))<(0.001*(overallvector.at(z+1))))
		{
			...
			addweighttotreespecial(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename).c_str(), decaytree, weightfinalkaon, "crossfeedweight");
			addweighttotreespecial(("modified_andcut_"+i2s(ntoy)+"_"+tags+filename2).c_str(), decaytree, weightfinalpion, "crossfeedweight");
