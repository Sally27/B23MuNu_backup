...
//	cuts4.push_back("P_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 0.0) && (nShared==0)_All");
	...
	cuts4.push_back("P_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 0.0) && (nShared==0) && ((MC12TuneV2_ProbNNmu - MC12TuneV2_ProbNNpi) > -0.6)_All");
 	...
 	for(int i(0);i<cuts4.size();i++)
 	{
		...
		string filename2=("/vols/lhcb/ss4314/prepare_misidvariations/FUMSB_NOTsimultaneous_NewTCK_misidvariation_extremeweights_2/generatedtuples/"+stripping+"_"+cleanNameString(cuts2.at(i))+"_"+i2s(c)+".root").c_str();
		string filename=("/vols/lhcb/ss4314/prepare_misidvariations/FUMSB_NOTsimultaneous_NewTCK_misidvariation_extremeweights_2/generatedtuples/"+stripping+"_"+cleanNameString(cuts.at(i))+"_"+i2s(c)+".root").c_str();
 		...
 		p=totmisidnobinofcorrmimp(kaonsample,pionsample,"kaon","pion",filename, (cuts.at(i)+"_new").c_str(), filename2, (cuts2.at(i)+"_new").c_str(), idPIDcalib, idcuts,idPIDcalib2, idcuts2, ktopimisidfilename, ktopimisidcuts, pitokmisidfilename, pitokmisidcuts ,(alltags.at(i)).c_str(), stripping, polarity,c);
 		eff.push_back(p);
                float l;
                l=totmisidproton(protonsample, "proton", filename4, (cuts4.at(i)).c_str(), idPIDcalib4, idcuts4, (alltags.at(i)).c_str(), stripping, polarity,c); 
 		double e;
 		double f;
 		double g;
