...
//	Double_t* yedges = &binningy[0];
	...
	for(int i=0; i<p; ++i){
        ...
        cout<<"Weight file: "<<weightfile<<endl;
        cout<<"Weight cuts: "<< wfilecuts<<endl;


	TFile* s = new TFile(weightfile.c_str());
        if(!s){cerr<<"no file found"<<endl;return;}
	TH2F *hname =(TH2F*)s->Get(wfilecuts.c_str());
        if(!hname){cerr<<"no hist found"<<endl;return;}        
	vector<float> effi;
	Int_t z(0);
...
//	Double_t* yedges = &binningy[0];
	...
	TFile* s = new TFile(weightfile.c_str());
        if(!s){cerr<<"no file found"<<endl;return;}
	TH2F *hname =(TH2F*)s->Get(wfilecuts.c_str());
        if(!hname){cerr<<"no histo found"<<endl;return;}
	vector<float> effi;
	Int_t z(0);
