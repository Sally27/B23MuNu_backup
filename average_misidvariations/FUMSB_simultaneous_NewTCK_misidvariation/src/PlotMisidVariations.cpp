...
void PlotMisidVariations::create_distributionhistograms()
{

        
	TFile *F1 = new TFile((bindir+"/BinInfo_"+i2s(numberofhistograms)+".root").c_str(),"RECREATE");
...
void PlotMisidVariations::create_bin_histograms(vector< vector<double> > values, string type)
{

        
	TFile *F1 = new TFile((bindir+"/BinInfo_"+i2s(numberofhistograms)+"_"+type+".root").c_str(),"RECREATE");

	for (int i(0); i<numberofhistograms; i++)
	{

		TH1D* h_bin = new TH1D(("BinInfo_"+i2s(i)).c_str(),("BinInfo_"+i2s(i)).c_str(), 100, 0.0, 1.5);
...
void PlotMisidVariations::print_mean_and_rms(string type)
{
	TFile* F1 = new TFile((bindir+"/BinInfo_"+i2s(numberofhistograms)+"_"+type+".root").c_str(),"READ");

        ofstream out;
        out.open((bindir+"BinInfo_"+type+"_Summary.txt").c_str());
	...
	TFile fw((workspacedir+"/RooWS_Error_Mean_MisidVariation_"+i2s(numberofhistograms)+"_"+i2s(numberoftoys)+"_"+type+".root").c_str(), "RECREATE");
...
void PlotMisidVariations::plot_bin_histograms(string type)
{
	TFile* F1 = new TFile((bindir+"/BinInfo_"+i2s(numberofhistograms)+"_"+type+".root").c_str(),"READ");
	...
	for (int l(0); l<numberofpdf; l++){
		...
		cv->Divide(3,3);
               ...
               cv->SaveAs((bindir+"/BinInfo_"+i2s(numberofhistograms)+"_"+i2s(l)+"_"+type+".pdf").c_str());
	}
...
vector<double> PlotMisidVariations::plotdistributions_perbin(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type, int toynumber, vector <string> &PID)
{
 ...
 //       Bplus_Corrected_Mass.setBins(15);
	...
	cout << dpw.weight() << endl;
        ...
        int desirednumofbins(numberofhistograms);
	...
	cout<< "Num of bins:" << binnedData.numEntries()<<endl; 
        ...
        ofstream out;
        out.open((bindir+"BinInfo_"+type+"_toynumber_"+i2s(toynumber)+".txt").c_str());
...
void PlotMisidVariations::plotmisidcomponents_pertoy(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type, int toynumber, vector <string> &PID)
{
        ...
        RooRealVar Bplus_Corrected_Mass("Bplus_Corrected_Mass","Bplus_Corrected_Mass",4000,7000);
        Bplus_Corrected_Mass.setRange("sideband",4000,6000);
        Bplus_Corrected_Mass.setBins(numberofhistograms);
        RooRealVar crossfeedweight(weights.at(0).c_str(),weights.at(0).c_str(), 0.0 ,1.0);
