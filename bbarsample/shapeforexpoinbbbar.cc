...
void plot(){
  ...
  l=3;
      ...
      if(l==3)
      {
  //    cutTree("PIDmuonbackground2.root", "DecayTree", "CAT110.root", "Bplus_BKGCAT==110"); 
        cutTree("CAT110BDT.root", "DecayTree", "CAT110BDTopt.root", "Combinatorial60TreesPfinaltuplesNTrees60_MinNodeSize2_MaxDepth3_SeparationTypeGiniIndex_PruneMethodNoPruning_DoPreselection > 0.56");     
      }
	  ...
	  framecom2->addPlotable(hpullcom,"P");
          ...
          TPaveLabel *t3 = new TPaveLabel(0.6,0.6,0.9,0.8, Form("Constant = %f",o.getVal()),"brNDC");
          framecom1->addObject(t3);
	  ...
	  canv2->SaveAs("FittingCombiUnBinnedPlotforANA.pdf");

          TCanvas* mine = new TCanvas("mine","mine",800,800) ;
          gPad->SetRightMargin(0.1) ; 
          gPad->SetLogy();
          frame6->GetYaxis()->SetTitleOffset(1.0) ;
          frame6->GetYaxis()->SetTitle("Events/375 MeV");
          frame6->GetXaxis()->SetTitle("B^{+} Corrected Mass");
          frame6->SetMinimum(1.0);
          frame6->SetMaximum(10000.0);
          frame6->Draw();
          mine->SaveAs("plofortanabbbarcombiDATA.pdf");
          cout<<"This is expo constant: "<<o<<endl;
          return(0); 
	  ...
	  y.setRange("sideband",4500,10000);
          y.setRange("signal",2500,10000);
          y.setRange("signalwindow",4500,5500);
	  ...
	  canv2->SaveAs("FittingCombiUnBinnedPlotforANA.pdf");
          ...
          frame6->Draw();
          mine->SaveAs("plofortanabbbarcombi.pdf");
          cout<<"This is expo constant: "<<o<<endl;
          break;
...
//  f->Close();
  ...
  TFile* f = new TFile("/vols/lhcbdisk05/ss4314/preparetuplesforBDTfinal/Combinatorialpreparetuple/B23MuNuSignal2012and2011data_corrm_Jpsi_mu1nShared_mu2nShared_mu3nShared_qmin.root");
  TTree* t = (TTree*)f->Get("DecayTree");
  TTree* tree = new TTree("tree","tree") ;
//  double notuseful;
