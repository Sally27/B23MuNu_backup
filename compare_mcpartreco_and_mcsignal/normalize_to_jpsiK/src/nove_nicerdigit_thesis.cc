...
int main(){
	...
	ofstream out;
	out.open("EfficiencyComparison_compared_toNormalisation_nove_nicedig_thesis_FULL.tex");
...
//        out << "$2.0\\times f(b\\rightarrow B^{+})$ "<<"& 2$\\times$"<<fragfrac.val<<"$\\pm$"<<fragfrac.err<<"& 2$\\times$"<<fragfrac.val<<"$\\pm$"<<fragfrac.err<<"& 2$\\times$"<<fragfrac.val<<"$\\pm$"<<fragfrac.err<<" \\\\ "<<endl;
        out << " $\\mathcal{B}(B^+ \\rightarrow K^+ \\pi^- \\mu^+ \\mu^{-})$ & "<<(overall.calculateProduct_BR("pr")).roundToError_diffbase(true)<<" &"<<(overall.calculateProduct_BR("pr")).roundToError_diffbase(true)<<" \\\\ "<<endl;
        out << "  $\\mathcal{B}(B^+ \\rightarrow (J/\\psi \\rightarrow \\mu^+ \\mu^{-}) K^{+})$ &"<<(overall.calculateProduct_BR("norm")).roundToError_diffbase(true)<<" & "<< (overall.calculateProduct_BR("norm")).roundToError_diffbase(true)<<" \\\\ "<<endl;
        
        out<<"\\hline"<<endl;
        out << "  $\\varepsilon_{gen}$ & " << (overall.return_gen_eff("pr")).roundToError_diffbase(true)<<" & Using 2012 " <<" \\\\ "<<endl;
//        out << "$\\varepsilon_{sel}$ &  " << round_to_digits(overall.calculate_total_seleff(double(number_bkk_nor)),3), round_to_digits((overall.return_file("pr","no"),"no").val,3)<<"$\\pm$"<<round_to_digits(overall.calculate_total_seleff(double(number_bkk_nor)),3), round_to_digits((overall.return_file("pr","no"),"no").err,3)<<" & Using 2012 " <<" \\\\ "<<endl;
	out << "  $\\varepsilon_{sel}$ &  " << ((overall.calculate_total_seleff(double(number_bkk_nor), overall.return_file("pr","no"),"no"))/effqmin).roundToError_diffbase(true)<<" & Using 2012 " <<" \\\\ "<<endl;

	out << "  $\\varepsilon_{total}$ &  " << (((overall.calculate_total_seleff(double(number_bkk_nor), overall.return_file("pr","no"),"no"))/effqmin)*(overall.return_gen_eff("pr"))).roundToError_diffbase(true)<<" & Using 2012 " <<" \\\\ "<<endl;
//----------------------------------------------------divide by qmin---------------- important//


        out<<"\\hline"<<endl;
        out << " $\\varepsilon_{gennorm}$ & " << (overall.return_gen_eff("norm")).roundToError_diffbase(true)<<" &  Using 2012" <<" \\\\ "<<endl;
//        out << "$\\varepsilon_{selnorm}$ & " << round_to_digits(overall.calculate_total_seleff(double(number_bkk_sig)),3), round_to_digits((overall.return_file("norm","no"),"no").val,3)<<"$\\pm$"<<round_to_digits(overall.calculate_total_seleff(double(number_bkk_sig)),3), round_to_digits((overall.return_file("norm","no"),"no").err,3)<<" & Using 2012 " <<" \\\\ "<<endl;
        out << " $\\varepsilon_{selnorm}$ & " << (overall.calculate_total_seleff(double(number_bkk_sig), overall.return_file("norm","no"),"no")).roundToError_diffbase(true)<<" & Using 2012 " <<" \\\\ "<<endl;
         out << " $\\varepsilon_{totalnorm}$ & " << ((overall.return_gen_eff("norm"))*(overall.calculate_total_seleff(double(number_bkk_sig), overall.return_file("norm","no"),"no"))).roundToError_diffbase(true)<<" &  Using 2012" <<" \\\\ "<<endl;
...
//        TTree fillinfotree("PR_Yield","PR_Yield");
        ...
        out << " \\hline "<<endl;
        out << " \\multicolumn{3}{|c|}{\\textbf{"<<(group.at(i))<<"FCME}} "<<" \\\\ "<<endl;
        out << " \\hline "<<endl;
        out << "$ \\mathcal{B} \\times \\varepsilon_{pr} $ "<<(group.at(i))<<" &  " <<(pr_prod).roundToError_diffbase(true)<<" & Using 2012"<<" \\\\ "<<endl;
        out << "$ \\mathcal{B} \\times \\varepsilon_{norm} $ " <<(group.at(i))<<" &  " <<(norm_prod).roundToError_diffbase(true)<<" & Using 2012"<<" \\\\ "<<endl;


        out << "$ \\frac{\\mathcal{B} \\times \\varepsilon_{pr}}{\\mathcal{B} \\times \\varepsilon_{norm}}$ "<<(group.at(i))<<"  &  "<< (pr_prod/norm_prod).roundToError_diffbase(true)<<" & Using 2012 "<<" \\\\ "<<endl;
        ...
        out<<"\\hline"<<endl;
        ValError totalpreff=forpr.calculateProduct_eff(group.at(i), "pr", lowpr.return_gen_eff("pr"), lowpr.calculate_total_seleff(double(number_bkk_nor), lowpr.return_file("pr",group.at(i)),group.at(i))/effqmin);
        ValError totaljpsikeff=fornorm.calculateProduct_eff(group.at(i), "norm", lownorm.return_gen_eff("norm"), lownorm.calculate_total_seleff(double(number_bkk_sig), lownorm.return_file("norm",group.at(i)),group.at(i)));
        ...
        double effi_ratio(0);
        double effi_ratio_err(0);

        double bdmunubr(0);
        double bdmunubr_err(0);

        double dkpimumubr(0);
        double dkpimumubr_err(0);
        ...
        fillinfotree.Branch(("effi_ratio_"+group.at(i)).c_str(),&effi_ratio, ("effi_ratio_"+group.at(i)+"/D").c_str());
        fillinfotree.Branch(("effi_ratio_err_"+group.at(i)).c_str(),&effi_ratio_err, ("effi_ratio_err_"+group.at(i)+"/D").c_str());

        fillinfotree.Branch("bdmunubr",&bdmunubr,"bdmunubr/D");
        fillinfotree.Branch("bdmunubr_err",&bdmunubr_err, "bdmunubr_err/D");


        fillinfotree.Branch("dkpimumubr",&dkpimumubr,"dkpimumubr/D");
        fillinfotree.Branch("dkpimumubr_err",&dkpimumubr_err, "dkpimumubr_err/D");

        numofevents=relnum.val;
        numofevents_err=relnum.err;

        effi_ratio=(totalpreff/totaljpsikeff).val;
        effi_ratio_err=(totalpreff/totaljpsikeff).err;
        
        bdmunubr=(forpr.return_brbdmunu()).val;
        bdmunubr_err=(forpr.return_brbdmunu()).err;

        dkpimumubr=(forpr.return_brdkpimumu()).val;
        dkpimumubr_err=(forpr.return_brdkpimumu()).err;
