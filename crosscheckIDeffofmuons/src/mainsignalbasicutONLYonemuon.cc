...
int main(int argc, char *argv[]){
	...
	cuts.push_back("(mu1_isMuon==1.0) && mu1_nShared==0");
        cuts.push_back("(mu1_isMuon==1.0) && mu1_nShared==0 && (mu1_PIDmu > 0.0)");
        cuts.push_back("(mu1_isMuon==1.0) && mu1_nShared==0 && (mu1_PIDmu > 0.0) && ((mu1_PIDmu - mu1_PIDK) > 0.0)");
	cuts.push_back("(mu1_isMuon==1.0) && (mu1_PIDmu > 0.0) && ((mu1_PIDmu - mu1_PIDK) > 0.0) && mu1_nShared==0 && (mu1_ProbNNmu > 0.3)");
	...
	for(int j(0); j<tuples.size(); j++){
		...
		pidweighthold=calculatePIDfromWeights(tuples.at(j), "DecayTree", "mu1weight");
		pidweight.push_back(pidweighthold);
	}

	ofstream out2;
	out2.open("EfficienciesIDPIDfromMCandPIDreweightBasicONLYoneMUON_mu1.tex");
	...
	ofstream out3;
	out3.open("Basicforpython_mu1.tex");


	TFile fillinfo("info_basic_mu1.root","RECREATE");
	...
	fillinfotree.Write();
        ...
        ofstream out;
	out.open("EfficienciesIDPIDfromMCafterAllSelectionNOFCMEBasicONLYoneMUON_mu1.tex");
