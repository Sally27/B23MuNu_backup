...
//	string pathname2 = "/vols/lhcb/ss4314/BDTrunonsamples/tuplesandbdtrun/MCSig2016_288888335_vs_DATACombi2016_2016only/";
	...
	tuples.push_back("Bu23MuNuMC2012_MCtruth_L0MuonDecisionTOS_trigger_Jpsi");
	tuples.push_back("Bu23MuNuMC2012_MCtruth_L0MuonDecisionTOS_trigger_Jpsi_CombBasic");
	tuples.push_back("Bu23MuNuMC2012_MCtruth_L0MuonDecisionTOS_trigger_Jpsi_CombBasic_MisidBasic");
	tuples.push_back("Bu23MuNuMC2012_MCtruth_L0MuonDecisionTOS_trigger_Jpsi_CombBasic_MisidBasic_FitCorM");


        vector<string> tags;
 
        tags.push_back("SigMC2012_Mine");
        tags.push_back("SigMC2012_Mine_aftercombi");
        tags.push_back("SigMC2012_Mine_aftermisid");
        tags.push_back("SigMC2012_Mine_afterhighfcme");
	...
	string cut2 = "MCSig2012_vs_DATACombiRun1NTrees60_MinNodeSize2_MaxDepth3_SeparationTypeGiniIndex_PruneMethodNoPruning_DoPreselection > 0.47";
        ...
        string cut5 = "(Bplus_MuMuMu_MCORRFULLERR/Bplus_Corrected_Mass)>0.0225";
        cuts.push_back(cut2);
        cuts.push_back(cut3);
        cuts.push_back(cut4);
        cuts.push_back(cut5);
	...
	for(int i(0); i<tuples.size(); i++)
	{
        ...
        ofstream out;
	out.open("EfficienciesCombiBDT_Mine_HighFCME.tex");
	...
	TFile *fillinfos = new TFile("../efficiencytuples_sig/Efficiencies_Mine_SigMC_2012_Combi_HighFCME.root","RECREATE");
	TTree *fillinfotrees = new TTree("MC_Sig_2012","MC_Sig_2012");
	FillEffInfo(fillinfotrees, "combibdt",double(collection.find("SigMC2012_Mine")->second),false);
	FillEffInfo(fillinfotrees, "misidbdt",double(collection.find("SigMC2012_Mine_aftercombi")->second),false);
	FillEffInfo(fillinfotrees, "fitrange",double(collection.find("SigMC2012_Mine_aftermisid")->second),false);
	FillEffInfo(fillinfotrees, "highfcme",double(collection.find("SigMC2012_Mine_afterhighfcme")->second),false);
	fillinfos->cd();
