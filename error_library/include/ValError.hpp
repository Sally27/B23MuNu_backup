...
class ValError
{
	public:
		...
		bool operator==(ValError const& ve) const;
                ...
                string roundToError_diffbasenoampe(bool wantLatex=false);
		string roundToError(bool wantLatex = false);
                string roundToError_percentbase(bool wantLatex=false);
		string roundToError_diffbase(bool wantLatex=false);
};
