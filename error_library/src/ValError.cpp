...
string ValError::roundToError_diffbasenoampe(bool wantLatex)
{
   string ret;
   if(err == 0)
   {
   ...
   int newpower(floor(TMath::Log10(val)));
   double newpowerasnumber=double(TMath::Power(10, newpower));
   double newval=(double(val)/double(newpowerasnumber));
   double newerr=(double(err)/double(newpowerasnumber));
   int newfixed=(nfixed/newpower);
   cout<<"This is newnfixed "<<newfixed<<endl;
   cout<<"with dbl2str This is error "<<dbl2str(newerr, newfixed)<<" and this is value "<<dbl2str(newval, newfixed)<<"\\times 10 ^{"<<newpowerasnumber<<" }"<<endl;
  
 
   if(!wantLatex) ret = "("+dbl2str(newval, 2)+"+-"+dbl2str(newerr, 2)+")\\times 10 ^{"+i2s(newpower)+" }";
   if(wantLatex) ret = "("+dbl2str(newval, 2)+"\\pm"+dbl2str(newerr, 2)+")\\times 10 ^{"+i2s(newpower)+" }";
   return ret;

}
string ValError::roundToError_diffbase(bool wantLatex)
{
   string ret;
   if(err == 0)
   {
...
string ValError::roundToError_percentbase(bool wantLatex)
{
   string ret;
   if(err == 0)
   {
...
//   int newpower(floor(TMath::Log10(val)));
//   double newpowerasnumber=double(2.0);
//   double newval=(double(val)/double(newpowerasnumber));
//   double newerr=(double(err)/double(newpowerasnumber));
//   int newfixed=(nfixed/newpower);
//   cout<<"This is newnfixed "<<newfixed<<endl;
//   cout<<"with dbl2str This is error "<<dbl2str(newerr, newfixed)<<" and this is value "<<dbl2str(newval, newfixed)<<"\\times 10 ^{"<<newpowerasnumber<<" }"<<endl;
   ...
   if(!wantLatex) ret = "("+dbl2str(val*100.0, nfixed)+"+-"+dbl2str(err*100.0, nfixed)+")\\%" ;
   if(wantLatex) ret = "$("+dbl2str(val*100.0, nfixed)+"\\pm"+dbl2str(err*100.0, nfixed)+")\\% $";
   return ret;
...
ValError getFullProduct(vector<ValError> vecVa)
{

  ValError finalres(1.0,0.0);
   for(unsigned int i(0); i<vecVa.size(); ++i)
   {
   finalres=finalres*vecVa.at(i);
   }
   return finalres;

}
ValError getInnerProduct(vector<ValError> vecVa, vector<double> weights)
{

  ValError finalres(0,0);
   for(unsigned int i(0); i<vecVa.size(); ++i)
   {
   finalres+=weights.at(i)*vecVa.at(i);
   }
   return finalres;
}
