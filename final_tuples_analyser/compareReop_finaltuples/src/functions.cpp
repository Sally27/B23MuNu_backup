...
double calculatePIDfromWeights2muons(string filename, string decaytreename, string muon1, string muon2)
{
        ...
        t->SetBranchAddress(muon1.c_str(), &weight1);
        t->SetBranchAddress(muon2.c_str(), &weight2);

//	t->SetBranchAddress(weightvar.c_str(), &weight);
	...
	for(int i=0; i<t->GetEntries(); ++i)
	{
        ...
        double weighteff(0);
        weighteff=(sumofweights/double(t->GetEntries()));
