...
int main(int argc, char *argv[]){
	...
	ofstream out2;
	out2.open("Efficiencies_2016_ALL_opt_4.tex");
	...
	ofstream out3;
	out3.open("EfficienciesPlay_2016_allcuts_opt_4.tex");
...
//	out3<<"\\begin{tabular}{| l | l | l | }"<<endl;
	...
	for(int z(0); z<maps.size(); z++){
		if ((maps.at(z)).cut=="opt_4_isMuonTight"){
			out3<<"Sample & ";
	...
	for(int z(0); z<maps.size(); z++){
		if ((maps.at(z)).cut=="opt_4_nShared2"){
			out3<<(maps.at(z)).efficiency<<" & ";
