...
int main(int argc, char *argv[]){
	...
	ofstream out2;
	out2.open("Efficiencies_2016_NShared0.tex");
	...
	ofstream out3;
	out3.open("EfficienciesPlay_2016_opt_1_nShared0.tex");
...
//	out3<<"\\begin{tabular}{| l | l | l | }"<<endl;
	...
	for(int z(0); z<maps.size(); z++){
		if ((maps.at(z)).cut=="opt_1_nShared0"){
			out3<<"Sample & ";
	...
	for(int s(0); s<maps.size(); s++){
        ...
        numberofb= getmyentries("dataCOMBI2016opt_1_minq2.root","DecayTree")+getmyentries("dataSS2016opt_1_minq2.root","DecayTree");
	...
	out3<<"\\caption{cuts: ";
		for(int i(0); i<cuts1.size();i++){
			out3<<" "<<cuts1.at(i);}
        out3<<", #epsilon_{s}="<<sigeff<<", combi"<<getmyentries("dataCOMBI2016opt_1_minq2.root","DecayTree")<<", misidss "<<getmyentries("dataSS2016opt_1_minq2.root","DecayTree")<<"numofbkg: "<<numberofb<<" ,PunziFOM for sigma 3: "<<punzifom<<"}"<<endl;
	out3<<"\\end{table}"<<endl;
