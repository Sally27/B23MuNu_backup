...
int main(int argc, char *argv[]){
	...
	vector<string> tuples4;
        ...
        tuples4.push_back("/vols/lhcb/ss4314/final_tuples_analyser/mc/norm_mc/2016/bin/B2JpsiKMC2016_MCtruth_misidstrippingfull_trigger");
	...
	ofstream out2;
	out2.open("Efficiencies_2016_FunCombi.tex");
	...
	ofstream out3;
	out3.open("EfficienciesPlay_2016_FunCombi.tex");
...
//	out3<<"\\begin{tabular}{| l | l | l | }"<<endl;
	...
	for(int z(0); z<maps.size(); z++){
		if ((maps.at(z)).cut=="opt_2_isMuonTight"){
			out3<<"Sample & ";
	...
	for(int z(0); z<maps.size(); z++){
		if ((maps.at(z)).cut=="opt_2_nSharedCombi"){
			out3<<(maps.at(z)).efficiency<<" & ";
	...
	for(int z(0); z<maps.size(); z++){
        ...
        vector<double> overal_efficiencies;
	for(int s(0); s<maps.size(); s++){
		...
		for(int z(0); z<maps.size(); z++){
			if ((maps.at(z)).filename==(maps.at(s)).filename){
                ...
                if (z==(maps.size()-1) && ((s+1)%(int(cuts1.size()))==1)){
                        out3<<overalleff<<" & "; 
			overal_efficiencies.push_back(overalleff);}
		}
	}
        ...
        sigeff=overal_efficiencies.at(0);
        numberofb= getmyentries("dataCOMBI2016opt_2_minq2.root","DecayTree")+getmyentries("dataSS2016opt_2_minq2.root","DecayTree");
        punzifom=sigeff/((sqrt(double(numberofb)))+double(3/2));
	...
	out3<<"\\caption{cuts: ";
		for(int i(0); i<cuts1.size();i++){
			out3<<" "<<cuts1.at(i);}
        out3<<", #epsilon_{s}="<<sigeff<<", combi"<<getmyentries("dataCOMBI2016opt_2_minq2.root","DecayTree")<<", misidss "<<getmyentries("dataSS2016opt_2_minq2.root","DecayTree")<<"numofbkg: "<<numberofb<<" ,PunziFOM for sigma 3: "<<punzifom<<"}"<<endl;
	out3<<"\\end{table}"<<endl;
