...
//	out3<<"\\begin{tabular}{| l | l | l | }"<<endl;
	...
	for(int s(0); s<maps.size(); s++){
        ...
        numberofb= getmyentries("dataCOMBI2016opt_3_minq2.root","DecayTree")+getmyentries("dataSS2016opt_3_minq2.root","DecayTree");
	...
	out3<<"\\caption{cuts: ";
		for(int i(0); i<cuts1.size();i++){
			out3<<" "<<cuts1.at(i);}
        out3<<", #epsilon_{s}="<<sigeff<<", combi"<<getmyentries("dataCOMBI2016opt_3_minq2.root","DecayTree")<<", misidss "<<getmyentries("dataSS2016opt_3_minq2.root","DecayTree")<<"numofbkg: "<<numberofb<<" ,PunziFOM for sigma 3: "<<punzifom<<"}"<<endl;
	out3<<"\\end{table}"<<endl;
