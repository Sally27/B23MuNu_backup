...
int main(){
	...
	out<<"or strip and reco & 0.144209 & 1277956(8861780) & TBD  \\\\ "<<endl;
        ...
        if (p==0)
        {
	string pathname = "/vols/lhcb/ss4314/final_tuples/data/B2jpsikst_data/2011/";
	string filename = "B2JpsiKst_Data_2011";
...
//"(sqrt(((sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(493.667*493.667)) + (K_PE))*(sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(493.667*493.667)) + (K_PE)))-(((K_PX+Pi_PX)*(K_PX+Pi_PX))+((K_PY+Pi_PY)*(K_PY+Pi_PY))+ ((K_PZ+Pi_PZ)*(K_PZ+Pi_PZ))))"	
	...
	varia.push_back("relevantcutsNOGHOST");
...
//        varia.push_back("kdllkismuon");
	...
	vals.push_back("B0_DIRA_OWNPV > 0.99996 && B0_ENDVERTEX_CHI2 < 7 && mu1_TRACK_CHI2 < 3.0 && mu2_TRACK_CHI2 < 3.0 && K_TRACK_CHI2 < 3.0 && Pi_TRACK_CHI2 < 3.0 && mu1_IPCHI2_OWNPV > 9.0 && mu2_IPCHI2_OWNPV > 9.0 && K_IPCHI2_OWNPV > 9.0 && Pi_IPCHI2_OWNPV > 9.0 ");
   ...
   //     vals.push_back("K_PIDK > 5 && K_isMuon==0.0");
        vals.push_back("K_PIDK > 0 && K_isMuon==0 && (K_PIDK-K_PIDp)> 0");
