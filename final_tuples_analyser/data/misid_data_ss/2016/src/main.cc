...
int main(){
	...
	out<<"\\begin{document}"<<endl;
        ...
        string pathname = "/vols/lhcb/ss4314/final_tuples/data/misid_data_ss/2016/";
        string filename = "Bu23MuNuData2016_SSmisid";
	...
	string filename2 = (filename+cuttag).c_str();
        ...
        vector<string> varia;
        varia.push_back("mu3isNotMuon_mu3InAccMuonAcc");
	...
	for(int i(2);i<4;i++)
	{
        ...
        string cutpion = "(mu3_PIDK < 0) && (mu3_PIDp < 5)";
	...
	string cutkaon = "(mu3_PIDK > 0) && (mu3_PIDp - mu3_PIDK < 5)";
	...
	idenkaon=cutTree((filenamekaon+ext).c_str(), "DecayTree", (namecuttreekaon+ext).c_str(), cutkaon);
        ...
        string cutprot = "(mu3_PIDp > 5.0) && (mu3_PIDp - mu3_PIDK > 5.0)";
	...
	addKFoldandW((filenameprot+cuttagprot).c_str(), "DecayTree",(filenameprot+cuttagprot).c_str() );
        addKFoldandW((filenamepion+cuttagpion).c_str(), "DecayTree", (filenamepion+cuttagpion).c_str());
        addKFoldandW((filenamekaon+cuttagkaon).c_str(), "DecayTree",(filenamekaon+cuttagkaon).c_str());
	...
	string filename3=(filenameclean+"_mu3isNotMuon_mu3InAccMuonAcc").c_str();
	...
	string filename4=(filenameclean+"_mu3isNotMuon_mu3InAccMuonAcc_L0MuonDecisionTOS_Hlt1TrackMuonDecisionTOS").c_str();        
...
//        string decaytreename = "B_Tuple2/DecayTree";
//        string ext = ".root";
//        string cuttag = "_mu3isNotMuon_mu3InAccMuonAcc";
//        string namecuttree2 = (filename+cuttag).c_str();
...
//        out << "$\\epsilon_{notMuonInAccMuon}$ &" << notmuoneff << " \\\\ "<<endl;
//
//
