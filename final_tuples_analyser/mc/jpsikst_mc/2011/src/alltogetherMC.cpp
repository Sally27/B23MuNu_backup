...
//	Double_t mu3_isMuon;
	...
	for(int i(0); i<t->GetEntries(); ++i)
	{
		...
		if (Bplus_BKGCAT<60)// ||Bplus_BKGCAT==50)
		{
			...
			if ((abs(mu1_TRUEID)==13) && (abs(mu2_TRUEID)==13) && (abs(K_TRUEID)==321) && (abs(Pi_TRUEID)==211) && mu1_MC_MOTHER_ID==443 && mu2_MC_MOTHER_ID==443 && abs(K_MC_MOTHER_ID)==313 && abs(Pi_MC_MOTHER_ID)==313 && abs(mu1_MC_GD_MOTHER_ID)==511 && abs(mu2_MC_GD_MOTHER_ID)==511 && abs(K_MC_GD_MOTHER_ID)==511 && abs(Pi_MC_GD_MOTHER_ID)==511)
                        {			
				count2++;
				t2->Fill();
//				//              cout<<"Executing"<<endl;
