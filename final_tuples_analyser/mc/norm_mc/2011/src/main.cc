...
int main(){
	...
	out<<"\\hline"<<endl;
	out<<"dec prod cut & 0.16379(md from simstat) & 3554(10000) & TBD \\\\ "<<endl;
//	out<<"or strip and reco & 0.33522 & 1365886(4074487) & TBD  \\\\ "<<endl;
...
//	string filename2 = (filename+cuttag).c_str();
 //       string filenameclean = filename;
        ...
        string pathname = "/vols/lhcb/ss4314/final_tuples/mc/norm_mc/2011/";
        string filename = "MC_B2JpsiK_2011";
        string decaytreename = "B2JpsiK_Tuple/DecayTree";
        string ext = ".root";
        ...
        out << "$\\epsilon_{reco}$ &" << double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(4074487) << " & "<<"("<<getmyentries((filename2+ext).c_str(),"DecayTree")<<")4074487" << " & "<< (sqrt(double(getmyentries((filename2+ext).c_str(),"DecayTree"))*(1.0-((double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(4074487))))))*(1/double(4074487)) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
