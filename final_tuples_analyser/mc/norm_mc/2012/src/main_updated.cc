...
//	out<<"or strip and reco & 0.33522 & 1365886(8043820) & TBD  \\\\ "<<endl;
        ...
        if (p==0)
        {
//	string pathname = "/vols/lhcb/ss4314/tuplesallvar/JpsiKmc/";
//	string filename = "BuKMuMuMCtotal";
...
//	string filename2 = (filename+cuttag).c_str();
 //       string filenameclean = filename;
        ...
        string pathname = "/vols/lhcb/ss4314/final_tuples/mc/norm_mc/2012/";
        string filename = "MC_B2JpsiK_2012";
        string decaytreename = "B2JpsiK_Tuple/DecayTree";
        string ext = ".root";
        string cuttag = "MCTruth_";
        string filename2 = (filename+cuttag).c_str();
        ...
        out << "$\\epsilon_{reco}$ &" << double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(8043820) << " & "<<"("<<getmyentries((filename2+ext).c_str(),"DecayTree")<<")8043820" << " & "<< (sqrt(double(getmyentries((filename2+ext).c_str(),"DecayTree"))*(1.0-((double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(8043820))))))*(1/double(8043820)) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;


        int num_evt_bkk(8043820);
	...
	FillEffInfo(fillinfotree, "numbkk",double(num_evt_bkk),true);
        FillEffInfo(fillinfotree, "strip",double( double(getmyentries("/vols/lhcb/ss4314/final_tuples/mc/norm_mc/2012/MC_B2JpsiK_2012.root","B2JpsiK_Tuple/DecayTree"))/double(num_evt_bkk)),true);
