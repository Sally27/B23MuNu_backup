...
int main(){
	...
	double toteff(1.0);
        ...
        string pathname = "/vols/lhcb/ss4314/final_tuples/mc/partreco_mc/2012/";
        string filename = "MC_PartReco";
	...
	mctrutheff=mctruth((pathname+filename).c_str(), decaytreename, (filename2).c_str());
        ...
        TFile *fillinfo = new TFile("Efficiencies_2012_UPD.root","RECREATE");
        TTree *fillinfotree = new TTree("PartReco_MC_2012","PartReco_MC_2012");
        double decprodcut2 =0.16025;
        int numberofevents = 2054607;
        FillEffInfo(fillinfotree, "decprodcut",decprodcut2,false);



	out << "$\\epsilon_{MC}$ &" << mctrutheff <<" & "<<double(getmyentries((filename2+ext).c_str(),"DecayTree"))<<" \\\\ "<<endl;

        
        FillEffInfo(fillinfotree, "numbkk",double(numberofevents),true);
        FillEffInfo(fillinfotree, "strip",double( double(getmyentries("/vols/lhcb/ss4314/final_tuples/mc/partreco_mc/2012/MC_PartReco.root","B2D3pimu_Tuple/DecayTree"))/double(numberofevents)),true);
        FillEffInfo(fillinfotree, "mctruth",mctrutheff,true);
//        FillEffInfo(fillinfotree, "reco",double( double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(numberofevents)),true);


        out << "$\\epsilon_{reco}$ &" << double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(2054607) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
...
//	vals.push_back("mu3_nShared == 0.0");
	vals.push_back("minq2 < 960400");
        ...
        toteff=double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(2054607);
	...
	for(int i(0);i<1;i++)
	{
		...
		out<<"$\\epsilon_{"+varia.at(i)+"}$ & "<<eff<<" & "<< double(getmyentries((filename2+"_"+varia.at(i)+ext).c_str(),"DecayTree"))<<" \\\\ "<<endl;
	...
	out << "$\\epsilon_{Jpsi}$ &" << jpsieff << " & "<<double(getmyentries((filename2+"_Jpsi.root").c_str(),"DecayTree"))<<" \\\\ "<<endl;
...
//	double effrecostrip= 0.075031;
	...
	double finalnum;
	finalnum=ppbbX*bBplus*brfr*datacoll*decprodcut*toteff; 
//        out<<"Calculated ppbbX*bBplus*brfr*datacoll*decprodcut*effrecostrip*toteff: "<<endl;
//        out<<"Does NOT include MC truth match eff!"<<endl;
        ...
        out<<"\\end{table}"<<endl;      
        out<<"Calculated ppbbX*bBplus*brfr*datacoll*decprodcut*toteff: "<<endl;
        out<<"Does include MC truth match eff!"<<endl;
        out<<"\\end{document}"<<endl;
