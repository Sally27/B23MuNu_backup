...
int main(){
	...
	ofstream out;
	out.open("EfficienciesRecalibrate_CutBased.tex");
	...
	string ext = ".root";
	string cuttag = "_MCtruth_CutBased";
	string filename2 = (filename+cuttag).c_str();

        string filename_trigger=filename2;
	...
	out << "$\\epsilon_{reco}$ &" << double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(1107715) << " & "<<"("<<getmyentries((filename2+ext).c_str(),"DecayTree")<<")1107715" << " & "<< (sqrt(double(getmyentries((filename2+ext).c_str(),"DecayTree"))*(1.0-((double(getmyentries((filename2+ext).c_str(),"DecayTree"))/double(1107715))))))*(1/double(1107715)) <<" \\\\ "<<endl;
	out<<"\\hline"<<endl;
	...
	string filename3=(filename_trigger).c_str();
	...
	string filename4=(filename_trigger+"_L0MuonDecisionTOS_Hlt1TrackMuonDecisionTOS").c_str();        
