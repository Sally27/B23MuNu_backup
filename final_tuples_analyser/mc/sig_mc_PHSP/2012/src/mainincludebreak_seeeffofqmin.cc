...
int main(){
        ...
        ofstream out;
        out.open("EfficienciesBreakNT_qmin.tex");
	...
	out<<"\\begin{document}"<<endl;
        ...
        string ext = ".root";
        string cuttag = "_MCtruth_checkingqmin";
        ...
        TFile *fillinfo = new TFile("Efficiencies_PHSP_2012_qmin.root","RECREATE");
...
//        FillEffInfo(fillinfotree, "numbkk",double(1114130),true);
        ...
        double qminup(0);
        qminup=cutTree((filename2+ext).c_str(), "DecayTree", (filename2+"_qmin"+ext).c_str(), "minq2 < 960400");
 
        out << "$\\epsilon_{qmin}$ &" << qminup <<" - & "<< " - & " << " \\\\ "<<endl;
//        FillEffInfo(fillinfotree, "qminup",qminup,true);
        return(1);
