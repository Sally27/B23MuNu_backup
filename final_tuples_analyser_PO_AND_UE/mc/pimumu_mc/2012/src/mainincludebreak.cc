...
int main(){
	...
	ofstream out;
	out.open("Efficiencies_2012_pimumu.tex");
	...
	TFile *fillinfo = new TFile("Efficiencies_2012_PiMuMu.root","RECREATE");
...
//	addcostheta(filename2, "DecayTree", filename2);
	...
	string ext = ".root";
         ...
         //copy the correct file here//
        cutTree((pathname+filename+ext).c_str(), decaytreename, (filename2+ext).c_str(),"1");//"Bplus_MM<300000");
	...
	out << "$\\epsilon_{qmin}$ &" << qmincut <<" \\\\ "<<endl;
	...
	out << "${\\epsilon^{GEN,minq^{2}}}\\times {\\epsilon^{REC,minq^{2}}}$  &" << try1_err << " \\\\ "<<endl;
        ...
        FillEffInfo(fillinfotree, "strip",double(qmincut),true);
