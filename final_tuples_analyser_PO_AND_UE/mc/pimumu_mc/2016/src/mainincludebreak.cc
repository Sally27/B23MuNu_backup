...
int main(){
	...
	ofstream out;
	out.open("Efficiencies_2016_pimumu.tex");
	...
	TFile *fillinfo = new TFile("Efficiencies_2016_Pimumu.root","RECREATE");
...
//
	...
	out << "${\\epsilon^{GEN,minq^{2}}}\\times {\\epsilon^{REC,minq^{2}}}$  &" << try1_err << " \\\\ "<<endl;
        FillEffInfo(fillinfotree, "effgentimeseffreco",try1_err.val,true);
