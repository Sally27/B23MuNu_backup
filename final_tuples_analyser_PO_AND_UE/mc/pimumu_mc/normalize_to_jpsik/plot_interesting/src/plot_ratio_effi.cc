...
int main()
{
        ...
        ofstream out;
        out.open("Summary_plot_individualeff_ratio.tex");
	...
	string filename="/vols/lhcb/ss4314/SingleEventSensitivity/SesFUMSB_NOTsimultaneous_NewTCK/bin/info_SES.root";
...
//		treenames.push_back(treename2);
		...
		string plotvariable2=variablestoplot.at(z+1);
		...
		for(int i(0);i<treenames.size();i++){
			...
			double numofevents(1.0);
                        double numofevents2(1.0);
			tree->SetBranchAddress("year", &year);
			tree->SetBranchAddress(plotvariable.c_str(), &numofevents);
                        tree->SetBranchAddress(plotvariable2.c_str(), &numofevents2);
			...
			for (int j(0);j<tree->GetEntries();j++)
			{
				...
				cout<<" Year "<<year<<endl;
				cout<<" Value "<<numofevents/numofevents2<<endl;
                                out<<" & "<<numofevents;              
				variable.push_back(numofevents/numofevents2);
			}
		...
		const Int_t n = 2;
		mg->SetTitle("Efficiency Overview Ratio");
	...
	mg->Draw("alp");
	mg->GetXaxis()->SetTitle("year");
	mg->GetYaxis()->SetTitle("Efficiency Ratio - Sig/Norm");
	//c1->Update();
        c1->BuildLegend(); 
	c1->SaveAs("Plot_ALL_Efficiencies_Ratio_Overview.pdf");
	delete c1;
