...
int main(){
	...
	ofstream out;
	out.open("EfficiencyComparison_compared_toNormalisation_nove.tex");
	...
	out<<"\\hline"<<endl;
	out<<"$\\varepsilon$ & $B^+ \\rightarrow J/\\psi K^{+}$  & $B^+ \\rightarrow \\pi^- \\mu^+ \\mu^{-}$ \\\\ "<<endl;
	out<<"\\hline"<<endl;
	out<<"\\hline"<<endl;
...
//        alleff.push_back(effmyrecosig); 
	...
	number_bkk_nor=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "numbkk");
	decprodcutjpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "effgen");
//	effstripjpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "effreco");
	effrecostrjpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "effreco");
	//effmyrecojpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "toteff");
	double efftriggerjpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "trigger")*returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "L0");
	double effjpsijpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "jpsi");
//	double effqmin=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "qmincut");

	combibdtjpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU_SECPART.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "combibdt");
	misidbdtjpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU_SECPART.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "misidbdt");
        double fitrangejpsik(0);
        fitrangejpsik=returnEff(("../tuples/Efficiencies_"+i2s(years.at(1))+"_PIMUMU_SECPART.root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "fitrange");
...
//        alleff_pr.push_back(effqmin);
	...
	out << "Events in Bookeeping &" << round_to_digits(number_bkk_sig,7) << " & " << round_to_digits(number_bkk_nor,7) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
	out << "$\\varepsilon_{strip}$ &" << round_to_digits(effstripsig,3) << "(reco) & " <<" - \\\\ "<<endl;
	out << "$\\varepsilon_{mctruth}$ &" << round_to_digits(effrecostrsig,3) << "(mctruth) & " << round_to_digits(effrecostrjpsik,3) <<" (reco) \\\\ "<<endl; 
...
//	out << "$\\varepsilon_{qmin}$ &" << " - " << " & " << round_to_digits(effqmin,3) <<" \\\\ "<<endl;
	...
	out<<"\\hline"<<endl;
	out << "$\\varepsilon_{generator}$ &" << round_to_digits(decprodcutsig,3) << " & " << round_to_digits(decprodcutjpsik,3) <<" \\\\ "<<endl;
...
//        out << "$2.0\\times f(b\\rightarrow B^{+})$ "<<"& 2$\\times$"<<fragfrac.val<<"$\\pm$"<<fragfrac.err<<"& 2$\\times$"<<fragfrac.val<<"$\\pm$"<<fragfrac.err<<"& 2$\\times$"<<fragfrac.val<<"$\\pm$"<<fragfrac.err<<" \\\\ "<<endl;
        out << "$\\mathcal{B}(B^+ \\rightarrow \\pi^- \\mu^+ \\mu^{-})$ & "<<round_to_digits((overall.calculateProduct_BR("pr").val),3)<<"$\\pm$"<<round_to_digits((overall.calculateProduct_BR("pr").err),3)<<" &"<< round_to_digits((overall.calculateProduct_BR("pr").val),3)<<"$\\pm$"<<round_to_digits((overall.calculateProduct_BR("pr").err),3)<<" \\\\ "<<endl;
...
//        TTree fillinfotree("PR_Yield","PR_Yield");
        ...
        out << "N($B^+ \\rightarrow \\pi^- \\mu^+ \\mu^{-}$) "<<(group.at(i))<<" & "<<"\\textbf{"<<round_to_digits(relnum.val,3)<<"$\\pm$"<<round_to_digits(relnum.err,3)<<"}"<<" & "<<"\\textbf{"<<round_to_digits(relnum_2016.val,3)<<"$\\pm$"<<round_to_digits(relnum_2016.err,3)<<"}"<<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
     ...
     //   fillinfotree.Write("",TObject::kOverwrite);
        ...
        out<<"\\end{tabular}"<<endl;
        out<<"\\end{center}"<<endl;
        out<<"\\caption{Calculation of number of events that comes from rare $B^+ \\rightarrow \\pi^- \\mu^+ \\mu^{-}$ backgrounds, assuming 2012 efficiencies but extrapolating to 2011,2012,2016 samples. $\\mathcal{B}(B^+ \\rightarrow K^+ \\pi^- \\mu^+ \\mu^{-})$ is obtained from PDG $B^+ \\rightarrow \\pi^- \\mu^+ \\mu^{-}$ = "<<round_to_digits(overall.calculateProduct_BR("pr").val,3)<<"$\\pm$"<< round_to_digits(overall.calculateProduct_BR("pr").err,3)<<". Normalisation channel $B^+ \\rightarrow (J/\\psi \\rightarrow \\mu^+ \\mu^{-}) K^{+}$ was used to normalize to using Run1 and 2016 yields. Assumes PIDratio 1 and qmin cut needs to be accounted for}"<<endl;
