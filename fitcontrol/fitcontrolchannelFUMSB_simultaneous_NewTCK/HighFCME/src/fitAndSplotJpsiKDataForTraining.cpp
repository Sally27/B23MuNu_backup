...
void FitAndSplotJpsiKDataForTraining::fitJpsiKDataHypathia3(bool binnedFit, string type, string dataset)
{
   ...
   TFile* f_pi = new TFile((plotdir+"myworkspaceWEIGHT_pimumu_"+type+".root").c_str());//for now remove type
   RooWorkspace* ws_pi = (RooWorkspace*)f_pi->Get("w");


//   RooRealVar* cbmeanrar=ws_pi->var("picbmeanrar_"); //for now remove type
//   RooRealVar* cbsigmarar=ws_pi->var("picbsigmarar_");//,0.1,50) ;
//   RooRealVar* cbsignalrar=ws_pi->var("picbsignalrar_") ;
//   RooRealVar* nral=ws_pi->var("pinral_");
//   RooRealVar* alpharal=ws_pi->var("pialpharal_");
//   RooRealVar* nrar=ws_pi->var("pinrar_");
//   RooRealVar* alpharar=ws_pi->var("pialpharar_");
//   MyCB blah("piblah", "piblah", *Bplus_MM, *cbmeanrar, *cbsigmarar, *nral , *alpharal, nrar, *alpharar);


   RooRealVar* cbmeanrar=ws_pi->var(("picbmeanrar_"+type).c_str()); //for now remove type
