...
void FitAndSplotJpsiKDataForTraining::plotprettyhypathia()
{ 
 ...
 //  model_total->paramOn(frame);
   ...
   canvTot.Print((plotdir+"HypatiaplotJpsiKFitPretty.pdf").c_str());
   canvTot.Print((plotdir+"HypatiaplotJpsiKFitPretty.root").c_str());

   canv.SetLogy();
   canvTot.cd(1);
   canv.DrawClonePad();
   canvTot.Print((plotdir+"HypatiaplotJpsiKFitLogyPretty.pdf").c_str());
   canvTot.Print((plotdir+"HypatiaplotJpsiKFitLogyPretty.root").c_str());
...
vector<double> FitAndSplotJpsiKDataForTraining::sigfitnoweight(string type)
{ 
...
vector<double> FitAndSplotJpsiKDataForTraining::sigfithypathia(string weightname, string type)
{ 
...
//        RooRealVar n2coeff(("n2coeff_"+type).c_str(),"l",3.35,10.0);
	...
	canv4->cd(3) ; gPad->SetLeftMargin(0.15) ; frame10->GetYaxis()->SetTitleOffset(1.4) ; frame10->Draw() ;
	canv4->cd(4) ; gPad->SetLeftMargin(0.15) ; frameun->GetYaxis()->SetTitleOffset(1.4) ; frameun->Draw() ;
	canv4->SaveAs((plotdir+"mcsigfit_HYP_NOTMAIN_"+type+".pdf").c_str());

        TCanvas* canvlog = new TCanvas("new","log",800,800) ;
        
        frame8->GetYaxis()->SetRangeUser(0.000001,60000) ;
        canvlog->SetLogy() ; gPad->SetLeftMargin(0.15) ; frame8->GetYaxis()->SetTitleOffset(1.4) ; frame8->Draw() ;
        canvlog->SaveAs((plotdir+"mcsigfit_HYP_NOTMAIN_LOGY_"+type+".pdf").c_str());
        delete canvlog;
...
vector<double> FitAndSplotJpsiKDataForTraining::sigfit(string weightname, string type)
{ 
...
void FitAndSplotJpsiKDataForTraining::fitJpsiKDataHypathia2(bool binnedFit, string type)
{
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   RooRealVar sig("sig","sig",40000 ,20000,70000);
