...
//string pidcut = "Pi_isMuon==1.0";// && Pi_nShared==0";// && Pi_ProbNNmu>0.5";
...
for(int i(0); i<13 ;i++) //number of bins wanted to be iterated above in momentum
{
	for(int j(0); j<1 ;j++){ //number of bins wanted to be iterated above in eta
		...
		run.calculateSweightsInBins("", pbin, etabin, pidcut, kinbinstring, pidcut, out);
                ...
                run.makeMasterTable("", pbin, etabin, pidcut, kinbinstring, pidcut, out3, i+1, j+1, "PerfHists_Pi_Strip21_MagDown_Jack_P_ETA.root", pidcalibcut);
	}
}
out<<"\\hline"<<endl;
...
//plotMergedFits(binningx, binningy,"Pi_isMuon==1.0","After","/vols/lhcb/ss4314/fitjpsikst/fitjpsikst_idPION_jackbin/NOFCME/workspace");
//
//out<<"\\hline"<<endl;
//out<<"\\end{tabular}"<<endl;
//out<<"\\end{center}"<<endl;
