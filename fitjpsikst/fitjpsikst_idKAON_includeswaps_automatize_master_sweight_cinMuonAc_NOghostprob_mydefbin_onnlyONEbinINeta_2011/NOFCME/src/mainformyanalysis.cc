...
//        TFile* f_info = new TFile((piddir+"/PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2012.root").c_str(),"UPDATE");
	...
	for(int i(0); i<allcuts.size() ;i++)
	{
		...
		TH2D *eff =(TH2D*)f_pid->Get("PIDHist_Sweights");
                ...
                TFile* f_info = new TFile("../PIDtables/PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2011.root","UPDATE");
	...
	for(int p(0); p<(binningy.size()-1); p++){        
			...
			for(int i(0); i<allcuts.size() ;i++)
			{
                        	...
                        	holder52=run_other.readEfficiency2DHist_all_formyanalysis("PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2011.root",allcuts.at(i),("PIDHist_Sweights_"+cleanNameString(allcuts.at(i))).c_str(), p);
				holder5_err2=run_other.readEfficiency2DHist_all_err_formyanalysis("PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2011.root",allcuts.at(i),("PIDHist_Sweights_"+cleanNameString(allcuts.at(i))).c_str(), p);
...
//        mg->GetXaxis()->SetRangeUser(0.,100000.0);
	...
	cv->BuildLegend(0.1,0.5,0.9,0.9);
        cv->SaveAs("Visualize_Weights_PionMisid_2011.pdf");    
