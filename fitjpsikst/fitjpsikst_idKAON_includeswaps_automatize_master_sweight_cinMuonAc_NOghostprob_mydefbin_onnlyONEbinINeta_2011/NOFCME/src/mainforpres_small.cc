...
//        holder5_err=run.readEfficiency2DHist_all_err(pidcut,"PIDHist_RatioSweights");
	...
	TCanvas *cv = new TCanvas("set1_cv", "set1_cv",200,10,700,500);




	TLegend *leg = new TLegend(.0,0.7,1.0,1.0);
   	leg->SetTextSize(0.038);
   	leg->SetBorderSize(0);
	leg->SetFillColor(0);
        TMultiGraph *mg = new TMultiGraph(); 
//   	mg->SetTextSize(0.032);
...
//                                delete set1_gr;
	...
	mg->GetYaxis()->SetTitle("Ratio of #pi misID: B #rightarrow J/#psi K^{*0} / PIDCalib");
//	cv->BuildLegend(0.1,0.5,0.8,0.9);
//        cv->BuildLegend(0.82,0.2,1.2,0.9);
        cv->SaveAs("Visualize_Ratios_2011_PionMisid_small.pdf");    
        ...
        TCanvas *cv2 = new TCanvas("set1_cv2", "set1_cv2",1000,500);
//        cv->BuildLegend(0.82,0.2,1.2,0.9);
        leg->Draw();
        canvTot.cd(2);
//        cv->BuildLegend(0.1,0.1,0.9,0.9);
        leg->Draw();
        gPad->SetPad(0.005, 0.005, 0.995, 0.2);
        cv2->DrawClonePad();
 
        canvTot.SaveAs("Visualize_Ratios_2011_PionMisid_small_2.pdf");
        cv2->SaveAs("Visualize_Ratios_2011_PionMisid_small_legend.pdf");
        cv2->SaveAs("Visualize_Ratios_2011_PionMisid_small_legend.root");
