...
void FitAndSplotJpsiKDataForTraining::plotprettyjpsikstrookeys_differentcolor(string dataset)
{ 
   ...
   B0_MM->setBins(60);
   B0_MM->setUnit("MeV/c^{2}");
//   RooRealVar B0_MM_Unit("B0_DTFM_M_zero",”B0 mass”,5.2794,”GeV”);
...
//   model_total->plotOn(frame, RooFit::Components("sigblah"), RooFit::Name("signal"), RooFit::FillColor(95), RooFit::LineColor(95), RooFit::DrawOption("F"));
   model_total->plotOn(frame, RooFit::Components("expo,sigBs,kest2,swap"), RooFit::Name("swapnum"),RooFit::FillColor(94), RooFit::LineColor(94), RooFit::DrawOption("F"));
   model_total->plotOn(frame, RooFit::Components("expo,sigBs,kest2"), RooFit::Name("lam"),RooFit::FillColor(99), RooFit::LineColor(99), RooFit::DrawOption("F"));
   model_total->plotOn(frame, RooFit::Components("expo,sigBs"), RooFit::Name("bs"), RooFit::FillColor(98), RooFit::LineColor(98), RooFit::DrawOption("F"));
   ...
   model_total->plotOn(frame, RooFit::Components("sigblah"), RooFit::Name("signal"), RooFit::LineStyle(kDashed), RooFit::LineColor(kBlack));

   TLegend *leg = new TLegend(0.6,0.73,0.9,0.92);
   leg->AddEntry(frame->findObject("signal"),"B^{0} #rightarrow J/#psi K^{*}","l");
   leg->AddEntry(frame->findObject("swapnum"),"K #pi swaps","l");

   leg->AddEntry(frame->findObject("lam"),"#Lambda_{b} #rightarrow J/#psi K^{-} p","l");
   leg->AddEntry(frame->findObject("bs"),"B_{s} #rightarrow J/#psi K^{*}","l");
   leg->AddEntry(frame->findObject("combinatorial"),"Combinatorial","l");

   TCanvas canv("canv", "canv", 600, 600);
   frame->GetXaxis()->SetTitle("M^{constr}(J/#psi K^{+} #pi^{-}) [MeV/c^{2}]");
   ...
   canvTot.Print((plotdir+"plotJpsiKstFitPretty_nicecolor.pdf").c_str());
   canvTot.Print((plotdir+"plotJpsiKstFitPretty_nicecolor.root").c_str());

   canv.SetLogy();
   canvTot.cd(1);
   canv.DrawClonePad();
   canvTot.Print((plotdir+"plotJpsiKstFitLogyPretty_nicecolor.pdf").c_str());
   canvTot.Print((plotdir+"plotJpsiKstFitLogyPretty_nicecolor.root").c_str());

   delete leg;
   fw.Close();
   fpull.Close();
}
vector<double> FitAndSplotJpsiKDataForTraining::readEfficiency2DHist_all_err_formyanalysis(string filename, string PIDcut, string branchname, int etabin){

   TFile* f_pid = new TFile((piddir+"/"+filename).c_str(),"READ");
   TH2D *eff =(TH2D*)f_pid->Get(branchname.c_str());
...
//   after->SetBinContent(binx+1,biny+1,sig.getVal());
   ...
   for (int i(1); i<=eff->GetNbinsX();i++ ){ 
	   ...
	   for (int j(etabin+1); j<(etabin+2) ;j++ ){
		   //cout<<"In bin "<<binx<<" and "<<biny<<" number of sig events"<< after->GetBinContent(binx+1,biny+1)<<endl;
		   cout<<"Efficiency "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<<eff->GetBinError(i,j)<<" \\\\ " <<endl;
//                   cout<<"After "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<< after->GetBinContent(i,j)<<" \\\\ " <<endl;
                    values.push_back(eff->GetBinError(i,j)); 
	   }
   }
...
vector<double> FitAndSplotJpsiKDataForTraining::readEfficiency2DHist_all_formyanalysis(string filename, string PIDcut, string branchname, int etabin){
...
//   after->SetBinContent(binx+1,biny+1,sig.getVal());
   ...
   for (int i(1); i<=eff->GetNbinsX();i++ ){ 
	   ...
	   for (int j(1+etabin); j<(etabin+2) ;j++ ){
		   //cout<<"In bin "<<binx<<" and "<<biny<<" number of sig events"<< after->GetBinContent(binx+1,biny+1)<<endl;
...
//                   cout<<"After "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<< after->GetBinContent(i,j)<<" \\\\ " <<endl;
                    values.push_back(eff->GetBinContent(i,j)); 
	   }
   }
...
void FitAndSplotJpsiKDataForTraining::readPIDCalibEfficiency2DHist(string pidcut){

   TFile* f_pid = new TFile((pidcalibdir+"/"+pidcalibfiledo).c_str(),"READ");
...
vector<double> FitAndSplotJpsiKDataForTraining::readEfficiency2DHist_all(string PIDcut, string branchname, int etabin){

   TFile* f_pid = new TFile((piddir+"/PID_2D_ALL_INFO_"+cleanNameString(PIDcut)+".root").c_str(),"READ");
   TH2D *eff =(TH2D*)f_pid->Get(branchname.c_str());
//   TH2D *after =(TH2D*)f_pid->Get("PIDHist_after");
   cout<<"Number of X bins "<<eff->GetNbinsX()<<endl;
   cout<<"Number of Y bins "<<eff->GetNbinsY()<<endl;
...
vector<double> FitAndSplotJpsiKDataForTraining::readEfficiency2DHist_all_err(string PIDcut, string branchname, int etabin){
...
//   after->SetBinContent(binx+1,biny+1,sig.getVal());
   ...
   for (int i(1); i<=eff->GetNbinsX();i++ ){ 
	   ...
	   for (int j(etabin+1); j<(etabin+2) ;j++ ){
		   //cout<<"In bin "<<binx<<" and "<<biny<<" number of sig events"<< after->GetBinContent(binx+1,biny+1)<<endl;
		   cout<<"Efficiency Err "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<<eff->GetBinError(i,j)<<" \\\\ " <<endl;
//                   cout<<"After "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<< after->GetBinContent(i,j)<<" \\\\ " <<endl;
                    values.push_back(eff->GetBinError(i,j)); 
	   }
   }
...
void FitAndSplotJpsiKDataForTraining::prepare2DhistMyAndPIDCalibRatio(string pidcut, string mypiddir, string mypidfile)
{
...
//   delete f_pid; 
   ...
   TFile* f_pidmine = new TFile((mypiddir+"/"+mypidfile).c_str(),"READ");
   ...
   for (int i(1); i<=before->GetNbinsX();i++ ){ 
	   ...
	   for (int j(1); j<=before->GetNbinsY();j++ ){
		   cout<<"PIDCalib "<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<< eff->GetBinContent(i,j)<<" \\\\ " <<endl;
                   ...
                   errorratio=double(ratio->GetBinContent(i,j))*sqrt(((eff->GetBinError(i,j)/eff->GetBinContent(i,j))*(eff->GetBinError(i,j)/eff->GetBinContent(i,j)))+((myeff->GetBinError(i,j)/myeff->GetBinContent(i,j))*(myeff->GetBinError(i,j)/myeff->GetBinContent(i,j))));
                   cout<<"The ratio of errors"<<errorratio<<endl;
                   out2<<xAxis->GetBinLowEdge(i)<<" & "<<xAxis->GetBinUpEdge(i)<<" & "<<yAxis->GetBinLowEdge(j)<<" & "<<yAxis->GetBinUpEdge(j)<<" & "<<eff->GetBinContent(i,j)<<"$\\pm$"<<eff->GetBinError(i,j)<<" & "<<myeff->GetBinContent(i,j)<<"$\\pm$"<<myeff->GetBinError(i,j)<<" & "<<ratio->GetBinContent(i,j)<<"$\\pm$"<<errorratio<<" \\\\ " <<endl;
	   }
   }
...
void FitAndSplotJpsiKDataForTraining::fitJpsiKstDataRookeysHyp(bool binnedFit, string dataset, string cut)
{
   ...
   TFile* f_fit_swap = new TFile((plotdir+"myworkspace_SwapsMCFit.root").c_str());
   RooWorkspace* ws_swap = (RooWorkspace*)f_fit_swap->Get("w");

   RooRealVar* cbmeanrar=ws_swap->var("cbmeanrar");//,"l",-2.44, -3.0, -2.1);
   RooRealVar* cbsigmarar=ws_swap->var("cbsigmarar");//,"l",1.0e-5, 1.0e-8, 1.0);
   RooRealVar* nral=ws_swap->var("nral");//;,"l",-2.20960e-04,-3.00960e-04,-1.20960e-04);
   RooRealVar* alpharal=ws_swap->var("alpharal");//,"l",8.37, 6.0, 9.0);//,8.0,10.0);//,0.1,50) ;
   RooRealVar* nrar=ws_swap->var("nrar");//,"l", 5280.0, 5278.0, 5282.0);
   RooRealVar* alpharar=ws_swap->var("alpharar");//,"l",2.75, 2.0, 3.0);
   
    if(!ws_swap)// || !cbmeanrar_lam || !cbsigmarar_lam || !nral_lam || !alpharal_lam || !nrar_lam || !alpharar_lam)
   {
      cerr<<"ERROR: in function FitAndSplotJpsiKDataForTraining, error downloading stuff from workspace"<<endl;
      return;
   }
   ...
   MyCB swap("swap", "swap", *B0_DTFM_M_zero, *cbmeanrar, *cbsigmarar, *nral , *alpharal, *nrar, *alpharar);
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   RooRealVar sig("sig","sig",140000.0,1000.0,250000);
   RooRealVar bkg("bkg","bkg",5000.0, 100.0 ,100000) ;
   RooRealVar Bs("Bs","Bs",800, 500.0 ,15000) ;
   RooRealVar Lam("Lam","Lam",5000.0, 100.0,30000) ;
   RooRealVar SwapNum("SwapNum","SwapNum",5000.0, 100.0,30000) ;

//   RooAddPdf sigCB_B0("sigCB0","sigCB0",RooArgList( cb1, cb2),RooArgList( vc1)) ;
   RooAddPdf model_total("model","model", RooArgList(blah, expo, sigBs, *kest2, swap) ,RooArgList(sig, bkg, Bs, Lam, SwapNum));
   ...
   model_total.plotOn(frame, RooFit::LineColor(kOrange), RooFit::Components("swap"));
   TCanvas canv("canv", "canv", 600, 600);
...
{ 
 ...
 //  model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("piblah"), RooFit::Name("pimumu"));
   ...
   model_total->plotOn(frame, RooFit::LineColor(kOrange), RooFit::Components("swap"), RooFit::Name("swapnum"));
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   RooRealVar sig("sig","sig",1000.0,10.0,250000);
   RooRealVar bkg("bkg","bkg",500.0, 1.0 ,100000) ;
   RooRealVar Bs("Bs","Bs",800, 1.0 ,15000) ;
   RooRealVar Lam("Lam","Lam",500.0, 1.0,30000) ;
   RooRealVar SwapNum("SwapNum","SwapNum",500.0, 1.0,30000) ;
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   ...
   RooRealVar SwapNum("SwapNum","SwapNum",5000.0, 1.0,30000) ;
...
vector<double> FitAndSplotJpsiKDataForTraining::fitswaps(string cut, string PIDcut)
{ 


        TFile* file_sig = new TFile((plotdir+"/MCSwaps_WS_prepared.root").c_str());
...
//		return(1.0);
	...
	RooRealVar cbmeanrar("cbmeanrar","cbmeanrar", 5283.0, 5260.8, 5330.0);
	RooRealVar cbsigmarar("cbsigmarar","cbsigmarar",8.6,5.0,100.0);//,0.1,50) ;
	RooRealVar cbsignalrar("cbsignalrar","cbsignalrar",5.0,0.0,10.0) ;
	RooRealVar nral("nral","",1.0,0.1,3.0);
	RooRealVar alpharal("alpharal","",1.0,0.1,10.0);
	RooRealVar nrar("nrar","",1.0,0.1,5.0);
	RooRealVar alpharar("alpharar","",1.0,0.1,10.0);

	MyCB blah("swap", "swap", *B0_DTFM_M_zero, cbmeanrar, cbsigmarar, nral , alpharal, nrar, alpharar);
	...
	canv4->SaveAs((plotdir+"mcswapsfit"+cleanNameString(cut)+"_"+cleanNameString(PIDcut)+".pdf").c_str());
        ...
        TFile *fm = new TFile((plotdir+"SwapsMCFit.root").c_str(),"RECREATE");
        canvnew->Write();
        fm->Close();
        delete fm;
        delete canvnew;
	...
	TFile wrkspc8((plotdir+"myworkspace_SwapsMCFit.root").c_str(),"RECREATE"); 
...
void FitAndSplotJpsiKDataForTraining::prepareWorkspaceSwapsMC()
{
   TFile f((mcswapdir+"/"+mcswapfile+".root").c_str());
   ...
   TFile fws((plotdir+"/MCSwaps_WS_prepared.root").c_str(), "RECREATE");
...
void FitAndSplotJpsiKDataForTraining::prepareWorkspaceSwapsMC(string cutP, string cutETA, string cutPID)
{
   TFile f((mcswapdir+"/"+mcswapfile+".root").c_str());
   TTree* t = (TTree*)f.Get(treename.c_str());
   TFile OKAPI("OKAPI.root", "RECREATE");
...
void FitAndSplotJpsiKDataForTraining::prepareWorkspaceLambdaMC()
{
   ...
   cout<<"Number of entries before is "<<t->GetEntries()<<endl;
   cout<<"Number of entries after is "<<t1->GetEntries()<<endl;
...
void FitAndSplotJpsiKDataForTraining::makeMasterTable(string extracut, string pbin, string etabin, string pidcut, string kinbinstring, string pidstring, ofstream &out, int i, int j,string pidcalibcut)
{
   ...
   tnocut->SetBranchAddress("SwapNum_sw", &SwapNum_sw);
   ...
   for(int i(0); i<tnocut->GetEntries(); ++i)
   {
    ...
    tnocut->GetEntry(i);
    total+=sig_sw+bkg_sw+Bs_sw+Lam_sw+SwapNum_sw;
    err+=(sig_sw+bkg_sw+Bs_sw+Lam_sw+SwapNum_sw)*(sig_sw+bkg_sw+Bs_sw+Lam_sw+SwapNum_sw);
   }
   ...
   tpidcut->SetBranchAddress("SwapNum_sw", &SwapNum_sw_pid);

   for(int i(0); i<tpidcut->GetEntries(); ++i)
   {
    
    tpidcut->GetEntry(i);
    total_pid+=sig_sw_pid+bkg_sw_pid+Bs_sw_pid+Lam_sw_pid+SwapNum_sw_pid;
//    err_pid+=(sig_sw_pid*sig_sw_pid)+(bkg_sw_pid*bkg_sw_pid)+(Bs_sw_pid*Bs_sw_pid)+(Lam_sw_pid*Lam_sw_pid);
    err_pid+=(sig_sw_pid+bkg_sw_pid+Bs_sw_pid+Lam_sw_pid+SwapNum_sw_pid)*(sig_sw_pid+bkg_sw_pid+Bs_sw_pid+Lam_sw_pid+SwapNum_sw_pid);
   }
...
//   errorratio=sqrt( TMath::Abs(  ((1.-2.*total_pid/total)*(sqrt(err_pid))*(sqrt(err_pid)) + (total_pid)*(total_pid)*(sqrt(err))*(sqrt(err)) / (total*total) ) / (total*total) ) );
   ...
   TFile* f_pidcalib = new TFile((pidcalibdir+"/"+pidcalibfiledo).c_str(),"READ");
   TH2D *eff_pidcalib =(TH2D*)f_pidcalib->Get(pidcalibcut.c_str());
   cout<<"Number of X bins "<<eff_pidcalib->GetNbinsX()<<endl;
   cout<<"Number of Y bins "<<eff_pidcalib->GetNbinsY()<<endl;
   TAxis* xAxis_pidcalib = eff_pidcalib->GetXaxis();
   TAxis* yAxis_pidcalib = eff_pidcalib->GetYaxis();
   
   double errorrefit(0);
   errorrefit=normalerror(eff->GetBinContent(i,j), eff->GetBinError(i,j),eff_pidcalib->GetBinContent(i,j), eff_pidcalib->GetBinError(i,j)); 
  
   double errorsweight(0);
   errorsweight=normalerror(double(total_pid)/double(total), errorratio, eff_pidcalib->GetBinContent(i,j), eff_pidcalib->GetBinError(i,j)); 
//   out<<cleanAbsolutelyNameString(kinbinstring)<<" & "<<d2s(double(total_pid)/double(total))<<" $\\pm$ "<<errorratio<<" & "<<eff->GetBinContent(i,j)<<" $\\pm$ "<<eff->GetBinError(i,j)<<"  \\\\ "<<endl;
   out<<cleanAbsolutelyNameString(kinbinstring)<<" & "<<d2s(double(total_pid)/double(total))<<" $\\pm$ "<<errorratio<<" & "<<eff->GetBinContent(i,j)<<" $\\pm$ "<<eff->GetBinError(i,j)<<" & "<<d2s((double(total_pid)/double(total))/(eff_pidcalib->GetBinContent(i,j)))<<" $\\pm$"<<errorsweight<<" & "<<d2s(eff->GetBinContent(i,j)/eff_pidcalib->GetBinContent(i,j))<<" $\\pm$"<<errorrefit<<"  \\\\ "<<endl;
    

   TFile* f_info = new TFile((piddir+"/PID_2D_ALL_INFO_"+cleanNameString(pidstring)+".root").c_str(),"UPDATE");
   TH2D *MyRefit =(TH2D*)f_info->Get("PIDHist_MyRefit");
   TH2D *Sweights =(TH2D*)f_info->Get("PIDHist_Sweights");
   TH2D *PIDCalib =(TH2D*)f_info->Get("PIDHist_PIDCalib");
   TH2D *RatioSweights =(TH2D*)f_info->Get("PIDHist_RatioSweights");
   TH2D *RatioMyRefit =(TH2D*)f_info->Get("PIDHist_RatioMyRefit");


   cout<<"Number of X bins "<<MyRefit->GetNbinsX()<<endl;
   cout<<"Number of Y bins "<<MyRefit->GetNbinsY()<<endl;
   MyRefit->SetBinContent(i,j,eff->GetBinContent(i,j));
   MyRefit->SetBinError(i,j,eff->GetBinError(i,j));
   Sweights->SetBinContent(i,j,double(total_pid)/double(total));
   Sweights->SetBinError(i,j,errorratio);
   PIDCalib->SetBinContent(i,j,eff_pidcalib->GetBinContent(i,j));
   PIDCalib->SetBinError(i,j,eff_pidcalib->GetBinError(i,j));
   RatioSweights->SetBinContent(i,j,(double(total_pid)/double(total))/(eff_pidcalib->GetBinContent(i,j)));
   RatioSweights->SetBinError(i,j,errorsweight);
   RatioMyRefit->SetBinContent(i,j,eff->GetBinContent(i,j)/eff_pidcalib->GetBinContent(i,j));
   RatioMyRefit->SetBinError(i,j,errorrefit); 

   MyRefit->Write("", TObject::kOverwrite);
   Sweights->Write("", TObject::kOverwrite);
   PIDCalib->Write("", TObject::kOverwrite);
   RatioSweights->Write("", TObject::kOverwrite);
   RatioMyRefit->Write("", TObject::kOverwrite);
...
void FitAndSplotJpsiKDataForTraining::prepare2DhistforAll(string name, vector<double> xbin, vector<double> ybin, string PIDcut)
{
	...
	TH2* h_before = new TH2D("PIDHist_MyRefit", "PIDHist_MyRefit", XBINS, PArr, YBINS, EtaArr);

        TH2* h_after =(TH2*)h_before->Clone(); 
        h_after->SetName("PIDHist_Sweights");

        TH2* h_PIDCalib =(TH2*)h_before->Clone(); 
        h_PIDCalib->SetName("PIDHist_PIDCalib");

        TH2* h_RatioSweight =(TH2*)h_before->Clone(); 
        h_RatioSweight->SetName("PIDHist_RatioSweights");

        TH2* h_RatioMyRefit =(TH2*)h_before->Clone(); 
        h_RatioMyRefit->SetName("PIDHist_RatioMyRefit");
	...
	for (int i=1; i<=XBINS; i++) {
		for (int j=1; j<=YBINS; j++) {
				h_before->SetBinContent(i,j,0.0);
                                ...
                                h_PIDCalib->SetBinContent(i,j,0.0);
                                h_RatioSweight->SetBinContent(i,j,0.0);
                                h_RatioMyRefit->SetBinContent(i,j,0.0);
				z++;
		}
	}
	...
	for (int i=1; i<=XBINS; i++) {
        ...
        TFile *F1 = new TFile((piddir+"/PID_2D_ALL_INFO_"+cleanNameString(PIDcut)+".root").c_str(),"UPDATE");
        h_before->Write("", TObject::kOverwrite);
        h_after->Write("", TObject::kOverwrite);
        h_PIDCalib->Write("", TObject::kOverwrite);
        h_RatioSweight->Write("", TObject::kOverwrite);
        h_RatioMyRefit->Write("", TObject::kOverwrite);
        ...
        delete h_after;
        delete h_PIDCalib;
        delete h_RatioSweight;
        delete h_RatioMyRefit; 
        delete canv;
...
void FitAndSplotJpsiKDataForTraining::calculateSweightsInBins(string extracut, string pbin, string etabin, string pidcut, string kinbinstring, string pidstring, ofstream &out)
{
   ...
   for(int i(0); i<tpidcut->GetEntries(); ++i)
   {
    ...
    total_pid+=sig_sw_pid+bkg_sw_pid+Bs_sw_pid+Lam_sw_pid+SwapNum_sw_pid;
//    err_pid+=(sig_sw_pid*sig_sw_pid)+(bkg_sw_pid*bkg_sw_pid)+(Bs_sw_pid*Bs_sw_pid)+(Lam_sw_pid*Lam_sw_pid);
    err_pid+=(sig_sw_pid+bkg_sw_pid+Bs_sw_pid+SwapNum_sw_pid)*(sig_sw_pid+bkg_sw_pid+Bs_sw_pid+Lam_sw_pid+SwapNum_sw_pid);
   }
...
void FitAndSplotJpsiKDataForTraining::makeSWeightedTree(string extracut, string label, string tuplename)
{
   ...
   RooRealVar* SwapNum = workspaceFit->var("SwapNum");   

   if(!data || !model_total || !sig || !bkg || !Bs || !Lam || !SwapNum)
   {
   ...
   RooStats::SPlot wdata("wData", "wData", *data, model_total, RooArgList(*sig, *bkg, *Bs, *Lam, *SwapNum));
...
//   nameNewFile.insert(nameNewFile.size()-5, label);
   ...
   t2->Branch("Bs_sw", &Bs_sw, "Bs_sw/D");
   t2->Branch("Lam_sw", &Lam_sw, "Lam_sw/D");
   t2->Branch("SwapNum_sw", &SwapNum_sw, "SwapNum_sw/D");
   ...
   for(int i(0); i<nEntries; ++i)
   {
      ...
      if(B0_MM > B0_MM_min && B0_MM < B0_MM_max) 
      {
         ...
         Bs_sw = wdata.GetSWeight(j,"Bs");
         Lam_sw = wdata.GetSWeight(j,"Lam");
         SwapNum_sw = wdata.GetSWeight(j,"SwapNum");
         if(ttf.EvalInstance()) t2->Fill();
         ++j;
      }
   }
