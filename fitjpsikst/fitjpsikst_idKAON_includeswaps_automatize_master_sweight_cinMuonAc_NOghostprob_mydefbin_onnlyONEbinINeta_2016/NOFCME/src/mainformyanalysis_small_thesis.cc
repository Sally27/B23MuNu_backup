...
//        allcuts.push_back(basicplusprob);
        ...
        string pidcut_t_isMuon = "Pi_isMuon==1.0";
        string pidcut_t_isMuon_nShared = "Pi_isMuon==1.0 && Pi_isMuonTight==1.0";
	string pidcut_t_both_probnnmu ="Pi_isMuon==1.0 && Pi_isMuonTight==1.0 && Pi_MuonNShared<2";
        string pidcut_t_dllmu= "Pi_isMuon==1.0 && Pi_isMuonTight==1.0 && Pi_MuonNShared<2 && Pi_MC15TuneV1_ProbNNmu>0.5";
        string pidcut_t_nearlycomp = "Pi_isMuon==1.0 && Pi_DLLmu>0";
//        string basic = "Pi_isMuon==1.0 && Pi_DLLmu>0 && Pi_DLLmu-Pi_DLLK>0";
//        string basicplusprob ="Pi_isMuon==1.0 && Pi_DLLmu>0 && Pi_DLLmu-Pi_DLLK>0 && Pi_isMuonTight==1.0";
        string beforelastcut_t="Pi_isMuon==1.0 && Pi_DLLmu>0 && Pi_DLLmu-Pi_DLLK>0 && Pi_isMuonTight==1.0 && Pi_MuonNShared<2";    
...
//        allcut_ts.push_back(basic);
//        allcut_ts.push_back(basicplusprob);
        allcut_ts.push_back(beforelastcut_t);
...
//        TFile* f_info = new TFile((piddir+"/PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2016.root").c_str(),"UPDATE");
	...
	for(int p(0); p<(binningy.size()-1); p++){        
			...
			for(int i(0); i<allcuts.size() ;i++)
			{
                        	...
                        	holder52=run_other.readEfficiency2DHist_all_formyanalysis("PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2016.root",allcuts.at(i),("PIDHist_Sweights_"+cleanNameString(allcuts.at(i))).c_str(), p);
				holder5_err2=run_other.readEfficiency2DHist_all_err_formyanalysis("PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2016.root",allcuts.at(i),("PIDHist_Sweights_"+cleanNameString(allcuts.at(i))).c_str(), p);
...
//        mg->GetXaxis()->SetRangeUser(0.,100000.0);
	...
	mg->GetYaxis()->SetTitle("#pi #rightarrow #mu misID from J/#psi K^{*0}");
        cv->SaveAs("Visualize_Weights_PionMisid_2016_small_thesis.pdf");    
	...
	leg->Draw();
        cv2->SaveAs("Visualize_Weights_PionMisid_2016_small_thesis_legend.pdf");    
