...
void FitAndSplotJpsiKDataForTraining::plotprettyjpsikstrookeys_thesis(string dataset)
{ 
 ...
 //  model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("piblah"), RooFit::Name("pimumu"));
   ...
   canvTot.Print((plotdir+"plotJpsiKstFitPretty_thesis.pdf").c_str());
   canvTot.Print((plotdir+"plotJpsiKstFitPretty_thesis.root").c_str());

   canv.SetLogy();
   canvTot.cd(1);
   canv.DrawClonePad();
   canvTot.Print((plotdir+"plotJpsiKstFitLogyPretty_thesis.pdf").c_str());
   canvTot.Print((plotdir+"plotJpsiKstFitLogyPretty_thesis.root").c_str());
...
//S   TFile* f_pidcalib = new TFile((pidcalibdir+"/"+pidcalibfiledo).c_str(),"READ");
//S   TH2D *eff_pidcalib =(TH2D*)f_pidcalib->Get(pidcalibcut.c_str());
//S   cout<<"Number of X bins "<<eff_pidcalib->GetNbinsX()<<endl;
//S   cout<<"Number of Y bins "<<eff_pidcalib->GetNbinsY()<<endl;
//S   TAxis* xAxis_pidcalib = eff_pidcalib->GetXaxis();
//S   TAxis* yAxis_pidcalib = eff_pidcalib->GetYaxis();
   
//S   double errorrefit(0);
//S   errorrefit=normalerror(eff->GetBinContent(i,j), eff->GetBinError(i,j),eff_pidcalib->GetBinContent(i,j), eff_pidcalib->GetBinError(i,j)); 
  
//S   double errorsweight(0);
//S   errorsweight=normalerror(double(total_pid)/double(total), errorratio, eff_pidcalib->GetBinContent(i,j), eff_pidcalib->GetBinError(i,j)); 
//S   out<<cleanAbsolutelyNameString(kinbinstring)<<" & "<<d2s(double(total_pid)/double(total))<<" $\\pm$ "<<errorratio<<" & "<<eff->GetBinContent(i,j)<<" $\\pm$ "<<eff->GetBinError(i,j)<<"  \\\\ "<<endl;
//S   out<<cleanAbsolutelyNameString(kinbinstring)<<" & "<<d2s(double(total_pid)/double(total))<<" $\\pm$ "<<errorratio<<" & "<<eff->GetBinContent(i,j)<<" $\\pm$ "<<eff->GetBinError(i,j)<<" & "<<d2s((double(total_pid)/double(total))/(eff_pidcalib->GetBinContent(i,j)))<<" $\\pm$"<<errorsweight<<" & "<<d2s(eff->GetBinContent(i,j)/eff_pidcalib->GetBinContent(i,j))<<" $\\pm$"<<errorrefit<<"  \\\\ "<<endl;
...
//S   TH2D *PIDCalib =(TH2D*)f_info->Get("PIDHist_PIDCalib");
//S   TH2D *RatioSweights =(TH2D*)f_info->Get("PIDHist_RatioSweights");
//S   TH2D *RatioMyRefit =(TH2D*)f_info->Get("PIDHist_RatioMyRefit");
...
//S   PIDCalib->SetBinContent(i,j,eff_pidcalib->GetBinContent(i,j));
//S   PIDCalib->SetBinError(i,j,eff_pidcalib->GetBinError(i,j));
//S   RatioSweights->SetBinContent(i,j,(double(total_pid)/double(total))/(eff_pidcalib->GetBinContent(i,j)));
//S   RatioSweights->SetBinError(i,j,errorsweight);
//S   RatioMyRefit->SetBinContent(i,j,eff->GetBinContent(i,j)/eff_pidcalib->GetBinContent(i,j));
//S   RatioMyRefit->SetBinError(i,j,errorrefit); 

   MyRefit->Write("", TObject::kOverwrite);
   Sweights->Write("", TObject::kOverwrite);
//S   PIDCalib->Write("", TObject::kOverwrite);
//S   RatioSweights->Write("", TObject::kOverwrite);
//S   RatioMyRefit->Write("", TObject::kOverwrite);
