...
int main(int argc, char *argv[]){
	...
	string pidcut_both_probnnmu ="Pi_isMuon==1.0 && Pi_PIDmu>0 && (Pi_PIDmu-Pi_PIDK)>0 && Pi_isMuonTight==1.0 && Pi_MuonNShared<2 && Pi_MC15TuneV1_ProbNNmu>0.2";
        ...
        string pidcut_nearlycomp = "Pi_isMuon==1.0 && Pi_PIDmu>0 && (Pi_PIDmu-Pi_PIDK)>0 && Pi_isMuonTight==1.0 && Pi_MuonNShared<2 && Pi_MC15TuneV1_ProbNNmu>0.3";
        string basic = "Pi_isMuon==1.0 && Pi_PIDmu>0 && (Pi_PIDmu-Pi_PIDK)>0 && Pi_isMuonTight==1.0 && Pi_MuonNShared<2 && Pi_MC15TuneV1_ProbNNmu>0.35";
...
//				out<<allcuts.at(i)<<" & ";      
	...
	for(int i(0); i<allcuts.size() ;i++)
	{
		...
		TH2D *eff =(TH2D*)f_pid->Get("PIDHist_Sweights");
                ...
                TFile* f_info = new TFile("../PIDtables/PID_2D_ALL_INFO_FORANALYSIS_PIONMISID_2016.root","UPDATE");
...
//        mg->GetXaxis()->SetRangeUser(0.,100000.0);
	...
	cv->BuildLegend(0.1,0.5,0.9,0.9);
        cv->SaveAs("Visualize_Weights_PionMisid_2016.root");    
        cv->SaveAs("Visualize_Weights_PionMisid_2016.pdf");    
}
