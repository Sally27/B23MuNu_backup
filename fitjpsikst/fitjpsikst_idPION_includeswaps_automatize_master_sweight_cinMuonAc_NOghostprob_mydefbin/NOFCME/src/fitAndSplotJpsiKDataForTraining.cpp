...
void FitAndSplotJpsiKDataForTraining::makeMasterTable(string extracut, string pbin, string etabin, string pidcut, string kinbinstring, string pidstring, ofstream &out, int i, int j, string pidcalibfile ,string pidcalibcut)
{
...
//   errorratio=sqrt( TMath::Abs(  ((1.-2.*total_pid/total)*(sqrt(err_pid))*(sqrt(err_pid)) + (total_pid)*(total_pid)*(sqrt(err))*(sqrt(err)) / (total*total) ) / (total*total) ) );
   ...
   TFile* f_pidcalib = new TFile((pidcalibdir+"/"+pidcalibfile).c_str(),"READ");
...
void FitAndSplotJpsiKDataForTraining::readPIDCalibEfficiency2DHist(string pidfile, string pidcut){

   TFile* f_pid = new TFile((pidcalibdir+"/"+pidfile).c_str(),"READ");
...
void FitAndSplotJpsiKDataForTraining::prepare2DhistMyAndPIDCalibRatio(string pidfile, string pidcut, string mypiddir, string mypidfile)
{
