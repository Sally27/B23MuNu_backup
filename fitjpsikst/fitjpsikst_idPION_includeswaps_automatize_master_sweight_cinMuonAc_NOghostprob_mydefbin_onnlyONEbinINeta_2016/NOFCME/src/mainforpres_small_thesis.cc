...
//                                delete set1_gr;
	...
	mg->GetYaxis()->SetTitle("#frac{K #rightarrow #mu misID from B #rightarrow J/#psi K^{*0}}{K #rightarrow #mu misID from PIDCalib}");
        ...
        cv->SaveAs("Visualize_Ratios_2016_KaonMisid_small_thesis.pdf");    

        TCanvas *cv2 = new TCanvas("set2_cv", "set2_cv",1200,600);
        leg->SetTextSize(0.05);
	leg->Draw();
        
        cv2->SaveAs("Visualize_Ratios_2016_KaonMisid_small_thesis_leg.root");    
        cv2->SaveAs("Visualize_Ratios_2016_KaonMisid_small_thesis_leg.pdf");    
