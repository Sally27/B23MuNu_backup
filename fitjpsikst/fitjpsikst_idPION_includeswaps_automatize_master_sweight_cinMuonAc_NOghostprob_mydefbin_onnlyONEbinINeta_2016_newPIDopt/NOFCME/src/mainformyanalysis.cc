...
int main(int argc, char *argv[]){
	...
	//RUN with hypathia
        ...
        string pidcut_isMuon = "K_isMuon==1.0 && K_PIDmu>0 && (K_PIDmu-K_PIDK)>0 && K_isMuonTight==1.0 && K_MuonNShared<2 && K_MC15TuneV1_ProbNNmu>0.1";
        string pidcut_isMuon_nShared = "K_isMuon==1.0 && K_PIDmu>0 && (K_PIDmu-K_PIDK)>0 && K_isMuonTight==1.0 && K_MuonNShared<2 && K_MC15TuneV1_ProbNNmu>0.15";
	string pidcut_both_probnnmu ="K_isMuon==1.0 && K_PIDmu>0 && (K_PIDmu-K_PIDK)>0 && K_isMuonTight==1.0 && K_MuonNShared<2 && K_MC15TuneV1_ProbNNmu>0.2";
        string pidcut_dllmu= "K_isMuon==1.0 && K_PIDmu>0 && (K_PIDmu-K_PIDK)>0 && K_isMuonTight==1.0 && K_MuonNShared<2 && K_MC15TuneV1_ProbNNmu>0.25";

        string pure="K_isMuon==1.0 && K_PIDmu>0 && (K_PIDmu-K_PIDK)>0 && K_isMuonTight==1.0 && K_MuonNShared<2 && K_MC15TuneV1_ProbNNmu>0.3";
        string pure_new="K_isMuon==1.0 && K_PIDmu>0 && (K_PIDmu-K_PIDK)>0 && K_isMuonTight==1.0 && K_MuonNShared<2 && K_MC15TuneV1_ProbNNmu>0.35";
...
//        holder5_err=run.readEfficiency2DHist_all_err(pidcut,"PIDHist_RatioSweights");
	...
	for(int i(0); i<allcuts.size() ;i++)
	{
		...
		TH2D *eff =(TH2D*)f_pid->Get("PIDHist_Sweights");
                ...
                TFile* f_info = new TFile("../PIDtables/PID_2D_ALL_INFO_FORANALYSIS_KAONMISID_2016.root","UPDATE");
...
//        mg->GetXaxis()->SetRangeUser(0.,100000.0);
	...
	cv->BuildLegend(0.1,0.5,0.9,0.9);
        cv->SaveAs("Visualize_Weights_KaonMisid_2016.pdf");    
