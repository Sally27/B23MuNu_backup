...
{ 
 ...
 //  model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("piblah"), RooFit::Name("pimumu"));
   ...
   model_total->plotOn(frame, RooFit::LineColor(kYellow), RooFit::Components("kest1"), RooFit::Name("lam"));
...
{ 
 ...
 //  model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("piblah"), RooFit::Name("pimumu"));
   ...
   model_total->plotOn(frame, RooFit::LineColor(kBlue), RooFit::Components("sigblah"), RooFit::Name("signal"));
   model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("sigBs"), RooFit::Name("bs"));
   model_total->plotOn(frame, RooFit::LineColor(kYellow), RooFit::Components("lam_blah"), RooFit::Name("lam"));
...
{ 
 ...
 //  model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("piblah"), RooFit::Name("pimumu"));
   ...
   model_total->plotOn(frame, RooFit::LineColor(kBlue), RooFit::Components("sigblah"), RooFit::Name("signal"));
   model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("sigBs"), RooFit::Name("bs"));
   model_total->plotOn(frame, RooFit::LineColor(kYellow), RooFit::Components("kest1"), RooFit::Name("kest1"));
...
//		return(1.0);
        ...
        RooAbsReal* nll = kest1.createNLL(*data, RooFit::NumCPU(8));
        RooMinuit m(*nll);
        cout<<"I am here"<<endl;
        m.minos();
...
vector<double> FitAndSplotJpsiKDataForTraining::lambdafitnoweight(string type, string mcfilename)
{ 
...
//		return(1.0);
	...
	RooRealVar cbmeanrar("cbmeanrar_lam","cbmeanrar", 5283.0, 5200.8, 5400.0);
	RooRealVar cbsigmarar("cbsigmarar_lam","cbsigmarar",8.6,0.1,200);//,0.1,50) ;
	RooRealVar cbsignalrar("cbsignalrar_lam","cbsignalrar",5.0,0.0,10.0) ;
	RooRealVar nral("nral_lam","",1.0,0.1,3.0);
	RooRealVar alpharal("alpharal_lam","",1.0,0.1,5.0);
	RooRealVar nrar("nrar_lam","",1.0,0.1,5.0);
	RooRealVar alpharar("alpharar_lam","",1.0,0.1,10.0);

	MyCB blah("blah_lam", "blah_lam", *B0_DTFM_M_zero, cbmeanrar, cbsigmarar, nral , alpharal, nrar, alpharar);
	...
	canv4->cd(3) ; gPad->SetLeftMargin(0.15) ; frame10->GetYaxis()->SetTitleOffset(1.4) ; frame10->Draw() ;
	canv4->cd(4) ; gPad->SetLeftMargin(0.15) ; frameun->GetYaxis()->SetTitleOffset(1.4) ; frameun->Draw() ;
	canv4->SaveAs((plotdir+"mclambdafit.pdf").c_str());
...
//   RooRealVar* cbsignalrar=ws->var("cbsignalrar");
   ...
   RooAbsPdf* kest1=ws_lam->pdf("kest1");
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   ...
   RooRealVar bkg("bkg","bkg",100, 2 ,600000) ;
   RooRealVar Bs("Bs","Bs",1000, 2 ,20000) ;
   RooRealVar Lam("Lam","Lam",1000, 0.1,20000) ;

//   RooAddPdf sigCB_B0("sigCB0","sigCB0",RooArgList( cb1, cb2),RooArgList( vc1)) ;
   RooAddPdf model_total("model","model", RooArgList(blah, expo, sigBs, *kest1) ,RooArgList(sig, bkg, Bs, Lam));
   ...
   model_total.plotOn(frame, RooFit::LineColor(kBlue), RooFit::Components("sigblah"));
   model_total.plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("sigBs"));
   model_total.plotOn(frame, RooFit::LineColor(kYellow), RooFit::Components("kest1"));
...
//   RooRealVar* cbsignalrar=ws->var("cbsignalrar");
   ...
   TFile* f_fit_lam = new TFile((plotdir+"myworkspace_LambdaMCFit.root").c_str());
   RooWorkspace* ws_lam = (RooWorkspace*)f_fit_lam->Get("w");
   RooRealVar* cbmeanrar_lam=ws_lam->var("cbmeanrar_lam");
   RooRealVar* cbsigmarar_lam=ws_lam->var("cbsigmarar_lam");//,0.1,50) ;
//   RooRealVar* cbsignalrar=ws->var("cbsignalrar");
   RooRealVar* nral_lam=ws_lam->var("nral_lam");
   RooRealVar* alpharal_lam=ws_lam->var("alpharal_lam");
   RooRealVar* nrar_lam=ws_lam->var("nrar_lam");
   RooRealVar* alpharar_lam=ws_lam->var("alpharar_lam");
   ...
   MyCB blah("sigblah", "sigblah", *B0_DTFM_M_zero, *cbmeanrar, *cbsigmarar, *nral , *alpharal, *nrar, *alpharar);
      MyCB lam_blah("lam_blah", "lam_blah", *B0_DTFM_M_zero, *cbmeanrar_lam, *cbsigmarar_lam, *nral_lam , *alpharal_lam, *nrar_lam, *alpharar_lam);
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   ...
   RooRealVar bkg("bkg","bkg",100, 2 ,600000) ;
   RooRealVar Bs("Bs","Bs",1000, 2 ,20000) ;
   RooRealVar Lam("Lam","Lam",1000, 2 ,20000) ;

//   RooAddPdf sigCB_B0("sigCB0","sigCB0",RooArgList( cb1, cb2),RooArgList( vc1)) ;
   RooAddPdf model_total("model","model", RooArgList(blah, expo, sigBs, lam_blah) ,RooArgList(sig, bkg, Bs, Lam));
   ...
   model_total.plotOn(frame, RooFit::LineColor(kBlue), RooFit::Components("sigblah"));
   model_total.plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components("sigBs"));
   model_total.plotOn(frame, RooFit::LineColor(kYellow), RooFit::Components("lam_blah"));
...
void FitAndSplotJpsiKDataForTraining::readEfficiency2DHist(){
...
//   after->SetBinContent(binx+1,biny+1,sig.getVal());
   ...
   ofstream out2;
   out2.open("../PIDResult/PIDTable.txt");
...
//   RooRealVar* cbsignalrar=ws->var("cbsignalrar");
   ...
   MyCB blah("sigblah", "sigblah", *B0_DTFM_M_zero, *cbmeanrar, *cbsigmarar, *nral , *alpharal, *nrar, *alpharar);
   MyCB blah_lam("lam_blah", "lam_blah", *B0_DTFM_M_zero, *cbmeanrar_lam, *cbsigmarar_lam, *nral_lam , *alpharal_lam, *nrar_lam, *alpharar_lam);
...
//   RooAddPdf sigCB_B0("sigCB0","sigCB0",RooArgList( cb1, cb2),RooArgList( vc1)) ;
   //la 
   RooAddPdf model_total("model","model", RooArgList(blah, expo, sigBs, blah_lam) ,RooArgList(sig, bkg, Bs, Lam));
...
{
   TFile f((mcfilelambda+".root").c_str());
   TTree* t = (TTree*)f.Get(treename.c_str());
   TFile OKAPI("OKAPI.root", "RECREATE");
   TTree* t1(NULL);
...
void FitAndSplotJpsiKDataForTraining::prepareWorkspaceData(string cutP, string cutETA, string cutPID)
{
   ...
   if(!t)
   {
      cerr<<"ERROR: in function prepareWorkspace data"<<endl;
      return;
   }
...
//        underp.push_back(0.0);
//        undereta.push_back(0.0);
//        underp.push_back(&xbin);
//        undereta.push_back(&ybin);
