...
//////-----------------Just fit all of JpsiKst for PID purposes with dcb-----------//
////
////////-------------Create PID table--------------------////
//ofstream out;
...
//for(int i(0); i<2 ;i++) //number of bins wanted to be iterated above in momentum
//{
//        for(int j(0); j<2 ;j++){ //number of bins wanted to be iterated above in eta
//
...
////-----------------Just fit all of JpsiKst for PID purposes with rookeys-----------//
//
//////-------------Create PID table--------------------////
...
ofstream out;
out.open("../PIDResult/TableBeforeRookeys.txt");
out<<"\\begin{table}[h]"<<endl;
out<<"\\begin{center}"<<endl;
out<<"\\begin{tabular}{| l | l | l | l | l |}"<<endl;
out<<"\\hline"<<endl;
...
//
ofstream out2;
out2.open("../PIDResult/TableAfterRookeys.txt");
out2<<"\\begin{table}[h]"<<endl;
out2<<"\\begin{center}"<<endl;
out2<<"\\begin{tabular}{| l | l | l | l | l |}"<<endl;
out2<<"\\hline"<<endl;
...
//	runPID.prepareWorkspaceSigMC(pbin.c_str(), etabin.c_str(),pidcut.c_str()); //prepare ws for mc
//	runPID.sigfitnoweight("",""); //fit mc ws
//        runPID.prepareWorkspaceLambdaMC(pbin.c_str(), etabin.c_str(),pidcut.c_str());
//        runPID.lambdafitnoweightrookeys("","");
//	runPID.prepareWorkspaceData(pbin.c_str(), etabin.c_str(),pidcut.c_str()); //prepare ws for data
////	runPID.fitJpsiKstData(false, "",("Pi_P_"+d2s(binningx.at(i))+"_Pi_P_"+d2s(binningx.at(i+1))+"_Pi_ETA_"+d2s(binningy.at(j))+"_Pi_ETA_"+d2s(binningy.at(j+1))).c_str()); //fitdata ws
//        runPID.fitJpsiKstDataAndMakePIDbeforerookeys(false, "",kinbinstring.c_str(), i, j, pidcut.c_str(),out); //fitdata ws and fill pidtable
...
//	runPIDafter.prepareWorkspaceSigMC(pbin.c_str(), etabin.c_str(),pidcutafter.c_str()); //prepare ws for mc
//	runPIDafter.sigfitnoweight("",""); //fit mc ws
//        runPIDafter.prepareWorkspaceLambdaMC(pbin.c_str(), etabin.c_str(),pidcut.c_str());
//        runPIDafter.lambdafitnoweightrookeys("","");
//	runPIDafter.prepareWorkspaceData(pbin.c_str(), etabin.c_str(),pidcutafter.c_str()); //prepare ws for data
////	runPID.fitJpsiKstData(false, "",("Pi_P_"+d2s(binningx.at(i))+"_Pi_P_"+d2s(binningx.at(i+1))+"_Pi_ETA_"+d2s(binningy.at(j))+"_Pi_ETA_"+d2s(binningy.at(j+1))).c_str()); //fitdata ws
//        runPIDafter.fitJpsiKstDataAndMakePIDafterrookeys(false, "",kinbinstring.c_str(), i, j, pidcutafter.c_str(),out2); //fitdata ws and fill pidtable
...
plotMergedFits(binningx, binningy,"Pi_PIDK<0","BeforeRookeys","/vols/lhcb/ss4314/fitjpsikst/fitjpsikst_lambdapkmumu/NOFCME/workspace");
plotMergedFits(binningx, binningy,"Pi_PIDK<-2","AfterRookeys","/vols/lhcb/ss4314/fitjpsikst/fitjpsikst_lambdapkmumu/NOFCME/workspace");
//
//
//out<<"\\hline"<<endl;
//out<<"\\end{tabular}"<<endl;
