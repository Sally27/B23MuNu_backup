...
//	f->Close();
	...
	vector<double> params;
        ...
        RooRealVar cbmeanr2(("cbmeanr2"),"cbmeanr", 4300.4 ,4000.0,4600.0);
...
//	  RooExponential newexpoun("newexpoun", "exponential",Bplus_Corrected_Mass,newexpocon);
	  ...
	  RooAddPdf misidpdf(("misidpdf"),"extended sum pdf",RooArgList(*pdf_misid_low), RooArgList(*nmisid_low)); 
          normpdf_low.Print("t");
	...
	if(!data_low || !Bplus_Corrected_Mass_low)
	{
        ...
        //-------Retrieve misiddataset-----//

	RooDataHist* misid_dataset = (RooDataHist*)wsmisid_low->data("dpw");

	if(!misid_dataset)
	{
		cerr<<"ERROR: in function fitJpsiKData, no data found in misid found in file "<<endl;
		return;
	}
...
//                        cout<<"My dataset is weighted!" << endl;
//                }
       RooRealVar x("x","x",4000,7000);
       RooRealVar weight("weight","weight",0,1);
       RooDataSet d2("d2","d2",RooArgSet(Bplus_Corrected_Mass,weight),"weight");    
       for(int i(0); i<60; i++)
       {
       double w;
       double v;
       const RooArgSet* currarg=misid_dataset->get(i);
        currarg->Print("v");
//        cout<<misid_dataset->get(i)->weight()<<endl;
//       v=currarg->weight();
//       w=currarg->weight();
//       x.setVal(v);
//       d2.addFast(RooArgSet(x),w);
       }
	...
	sample.defineType("NOFCME") ;
        sample.defineType("misidsample") ;
	//sample.defineType("HighFCME") ;

	// Construct combined dataset in (x,sample)
	RooDataSet combData("combData","combined data",Bplus_Corrected_Mass,Index(sample),Import("NOFCME",*data_low),Import("misidsample",*misid_dataset)) ;
  //      combData->weight();
        cout<<"hey babe"<<endl; 
	...
	RooSimultaneous simPdf("simPdf","simultaneous pdf",sample) ;
        ...
        simPdf.addPdf(misidpdf,"misidsample") ;
...
//	simPdf.plotOn(frame1,Slice(sample,"LowFCME"),RooFit::Range("low,high"),RooFit::NormRange("new"), LineColor(kGreen));
	...
	RooPlot* frame2 = Bplus_Corrected_Mass.frame(Bins(60),Title("Misid Fit")) ;
	combData.plotOn(frame2,Cut("sample==sample::misidsample")) ;
	simPdf.plotOn(frame2,Slice(sample,"misidsample"),ProjWData(sample,combData),RooFit::Range("low"),RooFit::NormRange("low"));
        //simPdf.plotOn(frame2,Slice(sample,"misidsample"),Components(("twocbs")),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kCyan));
	simPdf.plotOn(frame2,Slice(sample,"misidsample"),ProjWData(sample,combData),RooFit::Range("high"),RooFit::NormRange("high"));
        //simPdf.plotOn(frame2,Slice(sample,"misidsample"),Components(("twocbs")),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kCyan));
	...
	c2->cd(2) ; gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.4) ; frame2->Draw() ;
        c2->SaveAs((plotdir+"FitResult_Data_MisidData.pdf").c_str());
        delete c2;
