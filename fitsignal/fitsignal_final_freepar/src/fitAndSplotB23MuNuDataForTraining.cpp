...
//---- Setup all thing for 3 different fits--------//
	...
	vector<double> params;
        ...
        RooRealVar sig4(("sig4"),"frac of events in sig4",2.83,2.0,10.0);
        RooRealVar cbmeanr4(("cbmeanr4"),"cbmeanr", 4195.4, 3500.0, 4400.0);
        RooRealVar cbsigmar4(("cbsigmar4"),"cbsigmar",511.,400.,600.0);//,0.1,50) ;
        RooRealVar nr4(("nr4"),"",1.24);//,0.1, 500.0);//, 0.8, 5.0);
        RooRealVar alphar4(("alphar4"),"",0.6);//,0.1, 0.8);//, 0.1, 1.0);//
	RooCBShape twocbs3(("twocbs3"), "c3", Bplus_Corrected_Mass, cbmeanr4, cbsigmar4, alphar4, nr4); 
        RooExtendPdf two_ext4("two_ext4","two_ext4",twocbs3,sig4);
...
//         RooRealVar mu_nr4(("mu_nr4"),"mu_nr4",nr4.getVal());//,cbmeanr4.getVal()-cbmeanr4.getError(),cbmeanr4.getVal()+cbmeanr4.getError());
//         RooRealVar mu_alphar4(("mu_alphar4"),"mu_alphar4",alphar4.getVal());//,cbsigmar4.getVal()-cbsigmar4.getError(),cbsigmar4.getVal()+cbsigmar4.getError());//,0.1,50) ;
...
////---------3 param free-------------//
//         double oldval03=cov_chi[0][3];
//         double oldval13=cov_chi[1][3];
//         double oldval23=cov_chi[2][3];
//         double oldval33=cov_chi[3][3];
//
//         double oldval30=cov_chi[3][0];
//         double oldval31=cov_chi[3][1];
//         double oldval32=cov_chi[3][2];
...
////----------4 paramfree--------//
//	 cov_chi_new[0][3]=oldval03*100.0;
//	 cov_chi_new[1][3]=oldval13*100.0;
//	 cov_chi_new[2][3]=oldval23*100.0;
//	 cov_chi_new[3][3]=oldval33*10000.0;         
//	 cov_chi_new[3][0]=oldval30*100.0;
//	 cov_chi_new[3][1]=oldval31*100.0;
//	 cov_chi_new[3][2]=oldval32*100.0;
...
//5 param         RooMultiVarGaussian mvg("mvg", "mvg",RooArgSet(alphar4,cbmeanr4,cbsigmar4,nr4,misid_scaled),RooArgSet(mu_alphar4,mu_cbmeanr4,mu_cbsigmar4,mu_nr4,mu_sig4),cov_chi_new);
//4 param         RooMultiVarGaussian mvg("mvg", "mvg",RooArgSet(alphar4,cbmeanr4,cbsigmar4,misid_scaled),RooArgSet(mu_alphar4,mu_cbmeanr4,mu_cbsigmar4,mu_sig4),cov_chi_new);
...
{ 
 ...
 //       dr.SumW2(kTrue);
	...
	RooFitResult* sigfit=blah.fitTo(dr,SumW2Error(kTRUE),Save());
        ...
        ofstream out_chi2((plotdir+type+"FitResults_Signal.txt").c_str());
        ofstream out_simple((plotdir+type+"FitResults_Signal_simple.txt").c_str());
        saveFitInfoExtended(out_chi2, frame8, 6, &blah,sigfit);
        saveFitInfo(out_simple, frame8, 6, &blah);
...
//        RooGaussian npartreco_con("npartreco_con","npartreco_con",npartreco,pr_val,pr_error); 

 //       double cbsigmarar_val=cbsigmarar.getVal();
//        double cbsigmarar_error=cbsigmarar.getError();
//        RooRealVar cbsigmarar_new("cbsigmarar_new","cbsigmarar_new",cbsigmarar.getVal(),(cbsigmarar_val-(5.0*cbsigmarar_error)),(cbsigmarar_val+(5.0*cbsigmarar_error)));        
//        RooRealVar cbmeanrar_new("cbmeanrar_new","cbmeanrar_new",cbmeanrar.getVal(),(cbsigmarar_val-(5.0*cbsigmarar_error)),(cbsigmarar_val+(5.0*cbsigmarar_error)));        
 //       RooGaussian cbsigmarar_con("cbsigmarar_con","cbsigmarar_con",&cbsigmarar,cbsigmarar.getVal(),cbsigmarar.getError());
 //       RooGaussian cbmeanrar_con("cbmeanrar_con","cbmeanrar_con",&cbmeanrar,cbmeanrar.getVal(),cbmeanrar.getError());
	...
	w8->import(dr);
	w8->import(blah);
//	w8->import(cbsigmarar_con);
//	w8->import(cbmeanrar_con);
