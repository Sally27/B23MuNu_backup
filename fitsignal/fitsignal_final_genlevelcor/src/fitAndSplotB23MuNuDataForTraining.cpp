...
{ 
 ...
 //       dr.SumW2(kTrue);
	...
	RooRealVar cbsigmarar(("cbsigmarar"),"#sigma",0.1,100);//,0.1,50) ;
	RooRealVar cbsignalrar(("cbsignalrar"),"signal",8400,7000,9000) ;
	RooRealVar nral(("nral"),"n_{L}",0.1,0.4);
	RooRealVar alpharal(("alpharal"),"#alpha_{L}",0.1,7.0);
	RooRealVar nrar(("nrar"),"n_{R}",0.1,2.0);
...
//	RooFitResult* fitres0=blah0.fitTo(dr,SumW2Error(kTRUE),Save());
	...
	for (int i(0); i<20; i++)
	{
		...
		cbsigmarar.setVal(sigma.Uniform(0.1,100));//,30.0,250.0);//,0.1,50) ;
		cbsignalrar.setVal(signal.Uniform(7000,9000));
		nral.setVal(nl.Uniform(0.1,0.4));//,0.1,2.0);
		alpharal.setVal(al.Uniform(0.1,7.0));//,0.1,7.0);
		nrar.setVal(nr.Uniform(0.1,2.0));//,0.1,5.0);
