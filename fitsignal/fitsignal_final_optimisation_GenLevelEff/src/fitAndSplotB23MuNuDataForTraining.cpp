...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall_chi2(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type, string PIDcut)
{
...
//-------------------------------------------Learn about Errors in RooDataHist----------------------//
 	...
 	binnedData.get(i);
        ...
        TFile* fw = new TFile((misidvariationdir+"/RooWS_Error_Mean_MisidVariation_"+i2s(desirednumofbins)+"_100_"+type+"_"+PIDcut+".root").c_str(), "READ");
...
void FitAndSplotB23MuNuDataForTraining::plotprettysim_chi2(string type, string pidcut)
{ 
...
//            hist->GetErrorYlow(i), hist->GetErrorYhigh(i));
   ...
   canvTot.Print((plotdir+type+pidcut+"plotB23MuNuFitPretty_chi2.pdf").c_str());
   canvTot.Print((plotdir+type+pidcut+"plotB23MuNuFitPretty_chi2.root").c_str());

   canv.SetLogy();
   canvTot.cd(1);
   canv.DrawClonePad();
   canvTot.Print((plotdir+type+pidcut+"plotB23MuNuFitLogyPretty_chi2.pdf").c_str());
   canvTot.Print((plotdir+type+pidcut+"plotB23MuNuFitLogyPretty_chi2.root").c_str());
...
{ 
	...
	TFile* f = new TFile((sigdir+"/"+signalfilename+year+PIDcut+type+".root").c_str());
...
void FitAndSplotB23MuNuDataForTraining::calculateFOM_chi2(string type, string pidcut)
{
