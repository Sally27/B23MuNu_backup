...
{
	TFile f( (datadir+"/"+tuplename+"_Run1_"+PIDcut+"_cut_"+type+".root").c_str());
...
void FitAndSplotB23MuNuDataForTraining::prepareWorkspaceForSimultaneousFit_NoFitData(string type, string year ,string PIDcut)
{
	TFile f( (nofitdatadir+"/"+nofitdatafile+"_"+type+".root").c_str());
	TTree* t = (TTree*)f.Get(treename.c_str());
	...
	TFile fws((workspacedir+"/NoFitData_"+type+".root").c_str(), "RECREATE");
	workspaceData.Write("",TObject::kOverwrite );

	cout<<"Workspace containing data ready:"<<endl;
        cout<<"In "<<(workspacedir+"/NoFitData_"+type+".root").c_str()<<endl;
	workspaceData.Print();
...
//        RooExtendPdf two_ext4("two_ext4","two_ext4",twocbs3,sig4);
          ...
          RooRealVar sig4(("sig4_"+type).c_str(),"frac of events in sig4",1.0,0.7,20.0);
...
//--top        errorsheet<<"Read out errors from misid variation Run1"<<endl;
//--top        TFile* fw = new TFile((misidvariationdir+"/RooWS_Error_Mean_MisidVariation_"+i2s(desirednumofbins)+"_100_"+type+"_"+PIDcut_Run1+".root").c_str(), "READ");
//--top        RooWorkspace* ws_error = (RooWorkspace*)fw->Get("workspaceVar");
//--top        ws_error->Print();
//--top        for (int l(0); l<desirednumofbins; l++){
//--top                RooRealVar* error = ws_error->var(("ErrorVar_"+i2s(l)).c_str());
//--top                RooRealVar* mean = ws_error->var(("MeanVar_"+i2s(l)).c_str());
//--top                errorsheet<<"Mean from toys: "<<mean->getVal()<<" Error from toys: "<<error->getVal()<<endl;
//--top        }
//--top
//--top        errorsheet<<"Sum the error from misid variation "<<endl;
//--top	 
//--top        for (int i(0); i<binnedData.numEntries(); i++)
//--top        {
//--top	RooRealVar* error = ws_error->var(("ErrorVar_"+i2s(i)).c_str());
//--top        
//--top        double combined_err(0);
//--top        cout<<"combining run1 and 2016 errors"<<endl;
//--top        combined_err=error->getVal();
//--top
//--top 	binnedData.get(i);
//--top        binnedData.set(binnedData.weight(),addstatandsyst(binnedData.weightError(RooAbsData::SumW2),combined_err));
//--top        binnedData.weightError(RooAbsData::SumW2);
//--top        cout<<" Bin "<<i<<" x = "<<binnedData.get(i)->getRealValue("Bplus_Corrected_Mass")<<" Weight="<<binnedData.weight()<<"New Weight Error= "<<binnedData.weightError(RooAbsData::SumW2)<<endl;
//--top        errorsheet<<i<<" & "<<binnedData.get(i)->getRealValue("Bplus_Corrected_Mass")<<"& "<<binnedData.weight()<<" & "<<binnedData.weightError(RooAbsData::SumW2)<<endl;
//--top        }
//--top
//--top
// new binning //
...
{ 
	...
	TFile* f = new TFile((sigdir+"/"+signalfilename+"2012"+PIDcut+"Strip21_"+type+".root").c_str());
...
//   RooAbsPdf* model_total = workspaceFit->pdf(("normpdf"));
   ...
   TFile nofitdata((workspacedir+"NoFitData_"+type+".root").c_str());
   ...
   RooDataSet* datanofit = (RooDataSet*)workspacenofit->data(("data_"+type).c_str());
...
//            hist->GetErrorYlow(i), hist->GetErrorYhigh(i));
   ...
   model_total->plotOn(frame, RooFit::LineColor(kRed),RooFit::Components(("newexpoun_"+type).c_str()),RooFit::NormRange("low,high"), RooFit::Range("full"), RooFit::Name("combinatorial"));
   model_total->plotOn(frame, RooFit::LineColor(kCyan),RooFit::Components(("misidModel_"+type).c_str()),RooFit::NormRange("low,high"), RooFit::Range("full"), RooFit::Name("mis2"));
   model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components(("newpartreco_"+type).c_str()),RooFit::NormRange("low,high"), RooFit::Range("full"), RooFit::Name("partreco"));
