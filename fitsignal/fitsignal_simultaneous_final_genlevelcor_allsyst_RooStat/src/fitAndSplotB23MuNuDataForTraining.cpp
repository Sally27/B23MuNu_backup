...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig_chi2fit_100cor_RooStats()
{
...
//	// Create joint pdf (RooSimultaneous)
        ...
        RooWorkspace w_new("w_new"); 
	w_new.factory("SIMUL:simPdf");
        w_new.Print();

        RooAbsPdf* pdf = w_new.pdf("simPdf");
        RooRealVar* x = w_new.var("Bplus_Corrected_Mass");  // the observable
        RooCategory* index = w_new.cat("sample");  // the category

        RooStats::ModelConfig mc("ModelConfig",&w_new);
        mc.SetPdf(*pdf);
        mc.SetParametersOfInterest(*w_new.var("BR"));
        mc.SetObservables(RooArgSet(*w_new.var("Bplus_Corrected_Mass"),*w_new.cat("sample")));
	...
	w_new.writeToFile(fileName,true);
