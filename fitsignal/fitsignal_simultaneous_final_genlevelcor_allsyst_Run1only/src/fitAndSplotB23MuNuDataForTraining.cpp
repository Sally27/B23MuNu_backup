...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig_chi2fit_100cor(bool Run1only)
{
	...
	//Calculate observables //
        ...
        const char* formula;
        RooArgList forpr(eff_ratio_pr_low,jpsik_run1_low,BRnorm,BRpr);
	...
	{
		formula="(((@0)*(@1)*(@3/@2)) + ((@0)*(@4)*(@3/@2)))";
		forpr.add(jpsik_2016_low);
	}
        
        RooFormulaVar npartreco_low("npartreco_LowFCME","npartreco_LowFCME",formula,forpr);
	...
	RooFormulaVar ses_low_2016("ses_LowFCME_2016","ses_LowFCME_2016",formula_ses,RooArgList(eff_ratio_2016_low,jpsik_2016_low,BRnorm));
	...
	//	 RooGaussian npartreco_con("npartreco_con","npartreco_con",npartreco,pr_val,pr_error); //Create Constraint for number of partreco candidates
        ...
        RooArgList forprhigh(eff_ratio_pr_high,jpsik_run1_high,BRnorm,BRpr);

        if (Run1only==false)
        {
//        formula="(((@0)*(@1)*(@3/@2)) + ((@0)*(@4)*(@3/@2)))";
        forprhigh.add(jpsik_2016_high);
        }


	RooFormulaVar npartreco_high("npartreco_HighFCME","npartreco_HighFCME",formula,forprhigh);        
...
//
	...
	RooFormulaVar ses_high_2016("ses_HighFCME_2016","ses_HighFCME_2016",formula_ses,RooArgList(eff_ratio_2016_high,jpsik_2016_high,BRnorm));
	...
	if (Run1only==false)
	{
		formula_signal="(@1/@2)+(@1/@0)";
	}


        RooArgList fornsiglow(ses_low,BR);
        RooArgList fornsighigh(ses_high,BR);

	if (Run1only==false)
	{
		fornsiglow.add(ses_low_2016);
		fornsighigh.add(ses_high_2016);

	}

	RooFormulaVar nsigform_low("nsigform_LowFCME","nsigfromformula",formula_signal,fornsiglow);
	RooFormulaVar nsigform_high("nsigform_HighFCME","nsigfromformula",formula_signal,fornsighigh);
...
//        w_fit.factory(*pdf_sig_high);//,newexpoun_high,*pdf_misid_high,*pdf_pr_high);
	...
	RooArgSet allsignallow_con(eff_ratio_pr_low_con,eff_ratio_run1_low_con,jpsik_run1_low_con);
	RooArgSet allsignalhigh_con(eff_ratio_pr_high_con,eff_ratio_run1_high_con,jpsik_run1_high_con);

	if (Run1only==false)
	{
		allsignallow_con.add(eff_ratio_2016_low_con);
		allsignallow_con.add(jpsik_2016_low_con);
		allsignalhigh_con.add(eff_ratio_2016_high_con);
                allsignalhigh_con.add(jpsik_2016_high_con);
	}

        //uncorrelated systematics

	RooArgSet allsignallow_syst_con(eff_ratio_pr_low_con,eff_ratio_run1_low_err_tot_con,jpsik_run1_low_con);
	RooArgSet allsignalhigh_syst_con(eff_ratio_pr_high_con,eff_ratio_run1_high_err_tot_con,jpsik_run1_high_con);

	if (Run1only==false)
	{
		allsignallow_syst_con.add(eff_ratio_2016_low_err_tot_con);
		allsignallow_syst_con.add(jpsik_2016_low_con);
		allsignalhigh_syst_con.add(eff_ratio_2016_high_err_tot_con);
                allsignalhigh_syst_con.add(jpsik_2016_high_con);
	}
        //100% correlated systematics

	RooArgSet allsignallow_syst_100_con(eff_ratio_pr_low_con,eff_ratio_run1_low_err_tot_100_con,jpsik_run1_low_con);
	RooArgSet allsignalhigh_syst_100_con(eff_ratio_pr_high_con,eff_ratio_run1_high_err_tot_100_con,jpsik_run1_high_con);

	if (Run1only==false)
	{
		allsignallow_syst_100_con.add(eff_ratio_2016_low_err_tot_100_con);
		allsignallow_syst_100_con.add(jpsik_2016_low_con);
		allsignalhigh_syst_100_con.add(eff_ratio_2016_high_err_tot_100_con);
                allsignalhigh_syst_100_con.add(jpsik_2016_high_con);
	}
	...
	RooArgSet allbkglow_con(eff_ratio_pr_low_con,jpsik_run1_low_con);
	RooArgSet allbkghigh_con(eff_ratio_pr_high_con,jpsik_run1_high_con);

	if (Run1only==false)
	{
		allbkglow_con.add(jpsik_2016_low_con);
                allbkghigh_con.add(jpsik_2016_high_con);
	}
	...
	if (systematics==true && correlation==false)
	{
		...
		workspaceFit2.import(allsignalcon_syst);
	...
	if (systematics==true && correlation==true)
	{
		...
		RooFitResult* result_sig2=simPdfsig.fitTo(combData,RooFit::Range("low,high"),Extended(kTRUE),Save(kTRUE),ExternalConstraints(allsignalcon_syst_100));//, ExternalConstrain(rmg)) ; //externalconstraint
		...
		TFile file2((clsdir+"fitresults_forCLs_syst_100.root").c_str(), "RECREATE");
		...
		workspaceFit2.import(*data_high);
		workspaceFit2.import(combData);
		workspaceFit2.import(allsignalcon_syst_100);
		workspaceFit2.import(simPdf,RecycleConflictNodes());
		workspaceFit2.import(simPdfsig,RecycleConflictNodes());
		workspaceFit2.Write("", TObject::kOverwrite);
		cout<<"Workspace for fit has been saved:"<<endl;
...
void FitAndSplotB23MuNuDataForTraining::prepareWorkspaceForSimultaneousFit(string type, string year ,string PIDcut, bool Run1only)
{
...
//	TTree* t = (TTree*)f.Get(treename.c_str());
	...
	if (Run1only==true)
	{
                 ...
                 cout<<"=================================================================="<<endl;
                 cout<<"=========================RUN 1 ONLY ========================="<<endl;
                 f=new TFile( (datadir+"/"+tuplename+"_Run1_"+PIDcut+"_cut_"+type+".root").c_str());
		 ...
		 if(!f)
		 {
			 ...
			 cerr<<"ERROR: in function FitAndSplotB23MuNuDataForTraining::prepareWorkspace for simult, no file found in file "<<endl;
			// return;
		 }
	...
	if (Run1only==false)
	{
                 ...
                 cout<<"=================================================================="<<endl;
                 cout<<"=========================FULL DATA ========================="<<endl;
		 f = new TFile((datadir+"/"+tuplename+"_All_"+type+".root").c_str());
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall_chi2(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type, string PIDcut_Run1, string PIDcut_Run2, bool Run1only)
{
...
//                }
	...
	for (int i(0); i<binnedData.numEntries(); i++)
	{
		...
		RooRealVar* error_2016;
		if (Run1only==false)
		{
                        cout<<"===================="<<endl;
		      	error_2016 = ws_error_2016->var(("ErrorVar_"+i2s(i)).c_str());
		}
		if (Run1only==true)
		{


                        cout<<"===========ONLY RUN 1========="<<endl;
			error_2016= new RooRealVar("buf","buf",0);
		}
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitsignalcomponent(string weightname, string type, string year, string PIDcut, bool Run1only)
{ 
	...
	if (Run1only==true)
	{
                 ...
                 cout<<"=================================================================="<<endl;
                 cout<<"=========================RUN 1 ONLY ========================="<<endl;
                  f = new TFile((sigdir2+"/"+signalfilename+"2012"+PIDcut+"Strip21_"+type+".root").c_str());
		 if(!f)
		 {

                         
			 cerr<<"ERROR: in function FitAndSplotB23MuNuDataForTraining::fitsignalcomponent, no file found "<<endl;
			// return;
		 }
	...
	if (Run1only==false)
	{
                 ...
                 cout<<"=================================================================="<<endl;
                 cout<<"=========================FULL DATA ========================="<<endl;
		 f = new TFile((sigdir+"/"+signalfilename+"_2012_2016_opt_"+type+".root").c_str());
		 t = (TTree*)f->Get("DecayTree");
		 if(!f)
		 {

                         
			 cerr<<"ERROR: in function FitAndSplotB23MuNuDataForTraining::fitsignalcomponent, no file found in file "<<endl;
			// return;
		 }
...
//        RooFitResult* chi2_binned_fit=m.save();
	...
	RooRealVar cbsigmarar(("cbsigmarar_"+type).c_str(),"#sigma",100.0,30.0,250.0);//,0.1,50) ;
	RooRealVar cbsignalrar(("cbsignalrar_"+type).c_str(),("cbsignalrar0"+type).c_str(),8400,7000,9000) ;
	RooRealVar nral(("nral_"+type).c_str(),"n_{L}",0.45,0.1,5.0);
	RooRealVar alpharal(("alpharal_"+type).c_str(),"#alpha_{L}",2.4,0.1,7.0);
	RooRealVar nrar(("nrar_"+type).c_str(),"n_{R}",2.0,0.1,5.0);
	RooRealVar alpharar(("alpharar_"+type).c_str(),"#alpha_{R}",1.8,0.1,5.30);
	...
	for (int i(0); i<100; i++)
	{
		...
		cbsigmarar.setVal(mean.Uniform(30.0,250.0));//,30.0,250.0);//,0.1,50) ;
		cbsignalrar.setVal(mean.Uniform(7000,9000));
		nral.setVal(mean.Uniform(0.1,5.0));//,0.1,2.0);
	        alpharal.setVal(mean.Uniform(0.1,7.0));//,0.1,7.0);
		nrar.setVal(mean.Uniform(0.1,5.0));//,0.1,5.0);
		alpharar.setVal(mean.Uniform(0.1,10.30));//,0.1,10.30);
...
//		RooFitResult* fitres=blah.fitTo(dr,SumW2Error(kTRUE),Save());//;RooFit::Minimizer("Minuit2"),Save());
	...
	//	m.hesse();
        //        RooMinuit m(*nll);
                ...
                migradStatus= m.migrad();
         //       RooFitResult* fitres=m.save();
         //              cout<<"quality of the cov matrix from minimizing: "<< fitres->covQual()<<endl;
