...
//   for(Int_t i=0 ; i<dataHist_1->numEntries() ; i++) {
//   dataHist_1->get(i);
//   if dataHist_1->get(i)!=0
//   {
//   cout<<"mm"<<endl;
//   }
...
{ 
	...
	RooRealVar cbmeanrar(("cbmeanrar_"+type).c_str(),("cbmeanrar"+type).c_str(), 5200.0, 5100.8, 5300.0);
	RooRealVar cbsigmarar(("cbsigmarar_"+type).c_str(),("cbsigmarar"+type).c_str(),100.0,30.0,300.0);//,0.1,50) ;
	RooRealVar cbsignalrar(("cbsignalrar_"+type).c_str(),("cbsignalrar"+type).c_str(),8400,8000,7000) ;
	RooRealVar nral(("nral_"+type).c_str(),"",0.1,2.0);
	RooRealVar alpharal(("alpharal_"+type).c_str(),"",0.1,7.0);
	RooRealVar nrar(("nrar_"+type).c_str(),"",0.1,2.0);
	RooRealVar alpharar(("alpharar_"+type).c_str(),"",0.1,10.30);
	...
	myresults.push_back(alpharar.getVal());
        ...
        cbmeanrar.setConstant(true);
        cbsigmarar.setConstant(true);
        cbsignalrar.setConstant(true);
        nral.setConstant(true);
        alpharal.setConstant(true);
        nrar.setConstant(true);
        alpharar.setConstant(true);
...
void FitAndSplotB23MuNuDataForTraining::compareplots(string filename)
{
	...
	TFile *fhigh = new TFile((workspacedir+filename+"_"+typehigh+".root").c_str());
        ...
        TFile *flowmisid = new TFile((workspacedir+"plotmisid_"+typelow+".root").c_str());
        TFile *fhighmisid = new TFile((workspacedir+"plotmisid_"+typehigh+".root").c_str());
	...
	C->SaveAs((workspacedir+"Compare_Component_Distributions.pdf").c_str());
	flow->Close();
	fhigh->Close();
	delete p1;
	delete p2;
...
//        RooRealVar npartreco(("npartreco_"+type).c_str(),("npartreco_"+type).c_str(),num);
	...
	RooPlot* framepartreco1 = Bplus_Corrected_Mass.frame(Title(" ")) ;
...
double FitAndSplotB23MuNuDataForTraining::fitmisidone(string type, string dir ,string filename, string weight, string species, string PID)
{
	TFile* f = new TFile((dir+"/"+filename+"_"+type+"_"+PID+".root").c_str());
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall_chi2(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type, vector <string> &PID)
{

	cout<<"Start misid fit all components"<<endl;


	TFile* s = new TFile((dir.at(0)+"/"+misidfilenames.at(0)+"_"+type+"_"+PID.at(0)+".root").c_str());
...
//        Bplus_Corrected_Mass.setBins(60);
	...
	fitmisidone(type, dir.at(0) , misidfilenames.at(0), weights.at(0), species.at(0), PID.at(0));
	...
	for(int i(1); i<misidfilenames.size(); i++)
	{
		...
		fitmisidone(type, dir.at(i) , misidfilenames.at(i), weights.at(i), species.at(i), PID.at(i));

                
		TFile* f = new TFile((dir.at(i)+"/"+misidfilenames.at(i)+"_"+type+"_"+PID.at(i)+".root").c_str());
		TTree* treek = (TTree*)f->Get("DecayTree");
...
//        for (int i(0); i<desirednumofbins; i++)
//        {
//        dpw.get(i);
//        cout<<"Bin entry "<<i<<" content:"<dpw.weight(i)<<endl;
//        }
    //    binnedData.Scale(100.0);
...
//-------------------------------------------Learn about Errors in RooDataHist----------------------//
 	...
 	binnedData.get(i);
        ...
        errorsheet<<"Read out errors from misid variation "<<endl;
        TFile* fw = new TFile((misidvariationdir+"/RooWS_Error_Mean_MisidVariation_"+i2s(desirednumofbins)+"_100_"+type+".root").c_str(), "READ");
 	...
 	binnedData.get(i);
        binnedData.set(binnedData.weight(),addstatandsyst(binnedData.weightError(RooAbsData::SumW2),error->getVal()));
        binnedData.weightError(RooAbsData::SumW2);
	...
	ofstream out_chi2((plotdir+type+"_FitResults_Bin_CHI2.txt").c_str());
	...
	const TMatrixDSym& cov_chi = chi2_binned_fit->covarianceMatrix();
        ...
        Int_t nrows= cov_chi.GetNrows();
        Int_t ncols= cov_chi.GetNcols();
	...
	cout<<" -log(L) at minimum: "<< chi2_binned_fit->minNll()<<endl;
        ...
        ofstream out_ana((plotdir+type+"_FitResults_Bin_CHI2_forANA.tex").c_str());
        ...
        out_ana<<"\\hline"<<endl;
        out_ana<<"mean & "<<cbmeanr4.getVal()<<" $\\pm$ "<<cbmeanr4.getError()<<" \\\\"<<endl;
        out_ana<<"$\\sigma$ & "<<cbsigmar4.getVal()<<" $\\pm $ "<<cbsigmar4.getError()<<" \\\\"<<endl;
        out_ana<<"number & "<<sig4.getVal()*100.0<<"$ \\pm $"<<sig4.getError()*100.0<<" \\\\"<<endl;
        out_ana<<"$n$ (fixed) & "<<nr4.getVal()<<" \\\\"<<endl;
        out_ana<<"$\\alpha$ (fixed) & "<<alphar4.getVal()<<" \\\\"<<endl;
        out_ana<<"\\hline"<<endl;
        out_ana<<"\\end{tabular}"<<endl;
        out_ana<<"\\end{center}"<<endl;
        out_ana<<"\\caption{$\\chi^{2}$ of the fit: "<<chi2.getVal()<<" .}"<<endl;
...
//---------------------get values for mean from the chi2 fit------------------------------------------------------//
	RooRealVar mu_sig4(("mu_sig4_"+type).c_str(),"frac of events in sig4",sig4.getVal()*100.0);//,(sig4.getVal()-sig4.getError())*100.0,((sig4.getVal()+sig4.getError())*100.0));
	RooRealVar mu_cbmeanr4(("mu_cbmeanr4_"+type).c_str(),"cbmeanr",cbmeanr4.getVal());//,cbmeanr4.getVal()-cbmeanr4.getError(),cbmeanr4.getVal()+cbmeanr4.getError());
	RooRealVar mu_cbsigmar4(("mu_cbsigmar4_"+type).c_str(),"cbsigmar",cbsigmar4.getVal());//,cbsigmar4.getVal()-cbsigmar4.getError(),cbsigmar4.getVal()+cbsigmar4.getError());//,0.1,50) ;
        RooRealVar mu_nr4(("mu_nr4_"+type).c_str(),"",1.24);//,0.1,2.8);//,1.0,1.5);//,0.8, 5.0);//, 0.8, 5.0);
        RooRealVar mu_alphar4(("mu_alphar4_"+type).c_str(),"",0.6);//,0.1,1.0);//, 0.1, 1.0);
...
//	 RooRealVar scale_for_misid("scale_for_misid","scale_for_misid",100.0);
         ...
         RooRealVar misid_scaled(("misid_scaled_"+type).c_str(),"misid_scaled",(sig4.getVal()*100.0),(double(sig4.getVal()-(numberofsigma*sqrt(cov_chi[2][2])))*100.0),(double(sig4.getVal()+(numberofsigma*sqrt(cov_chi[2][2])))*100.0));
...
//		{
//
 //               RooRealVar element(("element_"+i2s(i)+i2s(j)+"_"+type).c_str(),("element_"+i2s(i)+i2s(j)+"_"+type).c_str(),cov_chi_new[i][j]);
//			if(j==ncols-1){
...
//         cbmeanr4.setConstant(true);
//        cbsigmar4.setConstant(true);

         //---------------------function that shall be passed------------------------------//
	 RooExtendPdf two_ext5(("two_ext5_"+type).c_str(),"two_ext5",twocbs3,misid_scaled);
	 RooMultiVarGaussian mvg(("mvg_"+type).c_str(), "mvg",RooArgSet(cbmeanr4,cbsigmar4,misid_scaled),RooArgSet(mu_cbmeanr4,mu_cbsigmar4,mu_sig4),cov_chi_new);

	 //RooMultiVarGaussian misidModel("misidModel", "misidModel",pars,RooArgSet(mu_sig4,mu_cbmeanr4,mu_cbsigmar4),cov_chi);

         //constraints 1d//
 //        RooGaussian fixcon_mean(("fixcon_mean_"+type).c_str(),"fixcon_mean",cbmeanr4,RooConst(cbmeanr4.getVal()),RooConst(1.0));
 //        RooGaussian fixcon_sigma(("fixcon_sigma_"+type).c_str(),"fixcon_sigma",cbsigmar4,RooConst(cbsigmar4.getVal()),RooConst(1.0));
 //        RooGaussian fixcon_num(("fixcon_num_"+type).c_str(),"fixcon_num",misid_scaled,RooConst(misid_scaled.getVal()),RooConst(1.0));
 //        fixcon_mean.Print("s");
 //        fixcon_sigma.Print("s");       
 //        fixcon_num.Print("s");
...
//-----------------------------------Dokonci-------------//
	...
	TFile *fm = new TFile((workspacedir+"plotmisid_"+type+".root").c_str(),"RECREATE");
	canvnew->Write();
	fm->Close();
	delete fm;
	delete canvnew;
...
//        w8->import(cov_chi_new);
 //       w8->import(fixcon_sigma);
 //       w8->import(fixcon_num);
	w8->import(frac_low);
	w8->import(frac_high);
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type, vector <string> &PID)
{
...
//	f->Close();
	...
	RooRealVar cbmeanr2(("cbmeanr2_"+type).c_str(),"cbmeanr", 4300.0 ,4000.0,4600.0);
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponent(string type)
{
	...
	TFile* f = new TFile((misiddir+"/"+kaonfilename+"_"+type+".root").c_str());
	TTree* treek = (TTree*)f->Get("DecayTree");
	...
	TFile* s = new TFile((misiddir+"/"+pionfilename+"_"+type+".root").c_str());
	TTree* treep = (TTree*)s->Get("DecayTree");
	...
	TFile* s3 = new TFile((misiddir+"/"+protonfilename+"_"+type+".root").c_str());
...
//          RooRealVar ses_high_2016("ses_HighFCME_2016","Single event sensitivity",seshigh2016,double(seshigh2016-seshigh2016_err),double(seshigh2016+seshigh2016_err));
	  ...
	  RooRealVar ses_low("ses_LowFCME","ses_LowFCME",seslowrun1,double(seslowrun1-(sigma*seslowrun1_err)),double(seslowrun1+(sigma*seslowrun1_err)));
	  ...
	  RooRealVar ses_low_2016("ses_LowFCME_2016","ses_LowFCME_2016",seslow2016,double(seslow2016-(sigma*seslow2016_err)),double(seslow2016+(sigma*seslow2016_err)));
	  ...
	  RooRealVar ses_high("ses_HighFCME","ses_HighFCME",seshighrun1,double(seshighrun1-(sigma*seshighrun1_err)),double(seshighrun1+(sigma*seshighrun1_err)));
	  ...
	  RooRealVar ses_high_2016("ses_HighFCME_2016","ses_HighFCME_2016",seshigh2016,double(seshigh2016-(sigma*seshigh2016_err)),double(seshigh2016+(sigma*seshigh2016_err)));
...
//	  RooExponential newexpoun("newexpoun", "exponential",Bplus_Corrected_Mass,newexpocon);
	...
	// Perform simultaneous fit of model to data and model_ctl to data_ctl
	simPdf.fitTo(combData,RooFit::Range("low,high"),Extended(kTRUE),Save(kTRUE),ExternalConstraints(RooArgSet(*npr_high_con,*npr_low_con,ses_high_2016_con,ses_high_con,ses_low_2016_con,ses_low_con)));//, ExternalConstrain(rmg)) ; //externalconstraint
...
//          seshigh2015=2.30508e-8;//*(285.0/3000.0);
          seslow2016=3.35e-9;//*double(285.0/3000.0);
          seshigh2016=4.29-9;//*(285.0/3000.0);
	  ...
	  RooRealVar BR("BR","branching fraction",1.0e-8);
	  RooRealVar ses_low("ses_LowFCME","Single event sensitivity",seslowrun1,double(seslowrun1-1.90e-10),double(seslowrun1+1.90e-10));
...
//	  RooExponential newexpoun("newexpoun", "exponential",Bplus_Corrected_Mass,newexpocon);
          ...
          RooRealVar newexpocon_low(("newexpocon_"+typelow).c_str(),"newexpocon_low",Double_t(-0.1),Double_t(0.0));
          RooExponential newexpoun_low(("newexpoun_"+typelow).c_str(), "exponential_low",Bplus_Corrected_Mass,newexpocon_low);

          RooRealVar newexpocon_high(("newexpocon_"+typehigh).c_str(),"newexpocon_high",Double_t(-0.1),Double_t(0.0));
          RooExponential newexpoun_high(("newexpoun_"+typehigh).c_str(), "exponential",Bplus_Corrected_Mass,newexpocon_high);
...
void FitAndSplotB23MuNuDataForTraining::simultaneouspdfminenew()
{
	  ...
	  RooRealVar newexpocon_low(("newexpocon_"+typelow).c_str(),"newexpocon_low",-0.1,0.0);
	  RooExponential newexpoun_low(("newexpoun_"+typelow).c_str(), "exponential_low",Bplus_Corrected_Mass,newexpocon_low);

          RooRealVar newexpocon_high(("newexpocon_"+typehigh).c_str(),"newexpocon_high",-0.1,0.0);
          RooExponential newexpoun_high(("newexpoun_"+typehigh).c_str(), "exponential",Bplus_Corrected_Mass,newexpocon_high);
	...
	//  Bplus_Corrected_Mass.setRange("low",4000,4499);
	  ...
	  // Display fit parameters.
	//RooAddPdf normsigpdf_low("normsigpdf_low","extended sum pdf",RooArgList(*pdf_sig_low,newexpoun,*pdf_misid_low,*pdf_pr_low), RooArgList(nsigform_low,nexp_low,*nmisid_low,*npr_low));
	RooAddPdf normpdf_low(("normpdf_"+typelow).c_str(),"extended sum pdf",RooArgList(newexpoun_low,*pdf_misid_low,*pdf_pr_low), RooArgList(nexp_low,*nmisid_low,*npr_low)); 
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   RooRealVar sig("sig","sig",110000 ,70000,150000);
   RooRealVar bkg("bkg","bkg",7000, 2 ,40000) ;
