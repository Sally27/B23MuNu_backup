...
//---- Setup all thing for 3 different fits--------//
        ...
        RooRealVar cbmeanr4(("cbmeanr4_"+type).c_str(),("cbmeanr4_"+type).c_str(), 4195.4, 3000.0, 4400.0);
        RooRealVar cbsigmar4(("cbsigmar4_"+type).c_str(),("cbsigmar4_"+type).c_str(),511.,400.,600.0);//,0.1,50) ;
        RooRealVar sig4(("sig4_"+type).c_str(),("sig4_"+type).c_str(),1.0,0.7,10.0);
        RooRealVar nr4(("nr4_"+type).c_str(),("nr4_"+type).c_str(),1.24);//,0.1,2.8);//,1.0,1.5);//,0.8, 5.0);//, 0.8, 5.0);
        RooRealVar alphar4(("alphar4_"+type).c_str(),("alphar4_"+type).c_str(),0.6);//,0.1,1.0);//, 0.1, 1.0);
        RooCBShape twocbs3(("twocbs3_"+type).c_str(), ("twocbs3_"+type).c_str(), Bplus_Corrected_Mass, cbmeanr4, cbsigmar4, alphar4, nr4); 
        RooExtendPdf two_ext4(("two_ext4_"+type).c_str(),("two_ext4_"+type).c_str(),twocbs3,sig4);
...
//-------------------------------------------Learn about Errors in RooDataHist----------------------//
 	...
 	binnedData.get(i);
        binnedData.set(binnedData.weight(),(binnedData.weightError(RooAbsData::SumW2)+error->getVal()));
        binnedData.weightError(RooAbsData::SumW2);
...
//---------------------get values for mean from the chi2 fit------------------------------------------------------//
	RooRealVar mu_sig4(("mu_sig4_"+type).c_str(),("mu_sig4_"+type).c_str(),sig4.getVal()*100.0);//,(sig4.getVal()-sig4.getError())*100.0,((sig4.getVal()+sig4.getError())*100.0));
	RooRealVar mu_cbmeanr4(("mu_cbmeanr4_"+type).c_str(),("mu_cbmeanr4_"+type).c_str(),cbmeanr4.getVal());//,cbmeanr4.getVal()-cbmeanr4.getError(),cbmeanr4.getVal()+cbmeanr4.getError());
	RooRealVar mu_cbsigmar4(("mu_cbsigmar4_"+type).c_str(),("mu_cbsigmar4_"+type).c_str(),cbsigmar4.getVal());//,cbsigmar4.getVal()-cbsigmar4.getError(),cbsigmar4.getVal()+cbsigmar4.getError());//,0.1,50) ;
        RooRealVar mu_nr4(("mu_nr4_"+type).c_str(),("mu_nr4_"+type).c_str(),1.24);//,0.1,2.8);//,1.0,1.5);//,0.8, 5.0);//, 0.8, 5.0);
        RooRealVar mu_alphar4(("mu_alphar4_"+type).c_str(),("mu_alphar4_"+type).c_str(),0.6);//,0.1,1.0);//, 0.1, 1.0);
...
//          seshigh=seshighrun1+seshigh2016;
	  ...
	  RooRealVar BR1("BR_low","branching fraction",1.0e-8);
	  RooRealVar BR2("BR_high","branching fraction",1.0e-8);
...
//          RooFormulaVar nsigform_high("nsigform_HighFCME","nsigfromformula","(@3/@2)+(@3/@1)+(@3/@0)",RooArgList(ses_high,ses_high_2015,ses_high_2016,BR));
	  ...
	  RooFormulaVar nsigform_low("nsigform_LowFCME","nsigfromformula","(@2/@1)+(@2/@0)",RooArgList(ses_low,ses_low_2016,BR1));
          RooFormulaVar nsigform_high("nsigform_HighFCME","nsigfromformula","(@2/@1)+(@2/@0)",RooArgList(ses_high,ses_high_2016,BR2));
...
//	  RooExponential newexpoun("newexpoun", "exponential",Bplus_Corrected_Mass,newexpocon);
	...
	// Perform simultaneous fit of model to data and model_ctl to data_ctl
	simPdf.fitTo(combData,RooFit::Range("low,high"),Extended(kTRUE),Save(kTRUE));//, ExternalConstrain(rmg)) ; //externalconstraint
