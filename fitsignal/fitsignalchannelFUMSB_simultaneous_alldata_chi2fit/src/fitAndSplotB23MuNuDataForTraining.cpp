...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall_chi2(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type)
{
	...
	TFile* s = new TFile((dir.at(0)+"/"+misidfilenames.at(0)+"_"+type+"_"+PID+".root").c_str());
...
//        Bplus_Corrected_Mass.setBins(60);
	...
	for(int i(1); i<misidfilenames.size(); i++)
	{
		...
		TFile* f = new TFile((dir.at(i)+"/"+misidfilenames.at(i)+"_"+type+"_"+PID+".root").c_str());
		TTree* treek = (TTree*)f->Get("DecayTree");
...
//---------------------get values for mean from the chi2 fit------------------------------------------------------//
	...
	//---------------------scale----------------//
	 RooRealVar scale_for_misid("scale_for_misid","scale_for_misid",100.0);
         ...
         RooRealVar misid_scaled(("misid_scaled_"+type).c_str(),"misid_scaled",(sig4.getVal()*100.0),(double(sig4.getVal()-sig4.getError())*100.0),(double(sig4.getVal()+sig4.getError())*100.0));
	 ...
	 RooMultiVarGaussian mvg(("mvg_"+type).c_str(), "mvg",RooArgSet(misid_scaled,cbmeanr4,cbsigmar4),RooArgSet(mu_sig4,mu_cbmeanr4,mu_cbsigmar4),cov_chi);
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type)
{
...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig_chi2fit()
{
	  ...
	  ws_low->Print();
         ...
         // misid_low->Close();
          ...
          seslowrun1=1.7321e-9;
          seshighrun1=2.008e-9;
          seslow2015=2.54836e-8;//*double(285.0/3000.0);
          seshigh2015=2.30508e-8;//*(285.0/3000.0);
          seslow2016=3.41290e-9;//*double(285.0/3000.0);
          seshigh2016=3.10853e-9;//*(285.0/3000.0);


          seslow=seslowrun1+seslow2015;
          seshigh=seshighrun1+seshigh2015;
          

	  RooRealVar BR("BR","branching fraction",1.0e-8);
	  RooRealVar ses_low("ses_LowFCME","Single event sensitivity",seslowrun1,double(seslowrun1-1.7e-10),double(seslowrun1+1.7e-10));
          RooRealVar ses_high("ses_HighFCME","Single event sensitivity",seshighrun1,double(seshighrun1-1.94e-10),double(seshighrun1+1.9432e-10));
          ...
          RooRealVar ses_low_2016("ses_LowFCME_2016","Single event sensitivity",seslow2016,double(seslow2016-5.27e-10),double(seslow2016+5.27e-10));
          RooRealVar ses_high_2016("ses_HighFCME_2016","Single event sensitivity",seshigh2016,double(seshigh2016-4.55e-10),double(seshigh2016+4.55e-10));
...
//	  RooExponential newexpoun("newexpoun", "exponential",Bplus_Corrected_Mass,newexpocon);
          ...
          RooRealVar newexpocon_low(("newexpocon_"+typelow).c_str(),"newexpocon_low",-0.3,0.3);
          RooExponential newexpoun_low(("newexpoun_"+typelow).c_str(), "exponential_low",Bplus_Corrected_Mass,newexpocon_low);

          RooRealVar newexpocon_high(("newexpocon_"+typehigh).c_str(),"newexpocon",-0.3,0.3);
