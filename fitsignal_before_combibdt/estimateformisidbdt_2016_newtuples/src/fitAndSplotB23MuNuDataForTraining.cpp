...
vector<double> FitAndSplotB23MuNuDataForTraining::fitmisidcomponentall(vector <string> &dir,vector <string> &misidfilenames, vector <string> &weights, vector <string> &species, string type)
{
	...
	for(int i(1); i<misidfilenames.size(); i++)
	{
		...
		RooDataSet dk("dk","dk",RooArgSet(Bplus_Corrected_Mass),Import(*treek));
		RooDataSet dkw("dkw","dkw", RooArgSet(Bplus_Corrected_Mass,crossfeedweight), Import(*treek), WeightVar(weights.at(i).c_str()),Cut("Bplus_Corrected_Mass>4000 && Bplus_Corrected_Mass<7000"));//, Import(*treew));
		dk.Print();
		dkw.Print();
		Bplus_Corrected_Mass.setBins(60);
...
//	f->Close();
	...
	RooAddPdf twocbs("twocbs","",RooArgList(cballr1,cballr2,newexpoun_low),RooArgList(frac1,frac2,frac3));
...
//	R/ooCBShape twocbs(("twocbs"), "crystal ball", Bplus_Corrected_Mass, cbmeanr2, cbsigmar2, alphar2, nr2);
...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig()
{
	  ...
	  RooRealVar ses_low("ses","Single event sensitivity",ses,9.0e-10,1.0e-9);
          ...
          RooRealVar nexp_low("nexp","n explow",0.1,55000);
