...
//---- Setup all thing for 3 different fits--------//
	...
	vector<double> params;
        ...
        RooRealVar sig4(("sig4"),"frac of events in sig4",2.83,2.0,80.0);
	...
	RooCBShape misidModel3(("misidModel3"), "c3", Bplus_Corrected_Mass, cbmeanr4, cbsigmar4, alphar4, nr4); 
        RooExtendPdf two_ext4("two_ext4","two_ext4",misidModel3,sig4);
...
//--------------------------------------Product of params function and error matrix-------------------------------------------//
	 ...
	 cov_chi_new.Print("v");
         ...
         RooExtendPdf two_ext5("two_ext5","two_ext5",misidModel3,misid_scaled);
...
//	savePullPlot_Sally(*ibou,"h_binnedData",&Bplus_Corrected_Mass,(workspacedir+"pullPlot_misid.root").c_str());
//	TFile fpull((workspacedir+"pullPlot_misid.root").c_str());
//	TCanvas* cpull = (TCanvas*)fpull.Get("pullplot");
	...
	canvTot.Print((workspacedir+"plotMisidFitPretty.pdf").c_str());
	canvTot.Print((workspacedir+"plotMisidFitPretty.root").c_str());
...
//	f->Close();
	...
	RooAddPdf misidModel("misidModel","",RooArgList(cballr1,cballr2,newexpoun_low),RooArgList(frac1,frac2,frac3));

//        RooRealVar sig2(("sig2"),"frac of events in sig2",544.0,450.0,650.0);
//        RooRealVar cbmeanr2(("cbmeanr2"),"cbmeanr", 4058.4 ,3800.0,4200.0);
//        RooRealVar cbsigmar2(("cbsigmar2"),"cbsigmar",400.,300.,1200.0);//,0.1,50) ;
//        RooRealVar nr2(("nr2"),"",0.32, 0.01, 5.0);
//        RooRealVar alphar2(("alphar2"),"",0.6, 0.1, 10.0);
//        RooCBShape misidModel(("misidModel"), "crystal ball", Bplus_Corrected_Mass, cbmeanr2, cbsigmar2, alphar2, nr2);

//	RooRealVar sig2(("sig2"+type).c_str(),"frac of events in sig2",544.0,450.0,650.0);
//	RooRealVar cbmeanr2(("cbmeanr2"+type).c_str(),"cbmeanr", 4058.4 ,4000.0,4200.0);
//	RooRealVar cbsigmar2(("cbsigmar2"+type).c_str(),"cbsigmar",400.,300.,800.0);//,0.1,50) ;
//	RooRealVar nr2(("nr2"+type).c_str(),"",0.32, 0.01, 5.0);
//	RooRealVar alphar2(("alphar2"+type).c_str(),"",0.6, 0.1, 10.0);
//	R/ooCBShape misidModel(("misidModel"), "crystal ball", Bplus_Corrected_Mass, cbmeanr2, cbsigmar2, alphar2, nr2);


	misidModel.fitTo(*binnedData,RooFit::Range("sideband"));
	misidModel.plotOn(ibou) ;
...
//	sig2.setConstant();
	...
	nr2.setConstant();
	alphar2.setConstant();
        cbmeanr1.setConstant();
        cbsigmar1.setConstant();
        nr1.setConstant();
        alphar1.setConstant();
        frac1.setConstant();
        frac2.setConstant();
	...
	Bplus_Corrected_Mass.setRange("high",5500,7000);
	Bplus_Corrected_Mass.setRange("full",4000,7000);
	RooAbsReal* integral_low = misidModel.createIntegral(Bplus_Corrected_Mass,NormSet(Bplus_Corrected_Mass),Range("low"));

	double int_low = integral_low->getVal(); 

	RooAbsReal* integral_high = misidModel.createIntegral(Bplus_Corrected_Mass,NormSet(Bplus_Corrected_Mass),Range("high"));

	double int_high = integral_high->getVal();

	RooAbsReal* integral_full = misidModel.createIntegral(Bplus_Corrected_Mass,NormSet(Bplus_Corrected_Mass),Range("full"));
	...
	RooPlot* ibou2 = Bplus_Corrected_Mass.frame(Title("Binned histogram")) ;
	binnedData->plotOn(ibou2,DataError(RooAbsData::SumW2));
	misidModel.plotOn(ibou2) ;
	misidModel.paramOn(ibou2);
	...
	// saveFitInfo(out, ibou2, 0, misidModel);
...
void FitAndSplotB23MuNuDataForTraining::fitJpsiKData4(bool binnedFit)
{
   ...
   pdf_b->plotOn(frame, RooFit::LineColor(kCyan), RooFit::NormRange("low,high"), RooFit::Components("misidModel"), RooFit::Range("low,high"));
...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig()
{
	  ...
	  ws_low->Print();
          ...
          sesrun1=9.43257e-10;
          ses2015=1.04425e-8*(double(285.0)/double(3000.0));

          ses=sesrun1;//+ses2015;
   

	  RooRealVar BR("BR","branching fraction",1.0e-8);
	  RooRealVar ses_low("ses","Single event sensitivity",ses,9.0e-10,1.0e-9);

          RooRealVar nexp_low("nexp","n explow",0.1,100000);
