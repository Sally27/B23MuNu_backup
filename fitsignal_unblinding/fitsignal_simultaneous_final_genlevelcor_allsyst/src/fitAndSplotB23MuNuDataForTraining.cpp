...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig_chi2fit_100cor()
{
	...
	RooRealVar BR("BR","branching fraction",0);
...
//unbl	RooFitResult* result_sig=simPdfsig.fitTo(combData,RooFit::Range("low,high"),Extended(kTRUE),Save(kTRUE),ExternalConstraints(allsignalcon));//, ExternalConstrain(rmg)) ; //externalconstraint
//unbl	RooFitResult* result_bkg=simPdf.fitTo(combData,RooFit::Range("low,high"),Extended(kTRUE),Save(kTRUE),ExternalConstraints(allbkgcon));//, ExternalConstrain(rmg)) ; //externalconstraint
//unbl
//unbl
//unbl
//unbl	RooArgSet* params = simPdfsig.getParameters(Bplus_Corrected_Mass);
//unbl	params->writeToFile((plotdir+"SimultaneousFitResults_bkgandsigpdf_variables.txt").c_str());
//unbl	ofstream outsbkg((plotdir+"SimultaneousFitResults_bkgandsigpdf_fullfitinfo.txt").c_str());
//unbl	saveFitInfo_simpdf(outsbkg, frame1, 10, &simPdfsig,result_sig);
//unbl
//unbl	RooArgSet* params2 = simPdf.getParameters(Bplus_Corrected_Mass);
//unbl	params2->writeToFile((plotdir+"SimultaneousFitResults_bkgpdf_variables.txt").c_str());
//unbl	ofstream outbkg((plotdir+"SimultaneousFitResults_bkgpdf_fullfitinfo.txt").c_str());
//unbl	saveFitInfo_simpdf(outbkg, frame1, 10, &simPdf,result_bkg);
//unbl
//unbl	bool conv(true);
//unbl	if (conv==true){
//unbl
//unbl		cout<<"starting expliciti bkg fit NO SYST "<<endl; 
//unbl
//unbl		cout<<"dependents "<<endl;
//unbl		RooArgSet* getSet = simPdf.getDependents(&combData);
//unbl		getSet->Print("v");
//unbl
//unbl		cout<<"parameters "<<endl;
//unbl
//unbl		RooArgSet* getVars = simPdf.getParameters(&combData);
//unbl		getVars->Print("v");
//unbl
//unbl		TCanvas* likel = new TCanvas("likel","likel",800,800) ;
//unbl		RooPlot* frame_likel = nmisid_low->frame(Range(310,350),Title("Likelihood plot for nmisidlow low")) ;
//unbl		RooPlot* frame_likel2 = nmisid_high->frame(Range(240,300),Title("Likelihood plot for nmisidlow high")) ;
//unbl		RooPlot* frame_likel3 = BRnorm.frame(Range(5.57e-5,6.67e-5),Title("Likelihood plot for brnorm")) ;
//unbl//		RooPlot* frame_likel4 = npartreco_low.frame(Range(10,60),Title("Likelihood plot for npartrecolow")) ;
//unbl		int migradStatus(5);
//unbl		int hesseStatus(5);
//unbl		int minosStatus(5);
//unbl
//unbl
//unbl		RooAbsReal* nll = simPdf.createNLL(combData,RooFit::Range("low,high"),Extended(kTRUE),ExternalConstraints(allbkgcon));
//unbl		RooAbsReal* pll = nll->createProfile(RooArgList(*nmisid_low,BRnorm,*nmisid_high));
//unbl		RooMinuit m(*nll);
//unbl		m.setStrategy(2);
//unbl		migradStatus=m.migrad();
//unbl		hesseStatus=m.hesse();
//unbl		minosStatus=m.minos();
//unbl		RooFitResult* result_bkg22=m.save();
//unbl		cout<<"migrad status "<<migradStatus<<endl;
//unbl		cout<<"hesse status "<<hesseStatus<<endl;
//unbl		cout<<"minos status "<<minosStatus<<endl;
//unbl		nll->plotOn(frame_likel,ShiftToZero());
//unbl		pll->plotOn(frame_likel,ShiftToZero(),LineColor(kRed));
//unbl		nll->plotOn(frame_likel2,ShiftToZero());
//unbl		pll->plotOn(frame_likel2,ShiftToZero(),LineColor(kRed));
//unbl		nll->plotOn(frame_likel3,ShiftToZero());
//unbl		pll->plotOn(frame_likel3,ShiftToZero(),LineColor(kRed));
//unbl//		nll->plotOn(frame_likel4,ShiftToZero());
//unbl//		pll->plotOn(frame_likel4,ShiftToZero(),LineColor(kRed));
//unbl
//unbl		likel->Divide(2,2) ;
//unbl		likel->cd(1) ; frame_likel->GetYaxis()->SetTitleOffset(1.4) ; frame_likel->Draw() ;
//unbl		likel->cd(2) ; frame_likel2->GetYaxis()->SetTitleOffset(1.4) ; frame_likel2->Draw() ;
//unbl		likel->cd(3) ; frame_likel3->GetYaxis()->SetTitleOffset(1.4) ; frame_likel3->Draw() ;
//unbl//		likel->cd(4) ; frame_likel4->GetYaxis()->SetTitleOffset(1.4) ; frame_likel4->Draw() ;
//unbl		//	frame_likel->Draw();
//unbl		likel->SaveAs((plotdir+"FitResult_LCME_HCME_Likelihood_bkg.pdf").c_str());     
//unbl
//unbl
//unbl	}
//unbl
	...
	if (conv==false){
		...
		cout<<"starting explicit sig and bkg fit ALL SYST "<<endl; 
		...
		for (int i(0); i<5; i++)
		{
			...
			RooAbsReal* nll = simPdfsig.createNLL(combData,RooFit::Range("low,high"),Extended(kTRUE),ExternalConstraints(allsignalcon_syst_100));
			RooMinuit m(*nll);
			m.setStrategy(2);
...
//				RooFitResult* result_sig22=m.save();
				...
				frame_likel->Draw();
				likel->SaveAs((plotdir+"FitResult_LCME_HCME_Likelihood_bkgANDsig.pdf").c_str());     
...
//		likel->SaveAs((plotdir+"FitResult_LCME_HCME_Likelihood_bkgsig.pdf").c_str());     
	...
	saveFitInfoExtended(out_sim, frame1, 2, &simPdfsig,result_sig2);
	...
	TCanvas* cyou = new TCanvas("you","you",600,600) ;
	//c->Divide(2) ;
	//c->cd(1) ; gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.4) ; frame1->Draw() ;
         gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.1) ; frame1->Draw() ;
	cyou->SaveAs((plotdir+"FitResult_LCME.pdf").c_str());

	TCanvas* cyou2 = new TCanvas("you","you",600,600) ;
	//c->Divide(2) ;
	//c->cd(1) ; gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.4) ; frame1->Draw() ;
        gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.1) ; frame2->Draw() ;
	cyou2->SaveAs((plotdir+"FitResult_HCME.pdf").c_str());
...
void FitAndSplotB23MuNuDataForTraining::plotprettysim_chi2_reallynice(string type, string pidcutRun1, string pidcutRun2, string nameofclsroot, string nameofpdf)
{ 
   cout<<"Plotting result of the fit"<<endl;
...
//unbl   model_total->plotOn(frame, RooFit::LineColor(kMagenta), RooFit::Components(("newpartreco_"+type).c_str()),RooFit::NormRange("low,high"), RooFit::Range("low,high"), RooFit::Name("partreco"));
   ...
   canvTot.Print((plotdir+type+pidcutRun1+pidcutRun2+"_"+nameofpdf+"_plotB23MuNuFitPretty_chi2.pdf").c_str());
   canvTot.Print((plotdir+type+pidcutRun1+pidcutRun2+"_"+nameofpdf+"_plotB23MuNuFitPretty_chi2.root").c_str());

   canv.SetLogy();
   canvTot.cd(1);
   canv.DrawClonePad();
   canvTot.Print((plotdir+type+pidcutRun1+pidcutRun2+"_"+nameofpdf+"_plotB23MuNuFitLogyPretty_chi2.pdf").c_str());
   canvTot.Print((plotdir+type+pidcutRun1+pidcutRun2+"_"+nameofpdf+"_plotB23MuNuFitLogyPretty_chi2.root").c_str());
