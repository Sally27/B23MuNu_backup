...
void saveFitInfo_abspdf_norf(ofstream& out, RooPlot* frame, int floatingParams, RooAbsPdf* pdf)
{
	...
	if(frame)
	{
		...
		out<<"Chi2 = "<<d2s(chi2)<<endl;
                out<<"Number of dof(bins)"<<i2s(ndof)<<endl;
                out<<"Number of floating params"<<i2s(ndof-floatingParams)<<endl;
		out<<"Chi2/(numofbins-floatingparams) = "<<d2s(chi2)<<" / "<<i2s(ndof-floatingParams)<<endl;
