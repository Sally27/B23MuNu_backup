...
void FitAndSplotB23MuNuDataForTraining::simfit_fit_both()
{
    TFile f_int((clsdir+"fitresults_sigandbkghypo_forCLs_syst_100.root").c_str(), "READ");
    RooWorkspace* ws_int = (RooWorkspace*)f_int.Get("workspaceFit");
//sally    double CLsBF(1.0e-8);
    ...
    TFile f_int2((clsdir+"fitresults_sigandbkghypo_forCLs_syst_100_old.root").c_str(), "READ");
    RooWorkspace* ws_int2 = (RooWorkspace*)f_int2.Get("workspaceFit");
//sally    double CLsBF(1.0e-8);


    RooAbsPdf* pdf_int2 = ws_int2->pdf("simPdfsig");
    RooAbsPdf* pdf_LL_int2 = ws_int2->pdf("normsigpdf_LowFCME");
    RooAbsPdf* pdf_DD_int2 = ws_int2->pdf("normsigpdf_HighFCME");
    RooAbsData* data_int2 = ws_int2->data("combData");
    RooCategory* cat_int2 = (RooCategory*)ws_int2->cat("sample");
    RooRealVar* BF_int2 = ws_int2->var("BR");
    RooRealVar* nmisidlow_int2 = ws_int2->var("misid_scaled_LowFCME");
    RooRealVar* mass_int2 = ws_int2->var("Bplus_Corrected_Mass");
    RooArgSet allsignallow_con2=ws_int2->argSet("eff_ratio_pr_low_con,eff_ratio_run1_low_err_tot_100_con,eff_ratio_2016_low_err_tot_100_con,jpsik_run1_low_con,jpsik_2016_low_con");
    RooArgSet allsignalhigh_con2=ws_int2->argSet("eff_ratio_pr_high_con,eff_ratio_run1_high_err_tot_100_con,eff_ratio_2016_high_err_tot_100_con,jpsik_run1_high_con,jpsik_2016_high_con");
    RooArgSet allsignalcon_common2=ws_int2->argSet("BRpr_con,BRnorm_con");
    RooArgSet allsignalboth_con2(allsignallow_con2,allsignalhigh_con2,"lol");
    RooArgSet constraints2(allsignalboth_con2,allsignalcon_common2,"lol2");

    pdf_int2->getParameters(*mass_int2)->Print("v") ;
    mass_int2->setRange("high",6999,7000);
    mass_int2->setRange("low",4000,6999);
    mass_int2->setRange("new",4000,7000);
    mass_int2->setRange("high1",6999,7000);
    mass_int2->setRange("low1",4000,6999);
    ...
    clsvalues.push_back(-3.0e-8);
    clsvalues.push_back(-2.5e-8);
    clsvalues.push_back(-2.4e-8);
    clsvalues.push_back(-2.3e-8);
    clsvalues.push_back(-2.2e-8);
    clsvalues.push_back(-2.1e-8);
    clsvalues.push_back(-2.0e-8);
    clsvalues.push_back(-1.95e-8);
    clsvalues.push_back(-1.9e-8);
    clsvalues.push_back(-1.85e-8);
    clsvalues.push_back(-1.8e-8);
    clsvalues.push_back(-1.75e-8);
    clsvalues.push_back(-1.7e-8);
    clsvalues.push_back(-1.65e-8);
    clsvalues.push_back(-1.6e-8);
    clsvalues.push_back(-1.55e-8);
    clsvalues.push_back(-1.5e-8);
    clsvalues.push_back(-1.4e-8);
    clsvalues.push_back(-1.3e-8);
    clsvalues.push_back(-1.2e-8);
    clsvalues.push_back(-1.1e-8);
    clsvalues.push_back(-1.0e-8);
    clsvalues.push_back(-0.5e-8);
    clsvalues.push_back(0.0);
    clsvalues.push_back(0.5e-8);
    ...
    BF_int2->setVal(CLsBF);
    BF_int2->setConstant(true);
    std::cout << "Value of BF_int 1:" <<BF_int->getVal()<<endl;
   ...
   // vector<double> minvals2;
     ...
     RooFitResult* data_intresult_free2 = pdf_int2->fitTo(*data_int2,RooFit::Extended(true),RooFit::Range("low,high"),ExternalConstraints(constraints2),RooFit::Save(true));

     double minimizedll2=data_intresult_free2->minNll();
     minvals2.push_back(minimizedll2);
...
//	likel->SaveAs((plotdir+"ProfileLikelihood_for_BF_own"+ cleanNameString(d2s(BF_int->getVal()))+".pdf").c_str());     
    ...
    TFile file3((clsdir+"fitresults_sigandbkghypo_forCLs_syst_100_final"+cleanNameString(d2s(BF_int->getVal()))+".root").c_str(), "RECREATE");
    RooWorkspace workspaceFit3("workspaceFit", "workspaceFit");
 ...
 //   workspaceFit3.import(*data_high);
    ...
    delete firstfit;
      ...
      vector<double> br_2;
      for (int m(0); m<sizeofvec ; m++)
      {
              double newvalue= br.at(m)-double(0.3e-8);
	      br_2.push_back(newvalue);
      }
      double* mybrval2=&br_2[0];
       



       vector<double> minll;
       vector<double> minll2;
       minll=minvals;
       minll2=minvals2;
       double* myminval=&minll[0];
       double* myminval2=&minll2[0];

       double cLower = *min_element(minll.begin(), minll.end());
       double cLower_2 = *min_element(minll2.begin(), minll2.end());
       double cMaxer = *max_element(minll.begin(), minll.end());

       vector<double> newvec_1;
       vector<double> newvec_2;

       vector<double> newvec_1_shift;
       vector<double> newvec_2_shift;


       for (int o(0); o<sizeofvec ; o++)
       {
       newvec_1.push_back((minll[o]-cLower));
       newvec_2.push_back((minll2[o]-cLower_2));
       newvec_1_shift.push_back((minll[o]-cLower));
       newvec_2_shift.push_back((minll2[o]-cLower_2));
       }
       ...
       double minimum_of_old(0);
       double minimum_of_new(0);
       for (int o(0); o<sizeofvec ; o++)
       {
	       if (cLower==minll.at(o))
               {
		minimum_of_new=br.at(o);
               }
       ...
       double* myminval_scaled=&newvec_1[0];
       double* myminval2_scaled=&newvec_2[0];
       double* myminval_scaled_s=&newvec_1_shift[0];
       double* myminval2_scaled_s=&newvec_2_shift[0];
       double cLower_scaled = *min_element(newvec_1.begin(), newvec_1.end());
       double cLower_2_scaled = *min_element(newvec_2.begin(), newvec_2.end());
       double cMaxer_scaled = *max_element(newvec_1.begin(), newvec_1.end());

       for (int o(0); o<sizeofvec ; o++)
       {
	       if (cLower_2==minll2.at(o))
               {
		minimum_of_old=br.at(o);
               }
...
//	c1->SetLogy();
        ...
        TGraph *grthird =  new TGraph(n,mybrval,myminval2);
        grsecond->Draw("l");
        grthird->SetLineColor(kRed);
        grthird->Draw("l");
        c1->Modified();
        c1->Update();
        auto legend = new TLegend(0.3,0.7,0.6,0.9);
//        legend->SetHeader("The Legend Title","C"); // option "C" allows to center the header
//        legend->AddEntry(h1,"Histogram filled with random numbers","f");
        legend->AddEntry(grsecond,("New:"+d2s(minimum_of_new)).c_str(),"l");
        legend->AddEntry(grthird,("Original:"+d2s(minimum_of_old)).c_str(),"l");
        legend->Draw("same");


        c1->SaveAs((plotdir+"MyOwnProfileLL_compare.pdf").c_str());

       
        TCanvas *c2 = new TCanvas("c2","CLS",200,10,700,500);
        //c1->DrawFrame(0,0,5,1, "CLs");
	c2->SetTitle("ProfileLL");
        gPad->SetLeftMargin(0.15);
        gPad->SetRightMargin(0.15);
	TH1F *hr_scaled = c2->DrawFrame(-5e-8,cLower_scaled,5e-8,cMaxer_scaled);
	hr_scaled->SetXTitle("B (B^{+} #rightarrow #mu^{+} #mu^{-} #mu^{+} #nu) [10^{-8}]");
	hr_scaled->SetYTitle("MinLL_offset");
	hr_scaled->GetYaxis()->SetTitleOffset(1.1);
//        gPad->SetLeftMargin(0.15);
//        gPad->SetRightMargin(0.15);

//	c1->SetLogy();
//        const Int_t n = sizeofvec;
        TGraph *grsecond_scaled =  new TGraph(n,mybrval,myminval_scaled);
        TGraph *grthird_scaled =  new TGraph(n,mybrval,myminval2_scaled);
        grsecond_scaled->Draw("l");
        grthird_scaled->SetLineColor(kRed);
        grthird_scaled->Draw("l");
        c2->Modified();
        c2->Update();
        auto legend_scaled = new TLegend(0.3,0.7,0.6,0.9);
//        legend->SetHeader("The Legend Title","C"); // option "C" allows to center the header
//        legend->AddEntry(h1,"Histogram filled with random numbers","f");
        legend_scaled->AddEntry(grsecond_scaled,"New","l");
        legend_scaled->AddEntry(grthird_scaled,"Original","l");
        legend_scaled->Draw("same");


        c2->SaveAs((plotdir+"MyOwnProfileLL_compare_scaled.pdf").c_str());

        TCanvas *c3 = new TCanvas("c3","CLS",200,10,700,500);
        //c1->DrawFrame(0,0,5,1, "CLs");
	c3->SetTitle("ProfileLL");
        gPad->SetLeftMargin(0.15);
        gPad->SetRightMargin(0.15);
	TH1F *hr_scaled_zoomed = c2->DrawFrame(-3.0e-8,cLower_scaled,2.0e-8,(cMaxer_scaled/2.5));
	hr_scaled_zoomed->SetXTitle("BF (B^{+} #rightarrow #mu^{+} #mu^{-} #mu^{+} #nu) [10^{-8}]");
	hr_scaled_zoomed->SetYTitle("MinLL offset zoomed");
	hr_scaled_zoomed->GetYaxis()->SetTitleOffset(1.1);
//        gPad->SetLeftMargin(0.15);
//        gPad->SetRightMargin(0.15);
...
//        TGraph *grthird_scaled =  new TGraph(n,mybrval,myminval2_scaled);
        ...
        c3->Modified();
        c3->Update();
        auto legend_z = new TLegend(0.3,0.7,0.6,0.9);
//        legend->SetHeader("The Legend Title","C"); // option "C" allows to center the header
//        legend->AddEntry(h1,"Histogram filled with random numbers","f");
//        legend_scaled->AddEntry(grsecond_scaled,"New","l");
//        legend_scaled->AddEntry(grthird_scaled,"Original","l");
        legend_z->AddEntry(grsecond,("New:"+d2s(minimum_of_new)).c_str(),"l");
        legend_z->AddEntry(grthird,("Original:"+d2s(minimum_of_old)).c_str(),"l");
        legend_z->Draw("same");


        c3->SaveAs((plotdir+"MyOwnProfileLL_compare_scaled_zoomed.pdf").c_str());


        TGraph *grsecond_scaled_s =  new TGraph(n,mybrval2,myminval_scaled_s);
        TGraph *grthird_scaled_s =  new TGraph(n,mybrval,myminval2_scaled_s);

        TCanvas *c4 = new TCanvas("c4","CLS",200,10,700,500);
        //c1->DrawFrame(0,0,5,1, "CLs");
	c4->SetTitle("ProfileLL");
        gPad->SetLeftMargin(0.15);
        gPad->SetRightMargin(0.15);
	TH1F *hr_scaled_zoomed_shifted = c4->DrawFrame(-3.0e-8,cLower_scaled,2.0e-8,(cMaxer_scaled/2.5));
	hr_scaled_zoomed_shifted->SetXTitle("BF (B^{+} #rightarrow #mu^{+} #mu^{-} #mu^{+} #nu) [10^{-8}]");
	hr_scaled_zoomed_shifted->SetYTitle("MinLL offset zoomed");
	hr_scaled_zoomed_shifted->GetYaxis()->SetTitleOffset(1.1);
//        gPad->SetLeftMargin(0.15);
//        gPad->SetRightMargin(0.15);
...
//        TGraph *grsecond_scaled =  new TGraph(n,mybrval,myminval_scaled);
//        TGraph *grthird_scaled =  new TGraph(n,mybrval,myminval2_scaled);
        grsecond_scaled_s->Draw("l");
        grthird_scaled_s->SetLineColor(kRed);
        grthird_scaled_s->Draw("l");
        c4->Modified();
        c4->Update();
        auto legend_z_shift = new TLegend(0.3,0.7,0.6,0.9);
//        legend->SetHeader("The Legend Title","C"); // option "C" allows to center the header
//        legend->AddEntry(h1,"Histogram filled with random numbers","f");
//        legend_scaled->AddEntry(grsecond_scaled,"New","l");
//        legend_scaled->AddEntry(grthird_scaled,"Original","l");
        legend_z_shift->AddEntry(grsecond,("New:"+d2s(minimum_of_new)).c_str(),"l");
        legend_z_shift->AddEntry(grthird,("Original:"+d2s(minimum_of_old)).c_str(),"l");
        legend_z_shift->Draw("same");


        c4->SaveAs((plotdir+"MyOwnProfileLL_compare_scaled_zoomed_shifted.pdf").c_str());



        TCanvas *c5 = new TCanvas("c5","CLS",200,10,700,500);
        //c1->DrawFrame(0,0,5,1, "CLs");
	c5->SetTitle("ProfileLL");
        gPad->SetLeftMargin(0.15);
        gPad->SetRightMargin(0.15);
	TH1F *hr_scaled_zoomed_shifted_zoom = c5->DrawFrame(-1.0e-8,cLower_scaled,2.0e-8,(cMaxer_scaled/2.5));
	hr_scaled_zoomed_shifted_zoom->SetXTitle("BF (B^{+} #rightarrow #mu^{+} #mu^{-} #mu^{+} #nu) [10^{-8}]");
	hr_scaled_zoomed_shifted_zoom->SetYTitle("MinLL offset zoomed");
	hr_scaled_zoomed_shifted_zoom->GetYaxis()->SetTitleOffset(1.1);
//        gPad->SetLeftMargin(0.15);
//        gPad->SetRightMargin(0.15);
...
//        legend_z_shift->AddEntry(grthird,("Original:"+d2s(minimum_of_old)).c_str(),"l");
        ...
        c5->SaveAs((plotdir+"MyOwnProfileLL_compare_scaled_zoomed_shifted_zoom2.pdf").c_str());
    ...
    RooArgSet* params3 = pdf_int->getParameters(*mass_int);
    params3->writeToFile((plotdir+"SimultaneousFitResults_bkgpdf_variables_syst_100.txt").c_str());
...
//	c1->SetLogy();
        ...
        TGraph *grsecond =  new TGraph(n,mybrval,myminval);
        grsecond->Draw("l");
        c1->SaveAs((plotdir+"MyOwnProfileLL.pdf").c_str());
...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig_chi2fit_100cor_old()
{
	...
	RooRealVar BR("BR","branching fraction",-1.0,1.0);
...
//         RooMsgService::instance().setGlobalKillBelow(RooFit::WARNING);
	...
	params2->writeToFile((plotdir+"SimultaneousFitResults_bkgpdf_variables_syst_100_old.txt").c_str());
	ofstream outbkg((plotdir+"SimultaneousFitResults_bkgpdf_fullfitinfo_syst_100_old.txt").c_str());
	saveFitInfo_simpdf(outbkg, frame1, 10, &simPdf,result_bkg2);

	TFile file2((clsdir+"fitresults_bkghypo_forCLs_syst_100_old.root").c_str(), "RECREATE");
...
//	workspaceFit2.import(simPdfsig,RecycleConflictNodes());
	...
	params->writeToFile((plotdir+"SimultaneousFitResults_bkgandsigpdf_variables_syst_100_old.txt").c_str());
	ofstream outsbkg((plotdir+"SimultaneousFitResults_bkgandsigpdf_fullfitinfo_syst_100_old.txt").c_str());
	saveFitInfo_simpdf(outsbkg, frame1, 10, &simPdfsig,result_sig2);  


	TFile file3((clsdir+"fitresults_sigandbkghypo_forCLs_syst_100_old.root").c_str(), "RECREATE");
...
//undid	RooRealVar newexpocon_low(("newexpocon_"+typelow).c_str(),"newexpocon_low",double(-0.1),double(0.0));
//undid	RooExponential newexpoun_low(("newexpoun_"+typelow).c_str(), "exponential_low",Bplus_Corrected_Mass,newexpocon_low);

//undid	RooRealVar newexpocon_high(("newexpocon_"+typehigh).c_str(),"newexpocon_high",double(-0.1),double(0.0));
//undid	RooExponential newexpoun_high(("newexpoun_"+typehigh).c_str(), "exponential_high",Bplus_Corrected_Mass,newexpocon_high);
