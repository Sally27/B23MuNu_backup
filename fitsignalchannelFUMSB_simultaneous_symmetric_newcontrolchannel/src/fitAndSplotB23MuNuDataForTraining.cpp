...
{
	TFile f( (datadir+"/"+tuplename+"_"+type+"_Probnnmu3.root").c_str());
	TTree* t = (TTree*)f.Get(treename.c_str());
...
void FitAndSplotB23MuNuDataForTraining::simfitseparateandsavesig()
{
	  ...
	  RooRealVar ses_low("ses_LowFCME","Single event sensitivity",1.89947e-9,1.8e-9,2.0e-9);
          RooRealVar ses_high("ses_HighFCME","Single event sensitivity",2.20e-9,2.1e-9,2.50e-9);
