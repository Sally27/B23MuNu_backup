...
// here hm has data ini it
   ...
   b->Sumw2();
   B23MuNu_gen.t->Draw("qsqmu1andmu2>>m1m2invmass_gen");
// here hg has data ini it
   ...
   TH1F* hm = new TH1F("m1m2invmassmunusel_sel", "m1m2invmassmunusel_sel", 50, -10 , 20000000);
   hm->Sumw2();
   myfile2.t->Draw("newqsqmu1andmu2>>m1m2invmassmunusel_sel");
   double nSel2 = myfile2.t->GetEntries();
   hm->Scale(generalscale);
// here hm has data ini it
   TH1F* hg = new TH1F("mplusmminusinvmassmunusel_gen", "mplusmminusinvmassmunusel_gen", 50, -10 , 20000000);
   hg->Sumw2();
   B23MuNu_gen2.t->Draw("qsqmu1andmu2>>mplusmminusinvmassmunusel_gen");
// here hg has data ini it
   double nGen2 = B23MuNu_gen2.t->GetEntries();

   TH1F* heff = new TH1F("eff", "eff", 50, -10 , 20000000);
   heff->Divide(hm,hg);
   ...
   TH1F* am = new TH1F("m1m2invmasstos_sel", "m1m2invmasstos_sel", 50, -10 , 20000000);
   am->Sumw2();
   myfile3.t->Draw("newqsqmu1andmu2>>m1m2invmasstos_sel");
   double nSel3 = myfile3.t->GetEntries();
   am->Scale(generalscale);
// here hm has data ini it
   TH1F* ag = new TH1F("mplusandmminusinvmasstos_gen", "mplusandmminusinvmasstos_gen", 50, -10 , 20000000);
   ag->Sumw2();
   
   B23MuNu_gen3.t->Draw("qsqmu1andmu2>>mplusandmminusinvmasstos_gen");
// here hg has data ini it
   double nGen3 = B23MuNu_gen3.t->GetEntries();

   TH1F* ha = new TH1F("eff", "eff", 50, -10 , 20000000);
   ha->Divide(am,ag);
   ...
   TH1F* af = new TH1F("m1m2invmasstosandmunu_sel", "m1m2invmasstosandmunu_sel", 50, -10 , 20000000);
   af->Sumw2();
   myfile4.t->Draw("newqsqmu1andmu2>>m1m2invmasstosandmunu_sel");
   double nSel4 = myfile4.t->GetEntries();
   af->Scale(generalscale);
// here hm has data ini it
   TH1F* ai = new TH1F("mplusandmminusinvmasstosandmunu_gen", "mplusandmminusinvmasstosandmunu_gen", 50, -10 , 20000000);
   ai->Sumw2();
   B23MuNu_gen4.t->Draw("qsqmu1andmu2>>mplusandmminusinvmasstosandmunu_gen");
// here hg has data ini it
   double nGen4 = B23MuNu_gen4.t->GetEntries();
   TH1F* aj = new TH1F("eff", "eff", 50, -10 , 20000000);
   aj->Divide(af,ai);
...
//   c->GetYaxis()->SetRangeUser(0.,3.);
   ...
   mp3->Update();
   mp3->SaveAs("plotforreportfinalnewMCzoomq2_20000000.pdf");
