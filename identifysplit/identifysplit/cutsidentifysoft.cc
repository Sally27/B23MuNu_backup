...
//              //veto J/psi
  ...
  TBranch *bpt6 = t2->Branch("identify",&identify,"identify/I");
  TBranch *bpt7 = t2->Branch("invmu1andmu2",&invmu1mu2,"invmu1mu2/D");
  TBranch *bpt8 = t2->Branch("invmu2andmu3",&invmu2mu3,"invmu2mu3/D");
  TBranch *bpt9 = t2->Branch("invmu1andmu3",&invmu1mu3,"invmu1mu3/D");
...
string base="_60andPPUNZIoptBDT";
string pion="B23MuNuFakeSameSignMuonSmallDataSample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut_PionPID";
string kaon="B23MuNuFakeSameSignMuonSmallDataSample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut_KaonPID";
string misid="B23MuNuFakeSameSignMuonSmallDataSample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut";
string mcsig="B23MuNuNewMCNShared_MCtruth_Jpsi_cut_mu1nShared_mu2nShared_mu3nShared_qmincut";
string partreco="B2D3pimufinal_MCtruth_Jpsi_cut_mu3nShared_qmin";
string combi="B23MuNuSignal2012and2011data_corrm_Jpsi_mu1nShared_mu2nShared_mu3nShared_qmin";

mytuples.push_back(pion);
mytuples.push_back(kaon);
mytuples.push_back(misid);
mytuples.push_back(mcsig);
mytuples.push_back(partreco);
mytuples.push_back(combi);

string cuttag = "_idsoft";


ofstream out;
out.open(("Cuts"+cuttag+".txt").c_str());
...
for(vector<string>::const_iterator i = mytuples.begin(); i != mytuples.end(); i++)
{
	string pathname = "/vols/lhcbdisk05/ss4314/cutonBDTs/fullsetwithJpsi60treesincludeP/";
	string filename = *i;
	string realfilename = (filename+base).c_str();
	string decaytreename = "DecayTree";
	string ext = ".root";
	//string cuttag = "_idsoft";
	string namecuttree2 = (realfilename+cuttag).c_str();
	string cut2 = "identify == 3.0 ";

	double probnncut;
	probnncut=cutTree((pathname+realfilename+ext).c_str(), decaytreename, (namecuttree2+ext).c_str(), cut2);
	out<<realfilename<<"  & "<<cut2<<" & "<< probnncut <<" \\\\ "<<endl;
...
out<<"\\end{center}"<<endl;
out<<"\\caption{Cut"<<cuttag<<".txt}"<<endl;
out<<"\\end{table}"<<endl;
...
for(vector<string>::const_iterator i = mytuples.begin(); i != mytuples.end(); i++)
{
	string pathname = "";
	string filename = *i;
	string realfilename = (filename+base+cuttag).c_str();
	string decaytreename = "DecayTree";
	string ext = ".root";
	string probnntag = "_probnnmumpi";
	string namecuttree2 = (realfilename+cuttag).c_str();
	string cut2 = " ";
