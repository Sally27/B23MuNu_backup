...
void copytree(string filename, string decaytreename, string newfilename, vector<string> variables) {


	TFile *oldfile = new TFile(filename.c_str());
	TTree *oldtree = (TTree*)oldfile->Get(decaytreename.c_str());

	oldtree->SetBranchStatus("*",0);

	for(int i(0); i<variables.size(); i++){

		oldtree->SetBranchStatus((variables.at(i)).c_str(),1);
	}
