...
void extremevaluesexchange(string filename, string decaytree, string newfilename, string mom, string eta)
{
	...
	t->SetBranchStatus(mom.c_str(),0);
	t->SetBranchStatus(eta.c_str(),0);
...
void extremevaluesFinal(string filename, string decaytree, string newfilename, string mom, string eta) {
        ...
        t->SetBranchAddress((mom+"_new").c_str(), &mu3_P_new);
        t->SetBranchAddress((eta+"_new").c_str(), &mu3_ETA_new);
        ...
        Double_t mu3_P;
        Double_t mu3_ETA;
        newtree->Branch(mom.c_str(),&mu3_P,(mom+"/D").c_str());
        newtree->Branch(eta.c_str(), &mu3_ETA, (eta+"/D").c_str());
...
void extremevalues(string filename, string decaytree, string newfilename, string mom, string eta, string relbinning) {
        ...
        t->SetBranchAddress(mom.c_str(), &mu3_P);
        ...
        newtree->Branch((mom+"_new").c_str(),&mu3_P_new,(mom+"_new/D").c_str());
        newtree->Branch((eta+"_new").c_str(), &mu3_ETA_new, (eta+"_new/D").c_str());
        ...
        if (relbinning=="muon")
        {
        cout<<"muon binning"<<endl;
        vector<double> binningx = binmuonx();
        }

        if (relbinning=="other")
        {
        cout<<"other binning"<<endl;
        vector<double> binningx = binmuonx();
        }
...
//        cout<<"sizeofntracks: "<<sizeofntracks;
        ...
        for(int n=0; n<t->GetEntries(); ++n)
        {
                ...
                if (mu3_P>binningx.at(sizeofp-1))
                {
                cout<<"Old value of momentum "<<mu3_P<<endl;
                (mu3_P_new=(binningx.at(sizeofp-1)-1));
                cout<<"New value of momentum "<<mu3_P_new<<endl;
                newtree->Fill();
                }

                if (mu3_ETA>binningy.at(sizeofeta-1))
                {
                cout<<"Old value of eta "<<mu3_ETA<<endl;
                (mu3_ETA_new=(binningy.at(sizeofeta-1)-0.01));
                ...
                if (mu3_P_new<binningx.at(0) || mu3_P_new>binningx.at(sizeofp-1) || mu3_ETA_new<binningy.at(0) || mu3_ETA_new>binningy.at(sizeofeta-1))
                {
                        cout<< "Need to thow away this"<<endl;
                        cout<< "Num of thrown away events: "<<numberofthrownevents++<<endl;
                        cout<< "Borders:"<< binningx.at(0) <<" "<<binningx.at(sizeofp-1)<< " "<<binningy.at(0)<< " "<<binningy.at(sizeofeta-1) << " "<<endl;
                }
...
double cutTree(string nametree, string decaytreename, string namecuttree, string cuts)
{
  ...
  //Read in  histogram
   ...
   Double_t Bplus_Corrected_Mass;
   t->SetBranchAddress("Bplus_Corrected_Mass", &Bplus_Corrected_Mass);
   TH1D *h = new TH1D("h","h",100,4000,10000);
   for(int i(0); i<t->GetEntries(); ++i)
   {
   t->GetEntry(i);
...
//     RooDataSet dr("dr","dr",RooArgSet(Bplus_Corrected_Mass),Import(*t));
    ...
    RooRealVar cbmeanrar("cbmeanrar","cbmeanrar", 5248.0, 5200.8, 5400.0);
    RooRealVar cbsigmarar("cbsigmarar","cbsigmarar",0.1,100);//,0.1,50) ;
    RooRealVar cbsignalrar("cbsignalrar","cbsignalrar",8400,8000,10000) ;
...
//     myresults.push_back(cbsignalrar.getVal());
     ...
     myresults.push_back(alpharar.getVal());
	  ...
	  canv4->SaveAs("mcsigfit_NOTMAIN.pdf");
 ...
 //     myresults.push_back(1.4);
       ...
       vector<double> paramfitcombi(string filename, string cut,string tag)
	{
           ...
           cutTree(("combi_cut"+tag+".root").c_str(), "DecayTree", (("combi_cut"+tag+"_LowFCME.root")).c_str(), "(Bplus_corMassERR/Bplus_Corrected_Mass)<0.0259167");
           cutTree(("combi_cut"+tag+".root").c_str(), "DecayTree", (("combi_cut"+tag+"_HighFCME.root")).c_str(), "(Bplus_corMassERR/Bplus_Corrected_Mass)>0.0259167");
...
//	  TTree* tree = gettreecomb(filename, cut,tag);
	  ...
	  canv2->cd(2) ; gPad->SetLeftMargin(0.15) ; framecom1->GetYaxis()->SetTitleOffset(1.4) ; framecom1->GetXaxis()->SetTitle("Corrected Mass") ; framecom1->Draw() ;
	  canv2->cd(3) ; gPad->SetLeftMargin(0.15) ; framecom2->GetYaxis()->SetTitleOffset(1.4) ; framecom2->GetXaxis()->SetTitle("Corrected Mass") ; framecom2->Draw() ;
	  canv2->SaveAs("combifit_NOTMAIN.pdf");
...
//                  params.push_back(alphar2.getVal());
		  ...
		  canvibou->SaveAs(("BinnedHistogramofMISID2cbs"+cut+"notinmain.pdf").c_str());         
                 ...
                 {      
                        ...
                        string partrecosource=filename;
                                              
                        TFile* fpr = new TFile((partrecosource).c_str());
                        TTree* treez = (TTree*)fpr->Get("DecayTree");

		//	TTree* treez =  gettreepartreco(partrecosource.c_str())i;
		        RooRealVar Bplus_Corrected_Mass("Bplus_Corrected_Mass","Bplus_Corrected_Mass",4000,10000);
			...
			RooRealVar cbsigmarpr("cbsigmarpr","cbsigmar",350,700);//,0.1,50) ;
		...
		//	RooArgSet* params=newpartreco.getParameters(Bplus_Corrected_Mass);
                      ...
                      // params.push_back(cballrpr.getVal());
                     ...
                     //  params.push_back(cballrpr2.getVal());
			...
			canvibou2->SaveAs("partrecofit_NOTMAIN.pdf");
                        delete canvibou2;
         ...
         vector<double> fitmisidcuts3samples2(string kaonsamplename, string pionsamplename, string protonsamplename ,string cut)
          {
...
//		  TTree* treep =  gettreemisidpion(pionsamplename, cut);
		  ...
		  canv5->SaveAs("Misid_Components_NOTMAIN.pdf");
		  ...
		  canvtot->Divide(2,2) ;
		  canvtot->cd(1) ; gPad->SetLeftMargin(0.15) ; misidtotal->GetYaxis()->SetTitleOffset(1.4) ; misidtotal->Draw() ;
		  canvtot->SaveAs(("totalmisid_NOTMAIN_"+cut+".pdf").c_str());
...
//                  RooRealVar sig2("sig2","frac of events in sig2",400.0,300.0,800.0);
//		  RooRealVar cbmeanr2("cbmeanr2","cbmeanr", 4058.4 ,3800.0,4200.0);
//		  RooRealVar cbsigmar2("cbsigmar2","cbsigmar",400.,300.,800.0);//,0.1,50) ;
//		  RooRealVar nr2("nr2","",0.32, 0.1, 0.5);
//		  RooRealVar alphar2("alphar2","",0.6, 0.3, 0.95);
...
//		  RooAddPdf twocbs("twocbs","",RooArgList(cballr2),RooArgList());
...
//                  params.push_back(frac1.getVal());
                  ...
                  twocbs.plotOn(ibou2) ;
//                  twocbs.plotOn(ibou2, Components(cballr1), LineColor(kGreen));
//                  twocbs.plotOn(ibou2, Components(cballr2), LineColor(kRed));
		  ...
		  canvibou->SaveAs(("totalmisidfit_NOTMAIN_"+cut+".pdf").c_str());         
...
void CopyTreeSelectedBranches(string filename, string newfilename)
{
        ...
        t->SetBranchStatus("Bplus_PT",1);
        t->SetBranchStatus("mu1_PT",1);
        t->SetBranchStatus("mu2_PT",1);
        t->SetBranchStatus("mu3_PT",1);
        t->SetBranchStatus("mu1_P",1);
        t->SetBranchStatus("mu2_P",1);
        t->SetBranchStatus("mu3_P",1);
        t->SetBranchStatus("Bplus_TAU",1);
        t->SetBranchStatus("Bplus_DIRA_OWNPV",1);
        t->SetBranchStatus("mu1_MINIPCHI2",1);
        t->SetBranchStatus("mu2_MINIPCHI2",1);
        t->SetBranchStatus("mu3_MINIPCHI2",1);
        t->SetBranchStatus("Bplus_pmu_ISOLATION_BDT1_weights",1);
        t->SetBranchStatus("Bplus_ENDVERTEX_CHI2",1);
        t->SetBranchStatus("Bplus_FD_CHI2",1);
        t->SetBranchStatus("KFold",1);
...
void multiply2branches(string filename, string first, string second, string name, string newfilename)
{
	TFile* f = new TFile(filename.c_str());
	TTree* t = (TTree*)f->Get("DecayTree");
...
void addrun1char(string filename, string name, string newfilename)
...

{
	...
	TTree* t = (TTree*)f->Get("DecayTree");
        ...
        double luminosity_2012(2.968);
        double luminosity_2016(1.612);
        double cross_2012(1.0);
        double cross_2016(2.0);
	double geneff_2012=(0.18643);
	double geneff_2016=(0.1977);
        double rec_2012=(1114130.0);
        double rec_2016=(1107715.0);

        double N2012=rec_2012/geneff_2012;
        double N2016=rec_2016/geneff_2016;
        double cond12=luminosity_2012*cross_2012;
        double cond16=luminosity_2016*cross_2016;
	...
	{
		t->GetEntry(i);
	        variable=double(((N2012+N2016)/(N2012*(1.0+double(cond16/cond12)))));
		newtree->Fill();
	}
...
void add2015char(string filename, string name, string newfilename)
...

{
	...
	{
		t->GetEntry(i);
		variable=double(((N2016+N2012)/(N2016*(1.0+double(cond12/cond16)))));
		newtree->Fill();
	}
...
void add2016char(string filename, string name, string newfilename)
...

{
	...
	double geneff_2016=(0.1977);
        ...
        double N2012=double(rec_2012/geneff_2012);
        double N2016=double(rec_2016/geneff_2016);
        double cond12=double(luminosity_2012*cross_2012);
        double cond16=double(luminosity_2016*cross_2016);
...
ValError calaveffofasamplemu1(string filename, string weightfile, string wfilecuts,string weightfilesup, string wfilecutssup ,string muon, string eta, string name, string newfilename) {
...
//			} //end of for
	...
	cout<<"Final numberofthrownevent: "<< numberofthrownevents<<endl;
        ...
        ValError passed(accumulate,sqrt(accumulate_err));
        ValError total(double(count),sqrt(double(count)));

        ValError res=getRatioWeightedBinomial(passed,total);


        cout<<" ValError: "<<res<<endl;
