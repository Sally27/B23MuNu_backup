...
TH3F firstkaon=((3.0)*(*kinkaon));
exit(EXIT_FAILURE);
TH3F firstpion = (*mispion)*(*kinkaon);
//TH3F lspion = *mispion;
...
int h(0);
  ...
  for (int i=1; i<XBINS+1; i++) {
    for (int j=1; j<YBINS+1; j++) {
      ...
      firstkaon.SetBinContent(i,j,k, (holderkaon*holderkaon2));
      holderkaon3+=firstkaon.GetBinContent(i,j,k);
      cout<<"Holder miskaon : "<<holderkaon <<" holder 2 kinkaon"<<holderkaon2<<" Result: "<< holderkaon3<<" in a bin i , j , k:" << i << " " << j << " "<< k << " "<< endl;
      h++;
  }
 }
}
...
int c(0);
  ...
  for (int i=0; i<XBINS; i++) {
    for (int j=0; j<YBINS; j++) {
      ...
      if (newkinematicaldispion.at(c)>10000 || newkinematicaldispion.at(c)<-10000){
      newkinematicaldispion.at(c)=0;
      ...
      if (newkinematicaldiskaon.at(c)>10000 || newkinematicaldiskaon.at(c)<-10000 ){
      cout<<"CHanging value!!!"<<endl;
      newkinematicaldiskaon.at(c)=0;
      }
...
cout<<"num of mis kaons 2nd it:"<<std::accumulate(jop2.begin(),jop2.end(),0.0)<<endl;
cout<<"num of mis pions 2nd it:"<<std::accumulate(jop1.begin(),jop1.end(),0.0)<<endl;

kaonvector.push_back(std::accumulate(jop2.begin(),jop2.end(),0.0));
pionvector.push_back(std::accumulate(jop1.begin(),jop1.end(),0.0));


cout<<"I did it"<<endl;

vector<float> newkinematicaldispioniterate;
vector<float> newkinematicaldiskaoniterate;

vector<float> jop1iterate;
vector<float> jop2iterate;
...
cout<<"num of mis kaons "<<z+2<<"th it:"<<std::accumulate(jop2iterate.begin(),jop2iterate.end(),0.0)<<endl;
cout<<"num of mis pions "<<z+2<<"th it:"<<std::accumulate(jop1iterate.begin(),jop1iterate.end(),0.0)<<endl;
