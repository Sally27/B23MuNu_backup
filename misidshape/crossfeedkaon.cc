...
void eff()
{
  ...
  TFile* f = new TFile("B23MuNuBackgroundDatacutKaonvetoJpsi.root");
...
//             					cout<<"Allocated to bin: "<< i << " "<< j << " " << k << " . Number of events in this bin" << MultiArr[i][j][k] <<endl;	
 //                                               cout<< "Multiarr: "<< i << " "<< j << " " << k << " :" << MultiArr[i][j][k]<<endl;
  ...
  TFile s("/vols/lhcbdisk05/ss4314/week26oct/vetojpsismallsamplenshared/PIDnshared/KisMuonNSharedzeroDLLmumorethanzeroDLLmuminusKmorethanzero/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root"); 
  TH3F *hname =(TH3F*)s.Get("K_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 0.0) && (nShared==0)_All");
  ...
  TFile s2("../../week20july/PIDefficiencies/KisnotMuonDLLKmorethanzeroDLLpKlessthanfive/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root");
...
//   cout<<MultiArr[i][j][k]<< " "<< newcount[c-1]<<endl;
  ...
  Double_t truekindistr[numofbins];
   
  
    for (int u=0; u<numofbins; u++)
  {
  truekindistr[u]=(double(newcount[u])/double(effi2[u]));
