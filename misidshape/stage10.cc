...
vector<double> numofparticles(string filename, string decaytree)
{
  ...
  int acc(0); 
   ...
   acc+=MultiArr[i][j][k];
   c++;
   cout<<MultiArr[i][j][k]<< " "<< newcount[c-1]<< "number of particles: "<< filename << " bin: "<< i << " " << j << " " << k <<endl;
  		 }
  	}
  }
  cout<<"My accumulated numbers"<<acc<<endl;
  return(newcount);  
...
//vector<double> iterationpion;
//vector<double> iterationkaonsub;
...
//{
//vector<int> row;
//iterationkaon.push_back(double(row));
//}

//break;
double arrkaon[10][288];
double arrpion[10][288];
double arrmiskaon[10][288];
double arrmispion[10][288];
double totpion[10];
double totkaon[10];
double totmisid[10];
...
cout<<z<<endl;
arrkaon[z][i] = double((numberofkaons[i] - (mypitokmisid[i]*newitpion[i]))/kaonideff[i]);
cout<<"iterationkaon: "<< arrkaon[z][i]<<endl;
//iterationkaon[z].push_back(double((numberofkaons[i] - (mypitokmisid[i]*newitpion[i]))/kaonideff[i]));
}
...
arrpion[z][i] = double(((numberofpions[i] - myktopimisid[i]*newitkaon[i])/pionideff[i]));
...
arrmispion[z][i]=(double(arrpion[z][i])*double(misidpiontomuon[i]));
...
arrmiskaon[z][i]=(double(arrkaon[z][i])*double(misidkaontomuon[i]));
...
cout<<z<<endl;
arrkaon[z][i] = double((numberofkaons[i] - (mypitokmisid[i]*arrpion[z-1][i]))/kaonideff[i]);
cout<<"iterationkaon: "<< arrkaon[z][i]<<endl;
//iterationkaon[z].push_back(double((numberofkaons[i] - (mypitokmisid[i]*newitpion[i]))/kaonideff[i]));
}
...
arrpion[z][i] = double(((numberofpions[i] - myktopimisid[i]*arrkaon[z-1][i])/pionideff[i]));
cout<<"iterationpion: "<< arrpion[z][i]<<endl;
}
