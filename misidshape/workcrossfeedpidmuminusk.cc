...
//efficace=hname->GetBinContent(resultx,resulty,resultz);
...
float totmisidnobinofcorrm(string misidPIDcalib, string misidcuts, string misidPIDcalib2, string misidcuts2) {
...
addweighttotree(("modified"+filename).c_str(), decaytree, misidPIDcalib, misidcuts, "misidPIDcalib", ("modified"+filename).c_str());

addweighttotree(filename2, decaytree2, idPIDcalib2, idcuts2, "idPIDcalib", ("modified"+filename2).c_str());
addweighttotree(("modified"+filename2).c_str(), decaytree2, misidPIDcalib2, misidcuts2, "misidPIDcalib", ("modified"+filename2).c_str());
//addweighttotree(filename, decaytree); 

///------------Cut the stupid entries------///
cutTree(("modified"+filename).c_str(), decaytree, ("modifiedandcut"+filename).c_str(), "(idPIDcalib != 1.0  && misidPIDcalib != 1.0)");
cutTree(("modified"+filename2).c_str(), decaytree, ("modifiedandcut"+filename2).c_str(), "(idPIDcalib != 1.0  && misidPIDcalib != 1.0)");



///----------- Calculate the probability of misid ----///

addratiofbranches(("modifiedandcut"+filename).c_str(), decaytree, "misidPIDcalib", "idPIDcalib", "misidoverid");
addratiofbranches(("modifiedandcut"+filename2).c_str(), decaytree2, "misidPIDcalib", "idPIDcalib", "misidoverid");


///---------- Reweigh Corrected Mass ------//
//addmultiplicationofbranches(("modifiedandcut"+filename).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM");
//addmultiplicationofbranches(("modifiedandcut"+filename2).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM");
addrealweight(("modifiedandcut"+filename).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species);
addrealweight(("modifiedandcut"+filename2).c_str(), decaytree, "Bplus_Corrected_Mass", "misidoverid", "ReweightedCorrM", species2);


///---------- Add it normalized ----------///
addnormalbranchweight(("modifiedandcut"+filename).c_str(), decaytree, "misidoverid", "normalizedmisidoverid");
addnormalbranchweight(("modifiedandcut"+filename2).c_str(), decaytree2, "misidoverid", "normalizedmisidoverid");
...
//Double_t Bcorrmkaon;
//TFile *F0 = new TFile("B23MuNuBackgroundDatacutKaonvetoJpsi.root");
//TTree *t0 = (TTree*)F0->Get("DecayTree");
//t0->SetBranchAddress("Bplus_Corrected_Mass", &Bcorrmkaon);
//TH1F* kaon = new TH1F(("kaonsample_"+i2s(numofcorrmbins)+"_"+f2s(mincorrm)+"_"+f2s(maxcorrm)).c_str(), ("kaonsample_"+i2s(numofcorrmbins)+"_"+f2s(mincorrm)+"_"+f2s(maxcorrm)).c_str(),numofcorrmbins, mincorrm, maxcorrm);
//for(int b(0); b < t0->GetEntries(); ++b)
//   {
//      t0->GetEntry(b);
//      kaon->Fill(Bcorrmkaon);
//   }


//Double_t Bcorrmpion;
//TFile *F1 = new TFile("B23MuNuBackgroundDatacutPionvetoJpsi.root");
//TTree *t1 = (TTree*)F1->Get("DecayTree");
//t1->SetBranchAddress("Bplus_Corrected_Mass", &Bcorrmpion);
//TH1F* pion = new TH1F(("pionsample_"+i2s(numofcorrmbins)+"_"+f2s(mincorrm)+"_"+f2s(maxcorrm)).c_str(), ("pionsample_"+i2s(numofcorrmbins)+"_"+f2s(mincorrm)+"_"+f2s(maxcorrm)).c_str() ,numofcorrmbins, mincorrm, maxcorrm);
//for(int a(0); a < t1->GetEntries(); ++a)
//   {
//      t1->GetEntry(a);
//      pion->Fill(Bcorrmkaon);
//   }
...
//kaon->SetDirectory(F2);
//pion->SetDirectory(F2);
//corrm->SetDirectory(F2);
corrm->Write("", TObject::kOverwrite);
//kaon->Write("", TObject::kOverwrite);
//pion->Write("", TObject::kOverwrite);
...
//string filename2="/vols/lhcbdisk05/ss4314/misidstudies/checkkaonmisid/samplewithpionpidcuts/dllmutight/PerfHists_Pi_Strip20_MagDown_P_ETA_nTracks.root";

string filename2="/vols/lhcbdisk05/ss4314/misidstudies/checkkaonmisid/samplewithpionpidcuts/dllmuminusktight/PerfHists_Pi_Strip20_MagDown_P_ETA_nTracks.root";
...
cuts2.push_back("Pi_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 3.0) && (nShared==0)_All");
cuts2.push_back("Pi_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 4.0) && (nShared==0)_All");
cuts2.push_back("Pi_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 5.0) && (nShared==0)_All");
cuts2.push_back("Pi_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 10.0) && (nShared==0)_All");

//string filename="/vols/lhcbdisk05/ss4314/misidstudies/checkkaonmisid/samplewithkaonpidcuts/dllmutight/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root";

string filename="/vols/lhcbdisk05/ss4314/misidstudies/checkkaonmisid/samplewithkaonpidcuts/dllmuminusktight/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root";
...
cuts.push_back("K_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 3.0) && (nShared==0)_All");
cuts.push_back("K_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 4.0) && (nShared==0)_All");
cuts.push_back("K_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 5.0) && (nShared==0)_All");
cuts.push_back("K_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 10.0) && (nShared==0)_All");

vector<float> eff;

ofstream myfile;
//myfile.open("MISIDDLLmu.txt");
myfile.open("MISIDDLLmuminusk.txt");
myfile<<"-------------------------MISID with DLLmu cuts on both KAON and PION----------------------"<<endl;
myfile<<"--------TOTAL MISID------------------------CUTS----------------------EFF OF CUT------------"<<endl;
for(int i=0;i<10;i++)
{
float p;
p=totmisidnobinofcorrm(filename, (cuts.at(i)).c_str(), filename2, (cuts2.at(i)).c_str());
eff.push_back(p);
if(i<3)
{
...
//p=totmisidnobinofcorrm(filename, th3nametenth);
//cout<<"TOTAL of MISID NO CORR M BINNING: "<<(p)<<"with cut: "<<cut<<endl;
...
//overallvector.push_back((numofkaonafteriteration(kaonbinmydata , mypitokmisid, truekinematicaldistributionpion, kaonideff, misidkaontomuon))+(numofpionafteriteration(pionbinmydata , myktopimisid, truekinematicaldistributionkaon, pionideff, misidpiontomuon)));



//int z(0);
// 
//while (abs((kaonvector.at(z+1)+pionvector.at(z+1))-(kaonvector.at(z)+pionvector.at(z)))<(0.1*(kaonvector.at(z)+pionvector.at(z))))
//{
//int v(0);
//
//cout<<"STATISTICS"<<endl;
//cout<<"tot at " <<z+1<<" it: "<<(kaonvector.at(z+1)+pionvector.at(z+1))<< endl;
//cout<<"tot at " <<z<< " it: "<<(kaonvector.at(z)+pionvector.at(z)) << endl;
//cout<<"Change: "<< abs((kaonvector.at(z+1)+pionvector.at(z+1))-(kaonvector.at(z)+pionvector.at(z)))<<endl;
//cout<<"0.1* tot at"<< z <<" it:"<<(0.1*(kaonvector.at(z)+pionvector.at(z)))<<endl;
//
//for (int i=0; i<XBINS; i++) {
//    for (int j=0; j<YBINS; j++) {
//      for (int k=0; k<ZBINS; k++) {
//
//      cout<<"I did it again"<<endl;
//      newkinematicaldispioniterate.push_back((pionbinmydata.at(v)-(myktopimisid.at(v)*newkinematicaldiskaon.at(v)))/pionideff.at(v));
//      cout<<"NewKinematicalpion : "<<newkinematicaldispioniterate.at(v)<< endl;
////      if (newkinematicaldispioniterate.at(v)>10000 || newkinematicaldispioniterate.at(v)<-10000){
////      newkinematicaldispioniterate.at(v)=0;
////      cout<<"CHanging value!!!"<<endl;
////      }
//      jop1iterate.push_back(newkinematicaldispioniterate.at(v)*misidpiontomuon.at(v));
//      newkinematicaldiskaoniterate.push_back((kaonbinmydata.at(v)-(mypitokmisid.at(v)*newkinematicaldispion.at(v)))/kaonideff.at(v));
//      cout<<"NewKinematicalkaon : "<<newkinematicaldiskaoniterate.at(v)<< endl;
////      if (newkinematicaldiskaoniterate.at(v)>10000 || newkinematicaldiskaoniterate.at(v)<-10000 ){
////      cout<<"CHanging value!!!"<<endl;
////      newkinematicaldiskaoniterate.at(v)=0;
////      }
//      jop2iterate.push_back(newkinematicaldiskaoniterate.at(v)*misidkaontomuon.at(v));
//      v++;
//  }
// }
//}
...
//
//
//cout<<"num of mis kaons "<<z+3<<"th it:"<<std::accumulate(jop2iterate.begin(),jop2iterate.end(),0.0)<<endl;
//cout<<"num of mis pions "<<z+3<<"th it:"<<std::accumulate(jop1iterate.begin(),jop1iterate.end(),0.0)<<endl;
//
//kaonvector.push_back(std::accumulate(jop2iterate.begin(),jop2iterate.end(),0.0));
//pionvector.push_back(std::accumulate(jop1iterate.begin(),jop1iterate.end(),0.0));
//
//newkinematicaldiskaon.clear();
//newkinematicaldispion.clear();
//
//for (int i=0; i<newkinematicaldiskaoniterate.size(); i++)
//{
//newkinematicaldiskaon.push_back(newkinematicaldiskaoniterate.at(i));
//newkinematicaldispion.push_back(newkinematicaldispioniterate.at(i));
//}
//
//newkinematicaldiskaoniterate.clear();
//newkinematicaldispioniterate.clear();
//
//z++;
//
//
...
//ma/cout<<"iterationkaon: "<< arrkaon[z][i]<<endl;
