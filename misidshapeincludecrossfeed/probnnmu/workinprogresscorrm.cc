...
float totmisidbutincorrm(string misidPIDcalib, string misidcuts, string misidPIDcalib2, string misidcuts2, string tags, Double_t lowboundary, Double_t highboundary) {
...
pionbinmydata=binmydatacorrm(filename2, decaytree2, species2, lowboundary, lowboundary);
...
while (abs(overallvector.at(z+2)-overallvector.at(z+1))>(0.001*(overallvector.at(z+1))))
{
	...
	cout<<"WHILE loop after it num of part:"<< numofpit <<endl;
...
if(l==0)
{
	...
	for (int i=0; i<numofcorrmbins; i++) {
		...
		h.push_back(totmisidbutincorrm(filename, (cuts.at(0)).c_str(), filename2, (cuts2.at(0)).c_str(), (tags.at(0)).c_str(), lowboundary, highboundary));
