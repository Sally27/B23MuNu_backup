...
float avcrossmisid(string filename, string cuts, int p, int eta, int ntracks)
{
  TFile s(filename.c_str());
  TH3F *hname =(TH3F*)s.Get(cuts.c_str());
  vector<float> effi(288);
...
vector<float> newbinmydata(string filename, string decaytree, string species) {
	...
	TH3* h3 = new TH3F(("bindata"+species).c_str(), ("bindata"+species).c_str(), XBINS, xedges, YBINS, yedges, ZBINS, zedges);
...
for (int i=0; i<(p); i++) {
   for (int j=0; j<(eta); j++) {
     for (int k=0; k<(ntracks); k++) {
       ...
       vectortoidentify[i][j][k]=myweights[z];
       cout<<"Give final weight for event"<<endl;
       cout<<vectortoidentify[i][j][k]<< " in a bin i , j , k:" << i << " " << j << " "<< k << " "<< endl;
...
weight=vectortoidentify[resultx][resulty][resultz];
cout<<"mu3_P: "<<mu3_P<<endl;
cout<<"mu3_ETA: "<<mu3_ETA<<endl;
cout<<"nTracks: "<<nTracks<<endl;
cout<<"Weight:" <<weight<<endl;
...
vector<float> newmyfavkin(vector<float> numberofparticles , vector<float> ideff, string species){
	...
	TH3* h3 = new TH3F(("kindistr"+species).c_str(), ("kindistr"+species).c_str(), XBINS, xedges, YBINS, yedges, ZBINS, zedges);
...
cout<<"Average K to pi misid rate:"<< avcrossmisid(ktopimisidfilename,ktopimisidcuts, p, eta, ntracks)<<endl;
cout<<"Average pi to K misid rate:"<< avcrossmisid(pitokmisidfilename,pitokmisidcuts, p, eta, ntracks)<<endl;
...
//                addweighttotreespecial(("modifiedandcut"+tags+filename).c_str(), decaytree, weightfinalkaon, "crossfeedweight", "try.root"); 
               }       
...
if(l==1)
{
	...
	string kaonsample="/vols/lhcbdisk05/ss4314/cutonBDTs/fullsetwithJpsi/SSmuonStrSmallDatapreparetuple/B23MuNuFakeSameSignMuonSmallDataSample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut_KaonPID_PUNZIoptBDT.root";
         ...
         string maintag="probnnmuminuspitighter";
...
string filename2="/vols/lhcbdisk05/ss4314/tightPIDinvestigation/probnnmuandpi/samples/rangezerotoone/pion/PerfHists_Pi_Strip20_MagDown_P_ETA_nTracks.root";
...
string filename="/vols/lhcbdisk05/ss4314/tightPIDinvestigation/probnnmuandpi/samples/rangezerotoone/kaon/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root";
...
cuts3.push_back("Mu_(IsMuon==1.0) && (DLLmu > 0.0) && ((DLLmu - DLLK) > 0.0) && (nShared==0) && ((V2ProbNNmu - V2ProbNNpi) > 0.8)_All");
	...
	for(int i=0;i<10;i++)
	{
		...
		double e;
		e=calaveffofasample("/vols/lhcbdisk05/ss4314/cutonBDTs/fullsetwithJpsi/MCSigpreparetuple/B23MuNuNewMCNShared_MCtruth_Jpsi_cut_mu1nShared_mu2nShared_mu3nShared_qmincut_PUNZIoptBDT.root","DecayTree", filename3 , (cuts3.at(i)).c_str(),"PIDCalib",("newmc"+tags.at(i)+".root").c_str());
	...
	out<<"\\end{center}"<<endl;
	out<<"\\caption{SPECIALprobnnmuminuspitigthertag.txt}"<<endl;
	out<<"\\end{table}"<<endl;
...
if(l==3)
{
	...
	string species = "kaon";
	string species2 = "pion";
	newbinmydata(filename, decaytree, species);
}
...
statistics= newbinmydata("B23MuNuFakeSameSignMuonSmallDataSample_mu3isNotMuon_mu3inMuonAcc_Jpsi_cut_mu1nShared_mu2nShared_qmincut_PionPID_PUNZIoptBDT.root", "DecayTree", "pion");
int numofpioninabin;
int num[18][4][4];
int z=0;
...
ntracks=binz();
        ...
        ofstream out;
        out.open("countthepions.tex");
        out<<"\\begin{table}[ht]"<<endl;
        out<<"\\begin{center}"<<endl;
        out<<"\\begin{tabular}{| l | l | l | l |}"<<endl;
        out<<"\\hline"<<endl;
        out<<"P & ETA & nTracks & Number of Events  \\\\ "<<endl;
        out<<"\\hline"<<endl;
        ...
        for (int i=0; i<18; i++) {
                for (int j=0; j<4; j++) {
                        for (int k=0; k<4; k++) {
                                ...
                                num[i][j][k]=statistics.at(z);
                                cout<< num[i][j][k] << " in a bin P: " << mymombin.at(i) <<" to "<< mymombin.at(i+1) << " ETA: " << eta.at(j) <<" to "<< eta.at(j+1) << " nTracks: " << ntracks.at(k) <<" to "<< ntracks.at(k+1) << " "<< endl;
                                out<< mymombin.at(i) <<" to "<< mymombin.at(i+1) << " & " << eta.at(j) <<" to "<< eta.at(j+1) << " & " << ntracks.at(k) << " to " << ntracks.at(k+1) << " & "<< num[i][j][k] << " \\\\ "<<endl;   
                                z++;
                        }
                }
        }
...
int togetherpioninbinetaandntracks;
 
      for (int g=0; g<18; g++) { 
      cout<< num[g][1][1] << " in a bin i , j , k:" << g << " " << 1 << " "<< 1 << " "<< endl;
       togetherpioninbinetaandntracks+=num[g][1][1];     
 }
