...
//-------------------------------------------PID Calib ID histograms and cuts---------------------------------------//
        ...
        string idPIDcalib =("/vols/lhcb/ss4314/PIDCalibEfficiencies/ideff/protontighter/PerfHists_P_"+stripping+"_"+polarity+"_P_ETA.root").c_str();
        string idcuts ="P_IsMuon==0.0 && InMuonAcc==1.0 && DLLp > 5.0 && ((DLLp - DLLK) > 5.0)_All";
...
//------------------------------------------Crossfeed Hitograms and Cuts--------------------------------------------//
        string ktopimisidfilename=("/vols/lhcb/ss4314/PIDCalibEfficiencies/crossfeedeff/protontopiontighter/PerfHists_P_"+stripping+"_"+polarity+"_P_ETA.root").c_str();
        string ktopimisidcuts="P_IsMuon==0.0 && InMuonAcc==1.0 && DLLK < 0.0 && DLLp < 5.0_All";

        string pitokmisidfilename=("/vols/lhcb/ss4314/PIDCalibEfficiencies/crossfeedeff/protontopiontighter/PerfHists_Pi_"+stripping+"_"+polarity+"_P_ETA.root").c_str();
        string pitokmisidcuts="Pi_IsMuon==0.0 && InMuonAcc==1.0 && DLLp > 5.0 && ((DLLp - DLLK) > 5.0)_All";



//-----------------------------------------Muon Sample and Muon PIDCalib Sample-----------------------------------------//
	string mcmuonsample="/vols/lhcb/ss4314/cutonBDTs/MisidBackTonomu1mu3ETAlesstightBDTcut/B23MuNuMCallvar_MCtruth_trigger_Jpsi_mu1nShared_mu2nShared_mu3nShared_qmincut_CombBasic_MisidBasic.root";
...
//         return(1);
 	...
 	myfile<<"-------------------------MISID with Probnn cuts on both PROTON and PION----------------------"<<endl;
 	myfile<<"--------TOTAL MISID------------------------CUTS----------------------EFF OF CUT------------"<<endl;
 
 	myfile0<<"-------------------------MISID with Probnn cuts on both PROTON and PION----------------------"<<endl;
 	myfile0<<"--------TOTAL MISID------------------------CUTS----------------------EFF OF CUT------------"<<endl;
 
 
 	myfile4<<"-------------------------Trade off  - Probnn cuts PROTON, PION, MUON----------------------"<<endl;
 	...
 	{
 		float p;
 		p=totmisidnobinofcorrmimp(protonsample,pionsample,"proton","pion",filename, (cuts.at(i)).c_str(), filename2, (cuts2.at(i)).c_str(), idPIDcalib, idcuts,idPIDcalib2, idcuts2, ktopimisidfilename, ktopimisidcuts, pitokmisidfilename, pitokmisidcuts ,(tags.at(i)).c_str(), stripping, polarity);
 		eff.push_back(p);
