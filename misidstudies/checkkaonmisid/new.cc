...
//
  ...
  TFile s4("justismuonandinmuondllmu0/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root");
  TH3F *hname4 =(TH3F*)s4.Get("K_IsMuon==1.0 && DLLmu > 0.0_All");

  TFile s5("justismuonandinmuondllmu0andddlk/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root");
  TH3F *hname5 =(TH3F*)s5.Get("K_IsMuon==1.0 && DLLmu > 0.0 && (DLLmu - DLLK > 0)_All");
  ...
  TH2D *h2f = new TH2D("h2f","MIS-ID rate as a function of kaon momentum",18,0,100000,18,0,0.03);
  ...
  for (int i=1; i<(p+1); i++) {
   ...
   h2f->Fill(plotvarx2[y2],plotvary2[y2],1);
   y2++;
   }
  }
  }
  ...
  TH2D *h2g = new TH2D("h2g","MIS-ID rate as a function of kaon momentum",18,0,100000,18, 0.0, 0.03);
  ...
  Double_t plotvary3 [19];
  Double_t plotvarx3 [19];
  Int_t y3(0);
  ...
  for (int i=1; i<(p+1); i++) {
   ...
   plotvary3[y3] = hname3->GetBinContent(i,j,k);
   cout<<" Efficiency: " << plotvary3[y3] << " in a bin i , j , k:" << i << " " << j << " "<< k << " "<< endl;
   plotvarx3[y3] = Double_t(PArr[i-1]);
   cout<<" Momwntum: " << plotvarx3[y3] << endl;
   h2g->Fill(plotvarx3[y3],plotvary3[y3],1);
   //h2g->Draw("same","SCAT");
   y3++;
  ...
  TH2D *h2m = new TH2D("h2m","MIS-ID rate as a function of kaon momentum",18,0,100000,18, 0.0, 0.03);
  h2m->SetMarkerStyle(27);
  h2m->SetMarkerColor(4);
  h2m->GetXaxis()->SetTitle("Momentum");
  h2m->GetYaxis()->SetTitle("MIS-ID rate");


  Double_t plotvary4 [19];
  Double_t plotvarx4 [19];
  Int_t y4(0);
  ...
  for (int i=1; i<(p+1); i++) {
   ...
   plotvary4[y4] = hname4->GetBinContent(i,j,k);
   cout<<" Efficiency: " << plotvary4[y4] << " in a bin i , j , k:" << i << " " << j << " "<< k << " "<< endl;
   plotvarx4[y4] = Double_t(PArr[i-1]);
   cout<<" Momwntum: " << plotvarx4[y4] << endl;
   h2m->Fill(plotvarx4[y4],plotvary4[y4],1);
      y4++;
  ...
  TH2D *h2n = new TH2D("h2n","MIS-ID rate as a function of kaon momentum",18,0,100000,18, 0.0, 0.03);
  h2n->SetMarkerStyle(28);
  h2n->SetMarkerColor(6);
  h2n->GetXaxis()->SetTitle("Momentum");
  h2n->GetYaxis()->SetTitle("MIS-ID rate");


  Double_t plotvary5 [19];
  Double_t plotvarx5 [19];
  Int_t y5(0);
  ...
  for (int i=1; i<(p+1); i++) {
   ...
   plotvary5[y5] = hname5->GetBinContent(i,j,k);
   cout<<" Efficiency: " << plotvary5[y5] << " in a bin i , j , k:" << i << " " << j << " "<< k << " "<< endl;
   plotvarx5[y5] = Double_t(PArr[i-1]);
   cout<<" Momwntum: " << plotvarx5[y5] << endl;
   h2n->Fill(plotvarx5[y5],plotvary5[y5],1);
      y4++;
