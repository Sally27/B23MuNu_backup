...
void plotvariable_one(string k, double low, double high, string xaxis, string filenamereco, string year, double scalecalc, string addon, string legtext)
{
...
//  cout<<"This is scale2: "<<scale<<endl;
  ...
  gStyle->SetOptStat(0);
   ...
   a->GetYaxis()->SetTitleOffset(1.4);
   a->SetMinimum(0.0);
   a->SetMaximum(yaxismax+(0.1*yaxismax));
   a->Draw();

   Double_t xl1=.65, yl1=0.6, xl2=xl1+.3, yl2=yl1+.3;
    TLegend *leg = new TLegend(xl1,yl1,xl2,yl2);
    leg->SetBorderSize(0);
    leg->AddEntry(a,legtext.c_str(),"l");   // h1 and h2 are histogram pointers
    leg->Draw("same");

  
   mp3->Update();
   mp3->SaveAs(("variable_"+addon+"_"+k+"_Scaled_ONLY"+year+"_"+d2s(scalecalc)+".pdf").c_str());
