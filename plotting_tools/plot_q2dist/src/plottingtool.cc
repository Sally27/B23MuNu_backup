...
int main()
{
	...
	plotvariable_partrecostudy_checkcorr("Bplus_Corrected_Mass", 2000, 10000, "Corrected #mu^{+} #mu^{-} #mu^{+} mass [MeV/c^{2}] ", "file1.root","file2.root",year, 1.0);
        ...
        string filepartreco2="/vols/lhcb/ss4314/final_tuples_analyser/mc/partreco_mc/2012/bin/MC_PartReco_MCtruth_trigger_Jpsi_mu3nShared.root"; 
        plotvariable_one("invmu1andmu2", 200.0, 1400.0, "M(#pi^{+}#pi^{-})[MeV/c^{2}]", filepartreco2, "2012", 1.0);
        plotvariable_one("invmu2andmu3", 200.0, 4000.0, "M(#pi^{-}#mu^{+})[MeV/c^{2}]", filepartreco2, "2012", 1.0);
        plotvariable_one("invmu1andmu3", 200.0, 4000.0, "M(#pi^{+}#mu^{+})[MeV/c^{2}]", filepartreco2, "2012", 1.0);
