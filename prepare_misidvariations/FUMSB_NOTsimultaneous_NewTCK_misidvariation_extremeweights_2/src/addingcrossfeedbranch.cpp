...
void createPIDvariation(string filename, string cuts, int p, int eta, string  polarity, string stripping, int toy)
{
       ...
       for (int ntoy(0); ntoy<toy; ntoy++)
       {

	TFile* s = new TFile((filename).c_str(),"READ");
	...
	TH2F *hname =(TH2F*)s->Get(cuts.c_str());
        ...
        cout<<"Creating Histogram "<<ntoy<<endl;
	TFile *F1 = new TFile(("../generatedtuples/"+stripping+"_"+cleanNameString(cuts)+"_"+i2s(ntoy)+".root").c_str(),"RECREATE");
	TH2D* newPIDhist =(TH2D*)hname->Clone();
	newPIDhist->SetName((cleanNameString(cuts)+"_new").c_str());
	...
	for (int i(1); i<(p+1); i++) {
                        ...
                        newvalue=gRandom->Gaus(hname->GetBinContent(i,1),hname->GetBinError(i,1)); //because only one bin of eta

                        for (int j(1); j<(eta+1); j++) { //because only one bin of eta
    
				int u(0);
				while (newvalue==0)
				{
					cout<<"Using "<<u<<" dozadu"<<endl;
					newvalue=gRandom->Gaus(hname->GetBinContent(i-u,1),hname->GetBinError(i-u,1));
					u++;
					if ((i-u)==0)
					{
				...
				if (newvalue>1)
				{
					...
					newvalue=1.0;
                        ...
                        newPIDhist->SetBinContent(i,j,newvalue);
			cout<<filename<<cuts<<"ID rate: " << hname->GetBinContent(i,j)<<" with error "<<hname->GetBinError(i,j)<< " in a bin i , j , k:" << i << " " << j << endl;
        		cout<<"Filling with "<<newvalue<<endl; 
      		        }

			
	}

       newPIDhist->Write("", TObject::kOverwrite);
...
void plotPIDvariation(string filename, string cuts, int p, int eta, string  polarity, string stripping, int toy)
{
       ...
       for (int i(1); i<(p+1); i++) {
	       for (int j(1); j<(eta+1); j++) {
                       ...
                       TH1F *hist = new TH1F(("Distribution_Bin_Momentum_"+i2s(i)+"_Eta"+i2s(j)).c_str(),"Distribution",100,0.0,0.01); 

		       for (int ntoy(0); ntoy<toy; ntoy++)
		       {

			       TFile* s = new TFile(("../generatedtuples/"+stripping+"_"+cleanNameString(cuts)+"_"+i2s(ntoy)+".root").c_str(),"READ");
			       if(!s){cerr<<"no file found"<<endl;return;}
			       TH2F *hname =(TH2F*)s->Get((cleanNameString(cuts)+"_new").c_str());
			       if(!hname){cerr<<"no hist found"<<endl;return;}
			       vector<float> effi;
			       Int_t z(0);
                               float value(0); 
			       value=hname->GetBinContent(i,j);
		      ...
		      TFile* ori = new TFile((filename).c_str(),"READ");
		      if(!ori){cerr<<"no file found"<<endl;return;}
		      TH2F *hnameori =(TH2F*)ori->Get(cuts.c_str());
		      if(!hnameori){cerr<<"no hist found"<<endl;return;}

                      TLine *l = new TLine(double(hnameori->GetBinContent(i,j)),c1->GetUymin(),double(hnameori->GetBinContent(i,j)),c1->GetUymax());
                      l->SetLineColor(kRed);
                      l->SetLineWidth(4);
                      l->Draw();
                      c1->Update();
                      TLine *l2 = new TLine(double(hnameori->GetBinContent(i,j))-double(hnameori->GetBinError(i,j)),0.0,double(hnameori->GetBinContent(i,j))+double(hnameori->GetBinError(i,j)),0.0);
                      l2->SetLineColor(kRed);
                      l2->SetLineWidth(4);
                      l2->Draw();


                      c1->SaveAs(("../generatedplots/"+stripping+"_"+cleanNameString(cuts)+"_RandomDistribution_Bin_Momentum_"+i2s(i)+"_Eta"+i2s(j)+".pdf").c_str());
                      delete ori;
                      delete l;
                      delete l2;
                      delete hist;
