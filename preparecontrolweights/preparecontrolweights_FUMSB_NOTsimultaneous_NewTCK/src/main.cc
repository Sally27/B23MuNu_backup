...
int main(int argc, char *argv[]){
	...
	if(stripping=="Turbo16"||stripping=="Turbo15")
	{
		...
		for(int h(0); h<1; h++)
		{
			...
			string kaoneffpath =("/vols/lhcb/ss4314/PIDCalibEfficienciesID_Urania_vs6r2p1/ideff/kaon/PerfHists_K_"+stripping+"_MagDown_P_ETA.root").c_str(); 
			string kaoneffpathup =("/vols/lhcb/ss4314/PIDCalibEfficienciesID_Urania_vs6r2p1/ideff/kaon/PerfHists_K_"+stripping+"_MagUp_P_ETA.root").c_str();

			string kaonidcut ="K_Brunel_IsMuon==0.0 && Brunel_InMuonAcc==1.0 && Brunel_DLLK > 0.0 && ((Brunel_DLLp - Brunel_DLLK) < 5.0)_All"; 
