	...
	//	canvshifttotal->Divide(4,4);
		...
		for(int h(14); h<15; h++)
		{
			...
			int indexofcut=h;
                        ...
                        string kaoneffpath =("/vols/lhcb/ss4314/PIDCalibEfficiencies/ideff/kaon/PerfHists_K_"+stripping+"_"+polarity+"_P_ETA.root").c_str(); 
...
//			string mcmuonhist="B23MuNuMCallvar_MCtruth_trigger_Jpsi_qmincut_CombBasic_MisidBasic.root";
			...
			mu3eff= calaveffofasamplekaon(mcmuonhist, kaoneffpath , kaonidcut, "mu3_P", "mu3_ETA", "mu3weight", mcmuonhist);
			...
			addmultiplicationofbranches(mcmuonhist, "mu1weight","mu2weight","mu3weight","mcweight",("B23MuNuMC"+mcname.at(indexofcut)+stripping+polarity+type+".root").c_str());
	}
