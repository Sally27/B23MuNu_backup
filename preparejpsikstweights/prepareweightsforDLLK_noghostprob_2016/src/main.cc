	...
	if(l==0.0)
	{
		...
		for(int h(14); h<15; h++)
		{
			...
			int indexofcut=h;
                        ...
                        string kaoneffpath =("/vols/lhcb/ss4314/PIDCalibEfficienciesID_for_JpsiKst/2016/id/kaon/PerfHists_K_"+stripping+"_MagDown_P_ETA.root").c_str(); 
                        string kaoneffpathup =("/vols/lhcb/ss4314/PIDCalibEfficienciesID_for_JpsiKst/2016/id/kaon/PerfHists_K_"+stripping+"_MagUp_P_ETA.root").c_str();
                        string kaonidcut ="K_Brunel_DLLK > 0.0 && Brunel_IsMuon==0.0 && (Brunel_DLLK-Brunel_DLLp) > 0.0_All"; 

                        //---------------GET MC EFFICIENCIES FROM PIDCALIB-------??
                        string muonideffpath=("/vols/lhcb/ss4314/PIDCalibEfficienciesID_for_JpsiKst/2016/id/muon/PerfHists_Mu_"+stripping+"_MagDown_P_ETA.root").c_str();
                        string muonideffpathup=("/vols/lhcb/ss4314/PIDCalibEfficienciesID_for_JpsiKst/2016/id/muon/PerfHists_Mu_"+stripping+"_MagUp_P_ETA.root").c_str();
                        string muonidcut="Mu_(Brunel_MC15TuneV1_ProbNNmu > 0.5)_All";
			...
			mu3eff= calaveffofasamplekaon(mcmuonhist, kaoneffpath , kaonidcut, kaoneffpathup , kaonidcut ,"K_P", "K_ETA", "Kweight", mcmuonhist);
			cutTree(mcmuonhist, "DecayTree", mcmuonhist, "Kweight != -1.0");

			cout<<"mu1 id efficiency: "<<mu1eff<<endl;
			cout<<"mu2 id efficiency: "<<mu2eff<<endl;
			cout<<"K id efficiency: "<<mu3eff<<endl;
			double finalmceff;
			...
			addmultiplicationofbranches(mcmuonhist, "mu1weight","mu2weight","Kweight","mcweight",(name+".root").c_str());
	}
        cout<<"la la"<<endl; 
