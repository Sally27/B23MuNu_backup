...
int main(){
	...
	ofstream out;
	out.open("EfficienciesQuickBothPiandKVetoKst.tex");
	...
	out<<"or strip and reco & 0.144209 & 1277956(8861780) & TBD  \\\\ "<<endl;
        ...
        if (p==0)
        {
	string pathname = "/vols/lhcb/ss4314/tuplesallvar/JpsiKst_fromB2XMuMu_Data/2012/";
	string filename = "BuKstMuMu2012";
...
//"(sqrt(((sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(493.667*493.667)) + (K_PE))*(sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(493.667*493.667)) + (K_PE)))-(((K_PX+Pi_PX)*(K_PX+Pi_PX))+((K_PY+Pi_PY)*(K_PY+Pi_PY))+ ((K_PZ+Pi_PZ)*(K_PZ+Pi_PZ))))"	
        varia.push_back("trigger");
        varia.push_back("relevantcuts");
        varia.push_back("KstVeto");        
        varia.push_back("PIDK");  
        varia.push_back("JpsiKVeto");
        varia.push_back("tightMuon");
        varia.push_back("tightKaon");
        varia.push_back("phiveto");
        varia.push_back("ptcut");       

        varia.push_back("PionAsMuon1Hyp");
        varia.push_back("PionAsMuon2Hyp");
  //      varia.push_back("PIDPi");
	...
	vals.push_back("B0_DIRA_OWNPV > 0.99996 && B0_ENDVERTEX_CHI2 < 7 && mu1_ghost < 0.35 && mu2_ghost < 0.35 && K_ghost < 0.35 && Pi_ghost < 0.35 && mu1_TRACK_CHI2 < 3.0 && mu2_TRACK_CHI2 < 3.0 && K_TRACK_CHI2 < 3.0 && Pi_TRACK_CHI2 < 3.0 && mu1_MINIPCHI2 > 9.0 && mu2_MINIPCHI2 > 9.0 && K_MINIPCHI2 > 9.0 && Pi_MINIPCHI2 > 9.0 ");
        vals.push_back("(abs(Kst_MM-895.81))<100");
        vals.push_back("((K_PIDp-K_PIDK) < 0) && K_PIDK > 0 && K_isMuon==0.0");
      ...
      //  vals.push_back("abs((sqrt(((sqrt(((K_PX*K_PX)+(K_PY*K_PY)+(K_PZ*K_PZ))+(105.658*105.658)) + (mu1_PE))*(sqrt(((K_PX*K_PX)+(K_PY*K_PY)+(K_PZ*K_PZ))+(105.658*105.658)) + (mu1_PE)))-(((mu1_PX+K_PX)*(mu1_PX+K_PX))+((mu1_PY+K_PY)*(mu1_PY+K_PY))+ ((mu1_PZ+K_PZ)*(mu1_PZ+K_PZ)))))-3096.9)>60");      
        ...
        vals.push_back("(K_ProbNNk*(1-K_ProbNNp))>0.05");
...
//        vals.push_back("Pi_PIDp < 0 && Pi_PIDK < 0 && Pi_isMuon==0.0");

        vals.push_back("abs((sqrt(((sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(105.658*105.658)) + (mu1_PE))*(sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(105.658*105.658)) + (mu1_PE)))-(((mu1_PX+Pi_PX)*(mu1_PX+Pi_PX))+((mu1_PY+Pi_PY)*(mu1_PY+Pi_PY))+ ((mu1_PZ+Pi_PZ)*(mu1_PZ+Pi_PZ)))))-3096.9)>60.0");      
        vals.push_back("abs((sqrt(((sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(105.658*105.658)) + (mu2_PE))*(sqrt(((Pi_PX*Pi_PX)+(Pi_PY*Pi_PY)+(Pi_PZ*Pi_PZ))+(105.658*105.658)) + (mu2_PE)))-(((mu2_PX+Pi_PX)*(mu2_PX+Pi_PX))+((mu2_PY+Pi_PY)*(mu2_PY+Pi_PY))+ ((mu2_PZ+Pi_PZ)*(mu2_PZ+Pi_PZ)))))-3096.9)>60.0");
       
        vals.push_back("abs((sqrt(((sqrt(((K_PX*K_PX)+(K_PY*K_PY)+(K_PZ*K_PZ))+(105.658*105.658)) + (mu1_PE))*(sqrt(((K_PX*K_PX)+(K_PY*K_PY)+(K_PZ*K_PZ))+(105.658*105.658)) + (mu1_PE)))-(((mu1_PX+K_PX)*(mu1_PX+K_PX))+((mu1_PY+K_PY)*(mu1_PY+K_PY))+ ((mu1_PZ+K_PZ)*(mu1_PZ+K_PZ)))))-3096.9)>60.0");      
        vals.push_back("abs((sqrt(((sqrt(((K_PX*K_PX)+(K_PY*K_PY)+(K_PZ*K_PZ))+(105.658*105.658)) + (mu2_PE))*(sqrt(((K_PX*K_PX)+(K_PY*K_PY)+(K_PZ*K_PZ))+(105.658*105.658)) + (mu2_PE)))-(((mu2_PX+K_PX)*(mu2_PX+K_PX))+((mu2_PY+K_PY)*(mu2_PY+K_PY))+ ((mu2_PZ+K_PZ)*(mu2_PZ+K_PZ)))))-3096.9)>60.0");
        vals.push_back("Pi_PIDp < 0 && Pi_PIDK < 0 && Pi_isMuon==0.0");
...
//	double errorj(0);
//	int numofentriesbeforej(0);
//	int numofentriesafterj(0);
//	double effj(0);
...
//	out<<"Jpsi  & "<<effj<<" & "<<numofentriesafterj<<" & "<<errorj<<" \\\\ "<<endl;
	...
	{
  //             double error(0);
    //           int numofentriesbefore(0);
      //         int numofentriesafter(0);
	//	double eff(0);
	 //       eff=cutTree((filename2+ext).c_str(), "DecayTree", (filename2+"_"+varia.at(i)+ext).c_str(), (vals.at(i)).c_str());
          //      numofentriesbefore= getmyentries((filename2+ext).c_str(),"DecayTree");
            //    numofentriesafter= getmyentries((filename2+"_"+varia.at(i)+ext).c_str(),"DecayTree");
