...
float mctruth(string filename, string decaytreename, string newfilename)
{
  Double_t mu3_isMuon;
  string ext=".root"; 
...
//              cout<<"Executing4"<<endl;
  ...
  effofcut = (float(count)/float(t->GetEntries()));
...
//        }
  ...
  effofcut = (float(count2)/float(t->GetEntries()));
...
vector<string> cuttuple(string pathname, string filename, string decaytreename, string ext, string cutvar, float cutval)
{
...
vector<string> cuttuplehere(string filename, string decaytreename, string ext, string cutvar, float cutval)
{
  Double_t cutvarholder;
  Double_t effofcut;
  vector<string> list;
...
float cutTree(string nametree, string decaytreename, string namecuttree, string cuts)
{
   ...
   float effofcut;
   effofcut = float(t2->GetEntries())/float(t->GetEntries());
...
string pathname = "/vols/lhcbdisk05/ss4314/tuples/B23MuNuMC/";
string filename = "B23MuNuNewMCNShared";
...
string ext = ".root";
string cuttag = "_mu1nShared_mu2nShared_mu3nShared";
string namecuttree2 = (filename+"_cut"+cuttag).c_str();
string cut2 = "mu1_nShared == 0.0 && mu2_nShared == 0.0 && mu3_nShared == 0.0";

float nSharedeff;
nSharedeff=cutTree((filename+ext).c_str(), decaytreename, (namecuttree2+ext).c_str(), cut2);
...
float qmineff;
qmineff=cutTree((filename+ext).c_str(), decaytreename, (namecuttree2+ext).c_str(), cut2);
...
float bBplus= 2*0.4;
float datacoll=3e15;
float decprodcut =0.185;
float effrecostrip= 0.111;

float finaleff;
finaleff=ppbbX*bBplus*brfr*datacoll*decprodcut*effrecostrip;//*mctrutheff*nSharedeff*qmineff;
