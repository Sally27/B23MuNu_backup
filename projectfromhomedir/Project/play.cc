...
void neutrinomass()
{
  ...
  TTree* t = (TTree*)f->Get("B23MuNu_Tuple/DecayTree");
   ...
   Double_t trial;
   Double_t sum0, sum1, sum2, sum3;
   TH1F* graph1 = new TH1F("missingneutrinomass", "missingneutrinomass", 50, -100 , 7000);
    ...
    for(int i(0); i<t->GetEntries(); ++i)
   {
      ...
      sum0 = Bplus_TRUEP_E - (mu3_TRUEP_E+mu2_TRUEP_E+mu1_TRUEP_E);
      trial= (sum0*sum0)-((sum1*sum1) + (sum2*sum2) + (sum3*sum3));
      graph1->Fill(trial);
      cout<<trial<<endl;	
   }
