...
// here hg has data ini it
   ...
   DataSample B23MuNu_gen3("myfile03.root", "t03");
   DataSample myfile3("myfile3.root","t3");
...
// here hg has data ini it
  ...
  /* TCanvas* m3p = new TCanvas("m3p", "m3p", 600, 600);*/   
   ...
   c->SetTitle("Efficiency[Not to scale]");
   c->SetXTitle("M^{2}(#mu^{+}#mu^{-})[(MeV/c^{2})^{2}]");
   c->SetYTitle("NCandidatesSel/NCandidatesGen");
   c->Draw();

   heff->SetLineColor(2);
   heff->SetTitle("Efficiency[Not to scale]");
  ...
  /* ha->SetLineColor(8);
   ...
   ha->Draw("same");*/
    ...
    leg->AddEntry(heff,"#mu_{2} and #mu_{3}","l");
