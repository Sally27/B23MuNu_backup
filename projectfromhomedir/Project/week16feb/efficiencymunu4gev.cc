...
// here hg has data ini it
   ...
   TH1F* hm = new TH1F("m1m2invmassmunusel_sel", "m1m2invmassmunusel_sel", 100, -10 , 2000000);
   hm->Sumw2();
   myfile2.t->Draw("m1andm2whereinvmunumore4gev>>m1m2invmassmunusel_sel");
   double nSel = myfile2.t->GetEntries();
// here hm has data ini it
   TH1F* hg = new TH1F("mplusmminusinvmassmunusel_gen", "mplusmminusinvmassmunusel_gen", 100, -10 , 2000000);
   hg->Sumw2();
   B23MuNu_gen2.t->Draw("mplusandmminusinvmass>>mplusmminusinvmassmunusel_gen");
// here hg has data ini it
   double nGen = B23MuNu_gen2.t->GetEntries();

   TH1F* heff = new TH1F("eff", "eff", 100, -10 , 2000000);
   heff->Divide(hm,hg);
   
   c->SetTitle("Efficiency of mu1 and mu2 with q^2(mu3 &nu)>4000 GeV");
