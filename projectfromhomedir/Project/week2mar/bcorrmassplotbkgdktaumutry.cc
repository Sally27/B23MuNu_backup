...
void invmass()
{
  ...
  TFile* f = new TFile("bkgdB2Ktaumu.root");
  TTree* t = (TTree*)f->Get("Bplus2Ktaumu_Tuple/DecayTree");

  TFile *f1 = new TFile("bcorrmassplotbkgdktaumu3try.root","RECREATE");
  TTree *t1 = new TTree("t3try","");
...
/*   TBranch *bpt2 = t1->Branch("mu3id",&mu3id,"mu3id/I");
   ...
   for(int i(0); i<t->GetEntries(); ++i)
   {
      ...
      if (mu2_TRUEID==13&&abs(mu3_TRUEID)==13&&abs(mu1_TRUEID)==13&&Bplus_TRUEID==521&&Bplus_L0Global_TOS==true)
        {
