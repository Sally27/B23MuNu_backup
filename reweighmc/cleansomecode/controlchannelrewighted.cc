...
void cloneTree(string filename, string decaytree, string newfilename){

        TFile* f = new TFile(filename.c_str());
        TTree* t = (TTree*)f->Get(decaytree.c_str());
        Double_t mu3_P, mu3_ETA, Bplus_Corrected_Mass, DataMCWeight;
        Int_t  nTracks;
        ...
        TBranch *bpt1 = newtree->Branch("nTracksrew",&nTracksrew,"nTracksrew/D");

        for(int n=0; n < t->GetEntries(); ++n)
        {
         t->GetEntry(n);
         
         newtree->Fill("nTracks","DataMCWeight");
        } 
...
double calaveffofasamplerew(string filename, string decaytree, string weightfile, string wfilecuts, string name, string newfilename) {
...
int  main()
{
	...
	if (l==2){
		//this is reweighting mc
                ...
                cute.push_back("(mu3_isMuon==1.0)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0) && (mu3_nShared==0)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0) && (mu3_nShared==0) && (mu3_NNmu >0.0)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0) && (mu3_nShared==0) && (mu3_NNmu >0.2)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0) && (mu3_nShared==0) && (mu3_NNmu >0.4)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0) && (mu3_nShared==0) && (mu3_NNmu >0.6)");
                cute.push_back("(mu3_isMuon==1.0) && (mu3_PIDmu > 0.0) && ((mu3_PIDmu - mu3_PIDK) > 0.0) && (mu3_nShared==0) && (mu3_NNmu >0.8)");

		cout<< "I am here: "<<endl;
                ofstream myfile2;

                myfile2.open("STATISTICSprobnnmu.txt");
                myfile2<<"------------------------------------ID EFF with Probnn cuts in PIDCalib-----------------"<<endl;
                myfile2<<"--------FILENAME------------------------CUTS----------------------EFF OF CUT------------"<<endl;
...
//                o = cutTree("/vols/lhcbdisk05/ss4314/preparetuplesforBDTincludeJpsi/MCSigpreparetuple/B23MuNuNewMCNShared_MCtruth_cut_mu1nShared_mu2nShared_mu3nShared_qmincut.root", "DecayTree", ("B23MuNuMC"+tags.at(i)+".root").c_str(), (cute.at(i)).c_str());
  ...
  //              cloneTree("/vols/lhcbdisk05/ss4314/tuplescontrolchannel/B23MuNuNewMCNShared_MCtruth_Jpsi_cut_mu1nShared_mu2nShared_mu3nShared_qmincut.root","DecayTree", "doit");
//
//effnew = calaveffofasample("/vols/lhcbdisk05/ss4314/tuplescontrolchannel/B23MuNuNewMCNShared_MCtruth_Jpsi_cut_mu1nShared_mu2nShared_mu3nShared_qmincut.root","DecayTree","/vols/lhcbdisk05/ss4314/tightPIDinvestigation/probnnmu/samples/muon/PerfHists_Mu_Strip20_MagDown_P_ETA_nTracks.root",(cuts3.at(i)).c_str(),"PIDCalib",("newmc"+tags.at(i)+".root").c_str());
...
//        	addweighttotree("/vols/lhcbdisk05/ss4314/preparetuplesforBDTincludeJpsi/MCSigpreparetuple/B23MuNuNewMCNShared_MCtruth_cut_mu1nShared_mu2nShared_mu3nShared_qmincut.root","DecayTree","/vols/lhcbdisk05/ss4314/tightPIDinvestigation/probnnmu/samples/muon/PerfHists_Mu_Strip20_MagDown_P_ETA_nTracks.root",(cuts3.at(i)).c_str(),"PIDCalib",("newmc"+tags.at(i)+".root").c_str());
               ...
               vector<double> efficiencies2;
                ...
                out.open("latexSTATISTICSprobnnmu.txt");
               ...
               if(wantLatex==1.0)
                ...
                cout<<"woohoooo2: number "<<i<<endl;
                 }
                out<<"\\hline"<<endl;
                out<<"\\end{tabular}"<<endl;
                out<<"\\end{center}"<<endl;
                out<<"\\caption{latexSTATISTICSprobnnmu.txt}"<<endl;
                out<<"\\end{table}"<<endl;
                }
         ...
         if(l==3)
         {
         cloneTree("/vols/lhcbdisk05/ss4314/tuplescontrolchannel/B23MuNuNewMCNShared_MCtruth_Jpsi_cut_mu1nShared_mu2nShared_mu3nShared_qmincut.root","DecayTree", "doit.root");  
         } 
