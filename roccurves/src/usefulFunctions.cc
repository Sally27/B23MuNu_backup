      ...
      )
{
   ...
   TCanvas canv("canv", "canv", 600,600);
   reth.Draw();
   TLegend *leg = new TLegend(0.1,0.7,0.1,0.3);
   leg->SetHeader(("Optimal Cut: "+d2s(reth.GetXaxis()->GetBinCenter(reth.GetMaximumBin()))).c_str());
   leg->SetTextSize(0.02);
   leg->Draw();
...
//   reth.SetTitle(("FOM with SigFile: "+fileSig+" bkgFile: "+fileBkg+"BDT var: "+bdtvar).c_str());
   ...
   reth.GetYaxis()->SetTitleOffset(1.5);
   ...
   if(nameplot.size()>1) canv.Print(("Scaled"+punzi+nameplot).c_str());
...
//   target[0]=reth.GetXaxis()->GetBinCenter(0.3);
   ...
   gr->GetYaxis()->SetTitle("eff(S)/(sqrt(B)+(3/2))");
   gr->GetXaxis()->SetTitle("eff(S)");
   gr->SetMarkerColor(1);
   gr->SetLineColor(1);
