...
//          gROOT->ProcessLine(".x lhcbSally.C");
	  ...
	  canvtot->SaveAs(("TotalMisIDfit"+tags.at(i)+".pdf").c_str());
          ...
          canvshift->SaveAs("misidshiftdllmu.pdf");          
	  ...
	  framenew2->addPlotable(hpullfit,"P"); 
          ...
          //Scaled//
          RooRealVar nsig00("nsig00","number of signal events",34.5) ;
          RooRealVar nbkg11("nbkg11","number of backgnd events",91) ;
          RooRealVar nbkg22("nbkg22","number of backgnd events2",7755) ;
          RooRealVar nbkg33("nbkg33","number of backgnd events3",53.4) ;
	  ...
	  // Display fit parameters.
          RooAddPdf normpdf("normpdf","extended sum pdf",RooArgList(blah,expoun,twocbs,cballpr), RooArgList(nsig00,nbkg11,nbkg22,nbkg33)); 
          RooDataSet *datafinalnorm = normpdf.generate(RooArgSet(y),1000) ;
          datafinalnorm->plotOn(finalfitframetruesig, MarkerSize(0.9));

	  normpdf.paramOn(finalfitframetruesig, Format("NELU", AutoPrecision(2)), Layout(0.1, 0.4, 0.9) );
	  ...
	  normpdf.plotOn(finalfitframetruesig, Components(twocbs), RooFit::Range("full"), LineColor(kCyan),RooFit::Name("misid2"));
          normpdf.plotOn(finalfitframetruesig, Components(cballpr), RooFit::Range("full"), LineColor(kMagenta),RooFit::Name("partreco2"));
	  // Plot total PDF.
	  ...
	  canvasfit->SaveAs("finalfitnormalized.pdf");
