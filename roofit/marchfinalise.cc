...
//          gROOT->ProcessLine(".x lhcbSally.C");
	  ...
	  RooRealVar cbsigmarar("cbsigmarar","cbsigmarar",99.7);//,0.1,50) ;
	  RooRealVar cbsignalrar("cbsignalrar","cbsignalrar",10,0.0,1000000) ;
	  RooRealVar nral("nral","",0.269);
	  RooRealVar alpharal("alpharal","",5.31);
	  RooRealVar nrar("nrar","",0.974);
	  RooRealVar alpharar("alpharar","",3.49);
 ...
 //         canvibou2->Divide(2,2) ;
          gPad->SetLeftMargin(0.15) ; framepartreco1->GetYaxis()->SetTitleOffset(1.4) ; framepartreco1->Draw() ;
...
//          tags.push_back("NNmuminuspimore08");
	  ...
	  canvtot->SaveAs(("TotalMisIDfit"+tags.at(i)+".pdf").c_str());
          ...
          canvshift->cd(i+1) ; gPad->SetLeftMargin(0.15) ; misidtotal->GetYaxis()->SetTitleOffset(1.4) ; misidtotal->Draw() ;      
...
//	  canvibou->Divide(2,2) ;
	  gPad->SetLeftMargin(0.15) ; ibou->GetYaxis()->SetTitleOffset(1.4) ; ibou->Draw() ;
	  ...
	  framenew2->addPlotable(hpullfit,"P"); 
          ...
          double numberofsigevents=34.5;
          double numberofcombi=91;
          double numberofpartreco=53.4;
	  ...
	  normpdf.plotOn(finalfitframetruesig,LineColor(kBlue),RooFit::Name("fit2")); // plot fit pdf
          ...
          TPaveLabel *t5 = new TPaveLabel(0.4,0.5,0.9,0.7, Form("TotMisID = %f, with %s",scaledmisid,(tags.at(i)).c_str()),"brNDC");
	  ...
	  canvasfit->SaveAs(("finalfitnormalized"+tags.at(i)+".pdf").c_str());
          ...
          canvshifttotal->cd(i+1) ; gPad->SetLeftMargin(0.15) ; finalfitframetruesig->GetYaxis()->SetTitleOffset(1.4) ; finalfitframetruesig->Draw(); leg2->Draw();
...
//          printLHCb("L","Prelim","");
 ...
 //         RooRealVar alpharar("alpharar","",1.01);
     ...
     { 
	  ...
	  RooRealVar cbmeanr("cbmeanr","cbmeanr",5188,5050,5300);
	  RooRealVar cbsigmar("cbsigmar","cbsigmar",150);//,0.1,50) ;
	  RooRealVar cbsignalr("cbsignalr","cbsignalr",10,0.0,1000000) ;
	  RooRealVar nr("nr","",0.0,5.0);
	  RooRealVar alphar("alphar","",0.0,10.0);
	  RooRealVar nl("nr","",0.0,5.0);
	  ...
	  canv3->Divide(2,2) ;
	  canv3->cd(1) ; gPad->SetLeftMargin(0.15) ; frame7->GetYaxis()->SetTitleOffset(1.4) ; frame7->Draw() ;
	  canv3->SaveAs("FittingMCUnBinnedtune.pdf");
...
//          RooRealVar c("c","c",-0.001,-0.05,0.05);
  ...
  TFile* f = new TFile("/vols/lhcbdisk05/ss4314/cutonBDTs/MCSigpreparetuple/B23MuNuNewMCNShared_MCtruth_cut_mu1nShared_mu2nShared_mu3nShared_qmincut_PUNZIoptBDT.root");
  TTree* t = (TTree*)f->Get("DecayTree");
  ...
  *py=Bplus_Corrected_Mass;
  ...
  TFile* f = new TFile("/vols/lhcbdisk05/ss4314/cutonBDTs/CombinatorialBackgroundpreparetuple/B23MuNuSignal2012and2011data_corrm_mu1nShared_mu2nShared_mu3nShared_qmin_PUNZIoptBDT.root");
  TTree* t = (TTree*)f->Get("DecayTree");
  TTree* tree = new TTree("tree","tree") ;
  Double_t Bplus_Corrected_Mass; 
  Double_t* py = new Double_t ;
...
//  }
  ...
  TFile* f = new TFile(("/vols/lhcbdisk05/ss4314/misidshape/modifiedandcut"+tags+"B23MuNuFakeSameSignMuonSmallDataSample_cut_mu1nShared_mu2nShared_mu3isNotMuon_mu3inMuonAcc_Jpsi_qmincut_KaonPID_cut_PUNZIoptBDT.root").c_str());
  TTree* t = (TTree*)f->Get("DecayTree");
  TTree* tree = new TTree("tree","tree") ;
  Double_t Bplus_Corrected_Mass;
  ...
  *px=misidoverid;
  ...
  TFile* f = new TFile(("/vols/lhcbdisk05/ss4314/misidshape/modifiedandcut"+tags+"B23MuNuFakeSameSignMuonSmallDataSample_cut_mu1nShared_mu2nShared_mu3isNotMuon_mu3inMuonAcc_Jpsi_qmincut_PionPID_cut_PUNZIoptBDT.root").c_str());
  TTree* t = (TTree*)f->Get("DecayTree");
  TTree* tree = new TTree("tree","tree") ;
  Double_t Bplus_Corrected_Mass;
  Double_t misidoverid;
  ...
  *px=normalizedmisidoverid;
  ...
  TFile* f = new TFile("/vols/lhcbdisk05/ss4314/cutonBDTs/B2D3pimupreparetuple/B2D3pimufinal_MCtruth_cut_mu3nShared_qmincut_PUNZIoptBDT.root");
  TTree* t = (TTree*)f->Get("DecayTree");
  TTree* tree = new TTree("tree","tree") ;
  Double_t Bplus_Corrected_Mass;
  Double_t* py = new Double_t ;
