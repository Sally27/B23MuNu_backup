...
void plot(){
  ...
  if(l==0.0)
  {
	  ...
	  RooRealVar nbkg2("nbkg2","number of backgnd events2",100,0,1000) ;
          ...
          //Scaled//
//          RooRealVar nsig("nsig","number of signal events",33.5,34.5) ;
//          RooRealVar nbkg1("nbkg1","number of backgnd events",90.000,91.1514) ;
//          RooRealVar nbkg2("nbkg2","number of backgnd events2",7755) ;
//          RooRealVar nbkg3("nbkg3","number of backgnd events3",53.42) ;
	  ...
	  RooPlot* finalfitframetruesig=y.frame(Bins(100), Name("myfinalfit"), Title("myfinalfit"));
	  // RooPlot finalfitframe(2500,10000);
	  // Plot histogram of r.

	  datafinal->plotOn(finalfitframetruesig, MarkerSize(0.9));

	  // Display fit parameters.

	  sume.paramOn(finalfitframetruesig, Format("NELU", AutoPrecision(2)), Layout(0.1, 0.4, 0.9) );


          // Plot all components
	  sume.plotOn(finalfitframetruesig, Components(expoun), RooFit::Range("truesig"), LineColor(kRed),RooFit::Name("combinatorial2"));
	  sume.plotOn(finalfitframetruesig, Components(blah), RooFit::Range("truesig"), LineColor(kGreen),RooFit::Name("signal2"));
	  sume.plotOn(finalfitframetruesig, Components(twocbs), RooFit::Range("truesig"), LineColor(kCyan),RooFit::Name("misid2"));
          sume.plotOn(finalfitframetruesig, Components(cballpr), RooFit::Range("truesig"), LineColor(kMagenta),RooFit::Name("partreco2"));
	  // Plot total PDF.

	  sume.plotOn(finalfitframetruesig,LineColor(kBlue),RooFit::Name("fit")); // plot fit pdf

          TLegend *leg2 = new TLegend(0.7,0.7,0.9,0.9);
          leg2->AddEntry(finalfitframetruesig->findObject("combinatorial2"),"Combinatorial","l");
          leg2->AddEntry(finalfitframetruesig->findObject("signal2"),"B^{+} -> #mu^{+}","l"); 
          leg2->AddEntry(finalfitframetruesig->findObject("misid2"),"MisID","l");
          leg2->AddEntry(finalfitframetruesig->findObject("partreco2"),"B^{+} -> D^{+} #Pi #mu #mu","l");
	  ...
	  //sume.paramOn(finalfitframelhcc, Format("NELU", AutoPrecision(2)), Layout(0.1, 0.4, 0.9) );
          ...
          // Plot all components
          dpw.plotOn(finalfitframelhcc,DataError(RooAbsData::SumW2), MarkerColor(kRed), LineColor(kRed), RooFit::Name("datamisid"));
	  sume.plotOn(finalfitframelhcc, Components(expoun), RooFit::Range("full"), LineColor(kRed),RooFit::Name("combinatoriallh"));
	  sume.plotOn(finalfitframelhcc, Components(blah), RooFit::Range("full"), LineColor(kGreen),RooFit::Name("signallh"));
	  sume.plotOn(finalfitframelhcc, Components(twocbs), RooFit::Range("full"), LineColor(kCyan),RooFit::Name("misidlh"));
          sume.plotOn(finalfitframelhcc, Components(cballpr), RooFit::Range("full"), LineColor(kMagenta),RooFit::Name("partrecolh"));
	  // Plot total PDF.

	  //sume.plotOn(finalfitframe,LineColor(kBlue),RooFit::Name("fit")); // plot fit pdf

          TLegend *leg3 = new TLegend(0.6,0.6,0.9,0.9);
          leg3->AddEntry(finalfitframelhcc->findObject("signallh"),"LHCb #mu^{+} #mu^{-} #mu^{+} #nu_{#mu} simulation","l");
          leg3->AddEntry(finalfitframelhcc->findObject("partrecolh"),"LHCb D^{0} (K^{+} #pi^{-} #mu^{+} #mu^{-}) #mu^{+} #nu_{#mu} simulation","l");
          leg3->AddEntry(finalfitframelhcc->findObject("combinatoriallh"),"LHCb Combinatorial Data","l"); 
          leg3->AddEntry(finalfitframelhcc->findObject("misidlh"),"LHCb MisID Data Pa","l");
          leg3->AddEntry(finalfitframelhcc->findObject("datamisid"),"LHCb MisID Data","l");
          //leg3->SetTextSize(0.015);


          TCanvas* canvaslhcc = new TCanvas("fit", "Fit",800,800);
          canvaslhcc->Divide(1,1) ;
          canvaslhcc->cd(1); 
          gPad->SetLeftMargin(0.17);
//          printLHCb("L","Prelim","");
          finalfitframelhcc->GetYaxis()->SetTitleOffset(1.4); 
          finalfitframelhcc->GetYaxis()->SetTitle("Number of Events");
 //         finalfitframelhcc->GetXaxis()->SetRange(2500,11000);
          finalfitframelhcc->GetXaxis()->SetTitle("Corrected #mu^{+}#mu^{-}#mu^{+}[MeV/c^{2}]");
...
//          RooCBShape xcballr1("xcballr1", "crystal ball", y, cbmeanr1, cbsigmar1, alphar1, nr1);
          ...
          RooPlot* xframepartreco1 = y.frame(Title("PartReco,two DBCB with common mean")) ;
