...
//  TTree* treew =  gettreemisidweight();
  ...
  RooRealVar p("p","p",2500,10000);
  RooRealVar pw("pw","pw", 0.0 ,1.0);
  RooDataSet dk("dk","dk",RooArgSet(p),Import(*treek));
  RooDataSet dkw("dkw","dkw", RooArgSet(p,pw), Import(*treek), WeightVar("pw"));//, Import(*treew));
  dk.Print();
  dkw.Print();
  p.setBins(150);
...
//  RooRealVar pw("pw","pw", 0.0 ,1.0);
  ...
  RooPlot* frame11 = p.frame(Title("MisIDShapeKaon")) ;
  RooPlot* frame12 = p.frame(Title("MisIDShapeNormalizedKaon")) ;
  dk.plotOn(frame11);
  ...
  cout << dpw.weight() << endl;
  RooPlot* misidtotal = p.frame(Title("MISIDTOTAL")) ;
  RooPlot* misidtotal2 = p.frame(Title("MISIDtotal2")) ;
...
//  RooRealVar cbmeanr("cbmeanr","cbmeanr", 3400.0);
//  RooRealVar cbsigmar("cbsigmar","cbsigmar",800.0,1000);//,0.1,50) ;
//  RooRealVar nr("nr","",2.5);
//  RooRealVar alphar("alphar","",1.0);
...
//  break;
  ...
  RooRealVar cbmeanr1("cbmeanr1","cbmeanr", 3340.0);
  RooRealVar cbsigmar1("cbsigmar1","cbsigmar",910.5);//,0.1,50) ;
  RooRealVar nr1("nr1","",2.48);
  RooRealVar alphar1("alphar1","",0.38);
  RooCBShape cballr1("cballr1", "crystal ball", p, cbmeanr1, cbsigmar1, alphar1, nr1);
  RooRealVar frac1("frac1","frac of events in cb 1",0.773); 

  RooRealVar cbmeanr2("cbmeanr2","cbmeanr", 3340.0);
  RooRealVar cbsigmar2("cbsigmar2","cbsigmar",3002.9);//,0.1,50) ;
  RooRealVar nr2("nr2","",0.00115);
  RooRealVar alphar2("alphar2","",0.0345);
  RooCBShape cballr2("cballr2", "crystal ball", p, cbmeanr2, cbsigmar2, alphar2, nr2);
  ...
  RooKeysPdf kest3("kest3","kest3",k,dkw,RooKeysPdf::NoMirror);
  RooKeysPdf kest4("kest4","kest4",k,dkw,RooKeysPdf::MirrorBoth);
  RooPlot* frame16 = k.frame(Title("Adaptive kernel estimation pdf with and withou mirroring"),Bins(20)) ;
  dkw.plotOn(frame16);
