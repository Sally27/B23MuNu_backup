...
//			TTree* treer = gettreemcsig(("B23MuNuMC"+mcname.at(indexofcut)+stripping+polarity+".root").c_str(),"mcweight");
			...
			//--------Fit with 2CBS Part Reco Fit-----------//
                        ...
                        paramspr=fitpartreco(partrecosource);
...
//				RooAddPdf twocbs("twocbs","",RooArgList(cballr1,cballr2),RooArgList(frac1));
				...
				RooRealVar nsig00("nsig00","number of signal events",numberofsigevents,0.1,10.0) ;
...
//				RooDataSet *datafinalnorm = .generate(RooArgSet(Bplus_Corrected_Mass),totalforfit)  ;
				...
				finaldataset.plotOn(works, MarkerSize(0.9), LineColor(kBlack),RooFit::Name("data2"));
                                ...
                                RooAbsPdf* modelsb= new RooAddPdf("modelsb","S+B pdf",RooArgList(blah,expoun,twocbs,newpartreco), RooArgList(nsigform,nbkg11,nbkg22,nbkg33));
                                RooAbsPdf* modelbkg= new RooAddPdf("modelbkg","B pdf",RooArgList(expoun,twocbs,newpartreco), RooArgList(nbkg11,nbkg22,nbkg33));                                                      
