...
double calculateL0effMC(string nametree, string decaytreename, string newfilename)
{
   ...
   double myprop[] = {0.00409485,0.024711,0.0180205,0.0553385,0.06731,0.0705092,0.0123352,0.228966,0.0411336,0.0151156,0.00461084,0.0213447,0.364867,0.024349,0.0472941,0.0};
...
//	Double_t mu3_isMuon;
	...
	Int_t Bplus_BKGCAT;
        Int_t mu1_TRUEID,mu2_TRUEID,mu3_TRUEID, Bplus_TRUEID;
	...
	for(int i(0); i<t->GetEntries(); ++i)
	{
		...
		if (((abs(mu1_TRUEID)==13) && (abs(mu2_TRUEID)==13) && (abs(mu3_TRUEID)==321) && (abs(Bplus_TRUEID)==521)) && ((abs(mu1_MC_MOTHER_ID)==443) && (abs(mu2_MC_MOTHER_ID)==443) && (abs(mu3_MC_MOTHER_ID)==521))&&((abs(mu1_MC_GD_MOTHER_ID)==521) && (abs(mu2_MC_GD_MOTHER_ID)==521)))
		{
			count2++;
			t2->Fill();
		}
...
double getSweightValue(string filename, string decaytreename)
{
	...
	TTree* t = (TTree*)s->Get(decaytreename.c_str());
        ...
        Double_t event_sw, sum_event_sw;
	//
	t->SetBranchAddress("event_sw", &event_sw);
