...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   RooRealVar sig("sig","sig",4000000 ,1000,5000000);
   RooRealVar bkg("bkg","bkg",1000000, 2 ,3000000) ;
   RooRealVar bkg2("bkg2","bkg2",1000000, 2 ,3000000) ;
...
void FitAndSplotJpsiKDataForTraining::prepareWorkspace(string tuplename)
{
   ...
   t->SetBranchStatus("*", 0);
   t->SetBranchStatus("Bplus_MM", 1);
   t->SetBranchStatus("nSPDHits", 1);

   RooRealVar Bplus_MM("Bplus_MM", "Bplus_MM",Bplus_MM_min,Bplus_MM_max);
   RooRealVar HLT1TCK("nSPDHits", "nSPDHits",0,700);
   RooArgSet vars(Bplus_MM,HLT1TCK);


   cout<<"Cut applied: "<<"Bplus_MM>"+d2s(Bplus_MM_min)+" && Bplus_MM <"+d2s(Bplus_MM_max)<<endl;
   cout<<"Cut applied: "<<"nSPDHits<450 "<<endl;

   RooDataSet data("data", "data", vars, Import(*t), Cut( ("nSPDHits<450 && Bplus_MM>"+d2s(Bplus_MM_min)+" && Bplus_MM <"+d2s(Bplus_MM_max)).c_str() ) );
