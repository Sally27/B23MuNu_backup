...
void FitAndSplotJpsiKDataForTraining::makeSWeightedTree(string extracut, string label, string tuplename)
{
   ...
   RooRealVar* bkg2 = workspaceFit->var("bkg2");

   if(!data || !model_total || !sig || !bkg || !bkg2)
   {
   ...
   RooStats::SPlot wdata("wData", "wData", *data, model_total, RooArgList(*sig, *bkg, *bkg2));


   double Bplus_MM;
   t->SetBranchAddress("Bplus_MM", &Bplus_MM);

   string nameNewFile("../Sweights/Data_WS_sweights_2012.root");
   ...
   t2->Branch("bkg2_sw", &bkg2_sw, "bkg2_sw/D");
   t2->Branch("event_sw", &event_sw, "event_sw/D");
   ...
   for(int i(0); i<nEntries; ++i)
   {
      ...
      if(Bplus_MM > Bplus_MM_min && Bplus_MM < Bplus_MM_max) 
      {
         ...
         bkg2_sw = wdata.GetSWeight(j,"bkg2");
         event_sw= sig_sw+bkg_sw+bkg2_sw;
...
void FitAndSplotJpsiKDataForTraining::GetYields(string dataset,string tag)
{
...
//		cout<<Bplus_Corrected_Mass<<" "<<data<<" "<<endl;
	...
	ofstream out;
	out.open(("../FOM/FOM_"+dataset+"_"+tag+".txt").c_str());
	...
	out<<"\\end{table}"<<endl;
        ...
        TFile *fillinfo = new TFile(("../FitResults/FitControl_"+dataset+"_"+tag+".root").c_str(),"RECREATE");
...
vector<double> FitAndSplotJpsiKDataForTraining::pimumufitweight(bool binnedFit, string type, string pimumufilename, string weightname, bool weighted)
{ 


        cout<<"Start Signal Component fit"<<endl;

	RooRealVar Bplus_Corrected_Mass("Bplus_MM","Bplus_MM",Bplus_MM_min,Bplus_MM_max);
	...
	TFile* f = new TFile((pimumufilename+type+".root").c_str());
...
//	blah.fitTo(dr);
	...
	RooDataHist* bdata;
	if(binnedFit) bdata = dr.binnedClone("binned", "binned") ;
	...
	if(binnedFit) nll = blah.createNLL(*bdata, RooFit::NumCPU(8) ) ;
	if(!binnedFit) nll = blah.createNLL(dr, RooFit::NumCPU(8) ) ;
...
//	RooRealVar weightnamevar(weightname.c_str(),weightname.c_str(), 0.0 ,1.0);
	...
	TFile* f = new TFile((mcfilename+type+".root").c_str());
	TTree* t = (TTree*)f->Get("DecayTree");
...
vector<double> FitAndSplotJpsiKDataForTraining::sigfithypathia(bool binnedFit,string weightname, string type, string mcfilename, string dataset, bool weighted)
{ 
...
//        RooRealVar l(("l_"+type).c_str(),"l",-4.0, -6.0, -0.1);
//	RooRealVar zeta(("zeta_"+type).c_str(),"l",1.0e-5, 1.0e-8, 5.0);
//        RooRealVar fb(("fb_"+type).c_str(),"l",-0.001,-0.1,-0.00001);
//        RooRealVar sigma(("sigma_"+type).c_str(),"l",9.81);//,8.0,10.0);//,0.1,50) ;
//        RooRealVar mean(("mean_"+type).c_str(),"l", 5280.0); 
//        RooRealVar a1coeff(("a1coeff_"+type).c_str(),"l",2.75, 1.0, 3.0);
...
//        RooRealVar n2coeff(("n2coeff_"+type).c_str(),"l",2.35,0.1,4.0);
        ...
        RooRealVar sigma(("sigma_"+type).c_str(),"l",9.81);//,8.0,10.0);//,0.1,50) ;
        RooRealVar mean(("mean_"+type).c_str(),"l", 5280.0);
        RooRealVar a1coeff(("a1coeff_"+type).c_str(),"l",2.15, 2.0, 3.0);
        RooRealVar n1coeff(("n1coeff_"+type).c_str(),"l",0.1,10.0);
        RooRealVar a2coeff(("a2coeff_"+type).c_str(),"a2co",3.17, 0.1,4.0);
        RooRealVar n2coeff(("n2coeff_"+type).c_str(),"l",2.35,0.1,4.0);
...
//   RooRealVar vc1("vc1", "vc1",0.75, 0.15,0.9);
   RooRealVar sig("sig","sig",4000000 ,1000,17000000);
   RooRealVar bkg("bkg","bkg",1000000, 2 ,10000000) ;
   RooRealVar bkg2("bkg2","bkg2",1000000, 2 ,10000000) ;
...
void FitAndSplotJpsiKDataForTraining::prepareWorkspace(string tuplename)
{
   ...
   cout<<" Going to prepare data workspace "<<endl;

   TFile f( (tuplename).c_str());
   TTree* t = (TTree*)f.Get(treename.c_str());

   if(!t)
   {
      cerr<<"ERROR: in function prepareWorkspace, no tree "<<treename<<" found in "<<"/"<<tuplename<<endl;
      return;
   }

   t->SetBranchStatus("*", 0);
   t->SetBranchStatus("Bplus_MM", 1);
   t->SetBranchStatus("HLT1TCK", 1);

   RooRealVar Bplus_MM("Bplus_MM", "Bplus_MM",Bplus_MM_min,Bplus_MM_max);
   RooRealVar HLT1TCK("HLT1TCK", "HLT1TCK",0,1e9);
   RooArgSet vars(Bplus_MM,HLT1TCK);


   cout<<"Cut applied: "<<"Bplus_MM>"+d2s(Bplus_MM_min)+" && Bplus_MM <"+d2s(Bplus_MM_max)<<endl;
   cout<<"Cut applied: "<<"HLT1TCK==288888335 "<<endl;

   RooDataSet data("data", "data", vars, Import(*t), Cut( ("HLT1TCK==288888335 && Bplus_MM>"+d2s(Bplus_MM_min)+" && Bplus_MM <"+d2s(Bplus_MM_max)).c_str() ) );
