...
int main(){
	...
	for (int z(0); z<nameofweight.size() ;z++)
	{
		...
		if (normalisation==1)
		{
			...
			//-------------Reco Efficiencies--------------//
			ValError sigy(double(t_initial_sig->GetEntries()),sqrt(double(t_initial_sig->GetEntries())));
			ValError sigz(number_bkk_sig,sqrt(number_bkk_sig));
			ValError normy(double(t_initial_norm->GetEntries()),sqrt(double(t_initial_norm->GetEntries())));
			...
			ValError jpsik_othereff_passed(double(getKinWeights_nameweight(files_phsp.back(),"DecayTree",todo)),sqrt(double(getKinWeights_nameweight(files_phsp.back(),"DecayTree",todo))));

			ValError sig_othereff_total(double(t_initial_sig->GetEntries()),sqrt(double(t_initial_sig->GetEntries())));
			ValError jpsik_othereff_total(double(getKinWeights_nameweight(files_phsp.at(0),"DecayTree",todo)),sqrt(double(getKinWeights_nameweight(files_phsp.at(0),"DecayTree",todo))));
			...
			ValError sig_pid(0.7152,0.0011);
			ValError jpsik_pid(0.7152,0.0011);
			...
			out<<"\\newline"<<endl;
			out<<"\\newline"<<endl;
			out<<"Sytematics from using reweighted model is "<<(100.0*(all_sig-all_norm)/(all_sig)).val<<" \\%"<<endl;
		...
		{  
			...
			//------------PID estuff no error so far-------//
			ValError sig_pid(0.648,0.005);
			ValError jpsik_pid(0.648,0.005);
		...
		out2<<"\\newline"<<endl;
		out2<<"Hence systematic is "<< (100.0*((signal_nominal/normalisation_nominal)-(signal_reweight/normalisation_reweight))/(signal_nominal/normalisation_nominal)).val<<endl;
		out2<<"\\end{document}"<<endl;

		TFile fillinfo("../../../KinRew_Systematic/KinRew_Systematic.root","UPDATE");

		TTree fillinfotree(("KinRew_Run1_"+todo).c_str(),("KinRew_Run1_"+todo).c_str());

		double systematic(0);

		fillinfotree.Branch("systematic",&systematic,"systematic/D");// "Cut/C");

		systematic=(100.0*((signal_nominal/normalisation_nominal)-(signal_reweight/normalisation_reweight))/(signal_nominal/normalisation_nominal)).val;
