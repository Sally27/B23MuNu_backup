...
int main(){
	...
	for (int z(0); z<nameofweight.size() ;z++)
	{
		...
		{
			ofstream out;
			out.open(("ComparisonNormMC_NormModelReweighted_2016"+todo+".tex").c_str());
			...
			ValError sig_pid(0.676,0.001);
			ValError jpsik_pid(0.676,0.001);
		...
		{  
			...
			ofstream out;
			out.open(("ComparisonSignalMC_SignalModelReweighted_2016"+todo+".tex").c_str());
			...
			//------------PID estuff no error so far-------//
			ValError sig_pid(0.624,0.006);
			ValError jpsik_pid(0.624,0.006);
		...
		ofstream out2;
		out2.open(("ImpactOnEffRatio_2016"+todo+".tex").c_str());
		...
		TTree fillinfotree(("KinRew_2016_"+todo).c_str(),("KinRew_2016_"+todo).c_str());
