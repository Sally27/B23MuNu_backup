...
int main(){
        ...
        ofstream out;
        out.open("ComparisonSignalMC_PHSPmodelefficienciesbelow1Gev_syst.tex");
...
//        out << "$\\epsilon_{corm}$ &" << cormsig << " & " << cormjpsik <<" \\\\ "<<endl;
        ...
        out<<"${\\epsilon^{other,minq^{2}}}$ & "<< return_nice_string(sig_othereff)  << " & "<< return_nice_string(jpsik_othereff) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
...
//        out<<"$\\epsilon\\_{pid}$ &"<<sig_pid <<" & "<<jpsik_pid<<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
        out<<" &"<<" $\\epsilon_{nom}$ = "<<return_nice_string(((try1_err/2.0)*(sig_othereff)*(sig_pid))) <<" & "<<" $\\epsilon_{alt}$ ="<<return_nice_string(((try2_err/2.0)*(jpsik_othereff)*(sig_pid)))<<" \\\\ "<<endl;
