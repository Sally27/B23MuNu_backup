...
//        double nrecoq_2(44086.0);
        ...
        out << "${\\epsilon^{GEN,minq^{2}}}\\times {\\epsilon^{REC,minq^{2}}}$ val err &" << try1_err << " & " << try2_err <<" \\\\ "<<endl;
 //       out<<"\\hline"<<endl;
...
//        out<<"$\\epsilon\\_{pid}$ &"<<sig_pid <<" & "<<jpsik_pid<<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
        out<<" &"<<" $\\epsilon_{nom}$ = "<<(try1_err)*(sig_othereff)*(sig_pid) <<" & "<<" $\\epsilon_{alt}$ ="<<(try2_err)*(jpsik_othereff)*(sig_pid)<<" \\\\ "<<endl;
  
        ValError all_sig=(try1_err)*(sig_othereff)*(sig_pid);
        ValError all_norm=(try2_err)*(jpsik_othereff)*(jpsik_pid);
