...
string return_nice_string(ValError object)
{
string nice;
nice=d2s(round_to_digits(object.val,3))+"$\\pm$"+d2s(round_to_digits(object.err,3));
return (nice);
}

double readPIDvar(string filename, string decaytreename, string branchname)
{

	cout<<"Reading PID"<<endl;
	...
	Double_t  PIDvar;
	//
	t->SetBranchAddress(branchname.c_str(), &PIDvar);
	...
	totalPID=(totalPID/double(t->GetEntries()));
