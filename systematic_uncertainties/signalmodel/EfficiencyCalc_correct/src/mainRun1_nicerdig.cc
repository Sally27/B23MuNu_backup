...
int main(){
        ...
        ofstream out;
        out.open("ComparisonSignalMC_PHSPmodelefficienciesbelow1Gev_syst_nicerdig.tex");
...
//	cormjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_NormMC_"+i2s(years.at(1))+"_Fitrange.root").c_str(), ("MC_Norm_"+i2s(years.at(1))).c_str(), "fitrange");
	...
	TFile f_final_sig("/vols/lhcb/ss4314/cutonBDTs/InvestigatePHSP_Combi/bin/Bu23MuNuMC2012_MCtruth_L0MuonDecisionTOS_trigger_Jpsi_CombBasic_MisidBasic_FitCorM.root","READ");
	...
	TFile f_final_norm("/vols/lhcb/ss4314/cutonBDTs/InvestigatePHSP_Combi/bin/B23MuNuMCPHSP_MCtruth_L0MuonDecisionTOS_trigger_Jpsi_CombBasic_MisidBasic_FitCorM.root","READ");
...
//	double crosscheckjpsikmclowerr=calcEffErr(int(number_bkk_nor),t_final_norm->GetEntries());//still need to add on error from dec prodcut
        ...
        ValError sig_pid(readPIDvar("/vols/lhcb/ss4314/systematic_uncertainties/signalmodel/preparesignalweights_correct/preparesignalweights_FUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights_correct_gen_level/bin/B23MuNuMC2012probnnmu035Strip21_selected.root", "DecayTree", "PIDweight"),0.005);
        ValError jpsik_pid(readPIDvar("/vols/lhcb/ss4314/systematic_uncertainties/signalmodel/preparesignalweights_correct/preparesignalweights_FUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights_correct_gen_level/bin/B23MuNuMCPHSP2012probnnmu035Strip21_selected.root", "DecayTree", "PIDweight"),0.008);
...
//        double nrecoq_2(44086.0);
        ...
        ValError numgen(numgen_1,sqrt(nbkk_1));
...
//        ValError firstie(eff_firstpart,0);
  //      ValError secondie(eff_secondpart,0);

       
        out<<"\\hline"<<endl;
        out << "${\\epsilon^{GEN,minq^{2}}}$ &" << return_nice_string(((getRatioBinomial(ninaccqmin,nglqmin))/2.0))<< " & " << return_nice_string(((getRatioBinomial(ninaccqmin_s,nglqmin_s))/2.0))<<" \\\\ "<<endl;
        out << "${\\epsilon^{REC,minq^{2}}}$ &" << return_nice_string( (getRatioBinomial(nrecqmin,(ninacc*getRatioBinomial(nglqmin,numgen))))) << " & " << return_nice_string((getRatioBinomial(nrecqmin_s,(ninacc_s*getRatioBinomial(nglqmin_s,numgen_s)))))<<" \\\\ "<<endl;
 ...
 //       out << "$\\epsilon_{reco+strip+mctruth}$ & "<<t_initial_sig_ORI->GetEntries()/number_bkk_sig <<" & "<< t_initial_norm_ORI->GetEntries()/number_bkk_nor<<" \\\\ "<<endl;
        ...
        out << "$\\epsilon\\_{"+cuts.at(i-1)+"}$ & "<<round_to_digits( (double(t_sig_cut_2->GetEntries())/double(t_sig_cut->GetEntries())),3) << " & " << round_to_digits((double(t_norm_cut_2->GetEntries())/double(t_norm_cut->GetEntries())),3)<<" \\\\ "<<endl;
        f_sig_cut.Close();
        f_norm_cut.Close();
        f_sig_cut_2.Close();
        f_norm_cut_2.Close();
...
//        out << "$\\epsilon_{corm}$ &" << cormsig << " & " << cormjpsik <<" \\\\ "<<endl;
        ...
        out<<"${\\epsilon^{other,minq^{2}}}$ & "<< sig_othereff.roundToError_percentbase(true)  << " & "<<jpsik_othereff.roundToError_percentbase(true) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
        out << "N$^{final}$ &" <<t_final_sig->GetEntries()  << " & " << t_final_norm->GetEntries() <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
        out << "$\\epsilon^{PID,minq^{2}}$ &" << return_nice_string(sig_pid) << " & " << return_nice_string(jpsik_pid) <<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
...
//        out<<"$\\epsilon\\_{pid}$ &"<<sig_pid <<" & "<<jpsik_pid<<" \\\\ "<<endl;
        out<<"\\hline"<<endl;
        out<<" &"<<" $\\epsilon_{nom}$ = "<<(((try1_err/2.0)*(sig_othereff)*(sig_pid))).roundToError_diffbase(true) <<" & "<<" $\\epsilon_{alt}$ ="<<(((try2_err/2.0)*(jpsik_othereff)*(sig_pid))).roundToError_diffbase(true)<<" \\\\ "<<endl;
...
//	double effrecostrip= 0.111;
        ...
        out<<"\\end{center}"<<endl;
        out<<"\\caption{Comparison of efficiencies of nominal and alternative signal model samples using 2012 datasets with minq$<$980 MeV/c$^{2}$ leads to systematic uncertainty "<< round_to_digits(((100.0*(all_sig-all_norm)/(all_sig)).val),2) <<" \\%."<<"The detector acceptance efficiency was computed on the top of minq2 selection and is obtained generating privately simulation with \\texttt{Pythia6} conditions.}"<<endl;
        out<<"\\end{table}"<<endl;
...
//        out<<"Sytematics from using different model after minq$^2$ cut + genlevelcut is "<<100.0*((((sig_othereff)*(sig_pid)*(truedecsig))-((truedecjpsik)*(jpsik_othereff)*(jpsik_pid)))/((truedecsig)*(sig_othereff)*(sig_pid)))<<" \\%"<<endl;
        ...
        TFile fillinfo("../Systematic_SigModel/Systematic_SigModel.root","UPDATE");

        TTree fillinfotree("SigModel_Run1","SigModel_Run1");

        double systematic(0);

        fillinfotree.Branch("systematic",&systematic,"systematic/D");// "Cut/C");

        systematic=(100.0*(all_sig-all_norm)/(all_sig)).val;
