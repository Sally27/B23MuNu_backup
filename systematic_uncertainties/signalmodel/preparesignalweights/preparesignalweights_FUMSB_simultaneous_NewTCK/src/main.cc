...
int main(int argc, char *argv[]){
	...
	if(stripping=="Strip21"||stripping=="Strip21r1")
	{
		...
		for(int h(14); h<15; h++)
		{
			...
			cout<<"Hence percentage of thrown away sample is"<< d2s(double(getmyentries(mcmuonsample, "DecayTree") - getmyentries(mcmuonhist, "DecayTree"))/double(getmyentries(mcmuonsample, "DecayTree")))<<endl;  
                        ...
                        multiply2branches(mcmuonhist, "Runweight", "PIDweight", "mcweight", (name+mcname.at(indexofcut)+stripping+type+".root").c_str());
			CopyTreeSelectedBranches((name+mcname.at(indexofcut)+stripping+type+".root").c_str(),(name+mcname.at(indexofcut)+stripping+type+"_selected.root").c_str());
			//----------------MULTIPLY  the EFFICIENCIES and make a branch----------------------------------------------------//

			//addmultiplicationofbranches(mcmuonhist, "mu1weight","mu2weight","mu3weight","mcweight",(name+mcname.at(indexofcut)+stripping+type+".root").c_str());
		}
...

	{
		...
		for(int h(0); h<1; h++)
		{
			...
			//---------------GET MC EFFICIENCIES FROM PIDCALIB-------??
			string muonideffpath=("/vols/lhcb/ss4314/PIDCalibEfficienciesID_Urania_vs6r2p1/ideff/muon/PerfHists_Mu_"+stripping+"_MagDown_P_ETA.root").c_str();
			string muonideffpathup=("/vols/lhcb/ss4314/PIDCalibEfficienciesID_Urania_vs6r2p1/ideff/muon/PerfHists_Mu_"+stripping+"_MagUp_P_ETA.root").c_str();
			//        string nameofmuonidhist=("PerfHists_Mu_"+stripping+"_"+polarity+"_P_ETA.root").c_str();
