...
{ 
 ...
 //       dr.SumW2(kTrue);
	...
	RooRealVar nral(("nral"),"",0.33);//0.01,0.9);
...
//	RooRealVar cbsignalrar(("cbsignalrar"),("cbsignalrar"+type).c_str(),8400,7000,9000) ;
//	RooRealVar nral(("nral"),"",0.01,0.9);
//	RooRealVar alpharal(("alpharal"),"",0.1,7.0);
//	RooRealVar nrar(("nrar"),"",0.01,5.0);
//	RooRealVar alpharar(("alpharar"),"",0.1,5.30);
...
//	RooRealVar cbsignalrar(("cbsignalrar"),("cbsignalrar"+type).c_str(),8400,7000,9000) ;
//	RooRealVar nral(("nral"),"",0.3,0.01,0.9);
//	RooRealVar alpharal(("alpharal"),"",2.8,0.1,7.0);
//	RooRealVar nrar(("nrar"),"",0.9,0.01,5.0);
...
//	RooRealVar nral_imp(("nral_imp"),"",nral.getVal(),0.01,0.9);
//	RooRealVar alpharal_imp(("alpharal_imp"),"",alpharal.getVal(),0.1,7.0);
//	RooRealVar nrar_imp(("nrar_imp"),"",nrar.getVal(),0.01,5.0);
...
//	RooRealVar nral_imp2(("nral_imp2"),"",nral_imp.getVal(),0.01,0.9);
//	RooRealVar alpharal_imp2(("alpharal_imp2"),"",alpharal_imp.getVal(),0.1,7.0);
//	RooRealVar nrar_imp2(("nrar_imp2"),"",nrar_imp.getVal(),0.01,5.0);
...
void FitAndSplotB23MuNuDataForTraining::plotpretty_plotnicely_paper_sigsim_drive_2model(bool parameters)
{
...
//	TFile wrkspc8((plotdir+"myworkspace_"+namews8+"_"+type+".root").c_str()); 
   ...
   cout<<"Plotting result of the fit"<<endl;
   TFile fw((workspacedir+"myworkspace_"+namews8+"_model2.root").c_str());
...
//   data->plotOn(frame);
   ...
   canvTot.Print((workspacedir+"SimSig_veryniceplot"+trueparam+"_paper_model2.pdf").c_str());
   canvTot.Print((workspacedir+"SimSig_veryniceplot"+trueparam+"_paper_model2.root").c_str());

   canv.Print((workspacedir+"SimSig_nopull_veryniceplot"+trueparam+"_paper_model2.pdf").c_str());
   canv.Print((workspacedir+"SimSig_nopull_veryniceplot"+trueparam+"_paper_model2.root").c_str());
...
//   frame->SetMaximum(200000.0);
   ...
   canv.Print((workspacedir+"SimSig_veryniceplot"+trueparam+"_nopull_logscale_paper_model2.pdf").c_str());
   canv.Print((workspacedir+"SimSig_veryniceplot"+trueparam+"_nopull_logscale_paper_model2.root").c_str());
...
//   canvTot.Update();
   ...
   canvTotn.Print((workspacedir+"SimSig_veryniceplot"+trueparam+"_logscale_paper_model2.pdf").c_str());
   canvTotn.Print((workspacedir+"SimSig_veryniceplot"+trueparam+"_logscale_paper_model2.root").c_str());
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitsignalcomponent_2model(string weightname, string type, string year, string PIDcut)
{ 
	...
	TFile* f = new TFile((sigdir+"/"+signalfilename2+".root").c_str());
	TTree* t = (TTree*)f->Get("DecayTree");
 ...
 //       dr.SumW2(kTrue);
	...
	RooRealVar cbsigmarar(("cbsigmarar"),"#sigma",0.01,200.0);//,0.1,50) ;
...
//	RooFitResult* fitres0=blah0.fitTo(dr,SumW2Error(kTRUE),Save());
	...
	for (int i(0); i<50; i++)
	{
		...
		cbsigmarar.setVal(sigma.Uniform(0.01,200.0));//,30.0,250.0);//,0.1,50) ;
		cbsignalrar.setVal(signal.Uniform(7000,9000));
		nral.setVal(nl.Uniform(0.01,0.9));//,0.01,5.0);
		...
		if (migradStatus!=0)
		{
			...
			ofstream out0((plotdir+type+"FitResults_SigMC_"+i2s(i)+"thiteration_model2.txt").c_str());
		...
		if (migradStatus==0)
		{
			...
			ofstream out0((plotdir+type+"FitResults_SigMC_"+i2s(i)+"thiteration_model2.txt").c_str());
			saveFitInfo_cb(out0, frame8, 2,blah,fitres);
			ofstream out1((plotdir+type+"FitResults_SigMC_FINAL_loop_model2.txt").c_str());
...
//        saveFitInfo_cb(out, frame8, 2,blah,fitres);
	...
	frame10->addPlotable(hpull,"P"); 
        ...
        savePullPlot(*frameun, workspacedir+"pullPlot_mc_model2.root");
        TFile fpull((workspacedir+"pullPlot_mc_model2.root").c_str());
        TCanvas* cpull = (TCanvas*)fpull.Get("pullplot");
        ...
        gPad->SetLeftMargin(0.15) ;
        cpull->DrawClonePad();
        canvTot.Print((workspacedir+"mcsigfit_NOTMAIN_moreinfo_"+type+"_model2.pdf").c_str());
        canvTot.Print((workspacedir+"mcsigfit_NOTMAIN_moreinfo_"+type+"_model2.root").c_str());
	...
	canv4->cd(3) ; gPad->SetLeftMargin(0.15) ; frame10->GetYaxis()->SetTitleOffset(1.4) ; frame10->Draw() ;
	canv4->cd(4) ; gPad->SetLeftMargin(0.15) ; frameun->GetYaxis()->SetTitleOffset(1.4) ; frameun->Draw() ;
	canv4->SaveAs((workspacedir+"mcsigfit_NOTMAIN_model2.pdf").c_str());
	...
	//
	string namews8="signal";
	TFile wrkspc8((workspacedir+"myworkspace_"+namews8+"_model2.root").c_str(),"RECREATE"); 
	w8->import(Bplus_Corrected_Mass);
...
vector<double> FitAndSplotB23MuNuDataForTraining::fitsignalcomponent_1model(string weightname, string type, string year, string PIDcut)
{ 
	...
	TFile* f = new TFile((sigdir+"/"+signalfilename+".root").c_str());
	TTree* t = (TTree*)f->Get("DecayTree");
