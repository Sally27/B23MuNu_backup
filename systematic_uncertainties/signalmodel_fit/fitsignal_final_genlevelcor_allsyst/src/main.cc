...
if (year=="2012")
{
	...
	cout<<"I am bad! these will be different due to only using 2012 mc"<<endl;
	FitAndSplotB23MuNuDataForTraining run;
	run.fitsignalcomponent_1model("PIDweight","", "", "");
	run.fitsignalcomponent_2model("PIDweight","", "", "");

        run.plotpretty_plotnicely_paper_sigsim_drive(false);
        run.plotpretty_plotnicely_paper_sigsim_drive(true);
        run.plotpretty_plotnicely_paper_sigsim_drive_2model(false);
        run.plotpretty_plotnicely_paper_sigsim_drive_2model(true);
