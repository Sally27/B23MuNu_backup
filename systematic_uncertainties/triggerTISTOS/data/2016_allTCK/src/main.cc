...
//	out<<"or strip and reco & 0.33522 & 1365886(8043820) & TBD  \\\\ "<<endl;
	...
	TFile *fillinfo = new TFile("Efficiencies_Data_2016_trigger_TISTOS.root","RECREATE");
	...
	FillEffInfo(fillinfotree, "decprodcut",decprodcut2,false);
        ...
        string ext=".root";
        string filenameclean="Bu23JpsiKData2016";


//        Int_t tcks[] = {287905280,287905283,287905284,287905285,288495113,288626185,288691721,288757257,288822793,288822798,288888329,288888334,ALLTCK,288888337,288888338,288888339};

//        vector<Int_t> mytcks(tcks, tcks+sizeof(tcks)/ sizeof(Int_t) );


        cutTree((filenameclean+"_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc"+ext).c_str(), "DecayTree", (filenameclean+"_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc_ALLTCK"+ext).c_str(), ""); //most repretck
	...
	vals.push_back("(Bplus_Hlt2TopoMu2BodyDecision_TOS==1.0 && Bplus_Hlt2TopoMu2BodyDecision_TIS==1.0) || (Bplus_Hlt2TopoMu3BodyDecision_TOS==1.0 && Bplus_Hlt2TopoMu3BodyDecision_TIS==1.0) || (Bplus_Hlt2DiMuonDetachedDecision_TOS==1.0 && Bplus_Hlt2DiMuonDetachedDecision_TIS==1.0) || (Bplus_Hlt2DiMuonDetachedHeavyDecision_TOS==1.0 && Bplus_Hlt2DiMuonDetachedHeavyDecision_TIS==1.0 )");

	varia.push_back("L0MuonDecisionTISTOS");
	varia.push_back("Hlt1TrackMuonDecisionTISTOS");
	varia.push_back("Hlt2orofDecisionsTISTOS");

	string filename3=(filenameclean+"_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc_ALLTCK").c_str();
	...
	for(int i(0);i<3;i++)
	{
		...
		eff=cutTree((filename3+ext).c_str(), "DecayTree", (filename3+"_"+varia.at(i)+ext).c_str(), (vals.at(i)).c_str());
                ...
                numbertistos=double(numofentriesafter);
	...
	vals.push_back("Bplus_Hlt2TopoMu2BodyDecision_TOS==1.0 && Bplus_Hlt2TopoMu2BodyDecision_TIS==1.0");
        vals.push_back("Bplus_Hlt2TopoMu3BodyDecision_TOS==1.0 && Bplus_Hlt2TopoMu3BodyDecision_TIS==1.0");
	...
	varia.push_back("Hlt2TopoMu2BodyDecisionTISTOS");
        varia.push_back("Hlt2TopoMu3BodyDecisionTISTOS");
        varia.push_back("Hlt2DiMuonDetachedDecisionTISTOS");
        varia.push_back("Hlt2DiMuonDetachedHeavyDecisionTISTOS");
        

	string filename4=(filenameclean+"_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc_ALLTCK_L0MuonDecisionTISTOS_Hlt1TrackMuonDecisionTISTOS").c_str();        
	...
	vals.push_back("(Bplus_Hlt2TopoMu2BodyDecision_TIS==1.0) || (Bplus_Hlt2TopoMu3BodyDecision_TIS==1.0) || (Bplus_Hlt2DiMuonDetachedDecision_TIS==1.0) || (Bplus_Hlt2DiMuonDetachedHeavyDecision_TIS==1.0 )");

	varia.push_back("L0MuonDecisionTIS");
	varia.push_back("Hlt1TrackMuonDecisionTIS");
	varia.push_back("Hlt2orofDecisionsTIS");

	string filename7=(filenameclean+"_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc_ALLTCK").c_str();
	...
	vals.push_back("Bplus_Hlt2TopoMu2BodyDecision_TIS==1.0");
        vals.push_back("Bplus_Hlt2TopoMu3BodyDecision_TIS==1.0");
        vals.push_back("Bplus_Hlt2DiMuonDetachedDecision_TIS==1.0");
        vals.push_back("Bplus_Hlt2DiMuonDetachedHeavyDecision_TIS==1.0");

	varia.push_back("Hlt2TopoMu2BodyDecisionTIS");
        varia.push_back("Hlt2TopoMu3BodyDecisionTIS");
        varia.push_back("Hlt2DiMuonDetachedDecisionTIS");
        varia.push_back("Hlt2DiMuonDetachedHeavyDecisionTIS");
        

	string filename8=(filenameclean+"_misidstrippingfull_mu3isNotMuon_mu3inMuonAcc_ALLTCK_L0MuonDecisionTIS_Hlt1TrackMuonDecisionTIS").c_str();        
