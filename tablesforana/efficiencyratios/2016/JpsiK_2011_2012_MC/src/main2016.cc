...
int main(int argc,char* argv[]){
	...
	vector<ValError> sigPID_vec_full;
	vector<ValError> jpsikPID_vec_full;
...
//		double cormjpsik=0.998631;
		...
		double sigPID=readPIDvar(("/vols/lhcb/ss4314/preparesignalweights/preparesignalweights_FUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/bin/B23MuNuMC2016"+PIDcut+"Turbo16_selected.root").c_str(), "DecayTree", "PIDweight");
		double jpsikPID=readPIDvar(("/vols/lhcb/ss4314/preparecontrolweights/preparecontrolweights_FUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/bin/JpsiKMC2016"+PIDcut+".root").c_str(), "DecayTree", "mcweight");
...
//		ValError sig_othereff_total(number_bkk_sig,sqrt(number_bkk_sig));
		...
		ValError sig_pid(readPIDvar(("/vols/lhcb/ss4314/preparesignalweights/preparesignalweights_FUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/bin/B23MuNuMC2016"+PIDcut+"Turbo16_selected.root").c_str(), "DecayTree", "PIDweight"),0.006);
		ValError jpsik_pid(readPIDvar(("/vols/lhcb/ss4314/preparecontrolweights/preparecontrolweights_FUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/bin/JpsiKMC2016"+PIDcut+".root").c_str(), "DecayTree", "mcweight"),0.0010);
	...
	out << "$\\epsilon\\_{REC}$ & "<<cormsig_full.val<<"$\\pm$"<<cormsig_full.err<< " & " << cormjpsik_full.val<<"$\\pm$"<<cormjpsik_full.err<<" \\\\ "<<endl;
	out << "$\\epsilon\\_{PID}$ & "<<sigPID_full.val<<"$\\pm$"<<sigPID_full.err<< " & " << jpsikPID_full.val<<"$\\pm$"<<jpsikPID_full.err<<" \\\\ "<<endl;

	out << "$\\epsilon\\_{total}$ &" << finalprod_sig.val<<"$\\pm$"<<finalprod_sig.err<<" & "<< finalprod_jpsik.val<<"$\\pm$"<<finalprod_jpsik.err<<" \\\\ "<<endl;
	...
	double effrecostrip= 0.111;
        ...
        ValError numberofobsevents(double(returnEff(("/vols/lhcb/ss4314/fitcontrol/fitcontrolchannelFUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/NOFCME/FitResults/FitControl_2016_"+PIDcut+".root").c_str(), "FitControl_2016", "NumNormObs")),double(returnEff(("/vols/lhcb/ss4314/fitcontrol/fitcontrolchannelFUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/NOFCME/FitResults/FitControl_2016_"+PIDcut+".root").c_str(), "FitControl_2016", "NumNormObs_Err")));
	...
	double numberofevents_low = returnEff(("/vols/lhcb/ss4314/fitcontrol/fitcontrolchannelFUMSB_NOTsimultaneous_NewTCK_PIDoptimisation_extremeweights/NOFCME/FitResults/FitControl_2016_"+PIDcut+".root").c_str(),"FitControl_2016", "NumNormObs");
	...
	out<<"Error is $\\pm$ "<<expsensitivity_low_error; 
        ...
        out<<"\\begin{equation}"<<endl;
        out<<"alpha^{26}_{NOFCME}(\\epsilon)=\\frac{\\mathcal{B}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+})}{N^{26}_{NOFCME}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+}) \\times R^{26}_{NOFCME}(\\epsilon)}="<<newses.val<<"\\pm"<<newses.err<<endl;
        out<<"\\end{equation}"<<endl;
        out<<"FOR ANANOTE diff base"<<endl;
        out<<"\\begin{equation}"<<endl;
        out<<"R^{26}_{NOFCME}(\\epsilon)=\\frac{"<<finalprod_sig.roundToError_diffbasenoampe(true)<<"}{"<<finalprod_jpsik.roundToError_diffbasenoampe(true)<<"}="<< (finalprod_sig/finalprod_jpsik).roundToError_diffbasenoampe(true)<<endl;
        out<<"\\end{equation}"<<endl;

        out<<"\\begin{equation}"<<endl;
        out<<"\\alpha^{26}_{NOFCME}=\\frac{\\mathcal{B}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+})}{N^{26}_{NOFCME}(B^{+} \\rightarrow (J/\\psi \\rightarrow \\mu^{+} \\mu^{-}) K^{+}) \\times R^{26}_{NOFCME}(\\epsilon)}="<<newses.roundToError_diffbasenoampe(true) <<endl;
        out<<"\\end{equation}"<<endl;
	...
	effi_pid_sig=sigPID_full.val;
	effi_pid_norm=jpsikPID_full.val;
	...
	effi_err_pid_sig=sigPID_full.err;
	effi_err_pid_norm=jpsikPID_full.err;
