...
int main(){
	...
	out<<"\\hline"<<endl;
        ...
        out<<" TCK & $\\epsilon_{c\\bar{c}}$ & $\\epsilon_{c\\bar{c}}$ " <<" \\\\ "<<endl;	
        out<<"\\hline"<<endl;
	...
	for(int z(0);z<mytcks.size();z++){
		...
		years.push_back(2016);
                ...
                decprodcutsig=returnEff(("../efficiencytuples_sig/Efficiencies_SigMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "jpsi");
  //              effrecostrsig=returnEff(("../efficiencytuples_sig/Efficiencies_SigMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "qmincut");
		...
		double cormjpsik(0);
        	...
        	decprodcutjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_NormMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Norm_"+i2s(years.at(1))).c_str(), "jpsi");
//        	effrecostrjpsik=(returnEff(("../efficiencytuples_norm/Efficiencies_NormMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Norm_"+i2s(years.at(1))).c_str(), "qmincut"));
...
//	effmyrecostrjpsik=inner_product( effmyrecostrjpsik_vec.begin(),  effmyrecostrjpsik_vec.end(), begin(weights), 0.0);
	...
	out <<"\\hline"<<endl;
	out << "$\\epsilon_{c\\bar{c}}$ &" << round_to_digits(decprodcutsig,3) << " & N/A \\\\ "<<endl;
//	out << "$\\epsilon_{minq^{2}}$ &" << effrecostrsig << " & N/A \\\\ "<<endl;
    //    out << "$\\epsilon_{HLT2}$ &" << effmyrecosig << " & " << effmyrecostrjpsik <<" \\\\ "<<endl;
	out<<"\\hline"<<endl;
	out<<"\\end{tabular}"<<endl;
	out<<"\\end{center}"<<endl;
