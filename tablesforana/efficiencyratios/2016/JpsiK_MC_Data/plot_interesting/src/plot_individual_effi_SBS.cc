...
int main(int argc, char *argv[])
{
	...
	string PIDcut_Run1=argv[1];
        ...
        lhcbSally();
        ofstream out;
        out.open("Summary_plot_individualeff_"+PIDcut_Run1+"_"+PIDcut_2016+"_OK_full_ERROR.tex");
		...
		for(int i(0);i<treenames.size();i++){
			...
			for (int j(0);j<tree->GetEntries();j++)
			{
				...
				cout<<" Value "<<numofevents<<endl;
                                out<<" & "<<round_to_digits(numofevents,3)<<"$\\pm$"<<round_to_digits(numofevents_err,3);              
...
//	mg->GetYaxis()->SetRightMargin(0.2);
	...
	c1->SaveAs("Plot_ALL_Efficiencies_Overview_OK_fullerr.pdf");
	delete c1;
