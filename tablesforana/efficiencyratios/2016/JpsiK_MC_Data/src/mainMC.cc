 ...
 //      Int_t tcks[] = {287905280,287905283,287905284,287905285,288495113,288626185,288691721,288757257,288822793,288822798,288888329,288888334,288888335,288888337,288888338,288888339};
   ...
   //     vector<Int_t> mytcks(tcks, tcks+sizeof(tcks)/ sizeof(Int_t) );
	...
	TTree* t_norm2016_cut_strip =(TTree*)f_norm2016_cut_strip.Get((decaytreename.at(0).c_str()));
        ...
        out<<"\\hline"<<endl;
        out << "$\\epsilon\\_{GEN}$ & "<<sig_decprod.val<<"$\\pm$"<<sig_decprod.err<< " & " << jpsik_decprod.val<<"$\\pm$"<<jpsik_decprod.err<<" & "<< jpsik2016_decprod.val<<"$\\pm$"<<jpsik2016_decprod.err<<" & "<< (jpsik_decprod/sig_decprod).val<< "$\\pm$"<< (jpsik_decprod/sig_decprod).err<<" & "<<printnice(jpsik2016_decprod/jpsik_decprod)<<" \\\\ "<<endl;
       out << "$\\epsilon\\_{REC}$ & "<<sig_reco.val<<"$\\pm$"<<sig_reco.err<< " & " << jpsik_reco.val<<"$\\pm$"<<jpsik_reco.err<<" & "<< jpsik2016_reco.val<<"$\\pm$"<<jpsik2016_reco.err<<" &"<< (jpsik_reco/sig_reco).val<< "$\\pm$"<< (jpsik_reco/sig_reco).err<<"& "<<printnice(jpsik2016_reco/jpsik_reco)<<" \\\\ "<<endl;
        out << "$\\epsilon\\_{striponly}$ & "<<sig_reco_strip.val<<"$\\pm$"<<sig_reco_strip.err<< " & " << jpsik_reco_strip.val<<"$\\pm$"<<jpsik_reco_strip.err<<" & "<<jpsik2016_reco_strip.val<<"$\\pm$"<<jpsik2016_reco_strip.err<<" & "<< (jpsik_reco_strip/sig_reco_strip).val<< "$\\pm$"<< (jpsik_reco_strip/sig_reco_strip).err<<" & "<< printnice(jpsik2016_reco_strip/jpsik_reco_strip)<<" \\\\ "<<endl;
	...
	for( int i(1); i<8; i++)
	{
		...
		out << "$\\epsilon\\_{"+cuts.at(i-1)+"}$ & "<<sig_effs.val<<"$\\pm$"<<sig_effs.err<< " & " << jpsik_effs.val<<"$\\pm$"<<jpsik_effs.err<< " & "<<jpsik2016_effs.val<<"$\\pm$"<<jpsik2016_effs.err<<" & "<< (jpsik_effs/sig_effs).val<< "$\\pm$"<< (jpsik_effs/sig_effs).err<<" & "<<printnice(jpsik2016_effs/jpsik_effs)<<" \\\\ "<<endl;
        ...
        out << "$\\epsilon\\_{PID}$ & "<<sig_pid.val<<"$\\pm$"<<sig_pid.err<< " & " << jpsik_pid.val<<" $\\pm$ "<<jpsik_pid.err<< " & "<<jpsik2016_pid.val<<" $\\pm$ "<<jpsik2016_pid.err<<" & "<< (jpsik_pid/sig_pid).val<< "$\\pm$"<< (jpsik_pid/sig_pid).err<<" & "<<printnice(jpsik2016_pid/jpsik_pid)<<" \\\\ "<<endl;
        out<<"$\\epsilon_{tot}$ &"<<finalprod_sig.val<<"$\\pm$"<<finalprod_sig.err<<" & "<<finalprod_jpsik.val<<"$\\pm$"<<finalprod_jpsik.err<< " & "<<finalprod_jpsik2016.val<<"$\\pm$"<<finalprod_jpsik2016.err<< " & "<< (finalprod_jpsik/finalprod_sig).val<< "$\\pm$"<< (finalprod_jpsik/finalprod_sig).err<<" & "<<printnice(finalprod_jpsik2016/finalprod_jpsik)<<" \\\\ "<<endl;

        out<<"\\hline"<<endl;
        out<<" Ratios & -  &$\\epsilon^{2012}_{tot}/\\epsilon^{2011}_{tot}$= "<<(finalprod_jpsik/finalprod_sig).val<<"$\\pm$"<<(finalprod_jpsik/finalprod_sig).err<<" & "<<"$\\epsilon^{2016}_{tot}/\\epsilon^{2012}_{tot}$="<<(finalprod_jpsik2016/finalprod_jpsik).val<<"$\\pm$"<<(finalprod_jpsik2016/finalprod_jpsik).err<<" \\\\ "<<endl;

        out<<"\\hline"<<endl;
        out<<"\\end{tabular}"<<endl;
        out<<"\\end{center}"<<endl;
        out<<"\\caption{Efficiency differences. Can be found in /vols/lhcb/ss4314/tablesforana/efficiencyratios/2016/JpsiK_MC_Data}"<<endl;
        out<<"\\end{table}"<<endl;
