...
int main(){
	...
	for(int z(0);z<mytcks.size();z++){
		...
		effrecostrsig=returnEff(("../efficiencytuples_sig/Efficiencies_"+i2s(years.at(2))+"_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Sig_"+i2s(years.at(2))).c_str(), "qmincut");
		decprodcutsig=returnEff(("../efficiencytuples_sig/Efficiencies_"+i2s(years.at(2))+"_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Sig_"+i2s(years.at(2))).c_str(), "jpsi");
           //     effmyrecosig=returnEff(("../efficiencytuples_sig/Efficiencies_"+i2s(years.at(2))+"_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Sig_"+i2s(years.at(2))).c_str(), "");
		...
		effrecostrjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "qmincut");
		decprodcutjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "jpsi");
	//	effmyrecostrjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_"+i2s(years.at(2))+"_Norm_"+i2s(mytcks.at(z))+".root").c_str(), ("MC_Norm_"+i2s(years.at(2))).c_str(), "trigger");
		...
		out<<mytcks.at(z)<<" & " << round_to_digits(decprodcutsig,3)<<" & "<<round_to_digits(effrecostrsig,3)<<" & N/A & N/A "<<" \\\\ "<<endl;
...
//	effmyrecostrjpsik=inner_product( effmyrecostrjpsik_vec.begin(),  effmyrecostrjpsik_vec.end(), begin(weights), 0.0);
	...
	out<<"\\hline"<<endl;
        out<<"Weighted efficiency" <<" & " << round_to_digits(decprodcutsig,3)<<" & "<<round_to_digits(effrecostrsig,3)<<" & "<< "  N/A & N/A \\\\ "<<endl;
	out<<"\\hline"<<endl;
	...
	out <<"\\hline"<<endl;
	out << "$\\epsilon_{c\\bar{c}}$ &" << round_to_digits(decprodcutsig,3) << " & " << round_to_digits(decprodcutjpsik,3) <<" \\\\ "<<endl;
	out << "$\\epsilon_{qmin}$ &" << round_to_digits(effrecostrsig,3) << " & " << round_to_digits(effrecostrjpsik,3) <<" \\\\ "<<endl;
	out<<"\\hline"<<endl;
	out<<"\\end{tabular}"<<endl;
	out<<"\\end{center}"<<endl;
