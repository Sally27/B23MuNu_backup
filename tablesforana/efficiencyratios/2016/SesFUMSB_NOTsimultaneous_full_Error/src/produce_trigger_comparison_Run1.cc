...
int main(){
	...
	out<<"\\hline"<<endl;
        ...
        out<<" TCK & $\\epsilon_{L0}$ & $\\epsilon_{HLT1}$ & $\\epsilon_{L0}$ & $\\epsilon_{HLT1}$" <<" \\\\ "<<endl;	
        out<<"\\hline"<<endl;
	...
	for(int z(0);z<mytcks.size();z++){
		...
		years.push_back(2016);
                ...
                decprodcutsig=returnEff(("../efficiencytuples_sig/Efficiencies_SigMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "L0");
                effrecostrsig=returnEff(("../efficiencytuples_sig/Efficiencies_SigMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Sig_"+i2s(years.at(1))).c_str(), "Hlt1TrackMuonDecisionTOS");
		...
		double cormjpsik(0);
        	...
        	decprodcutjpsik=returnEff(("../efficiencytuples_norm/Efficiencies_NormMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Norm_"+i2s(years.at(1))).c_str(), "L0MuonDecisionTOS");
        	effrecostrjpsik=(returnEff(("../efficiencytuples_norm/Efficiencies_NormMC_"+i2s(years.at(1))+".root").c_str(), ("MC_Norm_"+i2s(years.at(1))).c_str(), "Hlt1TrackMuonDecisionTOS"));
	...
	out <<"\\hline"<<endl;
	out << "$\\epsilon_{L0}$ &" << round_to_digits(decprodcutsig,3) << " & " << round_to_digits(decprodcutjpsik,3) <<" \\\\ "<<endl;
	out << "$\\epsilon_{HLT1}$ &" << round_to_digits(effrecostrsig,3) << " & " << round_to_digits(effrecostrjpsik,3) <<" \\\\ "<<endl;
        out << "$\\epsilon_{HLT2}$ &" << round_to_digits(effmyrecosig,3) << " & " << round_to_digits(effmyrecostrjpsik,3) <<" \\\\ "<<endl;
	out<<"\\hline"<<endl;
	out<<"\\end{tabular}"<<endl;
	out<<"\\end{center}"<<endl;
