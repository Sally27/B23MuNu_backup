...
int main(int argc, char *argv[])
{
	...
	string PIDcut_Run1=argv[1];
        ...
        ofstream out;
        out.open("Summary_plot_individualeff_BOTH_"+lowFCME+highFCME+"_FULLERROR_percent.tex");
		...
		for(int i(0);i<treenames.size();i++){
			...
			for (int j(0);j<tree->GetEntries();j++)
			{
				...
				cout<<" Value "<<numofevents<<endl;
                                ...
                                out<<" & "<<dbl2str(numofevents*100.0,2)<<" $\\pm$ "<<dbl2str(err*100.0,2);}
                                if (z==(variablestoplot.size()-1) || z==(variablestoplot.size()-2) ){
                                out<<" & "<<dbl2str(numofevents*100.0,4)<<" $\\pm$ "<<dbl2str(err*100.0,4);}
