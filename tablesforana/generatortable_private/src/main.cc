...
//        ValError try1 = ((ninaccqmin)/(nglqmin))*(nrecqmin/(ninacc*(nglqmin/numgen)));
        ...
        double effspythia8_down(try2_err.val);
        double effspythia8_down_err(try1_err.err);
        ...
        sig_2012.push_back((premiere/2.0));
        sig_2012.push_back(troisieme/2.0);
        ...
        ValError premiere_2016=(getRatioBinomial(ninaccqmin_2016,nglqmin_2016));;
...
//        double averageoffour_err=((effspythia6_up_err)+effspythia6_down_err+effspythia8_up_err+effspythia8_down_err)/4;
	...
	out<<""<<endl;
	out<<"	\\hline"<<endl;
	out<<"		Channel & Year & Conditions & $\\epsilon\\_{gen}$ & Average \\\\ \\hline"<<endl;
	out<<"		$B^{+} \\rightarrow \\mu^{+} \\mu^{-} \\mu^{+} \\nu$ & 2012 &  Sim08, Pyth6 &"<< premiere/2.0 <<" & \\multirow{2}{*}{"<<averageValError(sig_2012) <<"} \\\\"<<endl;
	out<<"		$B^{+} \\rightarrow \\mu^{+} \\mu^{-} \\mu^{+} \\nu$ & 2012 &  Sim08, Pyth8 &"<< troisieme/2.0 <<" & \\\\"<<endl;//<<averageValError(sig_2012)<<" \\\\"<<endl;
	out<<"		$B^{+} \\rightarrow \\mu^{+} \\mu^{-} \\mu^{+} \\nu$ & 2016 &  Sim09b, Pyth8 &"<< premiere_2016/2.0<<" & "<<averageValError(sig_2016)<<" \\\\"<<endl;
	out<<"		\\hline"<<endl;
	...
	out<<"		the polar angle region of [10, 400] mrad. These numbers were privately generated with 25000 events and qmin$^2$ cut is applied beforehand.}"<<endl;
	out<<"	\\label{tab:mygen}"<<endl;
	out<<"	\\end{table}"<<endl;
