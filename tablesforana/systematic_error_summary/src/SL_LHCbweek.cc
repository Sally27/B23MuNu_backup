...
//        string systematic_trigger_hlt2("/vols/lhcb/ss4314/systematic_uncertainties/triggerTISTOS/makeatable/Systematic_Trigger/Systematic_Trigger.root");
        ...
        string systematic_sigmodel("/vols/lhcb/ss4314/systematic_uncertainties/signalmodel/EfficiencyCalc_correct/Systematic_SigModel/Systematic_SigModel.root");
  ...
  //      suminquad_2012=;
        ...
        ofstream out;
	out.open("Summary_Systematics_SLWG_correct.tex");
	...
	//out<<"Trigger Data/Sim & "<<abs(round_to_digits(returnVar(systematic_trigger.c_str(),"Trigger_Run1","systematic"),2))<<" & "<< abs(round_to_digits(returnVar(systematic_trigger.c_str(),"Trigger_2016","systematic"),2))<<"& "<<round_to_digits(addstatandsyst(returnVar(systematic_trigger.c_str(),"Trigger_Run1","systematic"),returnVar(systematic_trigger.c_str(),"Trigger_2016","systematic")),2)<<"\\\\"<<endl;

        double kp=producecorrerror(0.039, 0.010, 0.754, 0.787);

	out<<"Trigger Data/Sim & "<<abs(round_to_digits(returnVar(systematic_trigger.c_str(),"Trigger_Run1","systematic"),2))<<" & "<< abs(round_to_digits(returnVar(systematic_trigger.c_str(),"Trigger_2016","systematic"),2))<<"& "<<round_to_digits((kp*100),2)<<"\\\\"<<endl;
	...
	suminquad_overall.push_back(kp*100);

        out<<"Trigger HLT2 & "<<" - "<<" & "<< " 1.5"<<"& "<<round_to_digits(addstatandsyst(0.0,1.5),2)<<"\\\\"<<endl;

        suminquad_2012.push_back(0.0);
        suminquad_2016.push_back(1.5);
	suminquad_overall.push_back(addstatandsyst(0.0,1.5));

        out<<"Kaon interaction probability & "<<" 2.0 "<<" & "<< " 2.0"<<"& "<<round_to_digits(2.0,2)<<"\\\\"<<endl;

        suminquad_2012.push_back(2.0);
        suminquad_2016.push_back(2.0);
	suminquad_overall.push_back(2.0);
...
//	out<<"Kinematic Reweighting & "<<abs(round_to_digits(returnVar(systematic_kinrew.c_str(),"KinRew_Run1_DataMCWeight2D_alt2_new","systematic"),2))<<" & "<< abs(round_to_digits(returnVar(systematic_kinrew.c_str(),"KinRew_2016_DataMCWeight2D_alt2_new","systematic"),2))<<" &"<< round_to_digits(addstatandsyst(returnVar(systematic_kinrew.c_str(),"KinRew_Run1_DataMCWeight2D_alt2_new","systematic"),returnVar(systematic_kinrew.c_str(),"KinRew_2016_DataMCWeight2D_alt2_new","systematic")),2)<<"\\\\"<<endl;
        ...
        double op=producecorrerror(0.004, 0.007, 0.380, 0.361);//(diff wrt to nom model, diff with respect to the nom model )

	out<<"Kinematic Reweighting & "<<abs(round_to_digits(returnVar(systematic_kinrew.c_str(),"KinRew_Run1_DataMCWeight2D_alt2_new","systematic"),2))<<" & "<< abs(round_to_digits(returnVar(systematic_kinrew.c_str(),"KinRew_2016_DataMCWeight2D_alt2_new","systematic"),2))<<" &"<< round_to_digits((op*100),2)<<"\\\\"<<endl;
        ...
        suminquad_overall.push_back(op*100);
...
//	out<<"Simulation statistics & "<<100*round_to_digits((0.000020/0.001581),2)<<" & "<<100*round_to_digits((0.000008/0.001182),2)<<" & "<<round_to_digits((addstatandsyst((0.000020/0.001581)*100.0,(0.000008/0.001182)*100.0)),2)<<" \\\\" <<endl;
	...
	out<<"Simulation Statistics & "<<100*round_to_digits((0.000020/0.001581),2)<<" & "<<100*round_to_digits((0.000008/0.001182),2)<<" & "<<round_to_digits((addstatandsyst(0.000020,0.000008)/(0.001581+0.001182))*100.0,2)<<" \\\\" <<endl;

        
        suminquad_2012.push_back((0.000020/0.001581)*100.0);
        suminquad_2016.push_back((0.000008/0.001182)*100.0);
	suminquad_overall.push_back((addstatandsyst(0.000020,0.000008)/(0.001581+0.001182))*100.0);
	...
	out<<" \\hline"<<endl;
//        out<<"Error on $B^{+} \\rightarrow J/\\psi K^{+}$ yield & "<<abs(round_to_digits((returnVar(stat_2012.c_str(),"Info_2012","numofevents_err"))/((returnVar(stat_2012.c_str(),"Info_2012","numofevents"))),2))*100<<" & "<< abs(round_to_digits((returnVar(stat_2016.c_str(),"Info_2016","numofevents_err"))/((returnVar(stat_2016.c_str(),"Info_2016","numofevents"))),2))*100<<" & "<< pleasework<<"\\\\"<<endl;
        double err1err20cor=addstatandsyst(returnVar(stat_2012.c_str(),"Info_2012","numofevents_err"),returnVar(stat_2016.c_str(),"Info_2016","numofevents_err"));
        double totalevs=double(returnVar(stat_2016.c_str(),"Info_2016","numofevents"))+double(returnVar(stat_2012.c_str(),"Info_2012","numofevents"));
        double pleasework=round_to_digits((double(err1err20cor)/double(totalevs))*100.0,3);
    //    out<<"I see tot &"<<err1err20cor<<" & "<<totalevs<<"\\\\"<<endl;
    //    out<<"I see 2012 &"<<err1err20cor<<" & "<<totalevs<<"\\\\"<<endl;
       
        double err1=double(returnVar(stat_2012.c_str(),"Info_2012","numofevents_err"));
        double num1=double(returnVar(stat_2012.c_str(),"Info_2012","numofevents"));
        double err2=double(returnVar(stat_2016.c_str(),"Info_2016","numofevents_err"));
        double num2=double(returnVar(stat_2016.c_str(),"Info_2016","numofevents"));
       
        
   //     out<<"I see 2012 &"<<err1<<" & "<<num1<<"\\\\"<<endl;
   //     out<<"I see 2016 &"<<err2<<" & "<<num2<<"\\\\"<<endl;

        out<<"Error on $B^{+} \\rightarrow J/\\psi K^{+}$ yield & "<<round_to_digits(((err1/num1)*100.0),2)<<" & "<<round_to_digits(((err2/num2)*100.0),2)<<" & "<< pleasework<<"\\\\"<<endl;
