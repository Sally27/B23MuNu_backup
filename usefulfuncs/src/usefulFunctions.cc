...
//   TH2F th2f("th2f", "th2f", 50,-1e5,1e5,50,-1e5,1e5);
   TH2F th2f("th2f", "th2f", 100,5000,6100,100,0,1.0);
   TH2D eff("eff", "eff", 100,5000,6100,100,0,1.0);
   ...
   TFile fSig("checkcorr2binsbdtcontrolsample/ControlBkgSamplevetoJpsi.root");
   TTree* tSig = (TTree*)fSig.Get("DecayTree");


  double nSig(getEntries("checkcorr2binsbdtcontrolsample/ControlBkgSamplevetoJpsi.root", "", ""));
  cout<<nSig<<"NSIG"<<endl;
  double mincut(-0.0);
  double maxcut(1.0);
  ...
  for (int i=0; i<t->GetEntries(); i++){
          ...
          cout<<"Var1:"<< var1<<endl;
          cout<<"nSig:"<<nSig<<endl;
          cout<<"Cut Value:"<<double(getEntries( tSig ,  extracut + " && " + var2name.c_str() + " > " + d2s(mincut+(i/(1.*n))*d), weightBranchSig ))<<endl;
          cout<<"check mincut"<<(mincut+(i/(1.*n))*d)<<endl;
          sigEff[i] = double(getEntries( tSig ,  extracut + " && " + var2name.c_str() + " > " + d2s(mincut+(i/(1.*n))*d), weightBranchSig ))/double(nSig); 
      ...
      if(draw)
   {
      ...
      eff.SetTitle(( var2name+"_vs_"+"SigEfficiency").c_str());
      eff.Draw("colz");
      TProfile *profile = eff.ProfileX();
      profile->Draw("same");
      canv4.Print((filename+cleanNameString("SigEff_vs_"+var2name)+".pdf").c_str());
...
//   return correl;
    ...
    if(draw)
   {
      TCanvas canv3("canv3", "canv3", 600, 600);
      th2f.GetXaxis()->SetTitle(var1name.c_str());
      th2f.GetYaxis()->SetTitle(var2name.c_str());
      th2f.SetTitle(( var1name+"_vs_"+var2name).c_str());
      TProfile *prof = th2f.ProfileX();
      prof->Draw();
      canv3.Print( (filename+cleanNameString("ProfileX"+var1name+"_vs_"+var2name)+".pdf").c_str());
   }
