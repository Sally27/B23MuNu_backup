...
void invmass()
{
  Double_t mu1_ghost, mu2_ghost, mu3_ghost;
  Double_t mu1_TRACK_CHI2, mu2_TRACK_CHI2, mu3_TRACK_CHI2;
  ...
  TFile* f = new TFile("B23MuNuCCdof4final.root");
  TTree* t = (TTree*)f->Get("B23MuNu_Tuple/DecayTree");

  TFile *f1 = new TFile("ghost.root","RECREATE");
...
/*  gStyle->SetOptStat(0);*/	
 ...
 /*  TH1F* hm = new TH1F("m1m2invmass", "m1m2invmass", 50, -10 , 10000);*/
   TBranch *bpt = t1->Branch("m1andm2whereinvtos",&invmassmu1mu2,"invmassmu1mu2/D");
   count=0;
   count2=0;
   ...
   for(int i(0); i<t->GetEntries(); ++i)
   {
      ...
      if ((abs(mu2_TRUEID)==13)&&(abs(mu3_TRUEID)==13)&&(abs(mu1_TRUEID)==13)&&(abs(Bplus_TRUEID)==521)&&(mu1_ghost<0.35)&&(mu2_ghost<0.35)&&(mu3_ghost)<0.35&&(mu1_TRACK_CHI2<3.0)&&(mu3_TRACK_CHI2<3.0)&&(mu2_TRACK_CHI2<3.0))
         {
