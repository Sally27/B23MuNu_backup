...
void eff()
{
  ...
  TFile* f = new TFile("B23MuNuBackgroundDatacutPion.root");
...
//                                                	cout<< "Multiarr woop: "<< i << " "<< j << " " << k << " :" << MultiArr[i][j][k]<<endl;
 ...
 //                                                       cout<<"Multiarr woop: "<< i << " "<< j << " " << k << " :" << MultiArr[i][j][k]<<endl;
  ...
  TFile s("PerfHists_Pi_Strip20_MagDown_P_ETA_nTracks.root"); 
  TH3F *hname =(TH3F*)s.Get("Pi_IsMuon==0.0 && DLLK < 0.0 && DLLp < 5.0_All");
  Double_t effi [288];
