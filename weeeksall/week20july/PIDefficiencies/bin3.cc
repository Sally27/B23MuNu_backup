...
//             					cout<<"Allocated to bin: "<< i << " "<< j << " " << k << " . Number of events in this bin" << MultiArr[i][j][k] <<endl;	
 //                                               cout<< "Multiarr: "<< i << " "<< j << " " << k << " :" << MultiArr[i][j][k]<<endl;
  ...
  TFile s("newpidsample/KisMuonDLLmumorethanzeroDLLmuminusKmorethanzero/PerfHists_K_Strip20_MagDown_P_ETA_nTracks.root"); 
...
//   cout<<MultiArr[i][j][k]<< " "<< newcount[c-1]<<endl;
  ...
  TH2D *h2e = new TH2D("h2e","MIS-ID rate as a function of kaon momentum",11,0,100000,100,0,0.005);
  ...
  m3p->SaveAs("kaonnewpidruntestbin3.pdf");
