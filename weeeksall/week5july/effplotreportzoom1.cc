...
// here hg has data ini it
   ...
   TH1F* hm = new TH1F("m1m2invmassmunusel_sel", "m1m2invmassmunusel_sel", 50, -10 , 1000000);
   hm->Sumw2();
   myfile2.t->Draw("newinvmassmu1andmu22gev>>m1m2invmassmunusel_sel");
   double nSel2 = myfile2.t->GetEntries();
   hm->Scale(0.025);
// here hm has data ini it
   TH1F* hg = new TH1F("mplusmminusinvmassmunusel_gen", "mplusmminusinvmassmunusel_gen", 50, -10 , 1000000);
   hg->Sumw2();
   B23MuNu_gen2.t->Draw("mplusandmminusinvmass>>mplusmminusinvmassmunusel_gen");
// here hg has data ini it
   double nGen2 = B23MuNu_gen2.t->GetEntries();

   TH1F* heff = new TH1F("eff", "eff", 50, -10 , 1000000);
   heff->Divide(hm,hg);
   ...
   TH1F* am = new TH1F("m1m2invmasstos_sel", "m1m2invmasstos_sel", 50, -10 , 1000000);
   am->Sumw2();
   myfile3.t->Draw("newinvmassmu1andmu2tos>>m1m2invmasstos_sel");
   double nSel3 = myfile3.t->GetEntries();
   am->Scale(0.025);
// here hm has data ini it
   TH1F* ag = new TH1F("mplusandmminusinvmasstos_gen", "mplusandmminusinvmasstos_gen", 50, -10 , 1000000);
   ag->Sumw2();
...
//   c->SetTitle("Efficiency of #mu_{+} and #mu{-} with q^2(#mu_{+} & #nu)>4000 GeV with TOS");
   ...
   mp3->Update();
   mp3->SaveAs("plotforreportfinalnewMCzoom1.pdf");
